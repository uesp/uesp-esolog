<?php
$ESO_QUEST_DATA = array(
	6413 => array(
			'name'    => "Another Khajiit's Tale",
			'journal' => "Zhasim, a Khajiit raised by Orcs in Wrothgar, came to Southern Elsweyr to help his uncle get his shipment of gourmet moon-sugar to market in Rimmen. But Zhasim and his guard Maezi were robbed and his cat Winks ran away.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2955 => array(
			'name'    => "Capture Glademist Farm",
			'journal' => "I accepted a mission to help capture Glademist Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4174 => array(
			'name'    => "Scouting the Mine",
			'journal' => "Tovisa wants help at Northwind Mine. The Reachmen have taken over the mine and we need to find out why.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5529 => array(
			'name'    => "Sacrament: Trader's Cove",
			'journal' => "I received a contract from the Speaker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	30 => array(
			'name'    => "Dark Brotherhood Assassination",
			'journal' => "Gotta kill 'em all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3344 => array(
			'name'    => "Satak was the First Serpent",
			'journal' => "I've come upon <<1>>, a sun-baked wanderer. He inhabits a murky oasis near Tu'whacca's Throne. The wanderer seems to be speaking in fables regarding Sep's Hunger and a pearl.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	51 => array(
			'name'    => "In Pursuit of Gargast",
			'journal' => "Countess Virolynn believes Gargast is bound for a town called Crosswych in Dwynnen.  Who ever he reports to, likely tied to this Shourded Thone, wait in Crosswych.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6508 => array(
			'name'    => "Pledge: Castle Thorn",
			'journal' => "Urgarlag Chief-bane of the Undaunted has challenged me to explore Castle Thorn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	35 => array(
			'name'    => "Example Strings",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6347 => array(
			'name'    => "Dragon Lore: Shroud Hearth",
			'journal' => "Dragons have not been seen in Tamriel for generations, and the Khajiit are unsure how best to fight against them. The ancient Nords recorded Dragon fighting techniques on scrolls they keep in their crypts. I need to find these scrolls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2637 => array(
			'name'    => "The Elder Scroll of Altadoon",
			'journal' => "Grand Warlord Dortene wants the <<1>> captured and brought into Covenant hands.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3675 => array(
			'name'    => "Last One Standing",
			'journal' => "<<1>> wants to get rid of <<2>>, the Khajiit in Alten Corimont. Since the Khajiit is running Dominion supply lines, her sudden demise would benefit both <<1>> and the rest of Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	57 => array(
			'name'    => "JR_NMSQ1",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4679 => array(
			'name'    => "The Shadow's Embrace",
			'journal' => "I decided to ask Faraniel for help getting into the tower. Before she shows me the secret, however, she wants me to help her convince the Wood Elves to leave the forest and stop working for Molag Bal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2687 => array(
			'name'    => "Scout Castle Roebeck",
			'journal' => "I accepted a mission to scout Castle Roebeck for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	105 => array(
			'name'    => "A Family Divided",
			'journal' => "While detouring through the swamp, the Bagot family was attacked by a powerful Bog Lurker. Their daughter Isabelle went missing in the chaos. Tristan, wounded, limped to the caravan to find help, while his wife, Aurore, left to look for Isabelle.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3659 => array(
			'name'    => "Unwanted Guests",
			'journal' => "I shared everything I learned with Councilor Ralden. He plans to seal Narsis so that no one can enter or leave. I've been granted an exception while I work for the councilor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	41 => array(
			'name'    => "Thieving from Thieves",
			'journal' => "Jes of Wayrest told me a shipment of weapons he sent to Eagle's Watch was taken by who bandits who prey on travelers near Camlorn. Jes doesn't want his weapons in the hands of these villains.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6365 => array(
			'name'    => "The Jewel of Baan Dar",
			'journal' => "I crossed paths with a scoundrel named Jakarn. He invited me to join him on a job that could make us both rich. Unfortunately, some seedy Khajiit are intent on stopping him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3338 => array(
			'name'    => "Mists of Corruption",
			'journal' => "Sir Lanis and the Knights of the Dragon are ready to march into Cath Bedraud to deal with Angof, but the mist rising from Angof's vines creates a hazard they can't overcome.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6518 => array(
			'name'    => "Circle of Cheaters",
			'journal' => "There's a cheater among Mor Khazgur's Circle of Champions and I have been tasked by the Swords of Solitude to deal with the troublesome champion before his actions provoke a civil war among the Orcs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	53 => array(
			'name'    => "A Crossroad",
			'journal' => "Oron awaits word from High King Fahara'jad as how to proceed with King Sebastien.  While he waits he asked me to ask Sebastien about the Shrouded Throne.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4787 => array(
			'name'    => "Mourning the Lost",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7156 => array(
			'name'    => "Pledge: Bedlam Veil",
			'journal' => "Urgarlag Chief-bane of the Undaunted has challenged me to explore Bedlam Veil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	45 => array(
			'name'    => "Aldcroft Manor",
			'journal' => "Katarin delivered Ranser's murder weapon to Countess Virolynn in Aldcroft manor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4779 => array(
			'name'    => "The Amronal of Valenwood",
			'journal' => "It appears a hunting band was overcome by a powerful wispmother, as their frozen forms are scattered about the mire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1615 => array(
			'name'    => "Next of Kin",
			'journal' => "I found a letter on a dead man in Bearclaw Mine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	423 => array(
			'name'    => "Missing Soldiers",
			'journal' => "Commander Lafavre has asked you to find some of his soldiers who have not reported back. He is afraid they are in danger.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3009 => array(
			'name'    => "Turning of the Trees",
			'journal' => "The forest itself seems to have turned against the village of Deleyn's Mill and the villagers are in danger. I should see if I can help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	56 => array(
			'name'    => "Spirits of the Forgotten",
			'journal' => "Abelard Yves has asked me to look into the issue of spirits who are haunting the roads outside Camlorn and disrupting trade. Camlorn is a big city, but the townspeople should be able to point me in the right direction.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3262 => array(
			'name'    => "Capture Alessia Mine",
			'journal' => "I accepted a mission to help capture Alessia Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	560 => array(
			'name'    => "dummyQuest",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6226 => array(
			'name'    => "Ruthless Competition",
			'journal' => "Cyrodilic Collections, a society dedicated to locating and restoring Argonian antiquities, seeks help in its mission. They want to stay ahead of their competition and preserve every piece of Argonian culture they can.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2776 => array(
			'name'    => "Scout Farragut Keep",
			'journal' => "I accepted a mission to scout Farragut Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4369 => array(
			'name'    => "The Will of the Worm",
			'journal' => "To save the members of the Mages Guild that were forced to compete in the Observer's game, I decided to side with Thallik. Now I just need to find a way to win the game.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	58 => array(
			'name'    => "KG The Knowledge of Age",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2823 => array(
			'name'    => "Scout Rayles Lumbermill",
			'journal' => "I accepted a mission to scout Rayles Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	406 => array(
			'name'    => "Crypt Cleansing",
			'journal' => "Vestrial asked you to cleanse the Crypt of all evil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2771 => array(
			'name'    => "Scout Fort Aleswell",
			'journal' => "I accepted a mission to scout Fort Aleswell for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5546 => array(
			'name'    => "Honest Work",
			'journal' => "Raynor and Kireth Vanos were recently marooned on the Gold Coast. The captain they hired to sail them to Sentinel delivered them to Anvil instead, and now they lack the funds to finish their journey. They asked me to help them find work.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2932 => array(
			'name'    => "Capture Castle Bloodmayne",
			'journal' => "I accepted a mission to help recapture Castle Bloodmayne for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	189 => array(
			'name'    => "Out of Darkness, Light OLD",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2558 => array(
			'name'    => "The Slavers",
			'journal' => "Margot Oscent has a son named Lucas who's missing, and she's worried he may have been taken by the slavers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5052 => array(
			'name'    => "An Offering to Azura",
			'journal' => "Sergeant Oorga is afraid she will die in her next battle and asked me to deliver an offering to Azura for her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	61 => array(
			'name'    => "The Lost Courier",
			'journal' => "Roche suggested I hurry to find the courier, as the Spriggans are exceptionally dangerous.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6445 => array(
			'name'    => "J'saad's Stone",
			'journal' => "I found the body of the Khajiit J'saad along with a letter. I should return the letter to his beloved Adhimba in Black Heights.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2987 => array(
			'name'    => "Capture Brindle Lumbermill",
			'journal' => "I accepted a mission to help capture Brindle Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4206 => array(
			'name'    => "Nature's Accord",
			'journal' => "Inside a broken tower, I found a wounded woman named Imwyn Frost-Tree. She told me of a powerful symbol of her faith, the Sigil of Nature's Accord.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	62 => array(
			'name'    => "Glenumbra Conclusion",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6780 => array(
			'name'    => "The Long Game",
			'journal' => "Marcelle Dantaine asked me to help break her innocent husband Valentyn out of the Amenos Station stockade before they finish processing him and banish him into the wilds of Amenos's jungle.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3322 => array(
			'name'    => "The Dresan Index",
			'journal' => "The spirit of Richard Dusant said I should take the Dresan Index to someone in the Mages Guild.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5833 => array(
			'name'    => "Dark Anchors in Glenumbra",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Glenumbra.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	67 => array(
			'name'    => "Ear to the Ground",
			'journal' => "Baron Winoc tells me that Camlorn is in an uneasy state of truce between his Hands of Northmoor and the Magis Order. He is unsure where public opinion lies, and is too high-profile to ask around himself. He's asked me to ask around for him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2822 => array(
			'name'    => "Scout Warden Lumbermill",
			'journal' => "I accepted a mission to scout Warden Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2797 => array(
			'name'    => "Scout Blue Road Mine",
			'journal' => "I accepted a mission to scout Blue Road Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6845 => array(
			'name'    => "An Eye for an Eye",
			'journal' => "Razum-dar asked for my help with a mission to stop a pirate captain named Redblade from raising an armada.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1079 => array(
			'name'    => "Join the Tavern Brawl",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5118 => array(
			'name'    => "The Reason We Fight",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5924 => array(
			'name'    => "Relics of Yasammidan",
			'journal' => "Numani-Rasi of the Urshilaku Ashlander tribe has requested that I explore the ruins of the Daedric shrine Yasammidan and gather as many historic relics as I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	418 => array(
			'name'    => "Choice Quest",
			'journal' => "Disheveled NPC asked me to find a magical tome and bring it back to him. He says when I find it, I should quickly deliver it to him - there's no telling the evils abound...",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4345 => array(
			'name'    => "The Veil Falls",
			'journal' => "I'm now a full member of the Veiled Heritance. I've been told of a grand hall where their leadership council meets. And they are meeting with the Veiled Queen herself!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3133 => array(
			'name'    => "Capture Alessia Mine",
			'journal' => "I accepted a mission to help capture Alessia Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2655 => array(
			'name'    => "The Elder Scroll of Altadoon",
			'journal' => "The Elder Scroll of Altadoon has been stolen by the enemy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4370 => array(
			'name'    => "A Bargain With Shadows",
			'journal' => "To save the Mages Guild members that were forced to compete in the Observer's game, I decided to side with Kinlady Estre. Now I just need to find a way to win the game.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3852 => array(
			'name'    => "Rescue and Revenge",
			'journal' => "The Forlorn One asked me to use the decanter of holy water to rescue two people trapped by the fires.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	73 => array(
			'name'    => "Incidental Damage",
			'journal' => "The lumberjacks are fine for now, but many of their tools were damaged.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3819 => array(
			'name'    => "Healing Hearts",
			'journal' => "Acolyte Eldri collected Dremora hearts for Healer Dalen. He can turn the hearts into healing potions and wanted them to help the troops at Eidolon's Hollow. Unfortunately, Dalen departed before Eldri could give him the hearts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7115 => array(
			'name'    => "Ruinous Evaluation",
			'journal' => "Commandant Salerius hired me to enter the ruins of Nonungalo and determine the structural stability of the ruin, look for signs of Ayleid hauntings, and locate any dangers. He gave me a journal to record my findings.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3584 => array(
			'name'    => "The Coral Heart",
			'journal' => "Captain Noris came to Vivec's Antlers to defend against an incursion by Alexandra Conele of the Covenant. Alexandra wants to retrieve an artifact known as the Coral Heart from Vivec's Antlers. The Pact is barely holding its ground.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	142 => array(
			'name'    => "Peering Into Darkness",
			'journal' => "There appears to be more to the Bloody Eye then it seems. Their leader keeps speaking of the battle against the flesh and the spirit, as if two different sources of a fight. I must find out what their true purpose is in order to get their assistance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7051 => array(
			'name'    => "Akacirn the Deathless",
			'journal' => "Cipher Plautis believes that an elder cipher named Akacirn lured the missing ciphers to their deaths as part of some dark rite. She asked me to find Akacirn and put a stop to his plans.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	78 => array(
			'name'    => "Warn Sebastien",
			'journal' => "Lord Drugo has requested that I travel to Camlorn to warn Lord Sebastien of a possible assassination in his court.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	657 => array(
			'name'    => "Fadeel's Freedom",
			'journal' => "I found the last wishes of someone named Handre. He was going to use a gem he stole from the bandits to buy Fadeel's freedom.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3026 => array(
			'name'    => "The Wyrd Sisters",
			'journal' => "<<1>> discovered that Reach magic has somehow corrupted the forest and silenced the elemental guardians.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2809 => array(
			'name'    => "Scout Dragonclaw Farm",
			'journal' => "I accepted a mission to scout Dragonclaw Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6552 => array(
			'name'    => "The End of Eternity",
			'journal' => "Verandis believes Rada al-Saran has set his final plan in motion. He seeks my help to put a stop to the Gray Host once and for all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2834 => array(
			'name'    => "Scout Alessia Lumbermill",
			'journal' => "I accepted a mission to scout Alessia Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6729 => array(
			'name'    => "Guidance for Guides",
			'journal' => "A Wood Elf writer named Calonir needs help conducting research for his travel brochure. If I can provide some information on the hazards and highlights of Vvardenfell, Summerset, and lands beyond, I can earn a bit of gold for my trouble.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5388 => array(
			'name'    => "Clothier Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Clothier goods to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	143 => array(
			'name'    => "Embracing the Darkness",
			'journal' => "I have simply sampled a taste of the spirit world known as the Veil as it seems I would be set to return on a journey into the darkness. Before I enter this realm, I must find what I must do in order to succeed in my goals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2504 => array(
			'name'    => "Azura's Relics",
			'journal' => "Supernal Dreamers attacked Azura's shrine at Weeping Giant, killed the Spirit Wardens, and stole many relics. The young novice, Sister Tabakah, asked me to help her recover them and look for a captured warden.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6343 => array(
			'name'    => "Lurcher Demonstration",
			'journal' => "Zahari asked me to help train new recruits for the Khajiit Defense Legion to fight enemies they may not have encountered in training.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2745 => array(
			'name'    => "Scout Aleswell Lumbermill",
			'journal' => "I accepted a mission to scout Aleswell Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3950 => array(
			'name'    => "Kill the Boss!",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5291 => array(
			'name'    => "Pledge: Direfrost Keep",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to explore the frosty ruin of Direfrost Keep, which lies flush against the mountains of Eastmarch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3797 => array(
			'name'    => "Plague Bringer",
			'journal' => "The Llodos plague has been unleashed and it's spreading through the gorge. Despite this, some Hlaalu guards are searching for the Maulborn alchemist.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	79 => array(
			'name'    => "Murder In Camlorn",
			'journal' => "There's been a murder in Camlorn. Someone named Ranser was killed, and because a neutral party is hard to find, they've asked me to investigate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6376 => array(
			'name'    => "Goutfang Pariah",
			'journal' => "A Goutfang master fled the adeptorium after stealing scrolls and earning the title of pariah. I agreed to help kill the master and return these scrolls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4527 => array(
			'name'    => "Securing Knowledge",
			'journal' => "<<1>> is worried about Cropsford's books being destroyed in a Goblin attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2921 => array(
			'name'    => "Capture Chalman Keep",
			'journal' => "I accepted a mission to help capture Chalman Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6706 => array(
			'name'    => "Mettle and Stone",
			'journal' => "A Dark Elf, Selveni Andalor, asked me to help him free the trapped spirits of his grandmother, Sovisa, and an Ordinator, Nilera, from the False Martyrs' Folly in the Sever.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3973 => array(
			'name'    => "Will of the Council",
			'journal' => "The Fighters Guild has sentenced <<1>> to death for the murder of <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7065 => array(
			'name'    => "Replication Elimination",
			'journal' => "There are more copies of Tho'at Replicanum creating chaos inside the Infinite Archive. Master Malkhest needs help eliminating the maligraphies and the copies of Tho'at Replicanum.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	730 => array(
			'name'    => "Blightbane",
			'journal' => "The Quarantine Master in Davon's Watch wants me to discover the fate of Fort Arand, which was recently engulfed in a Blightstorm.\n\nHe says this will prove that I am immune to the Blight, if I returned uninfected.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6423 => array(
			'name'    => "Dust Smote",
			'journal' => "A territorial guardian watches over a temple of Meridia in Southern Elsweyr. I agreed to help kill the guardian and gather some temple dust.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3806 => array(
			'name'    => "Undermined",
			'journal' => "The renegade Ashlanders have broken through a storage tunnel beneath Iliath Temple, taking the monks and their defenders by surprise. If the attack can't be repelled, the monastery is sure to fall.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	80 => array(
			'name'    => "Failing Crops",
			'journal' => "Tarrant Circe, a local farmer, is having a bad year. His crops are failing and he can't figure out why. He thinks he may be the victim of sabotage from a neighboring farmer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5041 => array(
			'name'    => "To Aid Davon's Watch",
			'journal' => "I met an Argonian named Onuja at Othrenis. He asked me to aid Davon's Watch in their battle against the Daggerfall Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3443 => array(
			'name'    => "Capture BalfieraMine",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3296 => array(
			'name'    => "Trials of the Hero",
			'journal' => "The spirits of four legendary Redguards stand around the central shrine atop HoonDing's Watch. Each poses a challenging choice based on a difficult decision they once had to make.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6348 => array(
			'name'    => "In Defense of Elsweyr",
			'journal' => "I found a notice urging me to take up arms in defense of Northern Elsweyr.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	387 => array(
			'name'    => "Breaking Bones",
			'journal' => "The Ashen Order have asked me to break the will of the undead skeletons infesting Silvermist Castle. Their camp is just inside the Silvermist Castle entrance, but they are unable to move further forward.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	100 => array(
			'name'    => "Shornhelm",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5383 => array(
			'name'    => "The Deal's Not Dead",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2694 => array(
			'name'    => "Scout Ash Mine",
			'journal' => "I accepted a mission to scout Ash Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2958 => array(
			'name'    => "Capture Dragonclaw Farm",
			'journal' => "I accepted a mission to help capture Dragonclaw Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4637 => array(
			'name'    => "The Message",
			'journal' => "I've agreed to deliver a letter for a mysterious Imperial named <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	412 => array(
			'name'    => "Lost and Found",
			'journal' => "Find Quest NPC's midget.  Apparently he likes to go and play with a 'puppy' somewhere in the zone.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3223 => array(
			'name'    => "Capture Drakelowe Keep",
			'journal' => "I accepted a mission to help capture Drakelowe Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5584 => array(
			'name'    => "The Covetous Countess",
			'journal' => "A wealthy patron of the Thieves Guild takes a peculiar pleasure in owning stolen goods. Her tastes are mercurial, but her payments are like Dwarven clockwork.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2677 => array(
			'name'    => "Scout Fort Dragonclaw",
			'journal' => "I accepted a mission to scout Fort Dragonclaw for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5761 => array(
			'name'    => "Elemental Army",
			'journal' => "The Celestial Thief appeared to me and asked for my help discovering why the atronachs that were summoned to destroy the Dark Anchors turned against the mages of Craglorn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	123 => array(
			'name'    => "Find Werewolf Evidence",
			'journal' => "Dr. Holcombe has asked me to find clues to the source of the werewolf attack on Northmoor. I should return to him once I have gathered enough information.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3385 => array(
			'name'    => "Undying Loyalty",
			'journal' => "I met a sad looking dog beside a path in the Alik'r Desert. She seems to want my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5242 => array(
			'name'    => "Q5242 Example Theater Quest",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	198 => array(
			'name'    => "Rallying Cries of Northmoor",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2724 => array(
			'name'    => "Scout Ash Farm",
			'journal' => "I accepted a mission to scout Ash Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	134 => array(
			'name'    => "Rallying for the Next Wave",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3011 => array(
			'name'    => "Swine Thief",
			'journal' => "Glutton, the prized pig that belongs to the Daggerfall swineherd Wickton, is missing. Wickton suspects pig thieves, or at least one particular pig thief.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2660 => array(
			'name'    => "The Elder Scroll of Ni-Mohk",
			'journal' => "Grand Warlord Sorcalin needs volunteers to capture the <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5537 => array(
			'name'    => "His Greatest Treasure",
			'journal' => "<<1>> deciphered her father's ledger. She asked for my help with what she uncovered.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1489 => array(
			'name'    => "Waylaid Wine Merchant",
			'journal' => "A wine merchant named <<1>> was robbed on the road to Ebonheart. Now he's deep in his cups, and he wants help getting his shipment back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6100 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3085 => array(
			'name'    => "Capture Fort Glademist",
			'journal' => "I accepted a mission to help capture Fort Glademist for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5406 => array(
			'name'    => "Enchanter Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Enchanter glyphs to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3932 => array(
			'name'    => "Stolen Dream Crystal",
			'journal' => "Julian Kalere told me that Supernal Dreamers have stolen his Crystal of Power and shattered it into five shards. They plan to use the shards as foci for Daedric summoning rituals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	153 => array(
			'name'    => "On the Tail of a Wolf",
			'journal' => "Caldrun has tasked me with destroying the abomination that is Gargast. I must track him down and rid the world of him",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5514 => array(
			'name'    => "Getting a Bellyful",
			'journal' => "An Orcish animal handler named Arushna is desperate to get her hands on special durzog feed that can only be found in the Kennel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	523 => array(
			'name'    => "The Blood-Cursed Town",
			'journal' => "The town of Crestshade has been subjected to a blood-curse that turned the townsfolk feral. While some refugees were able to escape to the south, the mayor remained behind to aid any survivors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2900 => array(
			'name'    => "Scout Blue Road Lumbermill",
			'journal' => "I accepted a mission to scout Blue Road Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	157 => array(
			'name'    => "Trouble in Grayborne",
			'journal' => "Brennoc has asked me to speak to Inquisitor Roche about a troublesome villager.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2675 => array(
			'name'    => "Scout Fort Ash",
			'journal' => "I accepted a mission to scout Fort Ash for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	701 => array(
			'name'    => "Queen's Emissary - Isque",
			'journal' => "I visited all the High Kinlords and Kinladies of Summerset, seeking their support for Queen Ayrenn's invasion of Tamriel. This approval was symbolized in the forms of Batons of Fidelity, which I entrusted to Canonreeve Lefalto's safekeeping.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2789 => array(
			'name'    => "Scout Ash Mine",
			'journal' => "I accepted a mission to scout Ash Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4540 => array(
			'name'    => "Timberscar Troubles",
			'journal' => "Ufgra is worried about a tribe of Goblins that has moved into Timberscar Hollow.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2942 => array(
			'name'    => "Capture Farragut Mine",
			'journal' => "I accepted a mission to help capture Farragut Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3657 => array(
			'name'    => "Carving Cuttle",
			'journal' => "Orona, one of the Deepcrag miners, mentioned that killing the kwama and letting them rot is a waste. She'd much rather harvest their cuttle, which apparently has a lot of profitable uses.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7086 => array(
			'name'    => "Ayleid Altercations",
			'journal' => "The noted explorer Narsis Dren is seeking an Ayleid treasure from the ruins of Wendir. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	235 => array(
			'name'    => "Binding the Grave",
			'journal' => "Cleric Sellan has armed me with a blessed chain with which to bind the evil in Silvermist Castle. He believes the source is a crypt in the graveyard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4717 => array(
			'name'    => "Prisoners of the Sphinx",
			'journal' => "Within Fort Sphinxmoth I encountered one of the chapel guards. She believes more of her companions were still alive elsewhere in the complex.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3416 => array(
			'name'    => "Signals of Dominion",
			'journal' => "Aldmeri Dominion forces have captured Farwatch Tower. They're using the tower to signal their fleet at sea.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4476 => array(
			'name'    => "Tip of the Spearhead",
			'journal' => "Now that I've recruited at least one of the people Kaleen wanted, we can pull off her heist whenever I'm ready.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2878 => array(
			'name'    => "Scout Glademist Farm",
			'journal' => "I accepted a mission to scout Glademist Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3593 => array(
			'name'    => "Intruders in Deshaan",
			'journal' => "Dark Elves have been disappearing along the road to Narsis, near the western mountains. An incursion of Orcs may be responsible. I offered a Dark Elf scout my assistance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7022 => array(
			'name'    => "Paths Unwalked",
			'journal' => "In his downtime Azandar continued his research into the frayed fates spun off by the Fateweaver Key experiment. He'd like my assistance to smooth his fateline and continue the experiment.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	171 => array(
			'name'    => "Artifacts of the Mundus",
			'journal' => "I've agreed to help Oron, High King Fahara'jad's emissary, prove King Sebastien is innocent of Ranser's murder. I'm to work with High Priest Canning of the Order of Arkay to summon Ranser's spirit, so Ranser's shade can name his killer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4653 => array(
			'name'    => "Stonefire Machinations",
			'journal' => "According to Centurion Gavo's confession, the Stonefire cult plans to summon a Dark Anchor within Arenthia.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3120 => array(
			'name'    => "Capture Warden Mine",
			'journal' => "I accepted a mission to help capture Warden Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	702 => array(
			'name'    => "Queen's Emissary - Orrery",
			'journal' => "I visited all the High Kinlords and Kinladies of Summerset, seeking their support for Queen Ayrenn's invasion of Tamriel. This approval was symbolized in the forms of Batons of Fidelity, which I entrusted to Canonreeve Lefalto's safekeeping.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2941 => array(
			'name'    => "Capture Kingscrest Mine",
			'journal' => "I accepted a mission to help capture Kingscrest Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	467 => array(
			'name'    => "Sir Hughes' Fate",
			'journal' => "After murdering Duchess Lakana, Sir Hughes fled to Firebrand Keep. I've come to confront him and if necessary, bring him to justice for his crimes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2710 => array(
			'name'    => "Scout Bloodmayne Mine",
			'journal' => "I accepted a mission to scout Bloodmayne Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4949 => array(
			'name'    => "Favor for the Queen",
			'journal' => "Queen Maraya has asked a personal favor. She's heard disturbing rumors about some of her father's enemies returning, so she's asked me to go to Sentinel in the Alik'r Desert and ensure that both the kingdom and her father are safe.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	180 => array(
			'name'    => "A Ritual in Camlorn",
			'journal' => "High Priest Canning is almost ready to complete the ceremony to summon Ranser's spirit. I must be ready for whatever comes next.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4654 => array(
			'name'    => "An Unusual Circumstance",
			'journal' => "I decided to ask Tzik'nith for help getting into the tower. Before she shows me the secret, however, she wants me to help her against the Wood Elves.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7439 => array(
			'name'    => "Blunt Steel",
			'journal' => "I discovered a missive requesting that I sabotage weapon stores across Sorrow's Friend. I should seek them out and blunt each of the stored weapons. Doing so may give us an edge against the district's challenges.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2774 => array(
			'name'    => "Scout Arrius Keep",
			'journal' => "I accepted a mission to scout Arrius Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5013 => array(
			'name'    => "Hushed Whispers",
			'journal' => "I've found a scroll bearing marks from the Aldmeri Dominion. It details a series of unsolved murders in <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6786 => array(
			'name'    => "Cold Blood, Old Pain",
			'journal' => "Ember says that her friend Jhaka might have a lead about the mercenaries that took her friends all those years ago. He's in Marbruk right now, and Ember wants to go there to meet him and see what we can do to help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1339 => array(
			'name'    => "A Predator's Heart",
			'journal' => "Knarstygg and Dro-Dara have decided it would be wise to go to Wayrest until the bandits clear out of the plantation. Knarstygg is concerned that Murk-Watcher might not want to leave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4929 => array(
			'name'    => "A Dagger to the Heart",
			'journal' => "Using the codebook I discovered, Adusa has determined that Montclair's nephew, Rohlbert, is behind the assassination plot. She believes that if we eliminate Rohlbert, we can end this part of the Montclair threat once and for all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	192 => array(
			'name'    => "Murder on the Docks",
			'journal' => "I found a body weighted with heavy rocks in the sea beneath Camlorn's docks. I've decided to investigate the murder.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5240 => array(
			'name'    => "Uncaged",
			'journal' => "Outside the ancient ruins of Skyreach Pinnacle, I met a member of the Scaled Court. He told me that the Court accidentally released creatures from magical wards the Nedes set up long ago. He plans to reestablish the wards, but he needs help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	196 => array(
			'name'    => "The Third Squad",
			'journal' => "Captain Blanche Geves sent me to look for the Third Squad soldiers who were scattered after a vicious undead attack. They are near the Redoubt and may be pinned down, unable to escape.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2924 => array(
			'name'    => "Capture Farragut Keep",
			'journal' => "I accepted a mission to help capture Farragut Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7217 => array(
			'name'    => "The Wing of the Gryphon",
			'journal' => "I picked up the Fable of the Gryphon as my next step to restoring the Scholarium. I'll need to uncover its secrets to open the Gryphon door, meet the Luminary within, and amplify the power of the Scribing Altar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	197 => array(
			'name'    => "Rallying the Redoubt",
			'journal' => "Inquisitor Blarae has seen Morholdt as well - he has attacked the Redoubt many times. Also, the failed ritual in Camlorn caused the number of undead attacking the Redoubt to greatly increase. They threaten to overrun the Redoubt and destroy Camlorn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5490 => array(
			'name'    => "Knowledge is Power",
			'journal' => "The Drake of Blades wants me to recover a book that the Daedra refuse to burn and whose contents must therefore be very important.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3142 => array(
			'name'    => "Capture Ash Lumbermill",
			'journal' => "I accepted a mission to help capture Ash Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2784 => array(
			'name'    => "Scout Castle Bloodmayne",
			'journal' => "I accepted a mission to scout Castle Bloodmayne for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	210 => array(
			'name'    => "Cleansing the Lake",
			'journal' => "Thetis has finished mixing a remedy he believes will clean out the lake.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3211 => array(
			'name'    => "Capture Bloodmayne Farm",
			'journal' => "I accepted a mission to help capture Bloodmayne Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6319 => array(
			'name'    => "The Song of Kingdoms",
			'journal' => "Iraya and Izanji, two siblings in charge of a museum in Riverhold called the House of Histories, have fallen on hard times after a recent theft. I intend to help them solve this theft and return the missing items.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4584 => array(
			'name'    => "Pilfered Urn",
			'journal' => "I found an exquisitely carved urn within Old Sord's Cave. It appears to belong to <<1>> of Windhelm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	617 => array(
			'name'    => "Meeting of the Minds",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7452 => array(
			'name'    => "Repeatable Quest Tracker",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	231 => array(
			'name'    => "Quelling the Dead",
			'journal' => "The Ashen Order (a.k.a. the Gravebinders) have asked me to help cull the horde of undead infesting Silvermist Castle. Their camp is just inside the Silvermist Castle entrance, but they are unable to move further forward.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2970 => array(
			'name'    => "Capture Bloodmayne Farm",
			'journal' => "I accepted a mission to help capture Bloodmayne Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4697 => array(
			'name'    => "To Rawl'kha",
			'journal' => "Clan Mother Kazirra told me to travel to the city of Rawl'kha, to bring word of the Mane's change of heart.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4303 => array(
			'name'    => "Lighting the Shadows",
			'journal' => "The Spider Cult has come to Fungal Grotto. Members of the Fighters Guild, led by Defender Two-Blades, are determined to stop the Cult from raising an army of shadows that will destabilize the region.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2697 => array(
			'name'    => "Scout Aleswell Mine",
			'journal' => "I accepted a mission to scout Aleswell Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4448 => array(
			'name'    => "Kalodar's Farewell",
			'journal' => "I met a Nord woman who's searching for her father in the northern hills of the Rift.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2850 => array(
			'name'    => "Scout Blue Road Keep",
			'journal' => "I accepted a mission to scout Blue Road Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	304 => array(
			'name'    => "The Thaw After the Frieze",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4030 => array(
			'name'    => "Shrine of Corruption",
			'journal' => "I met two Nord soldiers near the shrine of Kynesgrove. They claim that the shrine has been taken over by evil witches and that the rest of their brigade never returned from the shrine. Now they just wait, afraid to enter the shrine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	328 => array(
			'name'    => "Serpent Slayer",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7275 => array(
			'name'    => "Protecting the Besieged",
			'journal' => "<<1>> asked me to help patrol and defend the Stirk Fellowship siege camps located along the Writhing Wall from Worm Cult and Daedric attacks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5556 => array(
			'name'    => "A Flawless Plan",
			'journal' => "<<1>> wants to know what really happened to <<2>>'s dowry. She has a plan to uncover the truth of what happened during the guild's failed heist.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3050 => array(
			'name'    => "Cursed Treasure",
			'journal' => "The Bloodthorn cult is attempting to take control of the spirits of High Rock knights laid to rest at the Burial Mounds. They also seek to control the ancient kings buried there, and have stolen their regalia for some dark purpose.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6358 => array(
			'name'    => "Unhealthy Preoccupation",
			'journal' => "A follower of Peryite, the Daedric Prince of Pestilence, has asked me to rescue his friend from the flu-ridden city of Orcrest.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3855 => array(
			'name'    => "Mystery of Othrenis",
			'journal' => "<<1>> believes the spirit of a long-dead mage may know the secret to imprisoning Balreth again. An Argonian scout named <<2>> has already left for Othrenis.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	405 => array(
			'name'    => "Vestrial's Research Book",
			'journal' => "Vestrial has asked you to recover the missing pages of his research book.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3422 => array(
			'name'    => "Capture the Farm",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6514 => array(
			'name'    => "The Antiquarian Circle",
			'journal' => "An order of scholars known as the Antiquarian Circle has gathered in Solitude, intent on discovering historical relics all over Tamriel. Joining them could be an adventure worth undertaking.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3269 => array(
			'name'    => "Capture Bloodmayne Mine",
			'journal' => "I accepted a mission to help capture Bloodmayne Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	703 => array(
			'name'    => "Queen's Emissary - Dusk",
			'journal' => "I visited all the High Kinlords and Kinladies of Summerset, seeking their support for Queen Ayrenn's invasion of Tamriel. This approval was symbolized in the forms of Batons of Fidelity, which I entrusted to Canonreeve Lefalto's safekeeping.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3064 => array(
			'name'    => "Rally Cry",
			'journal' => "The city of Camlorn has fallen to Faolchu and his werewolf legion. The Lion Guard struggles to retake the city, but the odds are against them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4145 => array(
			'name'    => "Mine All Mine",
			'journal' => "Tervur Sadri, the co-owner of the Darkshade mine, wants me to help him find his partner, Selos, and get rid of the thugs who've displaced his miners.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3141 => array(
			'name'    => "Capture Glademist Lumbermill",
			'journal' => "I accepted a mission to help capture Glademist Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	575 => array(
			'name'    => "Vaermina's Gambit",
			'journal' => "Vaermina's ultimate plan has been revealed. Her consort, the Night Terror, has seized High King Emeric's mind and trapped him in a nightmare. I must face the Night Terror and save the king.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2936 => array(
			'name'    => "Capture Ash Mine",
			'journal' => "I accepted a mission to help capture Ash Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6233 => array(
			'name'    => "Grave Circumstances",
			'journal' => "Bolu of the Dead-Water tribe, which calls itself the Naga-Kur, wants me to go to Bok-Xul and accomplish the task he set before me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5916 => array(
			'name'    => "The Anxious Apprentice",
			'journal' => "Dubdil Alar's apprentice has hired me to stop one of her former master's experiments. I'm also required to gather his research notes for further study.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	410 => array(
			'name'    => "A Royal Summons",
			'journal' => "The Numinous are to report to Queen Ayrenn for a Royal Fete. Proceed to the Immaculate Tabernacle in the Turquoise district.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2791 => array(
			'name'    => "Scout Dragonclaw Mine",
			'journal' => "I accepted a mission to scout Dragonclaw Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	422 => array(
			'name'    => "The Largest Threats",
			'journal' => "Commander Lafavre has asked you to kill the bosses controlling the Halls of the Damned.\n- Nazgol the Undying\n- General Ripfang\n- Bone Colossus\n- Orutu\n- Anarose Direblood\n- Therion Direblood",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2787 => array(
			'name'    => "Scout Rayles Mine",
			'journal' => "I accepted a mission to scout Rayles Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2820 => array(
			'name'    => "Scout Black Boot Farm",
			'journal' => "I accepted a mission to scout Black Boot Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4649 => array(
			'name'    => "The Sorcerer Division",
			'journal' => "I am competing in the Sorcerer Division of Thizzrini Arena. I must fight through the ranks in order to fight Malarel, the Sorcerer Division Champion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	424 => array(
			'name'    => "In Crucial Need",
			'journal' => "High Mage Dilborn has asked me to find him some Nightmare Dust. He says it drops from the spectres that roam the lowest depths of the keep.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5524 => array(
			'name'    => "Reeking of Foul Play",
			'journal' => "Riekrs have been raiding caravans throughout Wrothgar, plundering cargo and taking travelers prisoners. The Riekrs, led by King-Chief Edu, must be dealt with.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3018 => array(
			'name'    => "Lineage of Tooth and Claw",
			'journal' => "The Duke told me that Gloria has a plan to learn more about Faolchu and how to defeat him. She wants me to capture a living werewolf for the ritual she needs to perform.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3527 => array(
			'name'    => "Sample Escort",
			'journal' => "Sample Escort",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4220 => array(
			'name'    => "The Mallari-Mora",
			'journal' => "I discovered a Mages Guild expedition at the ruins of Ezduiin. A scholar named Telenger is in charge.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4571 => array(
			'name'    => "Fang Collector",
			'journal' => "I've found the body of a hunter in the wilds. Nearby was a notice he dropped detailing a bounty on senche-tiger fangs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2981 => array(
			'name'    => "Capture Farragut Lumbermill",
			'journal' => "I accepted a mission to help capture Farragut Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7304 => array(
			'name'    => "The Roughest Pearl",
			'journal' => "I found a message in a bottle near a pile of volcanic pearls. The message is from a ship's captain in Shor's Stand who seems desperate to get the volcanic pearls back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5455 => array(
			'name'    => "Q5455 Orsinium Stateful",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3849 => array(
			'name'    => "A Final Release",
			'journal' => "<<1>> believes his mother's remains lie in <<2>>, away from the swamp waters. Her spirit is unable to return to the Hist.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6766 => array(
			'name'    => "The Ascendant Storm",
			'journal' => "The Ascendant Magus stands revealed as Count Damard Dufort. He intends to lead the Ascendant Order in an attack on All Flags Islet. Lady Arabelle needs me to help save the alliance leaders-and the hope for peace.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	427 => array(
			'name'    => "Blackmoore's End",
			'journal' => "My long pursuit of Baron Blackmoore ends here, in his own keep. His crimes will not go unpunished, and his death will be a message to all. Time for me to hunt him down in his own home.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7191 => array(
			'name'    => "For Cake's Sake",
			'journal' => "Mogh, Chef Donolon's son and apprentice, needs my help to embarrass his father's rival bakers and recover the ingredients he needs to make his Jubilee Cake.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	669 => array(
			'name'    => "Caught in the Web",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4572 => array(
			'name'    => "Requests for Aid",
			'journal' => "<<1>> asked me to leave supplies at a cache for messengers who're going for help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2974 => array(
			'name'    => "Capture Glademist Lumbermill",
			'journal' => "I accepted a mission to help capture Glademist Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6243 => array(
			'name'    => "The Swamp and the Serpent",
			'journal' => "Kassandra betrayed us-she claimed the sap-speaker's staff and set out after the Remnant on her own. I need to reach her before she takes the Remnant for herself.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3817 => array(
			'name'    => "The Seal of Three",
			'journal' => "A large Maulborn force used the sewers to sneak into Mournhold. I need to find them before they can complete their nefarious plan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	428 => array(
			'name'    => "Exposing the Baron",
			'journal' => "While I pursue the Baron through his keep, I need to keep watch for evidence of his dark magic.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4902 => array(
			'name'    => "Shornhelm Divided",
			'journal' => "Shornhelm, the largest city in Rivenspire, is a city divided. House Montclair has annexed part of the city. Darien Gautier suggested I go to Shornhelm and help Countess Tamrith and Baron Dorell deal with the situation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3280 => array(
			'name'    => "Imperial Infiltration",
			'journal' => "The city of Evermore is in dire straits. King Eamond is dead and Reachmen are bearing down on the city from the north. Duke Renchant plans to hand the city over to Imperial control, but I have agreed to work with Queen Arzhela to stop him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	499 => array(
			'name'    => "Pursuing the Shard",
			'journal' => "King Emeric believes Count Hosni has the Dream Shard stolen from Pariah Abbey. He wants me to ingratiate myself with the count in hopes of retrieving the shard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2742 => array(
			'name'    => "Scout Rayles Lumbermill",
			'journal' => "I accepted a mission to scout Rayles Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4981 => array(
			'name'    => "Conflicted Emotions",
			'journal' => "<<1>> of the Dragonstar Caravan Company asked for my assistance in fulfilling a contract for a mage in the Silaseli ruins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3610 => array(
			'name'    => "For Their Own Protection",
			'journal' => "The threat of the Llodos plague has allowed paranoia to run rampant. Many people in Narsis claim that friends and family members have vanished in the night.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1383 => array(
			'name'    => "The Perfect Burial",
			'journal' => "<<1>> had a Bosmer wife named Breloth who died recently. He's asked me to help him get things to give her a befitting burial.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3216 => array(
			'name'    => "Capture Fort Aleswell",
			'journal' => "I accepted a mission to help recapture Fort Aleswell for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	435 => array(
			'name'    => "Treasure Beyond Wonder",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2678 => array(
			'name'    => "Scout Chalman Keep",
			'journal' => "I accepted a mission to scout Chalman Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4917 => array(
			'name'    => "The Blacksap's Hold",
			'journal' => "The Vinedusk Rangers want me to help quash the remnants of the Blacksap Rebellion, the Wood Elf civil war that ended with the Dominion's formation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5629 => array(
			'name'    => "Contract: Rivenspire",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2935 => array(
			'name'    => "Capture Glademist Mine",
			'journal' => "I accepted a mission to help capture Glademist Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4784 => array(
			'name'    => "Swift Justice",
			'journal' => "Sorderion has charged me with apprehending Aldolin, a traitor to the Dominion who has been giving aid to the Ebonheart Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6306 => array(
			'name'    => "The Halls of Colossus",
			'journal' => "I helped Abnur Tharn decode the Wrathstone tablet and determine the location of the hidden weapon. Now we need to travel to the Halls of Colossus and find it before his half-sister, Euraxia. Khamira has decided to go with us.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3620 => array(
			'name'    => "The Ravaged Village",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4449 => array(
			'name'    => "Carzog's Demise",
			'journal' => "My investigations of the Bloodthorn Cult's activities have convinced Chief Tazgol that they threaten the Orcs on Betnikh.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	736 => array(
			'name'    => "The Flame of Dissent",
			'journal' => "The Knights of the Flame are divided over whether to attack the Redguard army at Alcaire Castle. Sir Croix and other knights would potentially destroy the Covenant. They need to be stopped. I've agreed to rally knights to Sir Edmund's cause.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5206 => array(
			'name'    => "Rest in Peace",
			'journal' => "The Keeper has an intriguing contract that he feels I am just right for.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	465 => array(
			'name'    => "The Blood-Splattered Shield",
			'journal' => "Captain Janeve, the shield of the Shornhelm Guard and Countess Tamrith's younger sister, has established a camp for refugees displaced by the vampiric menace.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3243 => array(
			'name'    => "Capture Drakelowe Lumbermill",
			'journal' => "I accepted a mission to help capture Drakelowe Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6223 => array(
			'name'    => "Morrowind Gate Reclaimation",
			'journal' => "I accepted a mission to help reclaim Farragut Keep, Arrius Keep, and Kingscrest Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5845 => array(
			'name'    => "Castle Charm Challenge",
			'journal' => "<<1>>, a New Life Herald in Eastmarch, spoke of <<2>>'s New Life Festival in Stormhaven.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4488 => array(
			'name'    => "Gentle Gardener",
			'journal' => "I met a Bosmer named Ongalion at the Falinesti Autumn Site. He's a tree-steward, helping to tend the site while the city is away.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	521 => array(
			'name'    => "Azura's Aid",
			'journal' => "Vaermina has taken control of King Emeric's dreams and Abbot Durak says we need Azura's guidance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	614 => array(
			'name'    => "A Look in the Mirror",
			'journal' => "<<1>> met me on the road into Wind Keep and asked me to help the countess. She's acting strangely and it's put the entire town on edge.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4332 => array(
			'name'    => "Prisoner Dilemma",
			'journal' => "Three Pact soldiers were kidnapped in the Lion's Den: Hilka, Hald, and Fjar. Their commander, Thragof, is certain one of them is a traitor. He has tasked me with identifying the traitor and freeing the others.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	973 => array(
			'name'    => "Weller's Counter Offer",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3453 => array(
			'name'    => "KIll an Aldmeri Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2608 => array(
			'name'    => "The Elder Scroll of Ghartok",
			'journal' => "The Elder Scroll of Ghartok is being held by the enemy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6417 => array(
			'name'    => "Pledge: Unhallowed Grave",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Unhallowed Grave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	728 => array(
			'name'    => "Repair Koeglin Lighthouse",
			'journal' => "The lighthouse south of Koeglin Village was attacked, and its signal fires were extinguished. Until someone repairs it, ship captains will have difficulty navigating the harbor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2975 => array(
			'name'    => "Capture Ash Lumbermill",
			'journal' => "I accepted a mission to help capture Ash Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4178 => array(
			'name'    => "Lost Companions",
			'journal' => "<<1>>, <<2>>, and <<3>> are missing and likely in trouble. Snorrvild has asked me to help find them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	737 => array(
			'name'    => "Retaking Firebrand Keep",
			'journal' => "Sir Edmund and the knights who have rallied to our cause are prepared to attack. We must retake Firebrand Keep and put an end to the plan that would threaten the Daggerfall Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3621 => array(
			'name'    => "Peril at the Pools",
			'journal' => "Forces of the Daggerfall Covenant are occupying the volcanic sulfur pools east of Heimlyn Keep. The Othrys family worked the site, but they've been run off their property.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3337 => array(
			'name'    => "Legitimate Interests",
			'journal' => "The Red Rook bandits have taken control of the town of Crosswych. Some of the townsfolk are trying to organize a resistance, but the bandits are determined to use the town as a base.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6254 => array(
			'name'    => "Death Among the Dead-Water",
			'journal' => "The Naga-Kur, or Dead-Water tribe, has sentenced Kishi, a member of the tribe, to death for interfering in the rites of passage and saving a few young Nagas from certain death. One of the survivors asked if I could help Kishi.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	939 => array(
			'name'    => "History of the Mad Architect",
			'journal' => "While searching for an exit to the Bone Vaults, I have uncovered a hidden library.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2804 => array(
			'name'    => "Scout Warden Farm",
			'journal' => "I accepted a mission to scout Warden Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4397 => array(
			'name'    => "The Enemy Within",
			'journal' => "I've helped track down the source of the poison in <<1>>, but the poisoner still remains to be found. I should work with <<2>> to bring the culprit to justice.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	966 => array(
			'name'    => "The Dwemer Ruins",
			'journal' => "I seem to have found some Dwemer ruins.  I should explore and see what I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3392 => array(
			'name'    => "Capture the Mine",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	974 => array(
			'name'    => "A Duke in Exile",
			'journal' => "Duke Sebastien of Camlorn has been kidnapped by Bloodthorn cultists.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2701 => array(
			'name'    => "Scout Kingscrest Mine",
			'journal' => "I accepted a mission to scout Kingscrest Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6572 => array(
			'name'    => "Strange Contamination",
			'journal' => "Animals in Gloomreach seem to have developed the ability to produce a strange fungal mold. The Reachfolk think magic might be at work. They want samples of the mold collected in order to study them further.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5865 => array(
			'name'    => "Culling the Swarm",
			'journal' => "The Missir-Dadalit Egg Mine has been overrun with kwama. I've been hired to clear out the infestation to help re-balance the colony. I'm also required to defeat a large kwama warrior who guards the queen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2854 => array(
			'name'    => "Scout Castle Roebeck",
			'journal' => "I accepted a mission to scout Castle Roebeck for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1058 => array(
			'name'    => "Spy Hunter",
			'journal' => "Kill the Red Rook Spy before he can report to his superior.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1070 => array(
			'name'    => "Killed Lumpy",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2831 => array(
			'name'    => "Scout Farragut Lumbermill",
			'journal' => "I accepted a mission to scout Farragut Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7451 => array(
			'name'    => "Express Delivery",
			'journal' => "I found a note left by some hungry explorers from my faction. They asked me to deliver food so that they don't have to take a break from their current mission.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2977 => array(
			'name'    => "Capture Dragonclaw Lumbermill",
			'journal' => "I accepted a mission to help capture Dragonclaw Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5220 => array(
			'name'    => "Kill Enemy Templars",
			'journal' => "Grand Warlord Sorcalin has authorized a reward for any Dominion soldier who slays 20 enemy Templars.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5909 => array(
			'name'    => "Writhing Sveeth Hunt",
			'journal' => "Huntmaster Sorim-Nakar of the Erabenimsun Ashlander tribe has given me a challenge. He wants me to hunt the deadly greater hive golem they call Writhing Sveeth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3670 => array(
			'name'    => "Desperate Souls",
			'journal' => "Rogue Dark Elves flaunting the Pact are enslaving Argonians at a mine near Ash Mountain. A powerful Daedra rebelled against the mine's master. Then the Daedra took most of the people at the mine into Oblivion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6762 => array(
			'name'    => "Buried at the Bay",
			'journal' => "I encountered a man named Ilyes Maul searching for his ancestor's lost treasure. Ilyes hired me to help him find it in Ghost Haven Bay.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4535 => array(
			'name'    => "The Dagger's Edge",
			'journal' => "Lord Diel of Daggerfall and his sons are investigating the recent harpy attacks all along the coast. They hired a group of mercenaries called the Daggers to help them with their research, but the Daggers never returned from the Shrieking Scar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1076 => array(
			'name'    => "The Battlebarge",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3454 => array(
			'name'    => "Kill Ten Enemy Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1085 => array(
			'name'    => "What Little Aid",
			'journal' => "Sister Alarvyne would like me to collect Mountain Tail to use in tea she makes to soothe the blighted.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6418 => array(
			'name'    => "Dreams of the Forsaken",
			'journal' => "A young Imperial named Catina recently lost her mother, Honoria. In the weeks that followed, images of the Forsaken Citadel plagued her dreams. Honoria clearly has unfinished business in the ruined academy, and she needs the living to help settle it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2968 => array(
			'name'    => "Capture Brindle Farm",
			'journal' => "I accepted a mission to help capture Brindle Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7112 => array(
			'name'    => "Trinkets from the Reach",
			'journal' => "Commandant Salerius tasked me to enter Varen's Watch and gather Reachfolk trinkets left there after battles with Varen Aquilarios's forces during his uprising to claim the Ruby Throne.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4751 => array(
			'name'    => "Warship Designs",
			'journal' => "A Breton shipbuilder named Jeromec Lemal has asked me to help him recover the warship designs from three ships under construction in the docks at Tava's Blessing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1804 => array(
			'name'    => "Sunken Knowledge",
			'journal' => "I've agreed to help the Hermaeus Mora cult of Primeval Seekers to prevent the Nereid of Halcyon Lake from using forbidden secrets to subvert the laws of nature.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1097 => array(
			'name'    => "Out of the Camp",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2828 => array(
			'name'    => "Scout Chalman Lumbermill",
			'journal' => "I accepted a mission to scout Chalman Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6253 => array(
			'name'    => "Antique Armor",
			'journal' => "Varo Hosidias of Cyrodilic Collections wants me to enter the Tsofeer Cavern and gather any pieces of Imperial armor I can find for the benefit of a benefactor named Tia Doran.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2795 => array(
			'name'    => "Scout Kingscrest Mine",
			'journal' => "I accepted a mission to scout Kingscrest Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4526 => array(
			'name'    => "Lost Bet",
			'journal' => "I've found a strange glowing artifact from the age of the Altmeri deep within the ruins of Bewan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5767 => array(
			'name'    => "The Blood of Nirn",
			'journal' => "The scholar Nendirume warned me that the Scaled Court has taken over the ruined city of Skyreach. She believes they're searching Skyreach Hold for more of the nirncrux element, which they refer to as \"the blood of Nirn.\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1145 => array(
			'name'    => "Cleansing Spirits",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2876 => array(
			'name'    => "Scout Warden Farm",
			'journal' => "I accepted a mission to scout Warden Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5912 => array(
			'name'    => "Old Stomper Hunt",
			'journal' => "Huntmaster Sorim-Nakar of the Erabenimsun Ashlander tribe has given me a challenge. He wants me to hunt the dangerous cliff strider they call Old Stomper.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4197 => array(
			'name'    => "Sounds of Alarm",
			'journal' => "A runner arrived from Fort Morvunskar, an installation to the west. Thane Mera believes Fildgor's forces have launched an attack against the fort.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2715 => array(
			'name'    => "Scout Farm 5",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6377 => array(
			'name'    => "Sword of the Serpent",
			'journal' => "The elderly job broker, Ri'hirr, urged me to defeat Sword Master Vhysradue-a peerless warrior who camps on the Hill of Shattered Swords.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6060 => array(
			'name'    => "Tasty Tongue Varnish",
			'journal' => "Brengolin, the cook in Slag Town, needs some assistance in creating some unconventional cuisine. This time he's asked me to procure him some Bosmer Root Liqueur.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3683 => array(
			'name'    => "What Lies Beneath",
			'journal' => "I volunteered to enter the ruined crypts beneath the fort to look for the retainer, Durel Gilveni, and the mysterious others mentioned in the General's dispatch. The entrance is by the river.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1165 => array(
			'name'    => "A Greater Threat than Vignette",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6579 => array(
			'name'    => "Pledge: The Cauldron",
			'journal' => "Urgarlag Chief-bane of the Undaunted has challenged me to explore the Cauldron.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2869 => array(
			'name'    => "Scout Drakelowe Mine",
			'journal' => "I accepted a mission to scout Drakelowe Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4726 => array(
			'name'    => "Siege Warfare",
			'journal' => "I've reached the siege range. Bugbesh told me to try each weapon. I should also talk to Marc and Jaleel to learn more about the weapons.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1216 => array(
			'name'    => "Sealed Spirits",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6790 => array(
			'name'    => "A Mother's Request",
			'journal' => "Isobel has been asked to return to Castle Navire to speak to Knight Commander Jourvel, Aurelia's mother.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4547 => array(
			'name'    => "Prove Your Worth",
			'journal' => "Garnikh bet his axe that I can't best the Seamount hunters in duels.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3642 => array(
			'name'    => "The Curse of Heimlyn Keep",
			'journal' => "Heimlyn Keep is under a terrible curse. A group of Covenant soldiers attacked, thinking they were assaulting a military target. Instead they loosed an ancient magic hidden beneath in the vaults.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1287 => array(
			'name'    => "Forbidden Worship - Old Ver.",
			'journal' => "Thorkan is worried that Normar's rumors of necromancy in Fell's Run will cause bloodshed between Orcs and Bretons. To keep the peace, he wants to stop to the necromantic rituals and put Normar's fears to rest.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6036 => array(
			'name'    => "Most Complicated Machine",
			'journal' => "Proctor Sovor Saryoni, a ranking Clockwork Apostle, is dying and has asked me to help save his life. He believes his only hope to continue his existence lies in an experiment conducted by a former apostle, but she was exiled long ago because of it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1294 => array(
			'name'    => "The Wizard's Tome",
			'journal' => "The wizard <<1>> in Davon's Watch told me his apprentice has stolen his spellbook. I offered to help find him, but there's a problem. To punish him for a previous theft, the wizard changed the warding rune on the book.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3144 => array(
			'name'    => "Capture Dragonclaw Lumbermill",
			'journal' => "I accepted a mission to help capture Dragonclaw Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1295 => array(
			'name'    => "Merchant Matters",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2965 => array(
			'name'    => "Capture Alessia Farm",
			'journal' => "I accepted a mission to help capture Alessia Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4822 => array(
			'name'    => "Mind of Madness",
			'journal' => "Gasteau Chamrond knows how to escape the structure I've fallen into. He'll help me get out if I lend him my strength.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3198 => array(
			'name'    => "Capture Aleswell Farm",
			'journal' => "I accepted a mission to help capture Aleswell Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1341 => array(
			'name'    => "Ogre Teeth",
			'journal' => "A glory-seeking Orc warrior asked me to help him gather ogre teeth for a trophy necklace from the battle. Why not?",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6162 => array(
			'name'    => "An Unexpected Betrayal",
			'journal' => "Renzir, a Khajiit merchant, was betrayed by his first mate Madja. He requested that I recover his trade shipments, kill Madja, and find out what happened to his wife, Nubani.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2712 => array(
			'name'    => "Scout Farm 2",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1346 => array(
			'name'    => "Ending the Ogre Threat",
			'journal' => "General Godrun is counting on me to finish this battle with the Ironhand Ogres. If I can collapse the cavern they're using to get through the mountain, that should solve the ogre problem for good.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2769 => array(
			'name'    => "Scout Fort Glademist",
			'journal' => "I accepted a mission to scout Fort Glademist for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6640 => array(
			'name'    => "Prankster's Carnival",
			'journal' => "I met an Argonian named <<1>> and offered to help her pull off a prank on an unsuspecting target.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2922 => array(
			'name'    => "Capture Arrius Keep",
			'journal' => "I accepted a mission to help capture Arrius Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4103 => array(
			'name'    => "Z'en and Mauloch",
			'journal' => "I found a journal written by a priest of Z'en, which details part of Bloodtoil Valley's history. The local spinner may find this information valuable if I can find the rest of the journals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3974 => array(
			'name'    => "Storming the Hall",
			'journal' => "The Reachmen attacked Shor's Stone and brought the giant Sinmur back to life. Now they've overrun Fallowstone Hall, an ancient seat of the Companions. Vigrod wants my help regaining the Hall.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3099 => array(
			'name'    => "Capture Castle Black Boot",
			'journal' => "I accepted a mission to help capture Castle Black Boot for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1380 => array(
			'name'    => "Divide and Conquer",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6155 => array(
			'name'    => "Pledge: Fang Lair",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Fang Lair.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4436 => array(
			'name'    => "Luck of the Albatross",
			'journal' => "<<1>> led a mutiny against the captain and crew of the Albatross as they searched <<Ac:2>> for ancient treasure.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3978 => array(
			'name'    => "Tomb Beneath the Mountain",
			'journal' => "Netapatuu and his colleagues came to Pinepeak Caverns to consult with the spirit of Hakra, one of Ysgramor's Companions. Worm Cultists attacked and drove them out. The cultists cannot be allowed to gain control of Hakra's spirit.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7158 => array(
			'name'    => "Honor Bound",
			'journal' => "Squire Albec asked for help finding Sir Cadwell's trusted steed, Honor. The bantam guar is loose somewhere on Solstice. The would-be squire believes Cadwell will officially accept him upon Honor's safe return.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3119 => array(
			'name'    => "Capture Bloodmayne Farm",
			'journal' => "I accepted a mission to help capture Bloodmayne Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1384 => array(
			'name'    => "Old Adventurers",
			'journal' => "The owners of the Dro-Dara Plantation have lost everything. First ogres attacked, and now bandits. The plantation was owned by a group of retired adventurers. The thief, Janne, asked me to rescue the others.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5906 => array(
			'name'    => "Siren's Song",
			'journal' => "A Gold Coast Trading Company ship was lured ashore by a powerful Nereid. I've been hired to defeat the Nereid and recover the cargo in the wreckage. She's protected by a group of sailors who she controls through a magical song.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1432 => array(
			'name'    => "Clean Up the Dead",
			'journal' => "Clear the graveyard of zombies, making sure you check the crypt.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4529 => array(
			'name'    => "Red Rook Resources",
			'journal' => "A note in Ilessan Tower has informed me that the Red Rooks are amassing resources for their allies. It also mentioned an attack on a manor north of Deleyn's Mill.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3955 => array(
			'name'    => "Tracking the Plague",
			'journal' => "The mysterious operative called Naryu has been investigating the evidence I discovered at the Maulborn camp.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1437 => array(
			'name'    => "General Godrun's Orders",
			'journal' => "Clan Murtag Orcs are fighting the Ironhand ogres in Shinji's Scarp. They've asked for my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4569 => array(
			'name'    => "Enemy Reinforcements",
			'journal' => "<<1>> has received word that more Dremora are entering the city from the north.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3357 => array(
			'name'    => "The Labyrinth",
			'journal' => "The final battle against Angof begins now. I need to get inside and navigate the labyrinth of catacombs to find a safe place for Gabrielle and the soldiers to teleport into.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6449 => array(
			'name'    => "Take Your Lumps",
			'journal' => "I found a lumpy sack abandoned in the wilderness. Its contents seem to be alive.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1477 => array(
			'name'    => "Waste of the Wood",
			'journal' => "Collect Wayward Branches for Alreal outside Elden Root.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4210 => array(
			'name'    => "Real Marines",
			'journal' => "While in Silsailen, I learned that the imposters have taken real Auridon Marines captive.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7390 => array(
			'name'    => "Death Hound's Devotion",
			'journal' => "I met a vampire hunter searching for her pet death hound, Bloodmite, who ran off after the pair were ambushed by a group of vampires. I agreed to help her find him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2839 => array(
			'name'    => "Scout Bloodmayne Lumbermill",
			'journal' => "I accepted a mission to scout Bloodmayne Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1485 => array(
			'name'    => "They Dragged Him Away",
			'journal' => "Adiel Charnis is distraught. Dreugh attacked her home in the Dreughside district, and her husband fought back. They wounded him and dragged him off toward their spawning pools. Adiel holds out hope her husband still lives.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3912 => array(
			'name'    => "Vigil's End",
			'journal' => "After picking up an ancient sword in <<1>>, I learned an Ayleid knight's spirit haunts the ruins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4566 => array(
			'name'    => "On to Glenumbra",
			'journal' => "My actions have convinced Chief Tazgol to join the Daggerfall Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7092 => array(
			'name'    => "Death and Titans",
			'journal' => "I met Toutis Mido ouside the Ayleid ruins of Silorn. I agreed to help him recover Aetherial shards from the ruins, which are overrun with minotaurs and necromancers, by using a device called the Aetherial alarum.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4715 => array(
			'name'    => "The Harvest Heart",
			'journal' => "Vampires occupy the Orchard beyond the Chasm, somehow empowered by Molag Bal's Harvest Heart. The vampires need to be dealt with before our forces can push forward toward the planar vortex.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1514 => array(
			'name'    => "To Bug Musk Screal",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1515 => array(
			'name'    => "To Nocent Abad",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4128 => array(
			'name'    => "Mystery Metal",
			'journal' => "Foreman Jomund discovered a mysterious metal in the mine. He hastily hid their research notes and the ore sample just as the Stormfists began taking over the mine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6344 => array(
			'name'    => "Lamia Demonstration",
			'journal' => "Zahari asked me to help train new recruits for the Khajiit Defense Legion to fight enemies they may not have encountered in training.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4495 => array(
			'name'    => "A Service for the Dead",
			'journal' => "I found the body of a House Redoran soldier. He carried a medallion inscribed with instructions for his retainer, Lladrelo.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2889 => array(
			'name'    => "Scout Warden Lumbermill",
			'journal' => "I accepted a mission to scout Warden Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2871 => array(
			'name'    => "Scout Faregyl Mine",
			'journal' => "I accepted a mission to scout Faregyl Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4720 => array(
			'name'    => "The Den of Lorkhaj",
			'journal' => "I have completed the first part of the Two Moons Path with the Lunar Champion. Now we must venture into the Den of Lorkhaj and complete the mystical journey.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5565 => array(
			'name'    => "A Secret Shame",
			'journal' => "Treasure-hunting pirates have disturbed the undead necromancers in <<1>>. Now the great seal that kept them trapped has been weakened and the necromancers may soon be free to threaten all of Hew's Bane.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6601 => array(
			'name'    => "To Burn Away Evil",
			'journal' => "The Wayward Guardians need silver ore to coat their arms and armor in order to better fight against the Gray Host. Ardanir asked me to bring him some from the Dwarven ruins of Bthar-Zel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5772 => array(
			'name'    => "Uncaged",
			'journal' => "Outside the ancient ruins of Skyreach Pinnacle, I met a member of the Scaled Court. He told me that the Court accidentally released creatures from magical wards the Nedes set up long ago. He plans to reestablish the wards, but he needs help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3907 => array(
			'name'    => "Contract: Armando",
			'journal' => "The Dark Brotherhood has given me a contract.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6242 => array(
			'name'    => "The Cursed Skull",
			'journal' => "Concordia Mercius of Cyrodilic Collections needs my help to stop Margus Derius and the Blackguards from obtaining the Golden Skull of Beela-Kaar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4385 => array(
			'name'    => "Lost in Study",
			'journal' => "The Mages Guild is investigating a strange Ayleid ruin, which has recently become much more active than in years past.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1517 => array(
			'name'    => "To Loqueach",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4386 => array(
			'name'    => "Heart of the Matter",
			'journal' => "<<1>> uncovered information proving <<Ac:2>> is definitely located within <<Ac:3>>. She needs my help in recovering it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3245 => array(
			'name'    => "Capture Faregyl Lumbermill",
			'journal' => "I accepted a mission to help capture Faregyl Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1518 => array(
			'name'    => "To Nocent Abad",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2767 => array(
			'name'    => "Scout Fort Warden",
			'journal' => "I accepted a mission to scout Fort Warden for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4204 => array(
			'name'    => "Bath Time",
			'journal' => "Dralof Waterwalker and his friends bathe in the hot springs south of Wittestadr. Their friend, Grida, went off to gather bath salts for the water, but she never returned.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1519 => array(
			'name'    => "To Sylvan Upland",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2702 => array(
			'name'    => "Scout Farragut Mine",
			'journal' => "I accepted a mission to scout Farragut Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4555 => array(
			'name'    => "Blood Relations",
			'journal' => "<<1>> and his cohorts have taken up residence in Spindleclutch. <<1>> has returned to find his home overrun, and his son <<2>> missing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5447 => array(
			'name'    => "A King-Sized Problem",
			'journal' => "King Kurog has given me a standing invitation to drink with him at the King's Cornerclub in Orsinium. I seemed to have impressed him, and he obviously needs someone to handle the more delicate problems facing his kingdom.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2758 => array(
			'name'    => "Scout Bloodmayne Lumbermill",
			'journal' => "I accepted a mission to scout Bloodmayne Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4212 => array(
			'name'    => "Torn Asunder",
			'journal' => "I wandered into a strange rift between the living world and the land of the dead. Here, I met the spirit of Sonya Lastblood, who said that her husband killed her and her three daughters.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1527 => array(
			'name'    => "The Sower Reaps",
			'journal' => "Arcady Charnis believes someone ordered men to plant dreugh eggs in the Dreughside neighborhood to cause the dreugh to attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6461 => array(
			'name'    => "Hounds of Hircine",
			'journal' => "A sickness is spreading through Dark Moon Grotto that is causing werewolves to act violent and feral. They were close to a cure, but the alchemist working on it, Alchemist Rasir, disappeared. I should investigate to see if the cure can be found.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4222 => array(
			'name'    => "Rites of the Queen",
			'journal' => "Queen Ayrenn must perform the final ritual at Tanzelwil in the ancestral crypts. Malicious spirits plague this holy shrine and the traitorous Norion lurks somewhere in the shadows.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1529 => array(
			'name'    => "Azura's Guardian",
			'journal' => "Abbot Durak took some Spirit Wardens up to the Cave of Dreams to find a way to deal with the largest fire in the fields. I should check up on them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2748 => array(
			'name'    => "Scout Arrius Lumbermill",
			'journal' => "I accepted a mission to scout Arrius Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1536 => array(
			'name'    => "Fire in the Fields",
			'journal' => "Bandits attacked Pariah Abbey and set its crops on fire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5067 => array(
			'name'    => "Proprietary Formula",
			'journal' => "Alchemist Nilyne Hlor doesn't trust the proprietary formula the Maulborn use to make their curative. She wants to examine a few bottles so she can determine how the elixir is made.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2851 => array(
			'name'    => "Scout Drakelowe Keep",
			'journal' => "I accepted a mission to scout Drakelowe Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4716 => array(
			'name'    => "The Shurgak Job",
			'journal' => "<<1>> asked me to deliver a message to his brother <<2>> in <<3>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7050 => array(
			'name'    => "Numinous Grimoire, Volume 3",
			'journal' => "I came across the body of a Nord cipher in the middle of Apocrypha. There was a mysterious note next to her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4311 => array(
			'name'    => "A Ritual in the Ragged Hills",
			'journal' => "There's a Worm Cult ritual going on in the Ragged Hills. Amitra needs my help in stopping it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3083 => array(
			'name'    => "Capture Fort Warden",
			'journal' => "I accepted a mission to help capture Fort Warden for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7355 => array(
			'name'    => "The Dragon's Hoard",
			'journal' => "I met a Khajiit in Rawl'kha who asked me to try and steal coin from a Dragon for his New Life festivity, and then give it to a needy Orc nearby. Apparently, its tricks are challenging to some. I agreed to participate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2753 => array(
			'name'    => "Scout Alessia Lumbermill",
			'journal' => "I accepted a mission to scout Alessia Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5508 => array(
			'name'    => "Scouting the Arboretum",
			'journal' => "My Alliance needs scouts to secure rally points across the Imperial City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1541 => array(
			'name'    => "A Prison of Sleep",
			'journal' => "Bandits set fires in the fields to distract the Spirit Wardens. While the wardens were outside fighting the fires, the thieves stole a priceless relic: the Dream Shard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2906 => array(
			'name'    => "Scout Black Boot Lumbermill",
			'journal' => "I accepted a mission to scout Black Boot Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3471 => array(
			'name'    => "Kill twenty Enemy Guards",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6635 => array(
			'name'    => "Destroying What Remains",
			'journal' => "Mim-Jasa, a priest of the Clutch of Nisswo, asked me to help perform last rites for the Shadowscales who were entombed in the cave long ago. Doing so will supposedly prevent a necromancer named Tumma-Maxath from using them for his own evil purposes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3104 => array(
			'name'    => "Capture Ash Farm",
			'journal' => "I accepted a mission to help capture Ash Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2925 => array(
			'name'    => "Capture Blue Road Keep",
			'journal' => "I accepted a mission to help capture Blue Road Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1554 => array(
			'name'    => "Blood Revenge",
			'journal' => "A Goblin tribe has invaded Cumberland's Watch. A soldier asked me to help fight them off.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5093 => array(
			'name'    => "Moons Over Grimwatch",
			'journal' => "A moon priest named Thalinfar told me that I was needed at Fort Grimwatch, to the north of Rawl'kha.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2068 => array(
			'name'    => "The Dreugh Threat",
			'journal' => "Dreugh are mysteriously attacking the Dreughside district. Sergeant Stegine is organizing a defense. I should lend my aid.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3431 => array(
			'name'    => "Capture Any Three Keeps",
			'journal' => "I accepted a mission to help capture any three keeps for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1562 => array(
			'name'    => "A Father's Plea",
			'journal' => "A few days ago some men attacked Jordel Trueblade and his family.  They killed his wife, took his children and left him for dead.  You must find his children.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7497 => array(
			'name'    => "The Ancestral Bow",
			'journal' => "<<1>> runs a finder's business in Glenumbra and uses the Thieves Guild to track down lost, mislaid, and stolen items. Helping her will surely reflect well on my skill as a thief-and be lucrative to boot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4620 => array(
			'name'    => "Cast Adrift",
			'journal' => "The shoreline of <<1>> is lined with the shattered hulls of Aldmeri Dominion sailing vessels. Equipment and personnel are scattered along the beach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3784 => array(
			'name'    => "Repairing the Cage",
			'journal' => "A Daedric creature is loose in Bonesnap Ruins, and <<1>> has found a way to contain the creature by repairing a welkynd stone that's been damaged by invading Goblins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1568 => array(
			'name'    => "A Means to an End",
			'journal' => "The Goblins attacked Cumberland's Watch because Captain Rama raised their sacred totem over one of the towers as a challenge. Commander Kurt has sent me to take it down.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2222 => array(
			'name'    => "Alasan's Plot",
			'journal' => "An army of Withered Hand soldiers necromancers attacked Tava's Blessing and killed everyone. Whatever they are planning here, I plan to stop them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5099 => array(
			'name'    => "Trial of Martial Knowledge",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3153 => array(
			'name'    => "Capture Roebeck Lumbermill",
			'journal' => "I accepted a mission to help capture Roebeck Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6333 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1591 => array(
			'name'    => "Injured Spirit Wardens",
			'journal' => "I found an injured Spirit Warden in Pariah Abbey's fields. Falice Menoit said she was attacked by Midnight Union outlaws. She'll be fine, but she's worried about others who were injured in the attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4606 => array(
			'name'    => "Keepers of the Shell",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6436 => array(
			'name'    => "Lilies for Remembrance",
			'journal' => "The Senchal job broker, Guybert Flaubert, has a client who wants some lilies planted in Moonlit Cove as a tribute to her late grandmother.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1633 => array(
			'name'    => "The Return of the Dream Shard",
			'journal' => "Now that I've retrieved the Dream Shard, I can finally return it to the Spirit Wardens at Pariah Abbey.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4002 => array(
			'name'    => "Sparking the Flame",
			'journal' => "I've helped the community leaders of Bleakrock Village. Rana's preparing the villagers to evacuate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1637 => array(
			'name'    => "Divert and Deliver",
			'journal' => "Slavers are using Steelheart Moorings as their base of operations and they're holding some sailors who they recently captured. The first mate has asked me to help free the sailors from the slavers' clutches.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3201 => array(
			'name'    => "Capture Arrius Farm",
			'journal' => "I accepted a mission to help capture Arrius Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1639 => array(
			'name'    => "Another Omen",
			'journal' => "After I returned the Dream Shard to Pariah Abbey, Abbot Durak told me of concerns that Vaermina also works to destabilize the eastern region of Gavaudon. I'm to warn High King Emeric and then meet Durak at Shinji's Scarp.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3228 => array(
			'name'    => "Capture Castle Black Boot",
			'journal' => "I accepted a mission to help capture Castle Black Boot for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6591 => array(
			'name'    => "The Lost Scout's Report",
			'journal' => "I found a faded scrap of parchment in the depths of Blackreach. It's an account written by a Nord scout, Falkfyr, after he became lost in the great cavern. If I wish to discover Falkfyr's fate, I should follow the directions in his notes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6085 => array(
			'name'    => "Birds of a Feather",
			'journal' => "The Divine Prosecution has requested I defeat the gryphons Haeliata and Nagravia. I'll also need to collect their feathers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2865 => array(
			'name'    => "Scout Arrius Mine",
			'journal' => "I accepted a mission to scout Arrius Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4344 => array(
			'name'    => "Like Moths to a Candle",
			'journal' => "The Sea Drakes have captured friends of <<1>>. They're being held at <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3917 => array(
			'name'    => "Enlightenment Needs Salt",
			'journal' => "Nojaxia wants to join the Mages Guild in Ebonheart. She's trying to create an elixir that replicates the visions an Argonian receives after licking a Hist Tree.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1678 => array(
			'name'    => "The Slumbering Farmer",
			'journal' => "Supernal Dreamers have attacked the Vanne Farm and captured the owner, Ingride Vanne, with the intention of subjecting her to some dark ritual.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7297 => array(
			'name'    => "The Stubbornness of Nords",
			'journal' => "Britt the Bold, ambassador for Nord Cultural Exchange and representative of the Stirk Fellowship, needs help convincing the Nords of Shor's Stand to join the fight against the Worm Cult.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2683 => array(
			'name'    => "Scout Blue Road Keep",
			'journal' => "I accepted a mission to scout Blue Road Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5570 => array(
			'name'    => "Everyone Has A Price",
			'journal' => "Walks-Softly wants some more of my help with the situation involving his old friend, Anais Velmont. The letter he left indicates he's come up with a solution that he wants to run by me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4884 => array(
			'name'    => "The Lightless Remnant",
			'journal' => "We've taken control of the Shrouded Pass and the way to the Doomcrag is open. The time has come to make our final assault and reach Montclair and  the cursed relic known as the Lightless Remnant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3402 => array(
			'name'    => "Kill an Ebonheart Player",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1687 => array(
			'name'    => "False Knights",
			'journal' => "Bandits are disguising themselves as Knights of the Flame and robbing people on the road to Wrothgar. I promised Sir Graham I'd get the tabards back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6616 => array(
			'name'    => "An Unexpected Adversary",
			'journal' => "The last surviving members of the Imperial Elder Council in Blackwood have sought refuge in Leyawiin Castle. I need to determine the true nature of the threat posed by the Dark Brotherhood and how it relates to Leovic's secret.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2898 => array(
			'name'    => "Scout Kingscrest Lumbermill",
			'journal' => "I accepted a mission to scout Kingscrest Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4255 => array(
			'name'    => "Ensuring Security",
			'journal' => "I arrived in the city of Vulkhel Guard, gateway to Auridon. Security is tight as Queen Ayrenn, the new leader of the Aldmeri Dominion, prepares to make a public appearance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1709 => array(
			'name'    => "Rozenn's Dream",
			'journal' => "The Supernal Dreamers also took Ingride's daughter, <<1>>, and they're apparently planning a similar ritual using her as the vessel for some Daedric spirit. I promised Ingride I would stop them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4078 => array(
			'name'    => "A Council of Thanes",
			'journal' => "The Dark Elves and Argonians are finally willing to work with Prince Irnskar and meet in a council of war. Now the Prince wants me to convince the Eastmarch thanes to attend the war council as well.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6189 => array(
			'name'    => "Pledge: March of Sacrifices",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Hircine's March of Sacrifices.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2731 => array(
			'name'    => "Scout Farragut Farm",
			'journal' => "I accepted a mission to scout Farragut Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4462 => array(
			'name'    => "The Dead King",
			'journal' => "The Worm Cult has invaded the ruins of Rulanyil's Fall, intent on raising an Ayleid warlord from the dead. <<1>> wants to ensure the body is not disturbed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3615 => array(
			'name'    => "Wake the Dead",
			'journal' => "An Argonian named Onuja has come to Othrenis. He wants to know how to rebind the Brother of Strife. He's hoping to contact the spirit of Mavos Siloreth, an ancient Dunmer scribe buried in the area.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7036 => array(
			'name'    => "Secrets Reclaimed",
			'journal' => "Ordinator Tilena needs someone to go to the Disquiet Study and find the books that an apostate scribe gave to Hermaeus Mora. I volunteered my services.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1735 => array(
			'name'    => "Unanswered Questions",
			'journal' => "Though the Omen of Fear has been dealt with and Alcaire is free of its influence, Duke Nathaniel is concerned. He's asked me to go seek the counsel of a Spirit Warden named Muran-to see if he can explain what happened.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5305 => array(
			'name'    => "Pledge: Blackheart Haven",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to explore the island of Blackheart Haven in Iliac Bay.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1749 => array(
			'name'    => "A Miner's Vengeance",
			'journal' => "I am to summon and kill the Dremora that slayed the miners, then report to Brek after the deed is done. The artifact to summon it should be near the tower on the hill. I can navigate through the mines to find a way there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3130 => array(
			'name'    => "Capture Farragut Mine",
			'journal' => "I accepted a mission to help capture Farragut Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1799 => array(
			'name'    => "A City in Black",
			'journal' => "The city of Evermore is in dire straits. King Eamond is dead, and a Reachmen army marches toward the city gates even now. The people of Evermore need help, and they need hope.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1802 => array(
			'name'    => "Mistress of the Lake",
			'journal' => "Followers of Hermaeus Mora have come to secure the ancient Ayleid ruins at Lake Halcyon, but they say a mysterious entity inhabiting the ruins has enslaved local villagers and is trying to subvert the laws of nature.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5520 => array(
			'name'    => "Flames of Forge and Fallen",
			'journal' => "I met an Orc named <<1>> in Old Orsinium. He belonged to a holy order that lights Malacath's pyre once every decade, but was ambushed before he could do so. I swore to head to the heart of Old Orsinium and light Malacath's pyre.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6323 => array(
			'name'    => "The Moonlight Blade",
			'journal' => "Mazza-Mirri, the last surviving member of the Hidden Moon Order, led me to an enchanted Moonlight Blade. The soul of an ancient Khajiiti warrior resides within. With this enchanted blade, and Vastarie's help, I may be able to stop Arum-Khal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2613 => array(
			'name'    => "The Elder Scroll of Alma Ruma",
			'journal' => "Grand Warlord Zimmeron is ordering Pact soldiers to capture the <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4982 => array(
			'name'    => "We Live In Fear",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3333 => array(
			'name'    => "Risen From the Depths",
			'journal' => "The Sentinel Docks have been overrun with undead and the guards don't appear to be doing anything in response. I told Captain Marck I would try to find out what's going on.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6425 => array(
			'name'    => "Fletching Fetching",
			'journal' => "A driver took a dangerous route and died at the hands of a Meridian temple guardian. Now the Shields of Senchal are short on arrows. I agreed to collect arrows and deal with this guardian.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2983 => array(
			'name'    => "Capture Drakelowe Lumbermill",
			'journal' => "I accepted a mission to help capture Drakelowe Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1803 => array(
			'name'    => "The Water Stone",
			'journal' => "The Nereid of Halcyon Lake is trying to save the lake from eventually drying up by using ancient and forbidden secrets to create a Water Stone. Hermaeus Mora cultists called Primeval Seekers are trying to stop her, but I've agreed to help her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6557 => array(
			'name'    => "Wild Talismans",
			'journal' => "A liaison for the Wildblood clan, <<1>>, asked that I enter Briar Rock Ruins and retrieve talismans from Wildblood warriors who were taken prisoner by a rival clan, the Thornroot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1815 => array(
			'name'    => "Greg's Training Quest",
			'journal' => "So-and-so has asked that you do something cool. Won't you do something cool.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4318 => array(
			'name'    => "Into the Outside",
			'journal' => "I found a note on the charred corpse of an assassin who was sent to kill a mage named Malsia. She stole Dwarven artifacts and plans to open a portal to someplace. The Mages Guild wants her dead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3191 => array(
			'name'    => "Reclaiming the Elements",
			'journal' => "Angof's Bloodthorn cultists threaten the Wyrd Tree. I should help the Beldama Wyrd save the tree.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1834 => array(
			'name'    => "Heart of Evil",
			'journal' => "The only way to ensure Evermore's security and drive the Reachmen from northern Bangkorai is to defeat their leader, the Hagraven Uela. I have agreed to help the wyresses in Jackdaw Cove toward this end.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5552 => array(
			'name'    => "The Shark's Teeth",
			'journal' => "Shark's Teeth Grotto has been overrun by pirates and Shalug needs some help dealing with them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5318 => array(
			'name'    => "The Hidden Harvest",
			'journal' => "The Orsimer army needs to get into Frostbreak Fortress to find and destroy the source of the briarheart warriors, but the Winterborn's defense are too strong. Between the briarhearts and the harpies, they can't breach the walls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2887 => array(
			'name'    => "Scout Black Boot Farm",
			'journal' => "I accepted a mission to scout Black Boot Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4266 => array(
			'name'    => "The First Patient",
			'journal' => "Velatosse is suspicious about the plague's origins. She's asked me to look into the source of this mysterious illness.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1835 => array(
			'name'    => "The Last Spriggan",
			'journal' => "The Reachmen at Jackdaw Cove are corrupting the spriggans and transforming them into monstrous lurchers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3353 => array(
			'name'    => "The Nature of Fate",
			'journal' => "An old Redguard sitting in the desert is contemplating fate. He believes all of our actions are predetermined.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2692 => array(
			'name'    => "Scout Rayles Mine",
			'journal' => "I accepted a mission to scout Rayles Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5569 => array(
			'name'    => "A Cordial Collaboration",
			'journal' => "The assassin Naryu Virian must kill a Dark Elf noble in Kvatch to fulfill a contract for her shadowy organization. However, she doesn't want to attract the attention of the Dark Brotherhood as she completes her work, so she asked for my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3688 => array(
			'name'    => "Unwelcome Guests",
			'journal' => "<<1>> says some mercenaries in Stormhold are stirring up trouble between the Dark Elves and Argonians.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	1839 => array(
			'name'    => "Box Clicking For the Win!",
			'journal' => "So and so has asked that you travel to the big red box and do some stuff!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2852 => array(
			'name'    => "Scout Castle Alessia",
			'journal' => "I accepted a mission to scout Castle Alessia for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2016 => array(
			'name'    => "Hallin's Burden",
			'journal' => "Hallin's Stand is under the control of the Seventh Legion, an Imperial military force.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2691 => array(
			'name'    => "Scout Warden Mine",
			'journal' => "I accepted a mission to scout Warden Mine  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5068 => array(
			'name'    => "Quest for the Cure",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3399 => array(
			'name'    => "Kill a Daggerfall Player",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6308 => array(
			'name'    => "Thick as Thieves",
			'journal' => "I met a Khajiiti woman by the name of Tasnasi who is putting together a heist to rob Khasda, the new leader of the Stitches.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2017 => array(
			'name'    => "The Lion's Den",
			'journal' => "Yarah needs to discover the identity of whomever betrayed Hallin's Stand and allowed the Seventh Legion to enter the city.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4443 => array(
			'name'    => "To Alcaire Castle",
			'journal' => "<<1>> has expressed concerns about events unfolding at Alcaire Castle. She asked me to go there and see if I can help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2853 => array(
			'name'    => "Scout Castle Faregyl",
			'journal' => "I accepted a mission to scout Castle Faregyl for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2902 => array(
			'name'    => "Scout Alessia Lumbermill",
			'journal' => "I accepted a mission to scout Alessia Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4629 => array(
			'name'    => "Secrets Revealed",
			'journal' => "I found a letter on the body of a bounty hunter. It mentions a woman named Wardiya in Bruma.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4291 => array(
			'name'    => "The Summer Site",
			'journal' => "Falinesti Summer Site is a legendary seasonal location for the Wood Elves' Walking City. But the city hasn't been seen in Tamriel for centuries. Now, something is happening at the Summer Site.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3898 => array(
			'name'    => "Proving the Deed",
			'journal' => "The Council of Province Generals has ordered an investigation into the death of the former Guildmaster, Jofnir Iceblade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6171 => array(
			'name'    => "Jewelry Crafting Certification",
			'journal' => "If I can show that I know how to make a ring, Felarian will certify me as a jewelry crafter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3745 => array(
			'name'    => "Stolen Crystal",
			'journal' => "Supernal Dreamers stole a crystal from Geoffrey Stictal and broke it into multiple pieces. He wants me to retrieve the pieces from them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2018 => array(
			'name'    => "A Thirst for Revolution",
			'journal' => "Ufa the Red Asp and I must work together to locate the traitor identified by Captain Dhakir's men in order to save the grandeya of Hallin's Stand from execution.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4452 => array(
			'name'    => "Reap What Is Sown",
			'journal' => "<<1>> has come to <<2>> to find out why the Drublog attacked Velyn Harbor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2046 => array(
			'name'    => "Dreams to Nightmares",
			'journal' => "Cultists called Supernal Dreamers are up to no good in Moonlit Maw. Master Muzgu, a monk from Pariah Abbey, led other monks here to stop them. But they were ambushed by the cultists.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4151 => array(
			'name'    => "A Bitter Pill",
			'journal' => "<<1>> does not believe <<2>>'s story about <<3>> becoming infected. Something isn't right.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7346 => array(
			'name'    => "Food for New Life",
			'journal' => "For Lanullin, celebrating New Life involves giving back to the esteemed elders of her community through good meals. I agreed to help her prepare and deliver them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2047 => array(
			'name'    => "The Gate to Quagmire",
			'journal' => "The Supernal Dreamers performed a ritual on Muzgu that infused a Daedric spirit into his body. He's contained it for now, but it's gaining strength. If the spirit takes control of him, it will combine its powerful magic with his own. Many will die.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5489 => array(
			'name'    => "The Lock and the Legion",
			'journal' => "The Drake of Blades' contact in Legion Zero has not checked in for weeks. The Drake has requested that I go try to establish contact myself.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2867 => array(
			'name'    => "Scout Farragut Mine",
			'journal' => "I accepted a mission to scout Farragut Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2130 => array(
			'name'    => "Rise of the Dead",
			'journal' => "An army of undead swarmed from the sea and attacked the Sentinel Docks. Watch Captain Zafira has asked me to inform King Fahara'jad.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4768 => array(
			'name'    => "Scars Never Fade",
			'journal' => "Dominion soldiers erected barricades around a tribe of Argonian refugees living in <<c:1>>. <<2>> needs help resolving the Argonians' situation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2146 => array(
			'name'    => "The Impervious Vault",
			'journal' => "Prince Azah says the necromancers of the Withered Hand are tunneling into the Impervious Vault to steal powerful relics known as the Ansei Wards. We must try to stop them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4354 => array(
			'name'    => "The Endless War",
			'journal' => "I discovered the Cliffs of Failure, where Molag Bal's failed servants compete to get back into his good graces. Lost members of the Mages Guild have been ensnared in this twisted game, and I need to join in to save them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3213 => array(
			'name'    => "Capture Fort Rayles",
			'journal' => "I accepted a mission to help recapture Fort Rayles for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2880 => array(
			'name'    => "Scout Aleswell Farm",
			'journal' => "I accepted a mission to scout Aleswell Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2161 => array(
			'name'    => "Monkey Magic",
			'journal' => "I met a talking monkey in Sentinel. The monkey claims he's really an Orc and that he was tricked by a Khajiit potion-maker named Unishi. He wants to be changed back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6599 => array(
			'name'    => "The Symbol of Storihbeg",
			'journal' => "I found the body of <<1>>. It looks like he died after stealing a sacred relic from the Reach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3001 => array(
			'name'    => "Farlivere's Gambit",
			'journal' => "The Red Rooks attacked a caravan and escaped to a noble's house on the outskirts of Daggerfall, where they've taken hostages. Captain Farlivere of the Daggerfall Guard has ordered her soldiers to surround the estate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4858 => array(
			'name'    => "Hope Lost",
			'journal' => "The hamlet of Moira's Hope has met with disaster. Bloodfiends fill the streets, and the few survivors that remain are trying desperately to stay alive. I agreed to help Adusa-daro provide aid and find out who is behind this attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2184 => array(
			'name'    => "Tu'whacca's Breath",
			'journal' => "The Withered Hand has taken over Tu'whacca's Throne and begun using the Ansei Wards to raise the consecrated remains of fallen heroes. I agreed to help the Throne Keeper activate defenses to cleanse the temple.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6669 => array(
			'name'    => "Tome of Shadow",
			'journal' => "An Argonian named Walks-Under-Shadow asked me to retrieve something called the Nightwielder's Tome from Xi-Tsei. Retrieving the tome will help him perform the proper rites for a nisswo priest named Nesh-Tei who died trying to find it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2187 => array(
			'name'    => "Revered Ancestors",
			'journal' => "The dead have risen at Tu'whacca's Throne, and an Ash'abah here is particularly worried that three ancient heroes have been raised. I agreed to visit their graves, and if they have been raised, then I will kill them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4942 => array(
			'name'    => "The Spider's Cocoon",
			'journal' => "I met a man named Beryn in the woods, recovering from an epic drinking bout at his camp. He's worried because he fears his friend <<1>> has wandered into a nearby spider cave on a dare.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2881 => array(
			'name'    => "Scout Dragonclaw Farm",
			'journal' => "I accepted a mission to scout Dragonclaw Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5941 => array(
			'name'    => "The Jester's Festival",
			'journal' => "I received an invitation to celebrate a festival of fools and foolishness across Tamriel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3702 => array(
			'name'    => "The Soldier's Alibi",
			'journal' => "I encountered a captive Dark Elf soldier named Saryvn in the ruins beneath Tal'Deic Fortress. He was captured by the Maulborn while he was avoiding his duties.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6666 => array(
			'name'    => "A Mother's Obsession",
			'journal' => "Mirri has something of a family issue. Her mother, a collector of antiquities, has gone missing. Mirri requested my assistance in finding her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2192 => array(
			'name'    => "A Reckoning with Uwafa",
			'journal' => "The Throne Keeper and I have lit the sacred braziers at the cardinal points of the temple. When we light the final brazier, Tu'whacca should send his \"Breath\" to come and help us cleanse the temple. Then we can kill Uwafa and recover the Ansei Wards.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4061 => array(
			'name'    => "One Victor, One King",
			'journal' => "Fildgor, King Jorunn's exiled brother, has returned to Skyrim. Nords from the Stormfist clan, loyal to Fildgor, have infiltrated the Konunleikar to get close to King Jorunn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3398 => array(
			'name'    => "Scout the Mine",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6241 => array(
			'name'    => "Whispers in the Wood",
			'journal' => "A band of notorious criminals known as the Blackguards are searching for a powerful Argonian relic called the Remnant of Argon. I'll need allies if I'm going to find it first.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4514 => array(
			'name'    => "The Spearhead's Crew",
			'journal' => "Captain Kaleen's cabin girl, Nicolene, informed me that Kaleen's crew is ready to start the heist as soon as I'm done recruiting.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3040 => array(
			'name'    => "Vital Inheritance",
			'journal' => "<<1>> was the sole survivor of the Bloodthorn cult's attack on the Aldcroft lighthouse. She asked me to retrieve her father's pipe from his cottage so she will have something to remember him by.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2193 => array(
			'name'    => "The Scholar of Bergama",
			'journal' => "Throne Keeper Farvad says I should continue my search for the remaining Ansei Wards in Bergama to the east.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6977 => array(
			'name'    => "Chronicle of Fate",
			'journal' => "Leramil says we have two more tasks before we're finished. We need to help Scruut update the Chronicle of Apocrypha, then help Gadayn return the relic to Necrom. There's also the matter of the stolen glyphic, but first things first.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4738 => array(
			'name'    => "Song of the Spinner",
			'journal' => "I discovered a Spinner named Melrethel imprisoned in the cellars of the Vile Manse. She claims to be from the Wood Elf village that Graccus razed and built his manor upon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2240 => array(
			'name'    => "Shiri's Research",
			'journal' => "The scholar's apprentice Shiri has studied a book from among Alasan's possessions. Shiri suggests I begin my search for the third Ansei Ward in HoonDing's Watch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2251 => array(
			'name'    => "Gone Missing",
			'journal' => "People are disappearing from the streets of Bergama, and I've decided to help investigate the matter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5006 => array(
			'name'    => "To Velyn Harbor",
			'journal' => "With <<1>> dead-again-and the new Silvenar revealed, <<2>> asked me to head to <<3>> in <<4>>. The Aldmeri Dominion needs to prepare for the wedding between the Green Lady and the newly-discovered Silvenar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4898 => array(
			'name'    => "Rising Against Onsi's Breath",
			'journal' => "Onsi's Breath was captured by Imperial soldiers who hold its inhabitants captive.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3757 => array(
			'name'    => "Rescuing Leonar",
			'journal' => "I found Lord Geoffroi's son trapped in a cage.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2255 => array(
			'name'    => "Crawling Chaos",
			'journal' => "Once the mines were a prime spot for prospectors, but with the influx of spiders, they've been shut down for safety reasons.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6070 => array(
			'name'    => "Nibbles and Bits",
			'journal' => "The Bursar of Tributes seeks items of flesh to add to the Blackfeather Court's hoard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2344 => array(
			'name'    => "Snakes in the Sands",
			'journal' => "A caravan was attacked by snakes at the oasis near Ogre's Bluff. The caravan guard is afraid of snakes and asked me to return and help the members of the caravan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2356 => array(
			'name'    => "March of the Ra Gada",
			'journal' => "The spirits of HoonDing's Watch have awoken to test me. I must vanquish the spirits of the Redguards' ancient foes and claim their essence to summon their leaders.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2364 => array(
			'name'    => "Feathered Fiends",
			'journal' => "Kozanset, a small settlement in the deserts of Alik'r, is having harpy troubles.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2403 => array(
			'name'    => "The Search for Shiri",
			'journal' => "Shiri, the scholar's apprentice, has disappeared along with the text she recovered from Tava's Blessing. I've agreed to help find Shiri … and the text.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5793 => array(
			'name'    => "Dark Anchors in Auridon",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Auridon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2404 => array(
			'name'    => "Imperial Incursion",
			'journal' => "Imperial forces have seized control of Satakalaam. King Fahara'jad's army is here to take the city back, but General Thoda needs to rescue four hostages the Imperials are holding first.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6134 => array(
			'name'    => "The New Life Festival",
			'journal' => "The New Life Festival is celebrated across Tamriel and all are welcome to join in. I should find out more about this celebration.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2408 => array(
			'name'    => "In Search of the Ash'abah",
			'journal' => "The Withered Hand stole the Ansei Wards, ancient swords that protect Alik'r from necromancy. King Fahara'jad has asked me to enlist the aid of the Ash'abah, a tribe of dishonored warriors who fight the undead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2741 => array(
			'name'    => "Scout Warden Lumbermill",
			'journal' => "I accepted a mission to scout Warden Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5110 => array(
			'name'    => "Gem of the Stars",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5825 => array(
			'name'    => "Madness in Malabal Tor",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Malabal Tor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2436 => array(
			'name'    => "One Last Game",
			'journal' => "In the Wayrest graveyard, I found the last Will and Testament of a scholar named Frodibert Fontbonne. Frodibert challenged his son Donel to solve a riddle to claim his inheritance. Donel gave up, but maybe I can solve the riddle.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2450 => array(
			'name'    => "A Woman Wronged",
			'journal' => "Serge Arcole asked me to help him incapacitate the smugglers. Then I'm to search the counting house for clues as to where Miranda's being held.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4831 => array(
			'name'    => "The Harborage",
			'journal' => "The Prophet contacted me through a magical projection. He asked me to seek out a place called the Harborage, where he has taken refuge. I should go to him and ensure that he's safe.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2451 => array(
			'name'    => "A Ransom for Miranda",
			'journal' => "Serge Arcole says the Midnight Union is holding his wife, Miranda, for ransom. He wants me to collect some items and bring them to him so he can bargain from a position of strength.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3533 => array(
			'name'    => "The Waking Darkness",
			'journal' => "Corpses are crawling out of the ground at Pelin Graveyard and taking up arms. Prince Adrien was to be entombed here.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2688 => array(
			'name'    => "Scout Castle Brindle",
			'journal' => "I accepted a mission to scout Castle Brindle for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3174 => array(
			'name'    => "A Lingering Hope",
			'journal' => "General Gautier believes his son Darien, the Captain of the Camlorn Guard is still alive inside the city. He hopes to use Darien's master key to unlock the Camlorn gates. Then the Lion Guard can surround Faolchu's werewolves and retake the city.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2481 => array(
			'name'    => "The Debt Collector's Debts",
			'journal' => "S'rashi works for the Midnight Union in Wayrest, but he's gotten himself in trouble. He owes money to his boss and needs help getting the gold together as fast as possible.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3717 => array(
			'name'    => "King of Dust",
			'journal' => "The Dominion is searching for a keystone in <<1>>. I need to find it before they do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2488 => array(
			'name'    => "A Gang of Thugs",
			'journal' => "I stopped some thugs from robbing an innocent man in Wayrest. I found a bag of stolen items on them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5116 => array(
			'name'    => "Elemental Army",
			'journal' => "The Celestial Thief appeared to me and asked for my help discovering why the atronachs that were summoned to destroy the Dark Anchors turned against the mages of Craglorn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2494 => array(
			'name'    => "Can't Leave Without Her",
			'journal' => "The Vanne Farm has been overrun by Supernal Dreamer cultists and daedra. Phinis Vanne is simply trying to escape, but his sister has barricaded herself in the house. He asked me to persuade her to leave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2495 => array(
			'name'    => "The Signet Ring",
			'journal' => "<<1>> wants his father's signet ring, and he's tasked me with getting it. If I can retrieve the ring, he says I can name my reward. The Dream Shard, perhaps?",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4722 => array(
			'name'    => "Welcome to Cyrodiil",
			'journal' => "I've arrived in Cyrodiil, ready to help the Pact win the war.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2801 => array(
			'name'    => "Scout Brindle Mine",
			'journal' => "I accepted a mission to scout Brindle Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2826 => array(
			'name'    => "Scout Aleswell Lumbermill",
			'journal' => "I accepted a mission to scout Aleswell Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6721 => array(
			'name'    => "A Paralyzing Lesson",
			'journal' => "Dagonists drugged Saraathu Tong operatives-along with other prisoners-and left them outside Taupezu Azzida's lair to be eaten. I need to give the prisoners the antidote and kill Taupezu Azzida before it's too late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3131 => array(
			'name'    => "Capture Blue Road Mine",
			'journal' => "I accepted a mission to help capture Blue Road Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4994 => array(
			'name'    => "Imperial Curiosity",
			'journal' => "The Imperials in Southern Bangkorai have set up some sort of dig around an ancient Redguard ruin called the Hall of Heroes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2496 => array(
			'name'    => "Evidence Against Adima",
			'journal' => "<<1>> believes that <<2>> is controlling her son, Hosni. If she can get proof that Adima is behind his actions, then she'll have the evidence she needs to prove her son's innocence.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5764 => array(
			'name'    => "The Truer Fangs",
			'journal' => "Safa al-Satakalaam, a priest of Satakal, is at odds with the Scaled Court. Three of her most powerful acolytes have deserted her to worship the Celestial Serpent. She wants me to punish these traitors and deny their power to the Scaled Court.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2497 => array(
			'name'    => "Saving Hosni",
			'journal' => "<<1>> believes Adima controls Hosni through his dreams. She thinks I can save her son, Hosni, by entering his dreams as well.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2713 => array(
			'name'    => "Scout Farm 3",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6584 => array(
			'name'    => "Guides to the Deep",
			'journal' => "A scout of the Ard's Stonehands spotted a strange new Dwarven automaton dropping odd beacons in a nearby cave. They want me to investigate and destroy the construct, as well as any beacons I find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2866 => array(
			'name'    => "Scout Kingscrest Mine",
			'journal' => "I accepted a mission to scout Kingscrest Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4223 => array(
			'name'    => "Corruption Stones",
			'journal' => "Strange stones emanating necromantic energy have appeared on the sacred grounds of Tanzelwil. Priestess Langwe believes they might be tied to the disturbed demeanor of the Ancestral Spirits.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3918 => array(
			'name'    => "Circus of Cheerful Slaughter",
			'journal' => "Sheogorath's book is supposed to lead me to the next trial, the \"<<1>>.\" Let's see what the Mad God has planned for me this time.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5259 => array(
			'name'    => "Crafting Certification",
			'journal' => "Master crafters are offering free training and certification in their professions. Crafting certification will allow me to fulfill crafting writs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2536 => array(
			'name'    => "Plowshares to Swords",
			'journal' => "Dro-Dara asked me to talk to the warrior Knarstygg about abandoning the plantation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2537 => array(
			'name'    => "King Aphren's Sword",
			'journal' => "The mage Blaise Pamarc asked me to recover the artifact, King Aphren's Sword, from the ruins. There's just one problem: the spirits have awakened and block my path.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2538 => array(
			'name'    => "Gift from a Suitor",
			'journal' => "After escaping from the mirror, the countess is asking me to help her uncover which of her suitors actually gave her a gift that turned out to be a trap.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2756 => array(
			'name'    => "Scout Brindle Lumbermill",
			'journal' => "I accepted a mission to scout Brindle Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3752 => array(
			'name'    => "A Storm Broken",
			'journal' => "Pash-Riha hasn't returned to town yet. She's still hunting a Covenant lieutenant. Seven-Bellies is concerned for her safety. He believes she may be hunting southwest of the town.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2549 => array(
			'name'    => "Abominations from Beyond",
			'journal' => "I encountered a Priestess of Stendarr who said yonder rocky hill crowned with ruins has been occupied by infernal abominations. They've captured several locals for nefarious purposes; I volunteered to rescue them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2550 => array(
			'name'    => "Curse of Skulls",
			'journal' => "I found a skull in some old ruins on a hill outside Wayrest. It spoke to me. It claims to be cursed, forced to serve as the eyes of Molag Bal. It begged me to destroy it and all others like it upon the charnel heap in the tower.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4277 => array(
			'name'    => "Silent Village",
			'journal' => "I discovered a statue outside the small town of Shattered Grove. The statue is remarkably lifelike and warm to the touch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4335 => array(
			'name'    => "Pride of the Lion Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7203 => array(
			'name'    => "The Wing of the Dragon",
			'journal' => "I picked up the Fable of the Dragon as my next step to restoring the Scholarium. I'll need to uncover its secrets to open the Dragon door, meet the Luminary within, and amplify the power of the Scribing Altar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2729 => array(
			'name'    => "Scout Arrius Farm",
			'journal' => "I accepted a mission to scout Arrius Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2552 => array(
			'name'    => "Army at the Gates",
			'journal' => "King Fahara'jad of Alik'r has sent an army to Alcaire Castle. They claim to be on a diplomatic mission, but the Knights of the Flame believe otherwise. This situation could threaten the very fabric of the Daggerfall Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4657 => array(
			'name'    => "The Spinner's Tale",
			'journal' => "Though Aranias was sent by the Veiled Heritance to kill him, the Wilderking believes she has actually come for another reason, and that our destinies are intertwined.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2553 => array(
			'name'    => "Bones of a Ghost",
			'journal' => "The miner Gathan was trapped inside <<1>> when the Daedra suddenly appeared. Unable to escape, he was caught by the creatures and dismembered. His restless ghost is now bound to haunt the cavern where he died so long as his body remains unburied.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2733 => array(
			'name'    => "Scout Drakelowe Farm",
			'journal' => "I accepted a mission to scout Drakelowe Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2556 => array(
			'name'    => "False Accusations",
			'journal' => "A Knight of the Flame claims to have been put in the stocks unjustly. I need to convince the constable to release her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2561 => array(
			'name'    => "A Family Affair",
			'journal' => "The Nurin family needs help. They awoke this morning to find their farm burning, overrun by Daedra, and the father, William Nurin, is missing. I volunteered to help find him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2564 => array(
			'name'    => "Two Sides to Every Coin",
			'journal' => "<<1>> needs someone to take a message to the leader of the Redguard camp outside  the gates. Sir Hughes suggested I act as her messenger and report on what she reveals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5441 => array(
			'name'    => "The Hand of Morkul",
			'journal' => "All is not right in Morkul Stronghold. <<1>>, the chief's eldest daughter, ran off to <<2>> without explanation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3232 => array(
			'name'    => "Capture Rayles Lumbermill",
			'journal' => "I accepted a mission to help capture Rayles Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6284 => array(
			'name'    => "Something About Stibbons",
			'journal' => "Lady Laurent and her manservant, Stibbons, have come to Murkmire in search of an antique Argonian renewal charm. Lady Laurent needs it or she won't be able to observe the annual rite of renewal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6089 => array(
			'name'    => "Replacing the Commutators",
			'journal' => "The Clockwork Apostles have tasked me to continue maintenance in the Halls of Regulation. I need to replace several brass commutators to prevent the galvanic engine from shutting down.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2566 => array(
			'name'    => "Life of the Duchess",
			'journal' => "<<1>> was apparently behind the assassination attempts on <<2>>. We must apprehend her quickly to ensure the duchess's safety.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5510 => array(
			'name'    => "Scouting the Arena District",
			'journal' => "My Alliance needs scouts to secure rally points across the Imperial City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2567 => array(
			'name'    => "The Safety of the Kingdom",
			'journal' => "<<1>> is dead, and the threat against <<2>> is over, at least for the moment.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5677 => array(
			'name'    => "Contract: Greenshade",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3572 => array(
			'name'    => "Kill Twenty Daggerfall Guards",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2569 => array(
			'name'    => "Legacy of the Three",
			'journal' => "I've discovered evidence of something amiss at the Windridge Caves in Alcaire. The fates of a knight and his comrades seem inexorably tied to the existence of The Three, a trio of bandit chieftains who once dwelled in these caves.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3421 => array(
			'name'    => "Capture the Farm",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2576 => array(
			'name'    => "Tracking Sir Hughes",
			'journal' => "Sir Hughes murdered Duchess Lakana in Alcaire Castle. I promised Duke Nathaniel that I will track Sir Hughes down to answer for his crimes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6296 => array(
			'name'    => "The Battle for Riverhold",
			'journal' => "Euraxia, the Usurper Queen of Rimmen, has ordered her forces to attack Riverhold. I need to help defend the town.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2578 => array(
			'name'    => "Scamp Invasion",
			'journal' => "The Supernal Dreamers have apparently built a few portals on the Nurin's farm through which they are summoning Scamps and possibly other Daedra. I agreed to destroy these portals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4447 => array(
			'name'    => "Yngrel the Bloody",
			'journal' => "A wanted bandit named Yngrel the Bloody has been causing trouble along the northern road in the Rift. I've been offered a reward to bring him down.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5542 => array(
			'name'    => "Welcome Home",
			'journal' => "I gained access to the Dark Brotherhood Sanctuary and met the Matron of this chapter, Astara Caerellius. She welcomed me into the family and suggested I meet the rest of my brothers and sisters in the guild.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2599 => array(
			'name'    => "Ash and Reprieve",
			'journal' => "The village of Deleyn's Mill is under siege. Fire threatens to burn the village to the ground while enraged forest creatures attack the remaining villagers. The witches of the Beldama Wyrd are trying to contain the situation. I should help them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5829 => array(
			'name'    => "Madness in Stonefalls",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2609 => array(
			'name'    => "The Elder Scroll of Chim",
			'journal' => "The Elder Scroll of Chim has been stolen by the enemy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6328 => array(
			'name'    => "The Heir of Anequina",
			'journal' => "Despite our victories and the events on the Plane of Jode, Khamira is not yet officially the queen. Before Khamira can receive the crown of Anequina, she needs to receive the blessing of the Mane.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2610 => array(
			'name'    => "The Elder Scroll of Altadoon",
			'journal' => "Grand Warlord Zimmeron wants the <<1>> to be captured for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4479 => array(
			'name'    => "Motes in the Moonlight",
			'journal' => "After the ritual at Rawl'kha, I traveled to meet the Lunar Champions at the temple called Moonmont. Here I'm told the Dark Mane has holed up. After what it did to Mane Akkhuz-ri, we must destroy it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2969 => array(
			'name'    => "Capture Black Boot Farm",
			'journal' => "I accepted a mission to help capture Black Boot Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4826 => array(
			'name'    => "Hunting the Mammoth",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5233 => array(
			'name'    => "Kill Enemy Sorcerers",
			'journal' => "Grand Warlord Dortene has authorized a reward for any Covenant soldier who slays 20 enemy Sorcerers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2611 => array(
			'name'    => "The Elder Scroll of Mnem",
			'journal' => "The <<1>> must be taken from the enemy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5226 => array(
			'name'    => "Kill Enemy Dragonknights",
			'journal' => "Grand Warlord Sorcalin has authorized a reward for any Dominion soldier who slays 20 enemy Dragonknights.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2612 => array(
			'name'    => "The Elder Scroll of Ni-Mohk",
			'journal' => "The Pact plans to steal the <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6421 => array(
			'name'    => "Helping the Healers",
			'journal' => "A rogue Goutfang monk trains students in forbidden techniques where a rare herb grows. I agreed to help kill the monk, plus gather herbs to help heal Dragon hunters.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2963 => array(
			'name'    => "Capture Blue Road Farm",
			'journal' => "I accepted a mission to help capture Blue Road Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6352 => array(
			'name'    => "Pledge: Lair of Maarselok",
			'journal' => "<<1>> of the Undaunted has challenged me to explore the Lair of Maarselok.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2634 => array(
			'name'    => "The Elder Scroll of Ni-Mohk",
			'journal' => "Grand Warlord Dortene reports that the Elder Scroll of Ni-Mohk is now in enemy hands.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2635 => array(
			'name'    => "The Elder Scroll of Alma Ruma",
			'journal' => "Grand Warlord Dortene reports that the Elder Scroll of Alma Ruma is being held by the enemy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4366 => array(
			'name'    => "To Mathiisen",
			'journal' => "Battlereeve Urcelmo has asked that I meet with Razum-dar near the town of Mathiisen. Raz is apparently investigating more threads in the Veiled Heritance plot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6356 => array(
			'name'    => "Dousing the Daedric Flame",
			'journal' => "Nisuzi's Baandari clan needs oil from Darkpool Mine, but a flock of Mehrunes Dagon-worshipers took control of the cavern-blocking access to the oil with hordes of newly summoned Daedra.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2638 => array(
			'name'    => "The Elder Scroll of Mnem",
			'journal' => "Grand Warlord Dortene asked for warriors to help capture the <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6132 => array(
			'name'    => "Buried Memories",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3117 => array(
			'name'    => "Capture Brindle Farm",
			'journal' => "I accepted a mission to help capture Brindle Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4105 => array(
			'name'    => "Kireth's Amazing Plan",
			'journal' => "Kireth Vanos has been exploring the ruins of Mzulft, looking for a valuable piece of forgotten lore. Her brother Raynor and her plan to use their findings to get invited into either Shad Astula or the Mages Guild.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2639 => array(
			'name'    => "The Elder Scroll of Ghartok",
			'journal' => "Covenant soldiers are seeking the <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6577 => array(
			'name'    => "Pledge: Black Drake Villa",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Black Drake Villa.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4338 => array(
			'name'    => "Eye of the Ancients",
			'journal' => "The Maormer struck <<1>>, and now lay waste to the town. Auridon troops tried and failed to retake the area, and now await reinforcements before trying again. I've been tasked with reactivating an ancient Aldmeri device on the nearby cliffs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3126 => array(
			'name'    => "Capture Chalman Mine",
			'journal' => "I accepted a mission to help capture Chalman Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2640 => array(
			'name'    => "The Elder Scroll of Chim",
			'journal' => "Covenant forces are gathering to capture the <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4887 => array(
			'name'    => "Back in Time",
			'journal' => "A novice from the Mages Guild named Gahgdar was duped by a witch into giving her a valuable book from the guildhall in Hallin's Stand. He asked me to help him get the book back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4130 => array(
			'name'    => "The Captain's Honor",
			'journal' => "Though the <<C:1>> had been destroyed, the Maormer still hold <<2>> captive at <<3>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2656 => array(
			'name'    => "The Elder Scroll of Mnem",
			'journal' => "The Elder Scroll of Mnem has been taken by enemy soldiers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3023 => array(
			'name'    => "Wicked Trade",
			'journal' => "Bloodthorn cultists attacked a group of Camlorn refugees. They kidnapped the group's women and dragged them into the Fens.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6125 => array(
			'name'    => "A Necessary Alliance",
			'journal' => "With the Ritemaster dead and the Heart of Transparent Law in Nocturnal's hands, events have taken a grim turn. I need to help the Psijic sages figure out a way to enter the Crystal Tower and stop Nocturnal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4256 => array(
			'name'    => "A Hostile Situation",
			'journal' => "At the request of the captain of the Vulkhel Watch, I've been running errands all over Auridon's port city. I even helped deal with a criminal named Fasion who claimed that Captain Astanya is working with a group called the Veiled Heritance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2658 => array(
			'name'    => "The Elder Scroll of Ghartok",
			'journal' => "Grand Warlord Sorcalin needs soldiers to capture the Elder Scroll of Ghartok.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5638 => array(
			'name'    => "Under Our Thumb",
			'journal' => "Some law-abiding citizens have been meddling in the affairs of the Thieves Guild. The guild wants its pickpockets to teach the meddlers to mind their own business. I should look for ornaments, writings, or other oddities, but any treasures will do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3017 => array(
			'name'    => "Back-Alley Murders",
			'journal' => "Someone's murdering Daggerfall's poor. A beggar named Matthew thinks he's going to be the next one to die. He asked me to find the killer before that happens.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2659 => array(
			'name'    => "The Elder Scroll of Chim",
			'journal' => "Plans are being made to capture the <<1>> from the enemy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5036 => array(
			'name'    => "Honrich Tower",
			'journal' => "<<1>> has been saved and Pinepeak is secure, but it seems Honrich Tower has been taken by the Worm Cult.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3207 => array(
			'name'    => "Capture Faregyl Farm",
			'journal' => "I accepted a mission to help capture Faregyl Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6628 => array(
			'name'    => "Relics and Rumblings",
			'journal' => "<<1>> asked me to help her recover relics that belonged to a Minotaur-hunter named Phalevon the Magnificent from the partially flooded depths of Undertow Cavern. Goblins and Minotaurs infest the cave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4251 => array(
			'name'    => "Trial of the Spirit",
			'journal' => "Widulf, a priest of Kyne, proctors the Trial of Spirit. It apparently involves drinking a somewhat poisonous mead and then running around the island. Widulf assures me the poison will connect me with my inner spirit.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2661 => array(
			'name'    => "The Elder Scroll of Alma Ruma",
			'journal' => "The <<1>> must be taken from the enemy and delivered into Dominion hands. So says Grand Warlord Sorcalin.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2672 => array(
			'name'    => "Scout Fort Warden",
			'journal' => "I have accepted a mission to travel to Fort Warden and scout that stronghold for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6481 => array(
			'name'    => "Daughter of the Wolf",
			'journal' => "Solitude has survived High King Svargrim's harrowstorm, but the damage done by his betrayal remains. Can Princess Svana pick up the pieces and carry on for the kingdom?",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3039 => array(
			'name'    => "Crocodile Bounty",
			'journal' => "I found a dead Orc in the bog. He carried a broadsheet, as well as a handful of sharp crocodile teeth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4242 => array(
			'name'    => "The Lich",
			'journal' => "A Shadowed Path necromancer has transformed into a lich in <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5171 => array(
			'name'    => "The Oldest Ghost",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6399 => array(
			'name'    => "Order of the New Moon",
			'journal' => "Sai Sahan wants me to infiltrate the Dragon cult and learn more about them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2673 => array(
			'name'    => "Scout Fort Rayles",
			'journal' => "I accepted a mission to scout Fort Rayles for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6392 => array(
			'name'    => "Kill Enemy Necromancers",
			'journal' => "Grand Warlord Sorcalin has authorized a reward for any Dominion soldier who slays 20 enemy Necromancers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2674 => array(
			'name'    => "Scout Fort Glademist",
			'journal' => "I accepted a mission to scout Fort Glademist for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4543 => array(
			'name'    => "Holes in the World",
			'journal' => "Mezha-dro, a Khajiit servant of Meridia, asked for help sealing tears in Oblivion before more material from Tamriel falls through.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5290 => array(
			'name'    => "Pledge: City of Ash I",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to explore the City of Ash, a Wood Elf village in Greenshade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2676 => array(
			'name'    => "Scout Fort Aleswell",
			'journal' => "I accepted a mission to scout Fort Aleswell for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6485 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3027 => array(
			'name'    => "Ripple Effect",
			'journal' => "The Mages Guild is studying the site of the Battle of Glenumbra Moors. It's said that Faolchu, the invader of Camlorn, was originally slain here. They hope to find out what killed him in the past.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2679 => array(
			'name'    => "Scout Arrius Keep",
			'journal' => "I accepted a mission to scout Arrius Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5040 => array(
			'name'    => "Taking Precautions",
			'journal' => "A Pact Mage named Sees-Many-Paths is examining the wards at Tormented Spire. She wants word sent to her egg-sister, Walks-in-Ash, that they are unstable.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3397 => array(
			'name'    => "Scout the Lumbermill",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2680 => array(
			'name'    => "Scout Kingscrest Keep",
			'journal' => "I accepted a mission to scout Kingscrest Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6489 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5660 => array(
			'name'    => "Contract: Grahtwood",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3047 => array(
			'name'    => "A Step Back in Time",
			'journal' => "The spirit of Alana Relin has led me to a crypt that will provide a doorway to the past. Now we'll finally learn how Faolchu was originally killed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2682 => array(
			'name'    => "Scout Farragut Keep",
			'journal' => "I accepted a mission to scout Farragut Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4158 => array(
			'name'    => "The Pride of a Prince",
			'journal' => "Stormfist spies have used Daedric alchemy to infiltrate Fort Amol. The King and the Pact delegates are in danger. I need to convince someone that the threat is real before it's too late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7418 => array(
			'name'    => "Sheogorath Takes a Holiday",
			'journal' => "Sheogorath, the Daedric Prince of Madness, wants to take a holiday in Tamriel as a mortal. His follower Noth is worried about the consequences. She asked me to serve as Sheogorath's guardian and oversee his holiday.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6087 => array(
			'name'    => "Run Aground",
			'journal' => "The Office of Divine Prosecution tasked me with killing Keelsplitter-a massive sea adder bred by the Sea Elves to hunt down High Elf ships. They also need me to recover intelligence logs from the ships Keelsplitter destroyed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2684 => array(
			'name'    => "Scout Drakelowe Keep",
			'journal' => "I accepted a mission to scout Drakelowe Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5720 => array(
			'name'    => "Sacrament: Smuggler's Den",
			'journal' => "I received a contract from the Speaker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4517 => array(
			'name'    => "Crown Point",
			'journal' => "Prefect Antias asked me to re-light the flame atop Crown Point tower.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3035 => array(
			'name'    => "Wyrd and Coven",
			'journal' => "To stop the hags, I need to defeat the coven's leader, Mother Murk. Otherwise, other refugees will be in danger.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3231 => array(
			'name'    => "Capture Warden Lumbermill",
			'journal' => "I accepted a mission to help capture Warden Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2685 => array(
			'name'    => "Scout Castle Alessia",
			'journal' => "I accepted a mission to scout Castle Alessia for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3224 => array(
			'name'    => "Capture Castle Alessia",
			'journal' => "I accepted a mission to help capture Castle Alessia for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2686 => array(
			'name'    => "Scout Castle Faregyl",
			'journal' => "I accepted a mission to scout Castle Faregyl for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2689 => array(
			'name'    => "Scout Castle Black Boot",
			'journal' => "I accepted a mission to scout Castle Black Boot for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5444 => array(
			'name'    => "Sorrow's Kiss",
			'journal' => "Lady Laurent and Stibbons have been hired by the House of Orsimer Glories to locate the shrine of Torug gro-Igron, which supposedly sits at the top of the mountain named Sorrow.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2842 => array(
			'name'    => "Scout Fort Glademist",
			'journal' => "I accepted a mission to scout Fort Glademist for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4288 => array(
			'name'    => "Blind Man's Bluff",
			'journal' => "A blind Redguard pirate is willing to share his treasure with anyone who can transport him to Anvil, even the Wood Elves harassing him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2690 => array(
			'name'    => "Scout Castle Bloodmayne",
			'journal' => "I accepted a mission to scout Castle Bloodmayne for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2693 => array(
			'name'    => "Scout Glademist Mine",
			'journal' => "I accepted a mission to scout Glademist Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2846 => array(
			'name'    => "Scout Chalman Keep",
			'journal' => "I accepted a mission to scout Chalman Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2698 => array(
			'name'    => "Scout Dragonclaw Mine",
			'journal' => "I accepted a mission to scout Dragonclaw Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6157 => array(
			'name'    => "A Rose's Beauty",
			'journal' => "The Divine Prosecution has tasked me to recover several blossoms of the Rose-Of-Archon flower, needed by Sapiarchs for their religious rites.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2699 => array(
			'name'    => "Scout Chalman Mine",
			'journal' => "I accepted a mission to scout Chalman Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4430 => array(
			'name'    => "The Burned Estate",
			'journal' => "The Shadowed Path has taken over an estate southwest of Cheydinhal. They are using Daedric stones to summon atronachs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2860 => array(
			'name'    => "Scout Glademist Mine",
			'journal' => "I accepted a mission to scout Glademist Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6150 => array(
			'name'    => "Sunhold Sundered",
			'journal' => "Kinlady Helenaere of Sunhold needs my help to fight back against a Maormer raiding party and rescue her city.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2700 => array(
			'name'    => "Scout Arrius Mine",
			'journal' => "I accepted a mission to scout Arrius Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4931 => array(
			'name'    => "Frightened Folk",
			'journal' => "Constable Agazu says the mayor of Fell's Run fired her for no reason and appointed a new constable. She says all the townspeople are acting strange as well, but no one's talking. She wants me to help find out what's going on.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4169 => array(
			'name'    => "Of Councils and Kings",
			'journal' => "Stormy-Eyes seems to be recovering from the Dreamstride experience. King Jorunn and Prince Irnskar have gone ahead to Jorunn's Stand, a large encampment near Skuldafn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2703 => array(
			'name'    => "Scout Blue Road Mine",
			'journal' => "I accepted a mission to scout Blue Road Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2848 => array(
			'name'    => "Scout Kingscrest Keep",
			'journal' => "I accepted a mission to scout Kingscrest Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3190 => array(
			'name'    => "Ash'abah Rising",
			'journal' => "I am now officially a member of the Ash'abah, and I am setting forth to destroy the Withered Hand. Talia suggested I visit Tu'whacca's Throne to see if the Withered Hand has been there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6154 => array(
			'name'    => "Pledge: Scalecaller Peak",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Scalecaller Peak.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2704 => array(
			'name'    => "Scout Drakelowe Mine",
			'journal' => "I accepted a mission to scout Drakelowe Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4951 => array(
			'name'    => "Fit to Rule",
			'journal' => "<<1>> must complete the ratification ceremony if her allies are to accept her as leader of the Aldmeri Dominion. At the heart of the ceremony is <<c:2>>, an ancient Ayleid artifact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5203 => array(
			'name'    => "The Serpent's Fang",
			'journal' => "A Wood Elf named Little Leaf claims she's on a mission to stop the Iron Orcs from building a troll army. She tried to appeal to the Orc leaders, but diplomacy didn't work. Now she wants me to come along and try a more forceful approach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2705 => array(
			'name'    => "Scout Alessia Mine",
			'journal' => "I accepted a mission to scout Alessia Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6576 => array(
			'name'    => "Burning Secrets",
			'journal' => "I found the Black Drake Villa, an old retreat for the Longhouse Emperors. The young adventurer Eveli Sharp-Arrow enlisted my help in searching for an important book hidden away inside.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2858 => array(
			'name'    => "Scout Warden Mine",
			'journal' => "I accepted a mission to scout Warden Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2706 => array(
			'name'    => "Scout Faregyl Mine",
			'journal' => "I accepted a mission to scout Faregyl Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4575 => array(
			'name'    => "The Flooded Grove",
			'journal' => "I'm helping the Wooded Eye prevent Daedra from escaping Rootwater Grove into Greenshade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5329 => array(
			'name'    => "For King and Glory",
			'journal' => "King Kurog of Orsinium struggles to defeat the Winterborn threat. His mother, Forge-Mother Alga, asked me to go to Frostbreak Fortress to aid the king.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6165 => array(
			'name'    => "The Abyssal Cabal",
			'journal' => "A Battlereeve sent a group of adventurers and a squad of marines to determine the extent of the Sea Sload's incursion into Summerset. After an initial report, the Battlereeve lost contact with the group. She asked me to investigate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2707 => array(
			'name'    => "Scout Roebeck Mine",
			'journal' => "I accepted a mission to scout Roebeck Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5482 => array(
			'name'    => "The Sublime Brazier",
			'journal' => "Molag Bal seeks to destroy the Sublime Brazier-the key to rekindling the Dragonfires. Working with The Drake of Blades, Captain Caudex, and Father Egnatius, I must find a way to protect the Dragonfire Cathedral, and the brazier within.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2868 => array(
			'name'    => "Scout Blue Road Mine",
			'journal' => "I accepted a mission to scout Blue Road Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6158 => array(
			'name'    => "Relic Runaround",
			'journal' => "The Divine Prosecution has tasked me to recover several stolen relics from a group known as the Sagacians. In addition, I'll need to collect any plans I find outlining the group's movements.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2708 => array(
			'name'    => "Scout Brindle Mine",
			'journal' => "I accepted a mission to scout Brindle Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3400 => array(
			'name'    => "Scout Fort Balfiera",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2709 => array(
			'name'    => "Scout Black Boot Mine",
			'journal' => "I accepted a mission to scout Black Boot Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2862 => array(
			'name'    => "Scout Aleswell Mine",
			'journal' => "I accepted a mission to scout Aleswell Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4203 => array(
			'name'    => "Lifeline",
			'journal' => "Thane Oda Wolf-Sister believes that the Ternion monks might be able to heal King Jorunn. But their complex rituals require special components.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3189 => array(
			'name'    => "Hidden in Flames",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2856 => array(
			'name'    => "Scout Castle Black Boot",
			'journal' => "I accepted a mission to scout Castle Black Boot for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2711 => array(
			'name'    => "Scout Farm 1",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5072 => array(
			'name'    => "Aid for Bramblebreach",
			'journal' => "A Hollow Guardian accosted me deep in the Valenwood. He asked me to go north and offer my assistance to the Bramblebreach clan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2714 => array(
			'name'    => "Scout Farm 4",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4953 => array(
			'name'    => "Trouble at the Tree",
			'journal' => "I was approached shortly after saving <<1>> from the Jackdaw attack. <<2>> approached me and revealed she works for the Wood Elf king. She said he needs the aid of impressive people such as myself.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2721 => array(
			'name'    => "Scout Warden Farm",
			'journal' => "I accepted a mission to scout Warden Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5476 => array(
			'name'    => "Awaken the Past",
			'journal' => "The House of Orsimer Glories, a museum in the rebuilt city of Orsinium, seeks to fill its displays with relics and other items of historic importance to both the Orcs and the land of Wrothgar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5315 => array(
			'name'    => "Alchemist Certification",
			'journal' => "If I can prove I know my way around an alchemy table, <<1>> will certify me as an alchemist.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2874 => array(
			'name'    => "Scout Black Boot Mine",
			'journal' => "I accepted a mission to scout Black Boot Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4320 => array(
			'name'    => "Fierce Beasts of Ivarstead",
			'journal' => "Nilwen is living in a shack outside Ivarstead. She tells of fierce beasts nearby and needs someone to slay them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7204 => array(
			'name'    => "The Wing of the Netch",
			'journal' => "I picked up the Fable of the Netch as my next step to restoring the Scholarium. I'll need to uncover its secrets to open the Netch door, meet the Luminary within, and amplify the power of the Scribing Altar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2722 => array(
			'name'    => "Scout Rayles Farm",
			'journal' => "I accepted a mission to scout Rayles Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2723 => array(
			'name'    => "Scout Glademist Farm",
			'journal' => "I accepted a mission to scout Glademist Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5100 => array(
			'name'    => "Trial of the True Path",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5415 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2726 => array(
			'name'    => "Scout Aleswell Farm",
			'journal' => "I accepted a mission to scout Aleswell Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2727 => array(
			'name'    => "Scout Dragonclaw Farm",
			'journal' => "I accepted a mission to scout Dragonclaw Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2872 => array(
			'name'    => "Scout Roebeck Mine",
			'journal' => "I accepted a mission to scout Roebeck Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2728 => array(
			'name'    => "Scout Chalman Farm",
			'journal' => "I accepted a mission to scout Chalman Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3086 => array(
			'name'    => "Capture Fort Ash",
			'journal' => "I accepted a mission to help capture Fort Ash for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5236 => array(
			'name'    => "Souls of the Betrayed",
			'journal' => "A devout knight of the Divines discovered that the dead have awakened in the depths of Skyreach Catacombs. If the legend of Virmaril is true, then a powerful Elven necromancer could be the source of the trouble.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2730 => array(
			'name'    => "Scout Kingscrest Farm",
			'journal' => "I accepted a mission to scout Kingscrest Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2732 => array(
			'name'    => "Scout Blue Road Farm",
			'journal' => "I accepted a mission to scout Blue Road Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4963 => array(
			'name'    => "Passage Denied",
			'journal' => "Redguard forces are massing inside an old Ayleid ruin, and the Dominion believes they've opened up a magical gateway from Alik'r to transport their troops into the heart of Grahtwood unopposed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2734 => array(
			'name'    => "Scout Alessia Farm",
			'journal' => "I accepted a mission to scout Alessia Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4587 => array(
			'name'    => "Trail of the Skin-Stealer",
			'journal' => "The vicecanons of Shadowfen tracked the Dominion's forces to the ancient Daedric ruin of <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6186 => array(
			'name'    => "Moonlight Ascent",
			'journal' => "Adonatus Varian, member of the Silver Dawn, has tasked me with taking down the werewolf pack within Moon Hunter Keep. I'll need to work my way through the keep in order to reach the pack's leader, Vykosa.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3581 => array(
			'name'    => "Cleansing the Past",
			'journal' => "Priestess Brela believes the tormented spirits of a Dark Elf army haunt Vivec's Antlers. They died in a great conflict with the Akaviri. She was captured while consecrating their bones.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2736 => array(
			'name'    => "Scout Faregyl Farm",
			'journal' => "I accepted a mission to scout Faregyl Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4306 => array(
			'name'    => "Finding Winter's Hammer",
			'journal' => "Sage Svari has asked me to recover <<1>> from bandits at Fort Greenwall.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3094 => array(
			'name'    => "Capture Drakelowe Keep",
			'journal' => "I accepted a mission to help recapture Drakelowe Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5235 => array(
			'name'    => "Defend Fort Warden",
			'journal' => "Defend this keep to the last warrior standing! Kill any who dare assault Fort Warden!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2737 => array(
			'name'    => "Scout Roebeck Farm",
			'journal' => "I accepted a mission to scout Roebeck Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2738 => array(
			'name'    => "Scout Brindle Farm",
			'journal' => "I accepted a mission to scout Brindle Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4607 => array(
			'name'    => "Castle of the Worm",
			'journal' => "I must find a way into the Castle of the Worm. There will be many dangers ahead, so I must make sure I'm prepared.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6197 => array(
			'name'    => "The Shattered Staff",
			'journal' => "An artifact known as the Staff of Towers has returned to Tamriel. Loremaster Celarus urged me to recover four pieces of the staff.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2739 => array(
			'name'    => "Scout Black Boot Farm",
			'journal' => "I accepted a mission to scout Black Boot Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6190 => array(
			'name'    => "A Time for Mud and Mushrooms",
			'journal' => "My allies in the Psijic Order discovered another cluster of time breaches in the northernmost regions of Black Marsh. I need to seal them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2740 => array(
			'name'    => "Scout Bloodmayne Farm",
			'journal' => "I accepted a mission to scout Bloodmayne Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3093 => array(
			'name'    => "Capture Blue Road Keep",
			'journal' => "I accepted a mission to help recapture Blue Road Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5462 => array(
			'name'    => "The Ashes of Our Fathers",
			'journal' => "<<1>> and <<2>> discovered a lost tomb, but they disagree on who rests within.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2743 => array(
			'name'    => "Scout Glademist Lumbermill",
			'journal' => "I accepted a mission to scout Glademist Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5104 => array(
			'name'    => "The Shards of Wuuthrad",
			'journal' => "The Worm Cult has stolen the legendary axe Wuuthrad and plans to perform dark deeds with it. There are even rumors it was shattered, in which case we must find the shards of Wuuthrad wherever they may be.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6194 => array(
			'name'    => "A Breach Amid the Trees",
			'journal' => "Loremaster Celarus charged me with sealing more time breaches for the Psijic Order-this time in the Wood Elf homeland of Valenwood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3461 => array(
			'name'    => "Kill Thirty Ebonheart Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2744 => array(
			'name'    => "Scout Ash Lumbermill",
			'journal' => "I accepted a mission to scout Ash Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3102 => array(
			'name'    => "Capture Rayles Farm",
			'journal' => "I accepted a mission to help capture Rayles Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7228 => array(
			'name'    => "The Lord of Fear",
			'journal' => "The Daedric Lord of Fear, Hollowjack, is once again active in Nirn. I met a priest of Stendarr named Maxien Roudille who told me the Daedric Lord is torturing his victims with fear and terror. I agreed to help stop him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2746 => array(
			'name'    => "Scout Dragonclaw Lumbermill",
			'journal' => "I accepted a mission to scout Dragonclaw Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4101 => array(
			'name'    => "Payment In Kind",
			'journal' => "A recent conflict between the Wood Elves and Drublog Orcs of Bloodtoil Valley threatens to end in bloodshed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7361 => array(
			'name'    => "The Hunt for Dakoi",
			'journal' => "I met an Argonian angler in Stormhold frustrated over not being able to catch the Dakoi, a special fish that's said to appear every New Life festival and bring good fortune. I agreed to hunt and catch the Dakoi for him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2747 => array(
			'name'    => "Scout Chalman Lumbermill",
			'journal' => "I accepted a mission to scout Chalman Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3440 => array(
			'name'    => "Wayward Scouts",
			'journal' => "A Lion Guard scouting party, made up mostly of new recruits, has gone missing near Farwatch Tower.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2749 => array(
			'name'    => "Scout Kingscrest Lumbermill",
			'journal' => "I accepted a mission to scout Kingscrest Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5175 => array(
			'name'    => "Iron and Scales",
			'journal' => "I met an Iron Orc shaman who had been banished by a younger rival. What's more, this rival, named Agganor, seems to be responsible for the Iron Orc and Scaled Court alliance. The old shaman wants me to avenge him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4348 => array(
			'name'    => "A Graveyard of Ships",
			'journal' => "The pirate captain One-Eye and her crew reached Coldharbour through a whirlpool while searching for a legend called the Lost Fleet. They seem intent on investigating the nearby wrecks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2750 => array(
			'name'    => "Scout Farragut Lumbermill",
			'journal' => "I accepted a mission to scout Farragut Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3101 => array(
			'name'    => "Capture Warden Farm",
			'journal' => "I accepted a mission to help capture Warden Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6193 => array(
			'name'    => "Checking on Cloudrest",
			'journal' => "I met a commoner named Vandoril in Alinor who's had troubling dreams about the city of Cloudrest. He's requested I seek out his mother Belarin and ensure her safety.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2751 => array(
			'name'    => "Scout Blue Road Lumbermill",
			'journal' => "I accepted a mission to scout Blue Road Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4466 => array(
			'name'    => "The Broken Spearhead",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4903 => array(
			'name'    => "Dream-Walk Into Darkness",
			'journal' => "I need to locate Verandis Ravenwatch and find out if he can tell me more about Baron Montclair and where he might be hiding. He may also know more about what set Montclair on this path of destruction.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3238 => array(
			'name'    => "Capture Chalman Lumbermill",
			'journal' => "I accepted a mission to help capture Chalman Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6465 => array(
			'name'    => "Meridia's Brilliance",
			'journal' => "A strange storm has devastated the area around Kilkreath Temple. Now the priests of the temple want my help to find the priests and pilgrims that were lost in the storm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4226 => array(
			'name'    => "Death to the Black Daggers!",
			'journal' => "<<1>> asked me to travel to <<2>> and take revenge on the Black Dagger bandits for the deaths of her friends.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2752 => array(
			'name'    => "Scout Drakelowe Lumbermill",
			'journal' => "I accepted a mission to scout Drakelowe Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7236 => array(
			'name'    => "Pledge: Exiled Redoubt",
			'journal' => "Urgarlag Chief-bane of the Undaunted has challenged me to explore Exiled Redoubt.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2754 => array(
			'name'    => "Scout Faregyl Lumbermill",
			'journal' => "I accepted a mission to scout Faregyl Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5499 => array(
			'name'    => "Wrecked",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5434 => array(
			'name'    => "Battleground",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2755 => array(
			'name'    => "Scout Roebeck Lumbermill",
			'journal' => "I accepted a mission to scout Roebeck Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2916 => array(
			'name'    => "Capture Fort Rayles",
			'journal' => "I accepted a mission to help capture Fort Rayles for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5681 => array(
			'name'    => "Contract: Shadowfen",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2757 => array(
			'name'    => "Scout Black Boot Lumbermill",
			'journal' => "I accepted a mission to scout Black Boot Lumbermill  for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6329 => array(
			'name'    => "Spider Demonstration",
			'journal' => "Zahari asked me to help train new recruits for the Khajiit Defense Legion to fight enemies they may not have encountered in training.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6012 => array(
			'name'    => "Kill Enemy Wardens",
			'journal' => "Grand Warlord Dortene has authorized a reward for any Covenant soldier who slays 20 enemy Wardens.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2759 => array(
			'name'    => "Kill Enemy Players",
			'journal' => "Grand Warlord Dortene has authorized a bonus for any Covenant soldier who slays 20 enemy combatants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3390 => array(
			'name'    => "Capture Fort Balfiera",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3237 => array(
			'name'    => "Capture Dragonclaw Lumbermill",
			'journal' => "I accepted a mission to help capture Dragonclaw Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2904 => array(
			'name'    => "Scout Roebeck Lumbermill",
			'journal' => "I accepted a mission to scout Roebeck Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3254 => array(
			'name'    => "Capture Aleswell Mine",
			'journal' => "I accepted a mission to help capture Aleswell Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4648 => array(
			'name'    => "The Summoner Division",
			'journal' => "I am competing in the Summoner Division of Thizzrini Arena. I need to work my way through the ranks in this division before I can challenge Tholbor, the Division Champion, in the main arena.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5493 => array(
			'name'    => "City on the Brink",
			'journal' => "The Daggerfall Covenant is locked in an epic struggle for control of the Imperial City. The forces of Molag Bal hold the city in an iron grip, but enemy bannermen also vie for power. The Covenant needs my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6218 => array(
			'name'    => "Jewelry Crafting Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Jewelry to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2768 => array(
			'name'    => "Scout Fort Rayles",
			'journal' => "I accepted a mission to scout Fort Rayles for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5015 => array(
			'name'    => "Eyes of the Enemy",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2770 => array(
			'name'    => "Scout Fort Ash",
			'journal' => "I accepted a mission to scout Fort Ash for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6222 => array(
			'name'    => "Morrowind Gate Crasher",
			'journal' => "I accepted a mission to help capture Farragut Keep, Arrius Keep, and Kingscrest Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2772 => array(
			'name'    => "Scout Fort Dragonclaw",
			'journal' => "I accepted a mission to scout Fort Dragonclaw for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4365 => array(
			'name'    => "To Dawnbreak",
			'journal' => "I revealed High Kinlady Estre is the Veiled Queen. Queen Ayrenn wants me to make sure Dawnbreak is safe for her to continue her campaign.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3464 => array(
			'name'    => "Scout the Lumbermill",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2773 => array(
			'name'    => "Scout Chalman Keep",
			'journal' => "I accepted a mission to scout Chalman Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6644 => array(
			'name'    => "A Proper Blessing",
			'journal' => "Deetum-Jas's friend Leric has been having a terrible run of bad luck. He asked me to gather the components necessary to give him a proper Argonian blessing to change his fortune.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2926 => array(
			'name'    => "Capture Drakelowe Keep",
			'journal' => "I accepted a mission to help capture Drakelowe Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4267 => array(
			'name'    => "All's Fair",
			'journal' => "The Rothalens have fallen on hard times. Sarisa has designs on Bar-Neeus's stall in the central market.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5622 => array(
			'name'    => "Contract: Alik'r Desert",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3253 => array(
			'name'    => "Capture Ash Mine",
			'journal' => "I accepted a mission to help capture Ash Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2920 => array(
			'name'    => "Capture Fort Dragonclaw",
			'journal' => "I accepted a mission to help capture Fort Dragonclaw for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2775 => array(
			'name'    => "Scout Kingscrest Keep",
			'journal' => "I accepted a mission to scout Kingscrest Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2777 => array(
			'name'    => "Scout Blue Road Keep",
			'journal' => "I accepted a mission to scout Blue Road Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6648 => array(
			'name'    => "Shattered and Scattered",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2930 => array(
			'name'    => "Capture Castle Brindle",
			'journal' => "I accepted a mission to help recapture Castle Brindle for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4287 => array(
			'name'    => "Ritual at the Dragonshrine",
			'journal' => "The Worm Cult is up at the top of the dragonshrine. I'm not sure what they're doing, but it can't be good.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7260 => array(
			'name'    => "Sunport Seeks Adventurers!",
			'journal' => "I found a notice posted by the Regency of Sunport. They seek adventurers willing to take on dangerous work. I should find <<1>> to learn more.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2778 => array(
			'name'    => "Scout Drakelowe Keep",
			'journal' => "I accepted a mission to scout Drakelowe Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5976 => array(
			'name'    => "A Masterful Shield",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4261 => array(
			'name'    => "Sever All Ties",
			'journal' => "In order to save Firsthold and all of Auridon from High Kinlady Estre, the Oblivion portals must be closed and Estre defeated for good.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2779 => array(
			'name'    => "Scout Castle Alessia",
			'journal' => "I accepted a mission to scout Castle Alessia for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5831 => array(
			'name'    => "Madness in the Rift",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from the Rift.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2940 => array(
			'name'    => "Capture Arrius Mine",
			'journal' => "I accepted a mission to help capture Arrius Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4915 => array(
			'name'    => "Blood Hunt",
			'journal' => "A group of vampire hunters has come to this area to cleanse it of its infestation, including a powerful vampire lord. They've asked for my assistance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3441 => array(
			'name'    => "Capture Fort Balfiera",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6493 => array(
			'name'    => "The Strength of Giants",
			'journal' => "<<1>>, of the Coterie of Organized Scholars, asked me to help him complete an alchemical tonic that will imbue those who drink it with the strength of Giants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4254 => array(
			'name'    => "Those She Devours",
			'journal' => "Necromancer Gullveig has captives imprisoned at the Worm Cult camp. I should try to rescue them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2780 => array(
			'name'    => "Scout Castle Faregyl",
			'journal' => "I accepted a mission to scout Castle Faregyl for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3472 => array(
			'name'    => "Kill an Aldmeri Guard.",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2781 => array(
			'name'    => "Scout Castle Roebeck",
			'journal' => "I accepted a mission to scout Castle Roebeck for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2934 => array(
			'name'    => "Capture Rayles Mine",
			'journal' => "I accepted a mission to help capture Rayles Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2782 => array(
			'name'    => "Scout Castle Brindle",
			'journal' => "I accepted a mission to scout Castle Brindle for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3261 => array(
			'name'    => "Capture Drakelowe Mine",
			'journal' => "I accepted a mission to help capture Drakelowe Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2928 => array(
			'name'    => "Capture Castle Faregyl",
			'journal' => "I accepted a mission to help recapture Castle Faregyl for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4663 => array(
			'name'    => "Out for a Walk",
			'journal' => "<<1>> is out in the wilds, looking for his son, Donus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6225 => array(
			'name'    => "Elsweyr Gate Reclaimation",
			'journal' => "I accepted a mission to help reclaim Castle Faregyl, Castle Black Boot, and Castle Bloodmayne for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2783 => array(
			'name'    => "Scout Castle Black Boot",
			'journal' => "I accepted a mission to scout Castle Black Boot for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4498 => array(
			'name'    => "Waypoint Test",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7268 => array(
			'name'    => "Black Worm Relics",
			'journal' => "Lector Volonaro asked me to enter a cave called the Lair of the Black Worm located on the eastern side of the island. Once inside, I should gather Worm Cult relics for the College of Tomes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2786 => array(
			'name'    => "Scout Warden Mine",
			'journal' => "I accepted a mission to scout Warden Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5403 => array(
			'name'    => "Sap and Stone",
			'journal' => "The peaceful Argonians of the Su-Zahleel tribe have been abducted and pressed into slavery by the Xit-Xaht-warrior fanatics that have been driven mad by their psychotic Hist. I must rescue the captives and put an end to the Xit-Xaht threat.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3449 => array(
			'name'    => "Scout the Mine",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5306 => array(
			'name'    => "Pledge: Blessed Crucible",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to survive the Blessed Crucible, an unforgiving arena deep in The Rift.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2788 => array(
			'name'    => "Scout Glademist Mine",
			'journal' => "I accepted a mission to scout Glademist Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5479 => array(
			'name'    => "A Cold Wind From the Mountain",
			'journal' => "I accomplished much since coming to Wrothgar. Now, it seems <<1>> has one final request from the House of Orsimer Glories.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2790 => array(
			'name'    => "Scout Aleswell Mine",
			'journal' => "I accepted a mission to scout Aleswell Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7267 => array(
			'name'    => "Samples Needed",
			'journal' => "Lector Volonaro asked me to gather samples of a Daedric planta called letifer orca for the College of Rituals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5926 => array(
			'name'    => "Relics of Maelkashishi",
			'journal' => "Numani-Rasi of the Urshilaku Ashlander tribe has requested that I explore the ruins of the Daedric shrine Maelkashishi and gather as many historic relics as I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2793 => array(
			'name'    => "Scout Chalman Mine",
			'journal' => "I accepted a mission to scout Chalman Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2818 => array(
			'name'    => "Scout Roebeck Farm",
			'journal' => "I accepted a mission to scout Roebeck Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2794 => array(
			'name'    => "Scout Arrius Mine",
			'journal' => "I accepted a mission to scout Arrius Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6246 => array(
			'name'    => "Sunken Treasure",
			'journal' => "An explorer named Famia Mercius is recruiting adventurers to join her on an expedition into a sunken Argonian xanmeer. Ancient treasures and mortal danger both await within this mysterious ruin.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2796 => array(
			'name'    => "Scout Farragut Mine",
			'journal' => "I accepted a mission to scout Farragut Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5027 => array(
			'name'    => "A Change of Heart",
			'journal' => "<<1>>, the betrothed of Arlie Edrald, has asked for my assistance in locating the Edrald family. He returned to prepare for his wedding to find that they had mysteriously vanished from their estate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2798 => array(
			'name'    => "Scout Drakelowe Mine",
			'journal' => "I accepted a mission to scout Drakelowe Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3149 => array(
			'name'    => "Capture Blue Road Lumbermill",
			'journal' => "I accepted a mission to help capture Blue Road Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2816 => array(
			'name'    => "Scout Alessia Farm",
			'journal' => "I accepted a mission to scout Alessia Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4265 => array(
			'name'    => "The High Cost of Lying",
			'journal' => "Lliae asked me to trigger a Black Dagger ambush north of Chorrol.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2799 => array(
			'name'    => "Scout Alessia Mine",
			'journal' => "I accepted a mission to scout Alessia Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6250 => array(
			'name'    => "Pledge: Frostvault",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Frostvault.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2800 => array(
			'name'    => "Scout Roebeck Mine",
			'journal' => "I accepted a mission to scout Roebeck Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2802 => array(
			'name'    => "Scout Black Boot Mine",
			'journal' => "I accepted a mission to scout Black Boot Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6261 => array(
			'name'    => "The Winds of Kyne",
			'journal' => "Jee-Lar's associates in Murkmire need accurate weather predictions to plan their expeditions. Luckily, a priestess of Kyne named Hodyette can supply them with accurate forecasts. I can find this mysterious prophetess in Eastmarch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2803 => array(
			'name'    => "Scout Bloodmayne Mine",
			'journal' => "I accepted a mission to scout Bloodmayne Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5450 => array(
			'name'    => "Invitation to Orsinium",
			'journal' => "I received an invitation to go to Wrothgar and present myself to Forge-Mother Alga in the city of Orsinium. It appears that the Orcs of Wrothgar need some sort of assistance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2836 => array(
			'name'    => "Scout Roebeck Lumbermill",
			'journal' => "I accepted a mission to scout Roebeck Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3496 => array(
			'name'    => "Scavenging for a Scarab",
			'journal' => "I have run into a strange man called <<1>> who was injured. He asked me to search Imperials for documents which are important.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2805 => array(
			'name'    => "Scout Rayles Farm",
			'journal' => "I accepted a mission to scout Rayles Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6548 => array(
			'name'    => "The Awakening Darkness",
			'journal' => "Arana believes that her former clan knows about dark Reach magic and old prophecies. If I can find the Ghostsong clan and convince the witches to share their lore, I may learn what Lady Belain plans for the Reach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2830 => array(
			'name'    => "Scout Kingscrest Lumbermill",
			'journal' => "I accepted a mission to scout Kingscrest Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4171 => array(
			'name'    => "The Rise of Sage Svari",
			'journal' => "Skald Svari has been proposed as the next Sage of Fallowstone Hall. She must pray at Fallowstone's shrines to ask the approval of the Companion spirits. I've been asked to accompany her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7471 => array(
			'name'    => "Protecting the Besieged",
			'journal' => "<<1>> asked me to help patrol and defend the Stirk Fellowship siege camps located along the Writhing Wall from Worm Cult and Daedric attacks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2806 => array(
			'name'    => "Scout Glademist Farm",
			'journal' => "I accepted a mission to scout Glademist Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5045 => array(
			'name'    => "The Fate of a Friend",
			'journal' => "I met an Ashlander hunter named Shuldrashi looking for her Argonian friend.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3157 => array(
			'name'    => "Kill Enemy Players",
			'journal' => "Grand Warlord Zimmeron has authorized a bonus for any Pact soldier who slays 20 enemy combatants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2824 => array(
			'name'    => "Scout Glademist Lumbermill",
			'journal' => "I accepted a mission to scout Glademist Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2807 => array(
			'name'    => "Scout Ash Farm",
			'journal' => "I accepted a mission to scout Ash Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6258 => array(
			'name'    => "Empty Nest",
			'journal' => "The influential Bright-Throat tribe broker alliances with their neighbors through a complex system of bonding rituals. It is a delicate time for the tribe and it seems that this season's preparations have been plagued by unforeseen complications.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2808 => array(
			'name'    => "Scout Aleswell Farm",
			'journal' => "I accepted a mission to scout Aleswell Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4401 => array(
			'name'    => "Lighthouse Attack Plans",
			'journal' => "I've discovered the Ember-Eye's plans for an assault on Koeglin Lighthouse. I should bring these to Koeglin Village.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7292 => array(
			'name'    => "Tides of the Heart",
			'journal' => "Shell-Tide Village is under duress from the presence of the Worm Cult. In return for aiding in a ceremony to cleanse the tides, the village's tide-reader has promised to send warriors to aid the Stirk Fellowship.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2810 => array(
			'name'    => "Scout Chalman Farm",
			'journal' => "I accepted a mission to scout Chalman Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6008 => array(
			'name'    => "Ashlander Relations",
			'journal' => "The Zainab tribe has encouraged other Ashlanders to seek a positive relationship with outlanders. Because I've shown respect for the Ashlander culture, I've been encouraged to help build that relationship.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4165 => array(
			'name'    => "The Dark Night of the Soul",
			'journal' => "<<1>> must take root in Treehenge before the Worm Cult can corrupt her soul.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2811 => array(
			'name'    => "Scout Arrius Farm",
			'journal' => "I accepted a mission to scout Arrius Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5863 => array(
			'name'    => "Haunted Grounds",
			'journal' => "A group of ghost hunters hired me to protect them on their current job. They've been contracted to clear out a fortress to the north that's infested with spirits. I need to keep them safe while they do whatever they do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2844 => array(
			'name'    => "Scout Fort Aleswell",
			'journal' => "I accepted a mission to scout Fort Aleswell for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3345 => array(
			'name'    => "The End of Extortion",
			'journal' => "Red Rook enforcers are extorting gold from the citizens of Crosswych. They claim it's to pay for the protection they're providing, but that's just an excuse.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4286 => array(
			'name'    => "Pinepeak Caverns",
			'journal' => "A group of researchers were ambushed by the Worm Cult at Pinepeak Caverns.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2812 => array(
			'name'    => "Scout Kingscrest Farm",
			'journal' => "I accepted a mission to scout Kingscrest Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2813 => array(
			'name'    => "Scout Farragut Farm",
			'journal' => "I accepted a mission to scout Farragut Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5239 => array(
			'name'    => "Dawn of the Exalted Viper",
			'journal' => "I met Titus Valerius in Belkarth. He was hoping to find Kelmen to assist him and the Thief with a ritual to locate a magical disturbance near Skyreach, but the mage is nowhere to be found.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2838 => array(
			'name'    => "Scout Black Boot Lumbermill",
			'journal' => "I accepted a mission to scout Black Boot Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2814 => array(
			'name'    => "Scout Blue Road Farm",
			'journal' => "I accepted a mission to scout Blue Road Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6257 => array(
			'name'    => "Bug Off!",
			'journal' => "Jee-Lar needs bug repellent candles for Cyrodilic Collections' explorers in Murkmire. He requested that I gather materials so his contact, Tamira Dobar, can create these candles.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2815 => array(
			'name'    => "Scout Drakelowe Farm",
			'journal' => "I accepted a mission to scout Drakelowe Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4530 => array(
			'name'    => "Goblin's Delight",
			'journal' => "I agreed to help <<1>> kill a Goblin that <<2>> has been feeding.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2832 => array(
			'name'    => "Scout Blue Road Lumbermill",
			'journal' => "I accepted a mission to scout Blue Road Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2817 => array(
			'name'    => "Scout Faregyl Farm",
			'journal' => "I accepted a mission to scout Faregyl Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2819 => array(
			'name'    => "Scout Brindle Farm",
			'journal' => "I accepted a mission to scout Brindle Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4684 => array(
			'name'    => "Congratulations!",
			'journal' => "Apphia Matia asked me to let Brielus Colus of Cheydinhal know that she is bearing his child.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3128 => array(
			'name'    => "Capture Kingscrest Mine",
			'journal' => "I accepted a mission to help capture Kingscrest Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2821 => array(
			'name'    => "Scout Bloodmayne Farm",
			'journal' => "I accepted a mission to scout Bloodmayne Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6280 => array(
			'name'    => "Art of the Nisswo",
			'journal' => "Nisswo Uaxal, a priest of Sithis, wishes to create a monument to honor Sithis and his people's connection to the concept of change.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4431 => array(
			'name'    => "Buried Secrets",
			'journal' => "A researcher by the name of Neramo is exploring the Dwemer ruins of Bthzark, looking for a way inside.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2825 => array(
			'name'    => "Scout Ash Lumbermill",
			'journal' => "I accepted a mission to scout Ash Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2827 => array(
			'name'    => "Scout Dragonclaw Lumbermill",
			'journal' => "I accepted a mission to scout Dragonclaw Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3503 => array(
			'name'    => "Generic Defend NPC Event",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2829 => array(
			'name'    => "Scout Arrius Lumbermill",
			'journal' => "I accepted a mission to scout Arrius Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6559 => array(
			'name'    => "Halt the Harrowstorms",
			'journal' => "<<1>> charged me with finding allies to assault the harrowstorm ritual sites springing up across the Reach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2833 => array(
			'name'    => "Scout Drakelowe Lumbermill",
			'journal' => "I accepted a mission to scout Drakelowe Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2835 => array(
			'name'    => "Scout Faregyl Lumbermill",
			'journal' => "I accepted a mission to scout Faregyl Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6547 => array(
			'name'    => "The Study of Souls",
			'journal' => "Count Verandis Ravenwatch tracked Lady Belain and Rada al-Saran to a mysterious Dwarven edifice. But with the keystone in hand, our enemies have a clear advantage. Braving this ruin and learning its secrets may be our best hope of stopping them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2837 => array(
			'name'    => "Scout Brindle Lumbermill",
			'journal' => "I accepted a mission to scout Brindle Lumbermill  for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4694 => array(
			'name'    => "Word from the Throne",
			'journal' => "The dead have risen at Tu'whacca's Throne. Throne Keeper Farvard insists we consult the king before taking any action. I have agreed to take word back to the palace in Sentinel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2840 => array(
			'name'    => "Scout Fort Warden",
			'journal' => "I accepted a mission to scout Fort Warden for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3204 => array(
			'name'    => "Capture Blue Road Farm",
			'journal' => "I accepted a mission to help capture Blue Road Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5057 => array(
			'name'    => "Bad Medicine",
			'journal' => "The virulent Llodos plague spreads throughout Deshaan, and the town known as the Serk suffers under the effects of the ravaging disease. A group known as the Maulborn arrived to help, but some of the townsfolk are dubious about their true intentions.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6567 => array(
			'name'    => "The Tainted Briarheart",
			'journal' => "A Reach clan called the Thornroot kidnapped members of a nearby clan, the Wildblood. Now the Thornroot intend to engage in a ritual that will turn their unwilling captives into briarhearts. I need to stop them from performing the ritual.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2841 => array(
			'name'    => "Scout Fort Rayles",
			'journal' => "I accepted a mission to scout Fort Rayles for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2843 => array(
			'name'    => "Scout Fort Ash",
			'journal' => "I accepted a mission to scout Fort Ash for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2845 => array(
			'name'    => "Scout Fort Dragonclaw",
			'journal' => "I accepted a mission to scout Fort Dragonclaw for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2847 => array(
			'name'    => "Scout Arrius Keep",
			'journal' => "I accepted a mission to scout Arrius Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3212 => array(
			'name'    => "Capture Fort Warden",
			'journal' => "I accepted a mission to help recapture Fort Warden for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5443 => array(
			'name'    => "Thukhozod the Eternal",
			'journal' => "I met a pair of adventurers outside a ruin who offered me a share of their gold if I help them complete a job. They've been hired to find a missing young Orc inside the ruins known as Thukhozod's Sanctum.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2849 => array(
			'name'    => "Scout Farragut Keep",
			'journal' => "I accepted a mission to scout Farragut Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2855 => array(
			'name'    => "Scout Castle Brindle",
			'journal' => "I accepted a mission to scout Castle Brindle for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4810 => array(
			'name'    => "Nirnroot Wine",
			'journal' => "I agreed to gather Red Mountain Ash for <<1>>, so he can make Nirnroot wine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3220 => array(
			'name'    => "Capture Kingscrest Keep",
			'journal' => "I accepted a mission to help capture Kingscrest Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6312 => array(
			'name'    => "Strange Messengers",
			'journal' => "I agreed to retrieve mysterious Akaviri relics from the Tomb of the Serpents so that a messenger named Kiseravi can share their secrets with the world.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4463 => array(
			'name'    => "The Merethic Collection",
			'journal' => "The Merethic Society hid several of its most valuable treasures throughout the ruins of Rulanyil's Fall.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2857 => array(
			'name'    => "Scout Castle Bloodmayne",
			'journal' => "I accepted a mission to scout Castle Bloodmayne for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2859 => array(
			'name'    => "Scout Rayles Mine",
			'journal' => "I accepted a mission to scout Rayles Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3407 => array(
			'name'    => "Kill Twenty Guards",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6571 => array(
			'name'    => "Magic Mycology",
			'journal' => "<<1>> needs someone to gather a strange kind of mushroom growing in Gloomreach that a few Reach clans are interested in. I volunteered my services.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2861 => array(
			'name'    => "Scout Ash Mine",
			'journal' => "I accepted a mission to scout Ash Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4340 => array(
			'name'    => "The White Mask of Merien",
			'journal' => "Tamien, an ex-Red Rook bandit, claims he wants to save the town of Eagle's Brook. The Lion Guard, however, don't trust him. Tamien says the Bloodthorns are turning townfolk into zombies. He wants my help to prepare a spell to protect the villagers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2863 => array(
			'name'    => "Scout Dragonclaw Mine",
			'journal' => "I accepted a mission to scout Dragonclaw Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3197 => array(
			'name'    => "Capture Ash Farm",
			'journal' => "I accepted a mission to help capture Ash Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2864 => array(
			'name'    => "Scout Chalman Mine",
			'journal' => "I accepted a mission to scout Chalman Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2870 => array(
			'name'    => "Scout Alessia Mine",
			'journal' => "I accepted a mission to scout Alessia Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3236 => array(
			'name'    => "Capture Aleswell Lumbermill",
			'journal' => "I accepted a mission to help capture Aleswell Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6471 => array(
			'name'    => "Digging Up Trouble",
			'journal' => "Miners working in Blackreach have gone missing. Some of the miners believe spirits are to blame, and ghost hunters were hired to investigate. I should look into these disappearances.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2873 => array(
			'name'    => "Scout Brindle Mine",
			'journal' => "I accepted a mission to scout Brindle Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4730 => array(
			'name'    => "Breaking the Shackle",
			'journal' => "I restored Vanus Galerion and disrupted activities at the Black Forge. Now we need to assault the Great Shackle and end the threat of the Planemeld once and for all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6621 => array(
			'name'    => "Shadows in the Swamp",
			'journal' => "A tribe of Argonians is at odds with a group of Imperial soldiers. The Argonians say their people are missing, and they blame the Imperials. Heem-Jas asked me to act as a mediator to figure out what's going on.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3082 => array(
			'name'    => "The Lion Guard's Stand",
			'journal' => "To hold back Angof's undead army, the Lion Guard built a redoubt on a hill outside Cath Bedraud. As they struggle to find a way to breach Angof's defenses, they have to fend off hordes of Bloodthorn cultists and zombie thralls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2875 => array(
			'name'    => "Scout Bloodmayne Mine",
			'journal' => "I accepted a mission to scout Bloodmayne Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2877 => array(
			'name'    => "Scout Rayles Farm",
			'journal' => "I accepted a mission to scout Rayles Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2879 => array(
			'name'    => "Scout Ash Farm",
			'journal' => "I accepted a mission to scout Ash Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5837 => array(
			'name'    => "Lava Foot Stomp",
			'journal' => "<<1>>, a New Life Herald in Eastmarch, spoke of an <<2>> New Life Festival in Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4480 => array(
			'name'    => "Oath of Excision",
			'journal' => "The Vinedusk Rangers are under siege by an invading Colovian force. They've offered me a chance to join the fight.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2882 => array(
			'name'    => "Scout Chalman Farm",
			'journal' => "I accepted a mission to scout Chalman Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2883 => array(
			'name'    => "Scout Arrius Farm",
			'journal' => "I accepted a mission to scout Arrius Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4748 => array(
			'name'    => "Saving Stibbons",
			'journal' => "I found a hastily written note from Lady Laurent's servant, Stibbons. He was captured by Daedra after we arrived in Coldharbour and is being held in a nearby tower.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2884 => array(
			'name'    => "Scout Faregyl Farm",
			'journal' => "I accepted a mission to scout Faregyl Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5249 => array(
			'name'    => "Blacksmith Certification",
			'journal' => "If I can prove I know my way around a forge, <<1>> will certify me as a blacksmith.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3192 => array(
			'name'    => "Forgotten Ancestry",
			'journal' => "I met Harald Winvale, a repentant Bloodthorn cultist who is being haunted by ancestral spirits for desecrating his family tombs. He asked me to help him demonstrate that he still honors and respects his ancestors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2885 => array(
			'name'    => "Scout Roebeck Farm",
			'journal' => "I accepted a mission to scout Roebeck Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6604 => array(
			'name'    => "Defenders of the Reach",
			'journal' => "The Wayward Guardians fear that Gray Host forces still lurk in the halls of Nchuand-Zel beneath Markarth. They need them driven from those halls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2886 => array(
			'name'    => "Scout Brindle Farm",
			'journal' => "I accepted a mission to scout Brindle Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3221 => array(
			'name'    => "Capture Farragut Keep",
			'journal' => "I accepted a mission to help capture Farragut Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2888 => array(
			'name'    => "Scout Bloodmayne Farm",
			'journal' => "I accepted a mission to scout Bloodmayne Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5590 => array(
			'name'    => "Contract: Glenumbra",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5396 => array(
			'name'    => "Woodworker Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Woodworker goods to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2890 => array(
			'name'    => "Scout Rayles Lumbermill",
			'journal' => "I accepted a mission to scout Rayles Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2891 => array(
			'name'    => "Scout Glademist Lumbermill",
			'journal' => "I accepted a mission to scout Glademist Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2892 => array(
			'name'    => "Scout Ash Lumbermill",
			'journal' => "I accepted a mission to scout Ash Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4611 => array(
			'name'    => "Mist and Shadow",
			'journal' => "Something strange is happening in Shademist Moors. The village of Longhaven needs someone who can uncover what might be behind it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6603 => array(
			'name'    => "Alone in the Dark",
			'journal' => "A scout reported several Reach exiles have been taken captive by strange blind creatures in the depths of Blackreach. The Wayward Guardians want me to rescue them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2893 => array(
			'name'    => "Scout Aleswell Lumbermill",
			'journal' => "I accepted a mission to scout Aleswell Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6612 => array(
			'name'    => "A Mortal's Touch",
			'journal' => "A strange messenger delivered a mysterious letter from someone who calls herself the Dread Lady. I should read the letter and determine what this is all about.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2894 => array(
			'name'    => "Scout Dragonclaw Lumbermill",
			'journal' => "I accepted a mission to scout Dragonclaw Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4361 => array(
			'name'    => "Rightful Inheritance",
			'journal' => "Elanwe's son, Sorondil, is the illegitimate son of the deceased High Kinsman Rilis XII. Sorondil has come to these ruins to retrieve a great treasure and his inheritance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2895 => array(
			'name'    => "Scout Chalman Lumbermill",
			'journal' => "I accepted a mission to scout Chalman Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3229 => array(
			'name'    => "Capture Castle Bloodmayne",
			'journal' => "I accepted a mission to help capture Castle Bloodmayne for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2896 => array(
			'name'    => "Scout Arrius Lumbermill",
			'journal' => "I accepted a mission to scout Arrius Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4631 => array(
			'name'    => "Past Due",
			'journal' => "I found a note of debt in Coldrock Diggings.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5521 => array(
			'name'    => "Nature's Bounty",
			'journal' => "A foul corruption besets the Wrothgar wilderness. This dark sorcery has blighted the land and driven wild animals mad. The Orcish shaman, Ushang, believes that we can cleanse the corruption through old magic and force of arms.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2899 => array(
			'name'    => "Scout Farragut Lumbermill",
			'journal' => "I accepted a mission to scout Farragut Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2901 => array(
			'name'    => "Scout Drakelowe Lumbermill",
			'journal' => "I accepted a mission to scout Drakelowe Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4758 => array(
			'name'    => "The Final Assault",
			'journal' => "The final assault to end the Planemeld is at hand. We need to fight our way through Molag Bal's defenses and reach the portal to the planar vortex. Only then can we end this threat and save our world.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3381 => array(
			'name'    => "A Winner for Onwyn",
			'journal' => "I've decided to help <<1>> find a lizard who can win in the local lizard races. Breeders specialize in providing varieties of racing reptiles.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5597 => array(
			'name'    => "A Ghost from the Past",
			'journal' => "Green-Venom-Tongue learned a few things from the Black Dragon's journal that seem to point to his former Sanctuary. He wants to go back there and try to confirm the Black Dragon's claims.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2903 => array(
			'name'    => "Scout Faregyl Lumbermill",
			'journal' => "I accepted a mission to scout Faregyl Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3268 => array(
			'name'    => "Capture Black Boot Mine",
			'journal' => "I accepted a mission to help capture Black Boot Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6631 => array(
			'name'    => "Giving Up the Ghost",
			'journal' => "Revus Demnevanni, a Dunmer Mage, needs help with an experiment he's performing in the ruins of Veyond.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2905 => array(
			'name'    => "Scout Brindle Lumbermill",
			'journal' => "I accepted a mission to scout Brindle Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2907 => array(
			'name'    => "Scout Bloodmayne Lumbermill",
			'journal' => "I accepted a mission to scout Bloodmayne Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2915 => array(
			'name'    => "Capture Fort Warden",
			'journal' => "I accepted a mission to capture Fort Warden for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4780 => array(
			'name'    => "Messages Across Tamriel",
			'journal' => "Vanus Galerion of the Mages Guild has a plan to stop Molag Bal's incursion of Tamriel. It requires convincing the three alliances to join forces with the Mages Guild and Fighters Guild to assault Coldharbour.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3463 => array(
			'name'    => "Scout the Farm",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4156 => array(
			'name'    => "The Soul Trap",
			'journal' => "Someone's soul is trapped in an Ayleid tomb within Abamath. A puzzle lock prevents her from escaping.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4507 => array(
			'name'    => "Offerings to Zenithar",
			'journal' => "<<1>> has asked me to collect offerings to Zenithar from several villagers around Cropsford.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3096 => array(
			'name'    => "Capture Castle Faregyl",
			'journal' => "I accepted a mission to help capture Castle Faregyl for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2917 => array(
			'name'    => "Capture Fort Glademist",
			'journal' => "I accepted a mission to help capture Fort Glademist for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3456 => array(
			'name'    => "Kill Twenty Guards",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6636 => array(
			'name'    => "The Face of Change",
			'journal' => "There are strange things afoot in the sleepy little town of Farmer's Nook, and not just the arrival of a troupe of colorful entertainers. People are vanishing, only to return somehow different.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2918 => array(
			'name'    => "Capture Fort Ash",
			'journal' => "I accepted a mission to help capture Fort Ash for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2919 => array(
			'name'    => "Capture Fort Aleswell",
			'journal' => "I accepted a mission to help capture Fort Aleswell for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2923 => array(
			'name'    => "Capture Kingscrest Keep",
			'journal' => "I accepted a mission to help capture Kingscrest Keep for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4142 => array(
			'name'    => "Awakening",
			'journal' => "<<1>> needs to join her fellow spirits in Treehenge, but something prevents her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2927 => array(
			'name'    => "Capture Castle Alessia",
			'journal' => "I accepted a mission to help recapture Castle Alessia for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6655 => array(
			'name'    => "A Plea from the Flames",
			'journal' => "I met an Argonian named Malosza who told me of a brutal tribe named the Sul-Xan who invaded his home. He requested I travel to Rockgrove if I wish to lend aid.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2929 => array(
			'name'    => "Capture Castle Roebeck",
			'journal' => "I accepted a mission to help recapture Castle Roebeck for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5553 => array(
			'name'    => "The One That Got Away",
			'journal' => "A matter involving <<1>> requires the guild's attention, and <<2>> wants me to handle it personally.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2931 => array(
			'name'    => "Capture Castle Black Boot",
			'journal' => "I accepted a mission to help recapture Castle Black Boot for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6643 => array(
			'name'    => "Help Wanted in Leyawiin",
			'journal' => "I found a notice that says the Chamber of Legates in Leyawiin seeks adventurers for dangerous work.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3112 => array(
			'name'    => "Capture Blue Road Farm",
			'journal' => "I accepted a mission to help capture Blue Road Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2933 => array(
			'name'    => "Capture Warden Mine",
			'journal' => "I accepted a mission to help capture Warden Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4790 => array(
			'name'    => "Breaking the Ward",
			'journal' => "The Vinedusks are spread thin and need my assistance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6535 => array(
			'name'    => "Reeh-La's Mining Samples",
			'journal' => "I found Reeh-La, an Argonian prospector, dead in an out-of-the-way corner of Blackreach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2937 => array(
			'name'    => "Capture Aleswell Mine",
			'journal' => "I accepted a mission to help capture Aleswell Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4794 => array(
			'name'    => "In the Belly of the Sea Hawk",
			'journal' => "A Khajiit trader was tricked by an Altmer merchant captain who gave her low quality fabric in place of the rare silks she traded for.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2938 => array(
			'name'    => "Capture Dragonclaw Mine",
			'journal' => "I accepted a mission to help capture Dragonclaw Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4665 => array(
			'name'    => "Ezreba's Fate",
			'journal' => "Bowenas, the Treethane of Greenhill, is having trouble deciding what to do with Ezreba. The Khajiit helped the cultists to subvert the town, but she was forced to do so against her will.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2939 => array(
			'name'    => "Capture Chalman Mine",
			'journal' => "I accepted a mission to help capture Chalman Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5314 => array(
			'name'    => "Enchanter Certification",
			'journal' => "If I can prove I know my way around an enchanting table, <<1>> will certify me as an enchanter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2943 => array(
			'name'    => "Capture Blue Road Mine",
			'journal' => "I accepted a mission to help capture Blue Road Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4146 => array(
			'name'    => "A Family Divided",
			'journal' => "Survivors of the plague from Narsis have built a refugee camp inside long-abandoned crypts. <<1>> and her husband came to join the group, but became separated.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3277 => array(
			'name'    => "Mastering the Talisman",
			'journal' => "The wizard Gabrielle Benele asked me to help her recharge the Midnight Talisman. It could be a powerful weapon in the battle against Angof the Gravesinger.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2944 => array(
			'name'    => "Capture Drakelowe Mine",
			'journal' => "I accepted a mission to help capture Drakelowe Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2945 => array(
			'name'    => "Capture Alessia Mine",
			'journal' => "I accepted a mission to help capture Alessia Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4544 => array(
			'name'    => "The Dark Mane",
			'journal' => "The Mane, on his way to the Handfasting in Silvenar, suddenly attacked his entourage and fled. Lord Gharesh-ri, the Speaker of the Mane, asked me to find him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2946 => array(
			'name'    => "Capture Faregyl Mine",
			'journal' => "I accepted a mission to help capture Faregyl Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2947 => array(
			'name'    => "Capture Roebeck Mine",
			'journal' => "I accepted a mission to help capture Roebeck Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4812 => array(
			'name'    => "History's Song",
			'journal' => "I was lead to an old instrument in the ruins of Thibaut's Cairn by a spectral woman. The recoder was engraved with \"Valencia Nasica, the Exalted Bard of Dune.\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2950 => array(
			'name'    => "Capture Brindle Mine",
			'journal' => "I accepted a mission to help capture Brindle Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5692 => array(
			'name'    => "Contract: The Rift",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2951 => array(
			'name'    => "Capture Black Boot Mine",
			'journal' => "I accepted a mission to help capture Black Boot Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5803 => array(
			'name'    => "Divine Conundrum",
			'journal' => "I encountered a priest of the Tribunal and a wounded Buoyant Armiger on a holy mission for Lord Vivec. They were attacked by Daedra while seeking guidance at an ancestral tomb and weren't able to complete their mission.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3438 => array(
			'name'    => "Past in Ruins",
			'journal' => "<<1>>, a scholar from Cyrodiil, asked me to search the Yokudan ruins at Ancestor's Landing for an ancient sword.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3285 => array(
			'name'    => "The Jeweled Crown of Anton",
			'journal' => "A Daggerfall noble named Lady Laurent seems to be some kind of treasure hunter. She's exploring the Tomb of Lost Kings to find something to add to her collection.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2952 => array(
			'name'    => "Capture Bloodmayne Mine",
			'journal' => "I accepted a mission to help capture Bloodmayne Mine for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4559 => array(
			'name'    => "Daughter of Seamount",
			'journal' => "The hunt-wife of the Seamount clan thinks the chief's daughter is not mindful of her duties to the clan. She believes I could help sort things out.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7427 => array(
			'name'    => "The Nowhere Keys",
			'journal' => "The plan to have the Lion Guard capture Gavin Koldane failed and he got away. In the chaos, Abrasia stole our only Nowhere Key and fled. We need keys to enter Sage Voernet's vault. Luckily, Edwane and Seline may know where to get more keys.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2953 => array(
			'name'    => "Capture Warden Farm",
			'journal' => "I accepted a mission to help capture Warden Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2954 => array(
			'name'    => "Capture Rayles Farm",
			'journal' => "I accepted a mission to help capture Rayles Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3105 => array(
			'name'    => "Capture Aleswell Farm",
			'journal' => "I accepted a mission to help capture Aleswell Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6406 => array(
			'name'    => "A Lonely Grave",
			'journal' => "Dirge Truptor gave me an old bandit map that will supposedly lead me to a lost grave from the Akaviri Invasion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2956 => array(
			'name'    => "Capture Ash Farm",
			'journal' => "I accepted a mission to help capture Ash Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4675 => array(
			'name'    => "Consuming Darkness",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3136 => array(
			'name'    => "Capture Brindle Mine",
			'journal' => "I accepted a mission to help capture Brindle Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2957 => array(
			'name'    => "Capture Aleswell Farm",
			'journal' => "I accepted a mission to help capture Aleswell Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3437 => array(
			'name'    => "A Mysterious Curio",
			'journal' => "I found a dusty old scroll beside an ancient Ayleid coffer in the vault beneath Dresan Keep.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4425 => array(
			'name'    => "Prisoners of War",
			'journal' => "The Imperials have captured two of Vyctoria Girien's soldiers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2959 => array(
			'name'    => "Capture Chalman Farm",
			'journal' => "I accepted a mission to help capture Chalman Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3446 => array(
			'name'    => "Kill Thirty Daggerfall Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2960 => array(
			'name'    => "Capture Arrius Farm",
			'journal' => "I accepted a mission to help capture Arrius Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4695 => array(
			'name'    => "Homeward",
			'journal' => "<<1>> asked me to travel to <<2>> and look for her sister, <<3>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2961 => array(
			'name'    => "Capture Kingscrest Farm",
			'journal' => "I accepted a mission to help capture Kingscrest Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2962 => array(
			'name'    => "Capture Farragut Farm",
			'journal' => "I accepted a mission to help capture Farragut Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3113 => array(
			'name'    => "Capture Drakelowe Farm",
			'journal' => "I accepted a mission to help capture Drakelowe Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6414 => array(
			'name'    => "The Frozen Isle",
			'journal' => "The Skald-King dispatched a party of sword-thanes to investigate unnatural storms swirling around Icereach. Alas, a coven of witches killed most of the warriors. I must venture into the aged keep, kill the witches within, and search for survivors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2964 => array(
			'name'    => "Capture Drakelowe Farm",
			'journal' => "I accepted a mission to help capture Drakelowe Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2966 => array(
			'name'    => "Capture Faregyl Farm",
			'journal' => "I accepted a mission to help capture Faregyl Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4693 => array(
			'name'    => "The Family Business",
			'journal' => "A moon-sugar plantation suffered a terrible fire. The family in charge of the plantation needs a hand to recover from their tragedy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3445 => array(
			'name'    => "Kill Thirty Enemy Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5302 => array(
			'name'    => "Woodworker Certification",
			'journal' => "If I can prove I know my way around a workbench, <<1>> will certify me as a woodworker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2967 => array(
			'name'    => "Capture Roebeck Farm",
			'journal' => "I accepted a mission to help capture Roebeck Farm for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4816 => array(
			'name'    => "A Blessing for the Dead",
			'journal' => "A villager from Tava's Blessing asked that I seek out a Thronekeeper named Luala al-Tava.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6136 => array(
			'name'    => "The Vault of Moawita",
			'journal' => "Relicmaster Glenadir of the Psijic Order has tasked me with recovering stolen relics, taken from the Vault of Moawita.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3121 => array(
			'name'    => "Capture Rayles Mine",
			'journal' => "I accepted a mission to help capture Rayles Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4293 => array(
			'name'    => "Putting the Pieces Together",
			'journal' => "I'm working with Razum-dar, at the town of Mathiisen. He's looking into reports of the Veiled Heritance in the area for Queen Ayrenn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4446 => array(
			'name'    => "Aiding Sigunn",
			'journal' => "The traveling merchant Sigunn has asked for help retrieving supplies he lost after a giant attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2972 => array(
			'name'    => "Capture Warden Lumbermill",
			'journal' => "I accepted a mission to help capture Warden Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3152 => array(
			'name'    => "Capture Faregyl Lumbermill",
			'journal' => "I accepted a mission to help capture Faregyl Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2973 => array(
			'name'    => "Capture Rayles Lumbermill",
			'journal' => "I accepted a mission to help capture Rayles Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6177 => array(
			'name'    => "Gjadil's Legacy",
			'journal' => "Igeke Rat-Bite no longer trusts her companions after the mysterious death of their cousin, Vareda. She's requested that I collect a nearby treasure chest, which can only be opened by her family's blood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4450 => array(
			'name'    => "Well-Armed Savages",
			'journal' => "I ran into a captive Khajiit in Toothmaul Gully. I've opted to free him and entertain his offer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2976 => array(
			'name'    => "Capture Aleswell Lumbermill",
			'journal' => "I accepted a mission to help capture Aleswell Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7460 => array(
			'name'    => "Ink-Parched",
			'journal' => "I discovered a field journal in the Parch filled with valuable information, but with important sections missing. I can visit the locations in the journal to fill in the missing contents and turn it in for a reward with my faction.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2978 => array(
			'name'    => "Capture Chalman Lumbermill",
			'journal' => "I accepted a mission to help capture Chalman Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2979 => array(
			'name'    => "Capture Arrius Lumbermill",
			'journal' => "I accepted a mission to help capture Arrius Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4844 => array(
			'name'    => "The Price of Longevity",
			'journal' => "I found a strange sapling that spoke to me, confused about its own ability to think and feel. It has vague memories of walking and a nearby cave. I agreed to help the sapling get some answers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2980 => array(
			'name'    => "Capture Kingscrest Lumbermill",
			'journal' => "I accepted a mission to help capture Kingscrest Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3520 => array(
			'name'    => "Window on the Past",
			'journal' => "Sorcerer Vunal at the Brothers of Strife monument says he can teach me about the past. The Brothers were apparently once mortal leaders of the citizens of Stonefalls. The past may hold clues about these powerful creatures.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2982 => array(
			'name'    => "Capture Blue Road Lumbermill",
			'journal' => "I accepted a mission to help capture Blue Road Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3317 => array(
			'name'    => "Tongues of Stone",
			'journal' => "Lady Laurent, a traveling archeologist, has \"lost\" her assistant, Stibbons. She is trying to recover him from her research site.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2984 => array(
			'name'    => "Capture Alessia Lumbermill",
			'journal' => "I accepted a mission to help capture Alessia Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4591 => array(
			'name'    => "Timely Intervention",
			'journal' => "<<1>> asked that I search for <<2>> and <<3>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7459 => array(
			'name'    => "Dark and Empty Spaces",
			'journal' => "I discovered an unfinished sketch book in Sorrow's Friend. I should see if I can fill in the last few pages and turn it in for a reward.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2985 => array(
			'name'    => "Capture Faregyl Lumbermill",
			'journal' => "I accepted a mission to help capture Faregyl Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7211 => array(
			'name'    => "Tracking the Trapper",
			'journal' => "Star-Gazer Sariyaah asked me to look for Trapper Marcien to find out why they never returned.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5492 => array(
			'name'    => "The Lifeblood of an Empire",
			'journal' => "Valga Atrius, an Imperial freedom fighter, has asked for my help in rescuing Imperial citizens held captive by the Daedra in the Arena District.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2986 => array(
			'name'    => "Capture Roebeck Lumbermill",
			'journal' => "I accepted a mission to help capture Roebeck Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6438 => array(
			'name'    => "An Answer in Blood",
			'journal' => "A new form of skooma plagues the coves and alleys of Southern Elsweyr. The Senchal job broker, Guybert Flaubert, urged me to collect blood samples from addicts in Moonlit Cove to help a healer develop a treatment.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2988 => array(
			'name'    => "Capture Black Boot Lumbermill",
			'journal' => "I accepted a mission to help capture Black Boot Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4707 => array(
			'name'    => "Best Left Unknown",
			'journal' => "I found a note meant for <<1>>, who apparently lives in <<2>>. He might appreciate getting it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2989 => array(
			'name'    => "Capture Bloodmayne Lumbermill",
			'journal' => "I accepted a mission to help capture Bloodmayne Lumbermill for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2997 => array(
			'name'    => "Amputating the Hand",
			'journal' => "The final Ansei Ward has been tracked to the Motalion Necropolis. I must help Prince Azah recover the Ward and put an end to the Withered Hand's plot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4854 => array(
			'name'    => "Eyes of Azura",
			'journal' => "<<1>>, a priestess of Azura, has long suffered at the hands of Molag Bal. Her pain is so great it has torn a hole between <<2>> and <<3>>, an Ayleid ruin in <<4>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	2998 => array(
			'name'    => "Restoring the Ansei Wards",
			'journal' => "I recovered all three Ansei Wards and defeated the heirs of Suturah. The Withered Hand is broken. Their campaign of terror has ended. All that remains is to restore the Ansei Wards to safekeeping in Sentinel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4725 => array(
			'name'    => "Welcome to Cyrodiil",
			'journal' => "I entered Cyrodiil, ready to help the Covenant win the war.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5416 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6450 => array(
			'name'    => "Masterful Jewelry",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3205 => array(
			'name'    => "Capture Drakelowe Farm",
			'journal' => "I accepted a mission to help capture Drakelowe Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3000 => array(
			'name'    => "Blood and the Crescent Moon",
			'journal' => "The bustling city of Daggerfall holds many mysteries. It appears that I stumbled right into one of them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5688 => array(
			'name'    => "Contract: Gold Coast Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of the Gold Coast.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4357 => array(
			'name'    => "To Firsthold",
			'journal' => "After aiding the people of Dawnbreak, Landare asked me to move down the road to Firsthold. If Dawnbreak faced the Daedra, chances are the city of Firsthold has as well.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3003 => array(
			'name'    => "Disorganized Crime",
			'journal' => "The nobles now have the wolf's woe plants. Unfortunately, the bandits set traps all around the estate. The nobles will never be able to slip out past those traps or the bandits.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6205 => array(
			'name'    => "Masterful Jewelry",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4478 => array(
			'name'    => "Into the Hills",
			'journal' => "Laganakh senses evil spreading on Betnikh. We need to know what we're facing before we can fight it. A totem may grant me visions that will help us.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3004 => array(
			'name'    => "Lady Eloise's Lockbox",
			'journal' => "The Red Rook leader, Tharkul, stole Lady Noellaume's jewelry box. She's grateful that her family is safe, but she would love to have her lockbox returned to her once she's safely away from the bandits.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3006 => array(
			'name'    => "Bloodthorn Assassins",
			'journal' => "Bloodthorn assassins plan to kill King Casimir. If the King dies, the entire Daggerfall Covenant could fall apart.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5937 => array(
			'name'    => "Royal Revelry",
			'journal' => "Jester King Emeric has asked for my help in impressing his subjects and visiting guests with a display using his illusion dazzlers. Unfortunately, his stock was confiscated and he needs my help to create replacements.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3013 => array(
			'name'    => "Wolves in the Fold",
			'journal' => "<<1>> thinks that werewolves may be trying to infiltrate and take over Aldcroft, just as they overthrew Camlorn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3016 => array(
			'name'    => "The Wyrd Tree's Roots",
			'journal' => "The Beldama Wyrd want to use the roots of the Wyrd Tree to help the villagers get away. But the same corruption that caused all this trouble has sapped the roots of their power.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3019 => array(
			'name'    => "The Ghosts of Westtry",
			'journal' => "The town of Westtry is cursed, haunted by the spirits of its inhabitants and the raiders who slaughtered them. The spirit of Leon Milielle, Westtry's priest, asked me to lift the curse.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4238 => array(
			'name'    => "Overdue Supplies",
			'journal' => "<<1>> asked me to find his friend <<2>>, who was carrying supplies for Weynon Priory.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3020 => array(
			'name'    => "Memento Mori",
			'journal' => "Leon made a pact with Molag Bal that placed a curse upon Westtry. The final twist was when Molag Bal turned Leon's wife and the captain of the guard, Rosalind, into a vengeful shade. She may hold the key to ending this curse.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4739 => array(
			'name'    => "A Storm Upon the Shore",
			'journal' => "Vicereeve Pelidil and his Maormer allies have attacked and seized control of Seaside Sanctuary.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3208 => array(
			'name'    => "Capture Roebeck Farm",
			'journal' => "I accepted a mission to help capture Roebeck Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3029 => array(
			'name'    => "Temple's Treasures",
			'journal' => "Imperial invaders ransacked the Temple of Satakalaam, killing the priests, defiling the temple, and stealing the relics. High Priest Zuladr has asked me to reclaim the relics so he can restore the temple.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5670 => array(
			'name'    => "Contract: Eastmarch",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3049 => array(
			'name'    => "The Nameless Soldier",
			'journal' => "Alana and her soldiers were ambushed on their way to confront Faolchu. Now Alana has been taken captive and the remaining soldiers are too few to divide their attention. Moreover, I still need to figure out how Faolchu was killed in this time period.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6517 => array(
			'name'    => "Moonlight Kidnapping",
			'journal' => "The Swords of Solitude tasked me with dealing with a pack of werewolves preying on a local goatherd's stock.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3059 => array(
			'name'    => "Servants of Ancient Kings",
			'journal' => "The dead lords and ladies of the mounds can't rest knowing the Bloodthorn cult has enthralled their servants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6510 => array(
			'name'    => "The Maelmoth Mysterium",
			'journal' => "A prideful historian named Narsis Dren found himself trapped in an illusion while researching an ancient mage. Assisting him in his investigation may lead to some exciting discoveries.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3060 => array(
			'name'    => "Seeking the Guardians",
			'journal' => "The Beldama Wyrd asked for my help. The wyrd rely on the elemental guardians for guidance and counsel, but the spirits have become mysteriously silent.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4141 => array(
			'name'    => "Do Kill the Messenger",
			'journal' => "Magister Osanne may have sent word to the Orcthane about the mysterious ore found in Lost Knife Cave. Tridi decided to scout the area on her own for any signs of a messenger.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3063 => array(
			'name'    => "Champion of the Guardians",
			'journal' => "Angof Gravesinger has imprisoned the elemental guardians that the Beldama Wyrd rely on for protection and counsel. I agreed to help the wyrd set the guardians free.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4296 => array(
			'name'    => "Soul Shriven in Coldharbour",
			'journal' => "I died, but that was only the beginning …. I awoke in Oblivion. My body is intact, but my soul has been torn from my body. I must find a way out of this nightmare place to reclaim what was lost to me, or I'll be damned for all eternity.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3084 => array(
			'name'    => "Capture Fort Rayles",
			'journal' => "I accepted a mission to help capture Fort Rayles for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3087 => array(
			'name'    => "Capture Fort Aleswell",
			'journal' => "I accepted a mission to help capture Fort Aleswell for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3583 => array(
			'name'    => "Suspicious Silence",
			'journal' => "Covenant troops are massing to the north of Fort Arand. They don't have the numbers for a frontal assault, but Fort Captain Hjolm believes they may be coordinating an attack with a second flank.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3088 => array(
			'name'    => "Capture Fort Dragonclaw",
			'journal' => "I accepted a mission to help capture Fort Dragonclaw for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6104 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3089 => array(
			'name'    => "Capture Chalman Keep",
			'journal' => "I accepted a mission to help recapture Chalman Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6629 => array(
			'name'    => "A Visit to Elsweyr",
			'journal' => "I came across a notice about visiting Elsweyr and completing various tasks in order to receive rewards. The notice says I should speak with <<1>> once I complete the tasks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5983 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3090 => array(
			'name'    => "Capture Arrius Keep",
			'journal' => "I accepted a mission to help recapture Arrius Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7058 => array(
			'name'    => "Bastion Nymic - Fallen Hues",
			'journal' => "Ordinator Tandasea told me of a dangerous threat to Bastion Nymic, a stronghold in Hermaeus Mora's realm of Apocrypha. I agreed to find my way to the bastion and defeat Hermaeus Mora's enemies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4689 => array(
			'name'    => "A Door Into Moonlight",
			'journal' => "I have been sent to the Moonlit Clearing to meet with a member of the lunar clergy. I'm supposed to find out if I am the Moon Hallowed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3091 => array(
			'name'    => "Capture Kingscrest Keep",
			'journal' => "I accepted a mission to help recapture Kingscrest Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6551 => array(
			'name'    => "Blood of the Reach",
			'journal' => "Lady Belain seeks a mysterious Dwarven relic called the Arkthzand Keystone for her ally, Rada al-Saran. Count Verandis wants me to find the keystone before it winds up in the hands of the Gray Host.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4682 => array(
			'name'    => "The Doctor's Bag",
			'journal' => "I have a doctor's bag belonging to <<1>>, who may be in <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3092 => array(
			'name'    => "Capture Farragut Keep",
			'journal' => "I accepted a mission to help recapture Farragut Keep for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3095 => array(
			'name'    => "Capture Castle Alessia",
			'journal' => "I accepted a mission to help capture Castle Alessia for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4765 => array(
			'name'    => "Pelidil's End",
			'journal' => "Seaside Sanctuary is back in the control of the Aldmeri Dominion, but Vicereeve Pelidil has escaped aboard a Maormer vessel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3097 => array(
			'name'    => "Capture Castle Roebeck",
			'journal' => "I accepted a mission to help capture Castle Roebeck for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5466 => array(
			'name'    => "Tinker Trouble",
			'journal' => "It seems that Raynor Vanos used the Dwarven power core I acquired for him in Zthenganaz to rebuild and modify a Dwarven Sphere. The construct went crazy and knocked out Kireth, and when she woke up both it and her brother were missing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7035 => array(
			'name'    => "Bound in Secrecy",
			'journal' => "Ordinator Tilena seeks specific materials from Quires Wind in Apocrypha, on behalf of Sotha Sil, one of her Living Gods. She seems unsettled by this, however.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3457 => array(
			'name'    => "Capture Fort Balfiera",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4676 => array(
			'name'    => "An Affront to Mara",
			'journal' => "Bandits attacked a chapel just over the border from Reaper's March. A group of temple guards and their priestess tracked them to the ruins of Fort Sphinxmoth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3098 => array(
			'name'    => "Capture Castle Brindle",
			'journal' => "I accepted a mission to help capture Castle Brindle for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3100 => array(
			'name'    => "Capture Castle Bloodmayne",
			'journal' => "I accepted a mission to help capture Castle Bloodmayne for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4583 => array(
			'name'    => "Dear Cousins",
			'journal' => "I found a diary on a dead man in Icehammer's Vault. It described how the man and his family took refuge from a storm in Icehammer's Vault. They were eventually killed by the draugr that roam the burial chambers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3103 => array(
			'name'    => "Capture Glademist Farm",
			'journal' => "I accepted a mission to help capture Glademist Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3465 => array(
			'name'    => "Scout the Mine",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6382 => array(
			'name'    => "The Senche of Decay",
			'journal' => "A Senche-raht necromancer named Thannar the Graveprowler stalks the ruins southwest of Rimmen, urging the corpses buried there to rise from their graves. Left unchecked, his ranks of undead followers could swell.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4525 => array(
			'name'    => "Tormented Souls",
			'journal' => "Orc spirits have risen around Moriseli. I've been asked to investigate the source of this disturbance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3106 => array(
			'name'    => "Capture Dragonclaw Farm",
			'journal' => "I accepted a mission to help capture Dragonclaw Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3459 => array(
			'name'    => "Capture the Mine",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6818 => array(
			'name'    => "Prison Problems",
			'journal' => "An advocate from the Society of the Steadfast wants someone to travel to Whalefall, an island off the western coast of Amenos, to bring back evidence of the prisoners sent there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3108 => array(
			'name'    => "Capture Chalman Farm",
			'journal' => "I accepted a mission to help capture Chalman Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4961 => array(
			'name'    => "Hircine's Gift",
			'journal' => "A strange woman met me at the gates to the city. She may be a werewolf. She told me I am on my way to becoming one as well.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3109 => array(
			'name'    => "Capture Arrius Farm",
			'journal' => "I accepted a mission to help capture Arrius Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3469 => array(
			'name'    => "Kill an Enemy Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3110 => array(
			'name'    => "Capture Kingscrest Farm",
			'journal' => "I accepted a mission to help capture Kingscrest Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4585 => array(
			'name'    => "Relative Matters",
			'journal' => "I encountered a Nord in the Bastard's Tomb. He had been captured by the Stormfist Brigade on his way to his sister's wedding. He promised me a reward if I rescued him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6404 => array(
			'name'    => "The Dragonguard",
			'journal' => "With Ra'khajin defeated and the Mask of Alkosh in our possession, Sai Sahan is determined to defeat Laatvulon once and for all. It's time to make our final stand against the Order of the New Moon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3970 => array(
			'name'    => "An Ill-Fated Venture",
			'journal' => "I found a letter on a dead man named Tavo in the Lost City of the Na-Totambu. His wife, Nahrina, wanted him to recover some Yokudan relics  and return to Sentinel to pay off a debt. I can recover the relics in his stead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4155 => array(
			'name'    => "Arithiel",
			'journal' => "The Worm Cult ambushed and captured several Wood Elves in <<1>>. The Cult may be preparing for a dark ritual.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3111 => array(
			'name'    => "Capture Farragut Farm",
			'journal' => "I accepted a mission to help capture Farragut Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3473 => array(
			'name'    => "Kill an Ebonheart Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6653 => array(
			'name'    => "Call of the Ruinach",
			'journal' => "Someone has summoned a Daedra known as Ghemvas the Harbinger to southern Blackwood. Locals say the Ruinach slays any who approach its lair and reaps their souls. I agreed to defeat Ghemvas and sanctify the bodies of its victims.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3114 => array(
			'name'    => "Capture Alessia Farm",
			'journal' => "I accepted a mission to help capture Alessia Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3115 => array(
			'name'    => "Capture Faregyl Farm",
			'journal' => "I accepted a mission to help capture Faregyl Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5710 => array(
			'name'    => "Contract: Stormhaven Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Stormhaven.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3467 => array(
			'name'    => "Kill an Aldmeri Player.",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3116 => array(
			'name'    => "Capture Roebeck Farm",
			'journal' => "I accepted a mission to help capture Roebeck Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3477 => array(
			'name'    => "Scout Drakelowe Farm",
			'journal' => "I accepted a mission to scout Drakelowe Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3118 => array(
			'name'    => "Capture Black Boot Farm",
			'journal' => "I accepted a mission to help capture Black Boot Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3481 => array(
			'name'    => "Scout Faregyl Mine",
			'journal' => "I accepted a mission to scout Faregyl Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6533 => array(
			'name'    => "Kelbarn's Mining Samples",
			'journal' => "I found the body of a Nord miner, Kelbarn. It looks like he died while prospecting.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3122 => array(
			'name'    => "Capture Glademist Mine",
			'journal' => "I accepted a mission to help capture Glademist Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7090 => array(
			'name'    => "Terror of the Weald",
			'journal' => "A gang of vampires led by Balandrus, the Terror of the Weald, has taken control of the town of Sutch. Maerenah, a cured vampire and former member of the gang, needs help restoring peace to the town and putting an end to the Terror.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4721 => array(
			'name'    => "Honoring the Dishonored",
			'journal' => "Honorless criminals are left in the Tears of the Dishonored. Darius begged me to find the remains of his wife, Zariya, and consecrate them as he believes she was innocent.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3123 => array(
			'name'    => "Capture Ash Mine",
			'journal' => "I accepted a mission to help capture Ash Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5073 => array(
			'name'    => "Aicessar's Invitation",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3475 => array(
			'name'    => "Scout Farragut Farm",
			'journal' => "I accepted a mission to scout Farragut Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6583 => array(
			'name'    => "The Scholar's Request",
			'journal' => "An outsider scholar reported the presence of an unknown Dwarven construct in a nearby cave that's producing new mechanical spiders.The Ard's Stonehands asked me to destroy it and test a Dwarven rod that supposedly renders constructs docile.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4714 => array(
			'name'    => "The Changing Kind",
			'journal' => "<<1>> in <<2>> is wanted by a pack of werewolves.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3124 => array(
			'name'    => "Capture Aleswell Mine",
			'journal' => "I accepted a mission to help capture Aleswell Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3125 => array(
			'name'    => "Capture Dragonclaw Mine",
			'journal' => "I accepted a mission to help capture Dragonclaw Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5494 => array(
			'name'    => "Long Live the King",
			'journal' => "Chief Bazrag wants to honor the memory of Kurog with a traditional Orc requiem for the dead. He believes such a ceremony will help heal the wounds Kurog inflicted on Orsinium and the clans.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3986 => array(
			'name'    => "Underfoot",
			'journal' => "A strange woman came up to me as I was walking through the woods. She said something about skeevers before leading me into the hills.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3127 => array(
			'name'    => "Capture Arrius Mine",
			'journal' => "I accepted a mission to help capture Arrius Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3132 => array(
			'name'    => "Capture Drakelowe Mine",
			'journal' => "I accepted a mission to help capture Drakelowe Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4487 => array(
			'name'    => "The Arbordawn Cult",
			'journal' => "Telenger the Artificer, a renowned sage, has come to the Falinesti Autumn Site in search of answers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3134 => array(
			'name'    => "Capture Faregyl Mine",
			'journal' => "I accepted a mission to help capture Faregyl Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6395 => array(
			'name'    => "The Dragonguard's Legacy",
			'journal' => "Blademaster Kasura has agreed to search a nearby Dragonguard tomb for a former student interested in finding some way to fight back against the Dragons in Elsweyr. I agreed to help her in this endeavor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4548 => array(
			'name'    => "Farsight",
			'journal' => "An Orc named Magula has seen visions at three shrines around the island. I should visit these shrines and see the visions.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3135 => array(
			'name'    => "Capture Roebeck Mine",
			'journal' => "I accepted a mission to help capture Roebeck Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6715 => array(
			'name'    => "A Discreet Pickup",
			'journal' => "Luna Beriel says a Gleaner operative left a shipment of goods at three drop sites in the Shambles, but they haven't been picked up yet. If those goods are left in the Shambles too long, they'll be stolen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3137 => array(
			'name'    => "Capture Black Boot Mine",
			'journal' => "I accepted a mission to help capture Black Boot Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4868 => array(
			'name'    => "The Grip of Madness",
			'journal' => "<<1>>'s citizens are killing one another in the streets. Dominion soldiers cordoned off the town, but are in dire need of assistance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5760 => array(
			'name'    => "The Missing Guardian",
			'journal' => "Hara has decided it's time to do something about the third guardian, the Thief. The Thief has been mysteriously absent except for when she appears to speak to me in private.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4557 => array(
			'name'    => "Will of the Broken",
			'journal' => "In the depths of <<1>>, I found a <<2>> surrounded by bodies. One of them stirred when I took the urn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3138 => array(
			'name'    => "Capture Bloodmayne Mine",
			'journal' => "I accepted a mission to help capture Bloodmayne Mine for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6857 => array(
			'name'    => "The Sea Runs Restless",
			'journal' => "I met Quen near the settlement of Llanshara. It seems the Sea Elves found a relic and someone hired her to retrieve it. She wants to do the job quickly and quietly, but her partner is missing, so I agreed to help her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3139 => array(
			'name'    => "Capture Warden Lumbermill",
			'journal' => "I accepted a mission to help capture Warden Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6850 => array(
			'name'    => "Tides of Ruin",
			'journal' => "Archdruid Orlaith and her Dreadsail allies captured Sir Stefan Mornard during the treacherous attack at Draoife Dell. We need to find Sir Stefan and rescue him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3140 => array(
			'name'    => "Capture Rayles Lumbermill",
			'journal' => "I accepted a mission to help capture Rayles Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4993 => array(
			'name'    => "Report to Evermore",
			'journal' => "The garrison at Bangkorai Pass asked that I deliver a message to Evermore Castle.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3143 => array(
			'name'    => "Capture Aleswell Lumbermill",
			'journal' => "I accepted a mission to help capture Aleswell Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3145 => array(
			'name'    => "Capture Chalman Lumbermill",
			'journal' => "I accepted a mission to help capture Chalman Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4628 => array(
			'name'    => "Supplies for Applewatch",
			'journal' => "<<1>> of Applewatch asked me to deliver a list of supplies he needs for his inn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3146 => array(
			'name'    => "Capture Arrius Lumbermill",
			'journal' => "I accepted a mission to help capture Arrius Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5902 => array(
			'name'    => "Divine Disaster",
			'journal' => "The Daedric hound Barbas tricked us into depleting Lord Vivec's energy so he could determine the location of Clockwork City. Before we deal with Barbas, we need to restore enough energy to Vivec to keep the moonlet from plummeting out of the sky.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3147 => array(
			'name'    => "Capture Kingscrest Lumbermill",
			'journal' => "I accepted a mission to help capture Kingscrest Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3148 => array(
			'name'    => "Capture Farragut Lumbermill",
			'journal' => "I accepted a mission to help capture Farragut Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3150 => array(
			'name'    => "Capture Drakelowe Lumbermill",
			'journal' => "I accepted a mission to help capture Drakelowe Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3151 => array(
			'name'    => "Capture Alessia Lumbermill",
			'journal' => "I accepted a mission to help capture Alessia Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6047 => array(
			'name'    => "Where Shadows Lie",
			'journal' => "Luciana and I cleared the way to the Cogitum Centralis. Now we must delve deep into the core of the Clockwork City to stop the Shadow of Sotha Sil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3154 => array(
			'name'    => "Capture Brindle Lumbermill",
			'journal' => "I accepted a mission to help capture Brindle Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3155 => array(
			'name'    => "Capture Black Boot Lumbermill",
			'journal' => "I accepted a mission to help capture Black Boot Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6615 => array(
			'name'    => "A Deadly Secret",
			'journal' => "Captain Rian Liore, commander of Leyawiin's Ivory Brigade, asked me to investigate the disappearance of Councilor Jirich, a former member of the Imperial Elder Council. She may be in danger because of a deadly secret tied to the Empire's past.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4746 => array(
			'name'    => "A Misplaced Pendant",
			'journal' => "I found a pendant with the names <<1>> and <<2>> engraved upon it while exploring the Cave of Trophies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3156 => array(
			'name'    => "Capture Bloodmayne Lumbermill",
			'journal' => "I accepted a mission to help capture Bloodmayne Lumbermill for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3172 => array(
			'name'    => "Left at the Altar",
			'journal' => "A Crown bride, Tiela, has been left at the altar because the Forebear groom, Casnar, hasn't shown up for the wedding. Both families are angry, and I've agreed to help settle the matter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5025 => array(
			'name'    => "The Corrupted Stone",
			'journal' => "The guardian star sign of the Warrior has failed to appear to protect the mortal world from the Serpent. One of the Star-Gazers needs help uncovering what might be happening.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3183 => array(
			'name'    => "To the Wyrd Tree",
			'journal' => "The elemental guardian asked me to head for the Wyrd Tree. I need to find Wyress Gwen and help save the Wyrd Tree.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3184 => array(
			'name'    => "The Glenumbra Moors",
			'journal' => "The Mages Guild has researchers looking into the details surrounding the way Faolchu originally died. It happened at an ancient battle site called the Glenumbra Moors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5431 => array(
			'name'    => "Pledge: White-Gold Tower",
			'journal' => "<<1>> has challenged me to explore the White-Gold Tower in the Imperial City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4785 => array(
			'name'    => "The Senche",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3187 => array(
			'name'    => "Seize the Moment",
			'journal' => "A Khajiit named Ildani was apparently caught trying to smuggle her wares into Sentinel. She's asked me to help her recover the confiscated goods.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3194 => array(
			'name'    => "Capture Warden Farm",
			'journal' => "I accepted a mission to help capture Warden Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6785 => array(
			'name'    => "Cold Trail",
			'journal' => "Ember opened up a bit about her past. Apparently, she was part of a tight-knit group of street kids for much of her younger life until her makeshift family mysteriously vanished from Skywatch. She's determined to find them and asked for my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3195 => array(
			'name'    => "Capture Rayles Farm",
			'journal' => "I accepted a mission to help capture Rayles Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5058 => array(
			'name'    => "All the Fuss",
			'journal' => "A bored member of the Queen's entourage let me know Her Majesty plans to speak soon in the city of Vulkhel Guard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6906 => array(
			'name'    => "A Gulf of Time",
			'journal' => "I met a spirit named Jocien Rane along the coast of Galen. After realizing I could see him, he asked me a favor. To take the ring from his body and deliver it to his wife in Vastyr.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3196 => array(
			'name'    => "Capture Glademist Farm",
			'journal' => "I accepted a mission to help capture Glademist Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4551 => array(
			'name'    => "A Poisoned Heart",
			'journal' => "While wading through the swamp, I stumbled into a cloud of roaming poison gas and passed out. Two Imperials rescued me and asked me to help them clean it up.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3199 => array(
			'name'    => "Capture Dragonclaw Farm",
			'journal' => "I accepted a mission to help capture Dragonclaw Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3200 => array(
			'name'    => "Capture Chalman Farm",
			'journal' => "I accepted a mission to help capture Chalman Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6660 => array(
			'name'    => "Heroes of Blackwood",
			'journal' => "We stopped Vandacia and saved Blackwood from Mehrunes Dagon. Now Governor Keshu wants to throw a victory celebration in the gardens behind the governor's mansion in Gideon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3202 => array(
			'name'    => "Capture Kingscrest Farm",
			'journal' => "I accepted a mission to help capture Kingscrest Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4923 => array(
			'name'    => "Archaic Relics",
			'journal' => "Knights of Arkay search for holy relics to help combat the rising undead threat spreading throughout Rivenspire. So far, the knights have yet to return.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3203 => array(
			'name'    => "Capture Farragut Farm",
			'journal' => "I accepted a mission to help capture Farragut Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6202 => array(
			'name'    => "Sinking Summerset",
			'journal' => "Battlereeve Tanerline charged me with finding allies to attack the abyssal geysers and destroy the pearls that power them before the Sea Sload cabal's plan to sink Summerset comes to fruition.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4871 => array(
			'name'    => "Bounty: Shadowed Path",
			'journal' => "I accepted a Fighters Guild contract from Sebazi in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3206 => array(
			'name'    => "Capture Alessia Farm",
			'journal' => "I accepted a mission to help capture Alessia Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3209 => array(
			'name'    => "Capture Brindle Farm",
			'journal' => "I accepted a mission to help capture Brindle Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4692 => array(
			'name'    => "A Pinch of Sugar",
			'journal' => "Awash in a sea of rats, Laughing Moons Plantation is in dire need of assistance. <<1>> asked me to help deal with the infestation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3210 => array(
			'name'    => "Capture Black Boot Farm",
			'journal' => "I accepted a mission to help capture Black Boot Farm for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3214 => array(
			'name'    => "Capture Fort Glademist",
			'journal' => "I accepted a mission to help recapture Fort Glademist for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3215 => array(
			'name'    => "Capture Fort Ash",
			'journal' => "I accepted a mission to help recapture Fort Ash for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6422 => array(
			'name'    => "A Rogue and His Rice",
			'journal' => "A rogue Goutfang master and his students waylaid a supply caravan, stealing all the rice. I agreed to deal with the master and return any rice I find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4691 => array(
			'name'    => "Cause and Effect",
			'journal' => "I found a letter from someone named Iocundus in <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3217 => array(
			'name'    => "Capture Fort Dragonclaw",
			'journal' => "I accepted a mission to help recapture Fort Dragonclaw for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6111 => array(
			'name'    => "Murder In Lillandril",
			'journal' => "The Sapiarchs of Lillandril imprisoned a member of the Mages Guild. The authorities are convinced this mage murdered one of Lillandril's sapiarchs, but the particulars seem murky. There may be more to this murder than meets the eye.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3218 => array(
			'name'    => "Capture Chalman Keep",
			'journal' => "I accepted a mission to help capture Chalman Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4817 => array(
			'name'    => "Tracking the Hand",
			'journal' => "Scout Yanul says Prince Azah has his soldiers searching the desert for the Withered Hand.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3219 => array(
			'name'    => "Capture Arrius Keep",
			'journal' => "I accepted a mission to help capture Arrius Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3222 => array(
			'name'    => "Capture Blue Road Keep",
			'journal' => "I accepted a mission to help capture Blue Road Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3225 => array(
			'name'    => "Capture Castle Faregyl",
			'journal' => "I accepted a mission to help capture Castle Faregyl for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5594 => array(
			'name'    => "Contract: Glenumbra",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3903 => array(
			'name'    => "School Daze",
			'journal' => "Arch-Mage Valeyn believes I'm here to help protect the Academy against some unnamed threat. While he determines what this threat is, he asked me to help him with another matter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6684 => array(
			'name'    => "Pledge: Red Petal Bastion",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Red Petal Bastion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3226 => array(
			'name'    => "Capture Castle Roebeck",
			'journal' => "I accepted a mission to help capture Castle Roebeck for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3227 => array(
			'name'    => "Capture Castle Brindle",
			'journal' => "I accepted a mission to help capture Castle Brindle for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6683 => array(
			'name'    => "A Rose of Many Thorns",
			'journal' => "Weylin, a priest of Azura, claims that the Knights of the Silver Rose pillaged his temple. Though he enlisted the help of the Dremora, Lyranth, he asked for my assistance reclaiming the stolen relics for his temple.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3233 => array(
			'name'    => "Capture Glademist Lumbermill",
			'journal' => "I accepted a mission to help capture Glademist Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4964 => array(
			'name'    => "Scion of the Blood Matron",
			'journal' => "I was bitten by a vampire. I have the option of embracing the disease or receiving the cure.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3234 => array(
			'name'    => "Capture Ash Lumbermill",
			'journal' => "I accepted a mission to help capture Ash Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3235 => array(
			'name'    => "Purifying the Wyrd Tree",
			'journal' => "The guardians wait inside the Wyrd Tree, ready to assist me against the <<1>>. Once the <<1>> is destroyed, the Wyrd Tree will be purified. Then the corruption will start to fade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3239 => array(
			'name'    => "Capture Arrius Lumbermill",
			'journal' => "I accepted a mission to help capture Arrius Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3240 => array(
			'name'    => "Capture Kingscrest Lumbermill",
			'journal' => "I accepted a mission to help capture Kingscrest Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3241 => array(
			'name'    => "Capture Farragut Lumbermill",
			'journal' => "I accepted a mission to help capture Farragut Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4724 => array(
			'name'    => "Reporting for Duty",
			'journal' => "Galsi Mavani says I have more to learn about the conflict in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3242 => array(
			'name'    => "Capture Blue Road Lumbermill",
			'journal' => "I accepted a mission to help capture Blue Road Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3244 => array(
			'name'    => "Capture Alessia Lumbermill",
			'journal' => "I accepted a mission to help capture Alessia Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3246 => array(
			'name'    => "Capture Roebeck Lumbermill",
			'journal' => "I accepted a mission to help capture Roebeck Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3247 => array(
			'name'    => "Capture Brindle Lumbermill",
			'journal' => "I accepted a mission to help capture Brindle Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3248 => array(
			'name'    => "Capture Black Boot Lumbermill",
			'journal' => "I accepted a mission to help capture Black Boot Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5495 => array(
			'name'    => "Priceless Treasures",
			'journal' => "<<1>> is looking for aid to protect the valuable artifacts scattered and defenseless across the Temple District.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6454 => array(
			'name'    => "The Coven Conspiracy",
			'journal' => "Jorunn the Skald-King requested I meet with his agent to look into a plot that threatens not only Skyrim, but potentially all of Tamriel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4723 => array(
			'name'    => "Siege Warfare",
			'journal' => "I've reached the siege range. Adalmor advised me to try each weapon. I should also talk to Looks-Forward and Lirtis to learn more about the weapons.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3249 => array(
			'name'    => "Capture Bloodmayne Lumbermill",
			'journal' => "I accepted a mission to help capture Bloodmayne Lumbermill for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6143 => array(
			'name'    => "The Mind Trap",
			'journal' => "After the cave collapsed, I found myself in a strange ruin. A spectral image appeared before me, a woman named Oriandra. She says she wants to help me get out of this place.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3250 => array(
			'name'    => "Capture Warden Mine",
			'journal' => "I accepted a mission to help capture Warden Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4849 => array(
			'name'    => "The Unfilled Order",
			'journal' => "I found a dead adventurer in Barrow Trench. They entered the mine to collect rare ingredients for an alchemist in Woodhearth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3251 => array(
			'name'    => "Capture Rayles Mine",
			'journal' => "I accepted a mission to help capture Rayles Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4842 => array(
			'name'    => "The Unquiet Dead",
			'journal' => "Explorers from the Mages Guild awoke an ancient threat at <<Ac:1>>. Undead creatures have slain or captured many within their expedition.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3252 => array(
			'name'    => "Capture Glademist Mine",
			'journal' => "I accepted a mission to help capture Glademist Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5616 => array(
			'name'    => "Contract: Rivenspire",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3255 => array(
			'name'    => "Capture Dragonclaw Mine",
			'journal' => "I accepted a mission to help capture Dragonclaw Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3256 => array(
			'name'    => "Capture Chalman Mine",
			'journal' => "I accepted a mission to help capture Chalman Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5626 => array(
			'name'    => "Contract: Rivenspire",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3257 => array(
			'name'    => "Capture Arrius Mine",
			'journal' => "I accepted a mission to help capture Arrius Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6716 => array(
			'name'    => "Dangerous Business",
			'journal' => "An Infernium called the Unmaker and multiple Havocrels wiped out a Daedric merchant caravan. Vaveli Indavel asked me to kill the monsters and bring back some of the lost supplies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3258 => array(
			'name'    => "Capture Kingscrest Mine",
			'journal' => "I accepted a mission to help capture Kingscrest Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5497 => array(
			'name'    => "Atypical Artistry",
			'journal' => "<<1>>, an artist, believes the ogres of Wrothgar possess a misunderstood artistic genius. He wishes to create a replica of ogre artwork for the House of Orsimer Glories.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5637 => array(
			'name'    => "Veteran Maelstrom Arena",
			'journal' => "I have been welcomed into the bizarre realm of the Daedric Demiprince Fa-Nuit-Hen. In order to test my mettle and demonstrate my skills, I've agreed to enter the Maelstrom Arena, where deadly challenges created by Fa-Nuit-Hen's Barons await me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4408 => array(
			'name'    => "Spirited Away",
			'journal' => "Ancalin, an old Altmer woman, asked me to pray at the grave markers of her four children. She hopes my travels around the island will give them some peace.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6849 => array(
			'name'    => "A Sea of Troubles",
			'journal' => "Vicious Sea Elf raids and strife among the druid circles beset the beautiful island of Galen. House Mornard requires capable adventurers to help restore peace to the isle.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3259 => array(
			'name'    => "Capture Farragut Mine",
			'journal' => "I accepted a mission to help capture Farragut Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3260 => array(
			'name'    => "Capture Blue Road Mine",
			'journal' => "I accepted a mission to help capture Blue Road Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3263 => array(
			'name'    => "Capture Faregyl Mine",
			'journal' => "I accepted a mission to help capture Faregyl Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6843 => array(
			'name'    => "Sojourn of the Druid King",
			'journal' => "A druid named Laurel hired me to help her deal with the ruthless Firesong Druids while she and her partner, the mage Dhulef, investigate the past movements of the ancient druids of Galen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3265 => array(
			'name'    => "Capture Roebeck Mine",
			'journal' => "I accepted a mission to help capture Roebeck Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4326 => array(
			'name'    => "The Veiled Choice",
			'journal' => "I met a mer named <<1>> near the ruin known as Quendeluun. He claims the Veiled Heritance took his daughter. He tracked them to the site.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3943 => array(
			'name'    => "The Prisoner",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6724 => array(
			'name'    => "Destruction Incarnate",
			'journal' => "Our only clue as to the next step in Sister Celdina and the Waking Flame's plans is an encoded note we can't decipher. Both Lyranth and the Anchorite believe that we can find someone in Fargrave who can translate it for us.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3266 => array(
			'name'    => "Capture Brindle Mine",
			'journal' => "I accepted a mission to help capture Brindle Mine for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6151 => array(
			'name'    => "A Duelist's Dilemma",
			'journal' => "Roguzog, an Orcish visitor to Summerset, wants to settle a grievance with a High Elf named Lirendel. We'll have to learn all the rules and customs of High Elf dueling before Roguzog can get bloody satisfaction.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4922 => array(
			'name'    => "The Orrery of Elden Root",
			'journal' => "If <<1>> is to prove herself worthy of leading the Aldmeri Dominion to glory, she must complete the ratification ceremony in <<c:2>> of <<3>>. Legends say it can reveal those worthy of becoming emperor … or empress.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3267 => array(
			'name'    => "The Fall of Faolchu",
			'journal' => "I stand with Captain Darien Gautier and the rescued Camlorn citizens. The time to retake Camlorn from Faolchu has arrived.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3281 => array(
			'name'    => "Leading the Stand",
			'journal' => "After we thwarted the Imperial plot in Evermore, Uela the crow paid me another visit to say we're enemies again now and that her Reachmen army is going to kill everyone in Evermore.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6391 => array(
			'name'    => "Kill Enemy Necromancers",
			'journal' => "Grand Warlord Zimmeron has authorized a reward for any Pact soldier who slays 20 enemy Necromancers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5034 => array(
			'name'    => "A Grave Situation",
			'journal' => "With Honrich Tower secure, <<1>> asked that I seek out <<2>> in Taarengrav. The tomb holds one of Ysgramor's great companions and must be protected from the Worm Cult.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3444 => array(
			'name'    => "Kill Thirty Aldmeri Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6408 => array(
			'name'    => "The Deadliest Prey",
			'journal' => "Kidnappers abducted Skordo the Knife, but their caravan fell prey to a Dragon attack. The abductors left the hard-nosed Orc behind, but dragged off two of his companions. Skordo wants to free his friends and put a stop to the kidnappers' schemes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3283 => array(
			'name'    => "Werewolves to the North",
			'journal' => "<<1>> said that the Wyrd Tree will recover, but another danger has appeared in the north.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3947 => array(
			'name'    => "Kill the Leader",
			'journal' => "Geoffroi has asked me to kill Remelie Frinck, the Fire Mage leading the Supernal Dreamers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3286 => array(
			'name'    => "Under Siege",
			'journal' => "A contingent of Shornhelm Guard soldiers was on patrol when it was attacked by House Montclair troops. The Guard survivors have taken refuge in an old tower on Hinault Farm, where they continue to be under siege by Montclair's forces.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4680 => array(
			'name'    => "Storm on the Horizon",
			'journal' => "An Aldmeri Dominion fleet was shipwrecked here by a hurricane. I wasn't part of the wreck, but perhaps I can use the chaos as cover.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3963 => array(
			'name'    => "The Returned",
			'journal' => "In the wake on an Imperial attack, homes are on fire and Daedric creatures are shambling through the village.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6170 => array(
			'name'    => "Divine Deputation",
			'journal' => "The Divine Prosecution seeks able-bodied adventurers to assist with the enforcement of laws by performing contract work throughout Summerset.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4967 => array(
			'name'    => "One of the Undaunted",
			'journal' => "The Khajiit, <<1>>, has invited me to join a group of adventurers called the Undaunted.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3302 => array(
			'name'    => "The Miners' Lament",
			'journal' => "The Red Rooks have taken over the Crosswych Mines, turning the miners into slaves. Further, anyone who resists them are sent into the mines as prisoners.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6596 => array(
			'name'    => "The Symbol of Hrokkibeg",
			'journal' => "I came across the body of <<1>>. She apparently died while bringing a sacred relic to a shrine in Markarth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4347 => array(
			'name'    => "Special Blend",
			'journal' => "I came across a dead body on the path to the Everfull Flagon. The dead body clutches a note.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3448 => array(
			'name'    => "Scout the Lumbermill",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3303 => array(
			'name'    => "Thwarting the Aldmeri Dominion",
			'journal' => "While exploring, Lady Clarisse Laurent mistakenly opened a portal allowing Aldmeri Dominion soldiers to invade the ruins of Salas En. I've agreed to help her close the portal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4380 => array(
			'name'    => "Last Night",
			'journal' => "I stumbled onto the ruins of a campsite by the beach. Sea Drakes attacked and kidnapped two people last night.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6606 => array(
			'name'    => "The Lost Shrine",
			'journal' => "The Wayward Guardians want to cleanse a sacred shrine atop the Lost Valley Redoubt from the taint of invaders and the Gray Host.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5952 => array(
			'name'    => "To the Victor",
			'journal' => "Battlemaster Rivyn has challenged me to win Battleground matches. I can join games and find worthy opponents via the Group and Activity Finder.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4237 => array(
			'name'    => "Life of the Party",
			'journal' => "<<1>> has been trapped in Sanguine's celebration for four years. He's convinced he can only escape if the other party goers are distracted by an entertainment troupe from Morrowind.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3330 => array(
			'name'    => "Retaking Camlorn",
			'journal' => "I returned from the past with the knowledge we need to defeat Faolchu. Fire weakens him and makes him vulnerable.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6755 => array(
			'name'    => "In Secret and Shadow",
			'journal' => "A talking skull near Death's Valor Keep is promising to give treasure to whoever helps it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3305 => array(
			'name'    => "The Oldest Orc",
			'journal' => "I discovered an Orc imprisoned in the dungeons below Salas En. He claims to have lived there for centuries, kept alive by Aldmeri enchantment. The stones of his prison glow with mysterious energies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4390 => array(
			'name'    => "Unbridled Wealth",
			'journal' => "<<1>>, a traveling Argonian merchant, lost her guar and a valuable ruby to crocodiles in the swamp. She's too hurt to round them up, so she asked me to search for the ruby and her favorite guar, <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6366 => array(
			'name'    => "Beware the Purring Liar",
			'journal' => "A ragged preacher warned me that my wealth and status attract a lot of dangerous attention in Elsweyr, particularly from the folk-hero-turned-god, Rajhin. The preacher solicited me to steal offerings to appease the Khajiit thief-god.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3863 => array(
			'name'    => "Taking the Tower",
			'journal' => "<<1>> turned up again in Ebonheart looking to collaborate on another \"business venture.\" He wants me to procure a bottle of <<2>> from an innkeeper.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4509 => array(
			'name'    => "Harvest Time",
			'journal' => "Produce around Cropsford is not being harvested. It's going to go bad unless something's done soon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7027 => array(
			'name'    => "A War of Scribes",
			'journal' => "A mage called Valinna staged a coup and now leads the Scribes of Mora. An old Khajiit named Keshargo needs my help to reclaim the Itinerant Staff and remove Valinna from her position of power.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4150 => array(
			'name'    => "Sleeping on the Job",
			'journal' => "Something is definitely happening at Fort Amol. Groups of high-ranking Nords, Argonians, and Dark Elves are on hand, and the guards are turning travelers away from the inn. I should try to figure out what's going on.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3314 => array(
			'name'    => "The Corpse Horde",
			'journal' => "I encountered an injured Lion Guard knight named Sir Malik Nasir in Merovec's Folly. He wants to stop the Bloodthorn cultists from desecrating the dead Lion Guard soldiers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3315 => array(
			'name'    => "The Hidden Treasure",
			'journal' => "I encountered the spirit of Richard Dusant, who died searching for the hidden treasure of Dresan Keep. He wants me to finish his search for the Keep's ancient secrets.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6244 => array(
			'name'    => "The Remnant of Argon",
			'journal' => "Kassandra, having evaded Jaxsik-Orrn and Xukas, entered a second xanmeer called Vakka-Bok. Famia and I must enter this vast ruin and prevent Kassandra from acquiring the Remnant of Argon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3810 => array(
			'name'    => "Motive for Heresy",
			'journal' => "With Vox's wards shattered, the path to her lair is open. Aspera the Forgotten suggests this creates an opportunity to defeat Vox, but I'm not sure I can trust this strange projection.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4379 => array(
			'name'    => "Lover's Torment",
			'journal' => "Alanwe lives a tortured existence in the Crypt of Hearts. Her soul has been shattered, and the pieces strewn throughout the Crypt. I have promised to help her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3335 => array(
			'name'    => "The Charge of Evermore",
			'journal' => "Soldiers from Evermore were defeated at Murcien's Hamlet by a Reachman attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6507 => array(
			'name'    => "Blood of the Past",
			'journal' => "Gwendis arrived at Castle Thorn to investigate a story that Lady Thorn has started amassing an army. She wants to know what the lady is planning and asked me to help uncover it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4276 => array(
			'name'    => "The Troubleshooter",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3818 => array(
			'name'    => "A Saint Asunder",
			'journal' => "The Shrine of Saint Veloth has been desecrated by Magistrix Vox. This Daedric attack has left the spirit of Veloth in a weakened state.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3343 => array(
			'name'    => "Crosswych Reclaimed",
			'journal' => "<<1>> says the Crosswych resistance has gained enough momentum. It's time to deal with the Red Rooks' leader, <<2>> himself.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6627 => array(
			'name'    => "The Emperor's Secret",
			'journal' => "Eveli Sharp-Arrow and Lyranth the Dremora are chasing the same mystery-a secret concerning the last Longhouse Emperor, Leovic. Clues point to his steward Farrul Lupus, who hired Eveli to acquire a strange book. We need to bring it to him in Riften.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4774 => array(
			'name'    => "The Citadel Must Fall",
			'journal' => "King Dynar has devised a two-pronged attack to take control of the Reaver Citadel. It utilizes the old Fighters and Mages Guilds' halls located in this ancient Nirn city.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3367 => array(
			'name'    => "The Nature of Fate: Part Two",
			'journal' => "A Redguard named Hadoon believes ones fate is not predetermined. He believes one can break from destiny but needs help proving his theory.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5340 => array(
			'name'    => "Blood and the Sacred Words",
			'journal' => "The Vosh Rakh attacked Fharun Stronghold, killing people in Trinimac's name. A dying archivist claimed they sought a secret sanctuary and the sacred words of Malacath that are hidden within.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3742 => array(
			'name'    => "Geoffroi's Shattered Pride",
			'journal' => "Lord Geoffroi Stictal wants the shards of his Crystal of Power back so his wizard can repair it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4977 => array(
			'name'    => "Ancient Stones, Ancient Words",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3379 => array(
			'name'    => "Angof the Gravesinger",
			'journal' => "Darien discovered that we need three shard-keys to open the three locks that protect Angof's inner sanctum.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5599 => array(
			'name'    => "Questions of Faith",
			'journal' => "Hildegard never returned to the Sanctuary after going out to pray for Tanek. Kor asked me to help him find her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3730 => array(
			'name'    => "Whispers of the Wisps",
			'journal' => "<<1>> is plagued by wisps. They're stealing away travelers at the gravesite. No one is sure what caused their sudden aggression.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3383 => array(
			'name'    => "Badwater Mine",
			'journal' => "I met a miner named Samsi at the Badwater mine. She tells me that the mine is collapsing and her partner is trapped inside. I told her I would go in and find her partner. If he's dead, she still needs his supply pack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5053 => array(
			'name'    => "The Lost Patrol",
			'journal' => "In the tangled paths of <<1>>, I found the bodies of a Lion Guard officer and his patrol.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5232 => array(
			'name'    => "Kill Enemy Sorcerers",
			'journal' => "Grand Warlord Sorcalin has authorized a reward for any Dominion soldier who slays 20 enemy Sorcerers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4601 => array(
			'name'    => "Right of Theft",
			'journal' => "The Shade of Prince Naemon and the Veiled Heritance have somehow raised the infamous warriors of the Blackroot clan from the dead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3389 => array(
			'name'    => "Capture the Farm",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6651 => array(
			'name'    => "A Reign of Frogs",
			'journal' => "I agreed to hunt down a dangerous death hopper and collect singing frogs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4292 => array(
			'name'    => "A Ritual in Smokefrost Peaks",
			'journal' => "The Worm Cult is performing a ritual at an altar somewhere in the Smokefrost Peaks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3391 => array(
			'name'    => "Capture the Lumbermill",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6971 => array(
			'name'    => "Fate's Proxy",
			'journal' => "A dark conspiracy against Hermaeus Mora threatens both Nirn and his realm of Apocrypha, placing all of reality in danger. The Daedric Prince of Knowledge requires a mortal agent to help preserve reality.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3393 => array(
			'name'    => "Kill Thirty Enemy Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6016 => array(
			'name'    => "A Masterful Weapon",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4301 => array(
			'name'    => "Relic Rescue",
			'journal' => "Torinaan's strange little relic-keeper worries the holy relics will be corrupted by Daedric energies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3394 => array(
			'name'    => "Kill Thirty Ebonheart Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7113 => array(
			'name'    => "A Calamitous Error",
			'journal' => "<<1>> wants me to go to Fort Colovia and burn crates full of misprinted invitations before Count Calantius becomes aware of the derisive error.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3395 => array(
			'name'    => "Kill Thirty Daggerfall Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5260 => array(
			'name'    => "Pledge: Spindleclutch I",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to explore the cavern called Spindleclutch beneath Glenumbra.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7106 => array(
			'name'    => "Pledge: Oathsworn Pit",
			'journal' => "Urgarlag Chief-bane of the Undaunted has challenged me to explore Oathsworn Pit.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3396 => array(
			'name'    => "Scout the Farm",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4737 => array(
			'name'    => "The Waking Dreamer",
			'journal' => "I found the journal of Graccus, an Imperial who destroyed a Bosmer village to find a treasure buried below.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5646 => array(
			'name'    => "Crime Spree",
			'journal' => "Rumors are floating around that the Thieves Guild isn't a threat anymore. The Thieves Guild wants to quash the rumors with a series of conspicuous thefts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3403 => array(
			'name'    => "Kill an Enemy Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3404 => array(
			'name'    => "Kill a Daggerfall Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3405 => array(
			'name'    => "Kill an Ebonheart Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5771 => array(
			'name'    => "Dawn of the Exalted Viper",
			'journal' => "I met Titus Valerius in Belkarth. He was hoping to find Kelmen to assist him and the Thief with a ritual to locate a magical disturbance near Skyreach, but the mage is nowhere to be found.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3406 => array(
			'name'    => "Kill Ten Enemy Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3412 => array(
			'name'    => "A Dangerous Dream",
			'journal' => "A Lion Guard patrol on its way from Daggerfall to Wayrest was attacked by Daedric Cultists who referred to themselves as 'Supernal Dreamers'. The cultists kidnapped the guard captain.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3414 => array(
			'name'    => "Legacy of Baelborne Rock",
			'journal' => "<<1>> just learned that he inherited an estate. Unfortunately, the estate appears to be haunted and is overrun by imps.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3782 => array(
			'name'    => "JB - Generic Wave Event - Surv",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3419 => array(
			'name'    => "Kill Ten Enemy players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3436 => array(
			'name'    => "Foul Deeds in the Deep",
			'journal' => "I found <<1>> within the tunnels of <<2>>. She claims she and her husband, <<3>>, were kidnapped by the Oathbound, at the behest of an Orc named <<4>>. She managed to escape, but needs help rescuing her husband.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6341 => array(
			'name'    => "Skeleton Demonstration",
			'journal' => "Zahari asked me to help train new recruits for the Khajiit Defense Legion to fight enemies they may not have encountered in training.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5823 => array(
			'name'    => "Madness in Grahtwood",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Grahtwood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3442 => array(
			'name'    => "Capture Balfiera Lumbermill",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4630 => array(
			'name'    => "Regret",
			'journal' => "I found a note on the body of an Ebonheart Pact soldier. It mentions an egg brother named Bosekus in the town of Cropsford.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3794 => array(
			'name'    => "Divine Favor",
			'journal' => "<<1>> knows his monks cannot defend <<2>> against a determined assault from the Kasgesh Ashlanders. He's asked me to appeal to the Tribunal for aid.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3447 => array(
			'name'    => "Scout the Farm",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3450 => array(
			'name'    => "Scout Fort Balfiera",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7041 => array(
			'name'    => "Devourer of Dreams",
			'journal' => "Valkynaz Dek of the Dreamcarver Clan captured some Ordinator agents. A powerful Dremora like Dek operating in Apocrypha is a great concern.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3451 => array(
			'name'    => "Kill a Daggerfall Player",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4802 => array(
			'name'    => "To Moonmont",
			'journal' => "A mysterious Khajiit named Hunal spoke to me outside of the city of Dune. He told me some important task awaits me at the temple known as Moonmont.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3452 => array(
			'name'    => "Kill an Enemy Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4295 => array(
			'name'    => "The Wayward Dagger",
			'journal' => "A Covenant officer and her husband attempted to defect to the Aldmeri Dominion. Covenant troops pursued the Orc couple all the way to Auridon, and they've taken North Beacon in an attempt to capture the Orcs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6236 => array(
			'name'    => "Grave Expectations",
			'journal' => "Bolu of the Dead-Water tribe, which calls itself the Naga-Kur, wants me to go to Bok-Xul and accomplish the task he set before me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6041 => array(
			'name'    => "Enchanted Accumulation",
			'journal' => "<<1>> requested I gather a magical substance from the runestones in Clockwork City. I'll also need to collect a special crystal that causes the residue to glow.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3802 => array(
			'name'    => "What Happened at Murkwater",
			'journal' => "<<1>> wants me to take word of our accomplishments to Hissmir.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5682 => array(
			'name'    => "Contract: Shadowfen",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3455 => array(
			'name'    => "Kill a Daggerfall Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3458 => array(
			'name'    => "Capture the Lumbermill",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4667 => array(
			'name'    => "Dark Knowledge",
			'journal' => "A spirit named <<1>> warned me of dark knowledge sealed in three forbidden tomes. She asked me to find all three and destroy them in a shrine on <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3460 => array(
			'name'    => "Kill Thirty Enemy Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6305 => array(
			'name'    => "Cadwell the Betrayer",
			'journal' => "We recovered the Dragonhorn, but we have no idea what to do with it. While Abnur Tharn attempts to learn more, Khamira wants me to stop Zumog Phoom from reassembling Cadwell's original body-the evil Cadwell the Betrayer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3611 => array(
			'name'    => "Burning Revenge",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4578 => array(
			'name'    => "Over the Edge",
			'journal' => "An associate of Cariel's stopped me outside of Arenthia to ask for help. She's lost some critical supplies and letters, and wants my help in retrieving them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4615 => array(
			'name'    => "Lost in the Mist",
			'journal' => "Destroying the cursed skull held by the Shademist vampires has cleared the moors of corruption, but Thorinor says his wife was among the missing and never returned.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3462 => array(
			'name'    => "Kill Thirty Aldmeri Players",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6646 => array(
			'name'    => "The Gates of Adamant",
			'journal' => "I awoke imprisoned in a strange ruin. An Elf seems willing to help me, but escaping this place may not be easy. Danger lurks around every corner, and I sense that some darkness arrived with me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3631 => array(
			'name'    => "Recovering the Guar",
			'journal' => "The herd of guar at the sulfur pools stampeded during the Covenant attack. Now they're scattered. If I can calm them and fit them with the proper harnesses, the animals should return to the stable on their own.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3466 => array(
			'name'    => "Scout Fort Balfiera",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3468 => array(
			'name'    => "Kill an Ebonheart Player",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5835 => array(
			'name'    => "If the Spell Fits",
			'journal' => "A naked Nord stands frozen in place in the wilds of Vvardenfell. He claims a Dark Elf witch cast a spell on him, and only she can remove it. He asked me to find her and get her to set him free.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3470 => array(
			'name'    => "Kill an Ebonheart Guard",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6654 => array(
			'name'    => "Of Stone and Steam",
			'journal' => "A savage band of Sul-Xan Argonians raided the ancient city of Rockgrove. Led by Flame-Herald Bahsei, they plan to summon something named Xalvakka. I must purge the area of these brutes and stop their plan before this creature enters our realm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6000 => array(
			'name'    => "To Tel Fyr",
			'journal' => "I met an apprentice to Divayth Fyr in Vivec City, she mentioned an incident at her master's tower involving some kind of rift.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3639 => array(
			'name'    => "Restoring Order",
			'journal' => "A strong Covenant raiding force landed west of Ebonheart. The citizens are too busy with their own internal squabbles to unite against invaders. <<1>> fears the town may fall to Alexandra Conele's forces.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4285 => array(
			'name'    => "A Father's Promise",
			'journal' => "Drublog Orcs attacked the village of Tanglehaven. <<1>> said the survivors barely managed to fend them off. After the massacre, the surviving Tanglehaven militia chased the Drublog into the forest-including <<2>>, <<1>>'s daughter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5855 => array(
			'name'    => "Fish Boon Feast",
			'journal' => "<<1>>, a New Life Herald in Eastmarch, spoke of <<2>>'s New Life Festival in Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3474 => array(
			'name'    => "Scout Kingscrest Farm",
			'journal' => "I accepted a mission to scout Kingscrest Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3476 => array(
			'name'    => "Scout Blue Road Farm",
			'journal' => "I accepted a mission to scout Blue Road Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3627 => array(
			'name'    => "Kinsman's Revenge",
			'journal' => "<<1>> has been taken captive by the Goblins, and Fafnyr was wounded in the fighting. The Nord warrior seeks revenge against the hated Goblins and their spider allies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3478 => array(
			'name'    => "Scout Alessia Farm",
			'journal' => "I accepted a mission to scout Alessia Farm for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4872 => array(
			'name'    => "Bounty: Goblins",
			'journal' => "I accepted a Fighters Guild contract in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3647 => array(
			'name'    => "Shattering Mirror",
			'journal' => "Ra'shadda, the Khajiiti shaman who summoned the monster, didn't leave the plantation. Ruvali speculates that he wanted to stay behind to watch the monster tear them apart.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3482 => array(
			'name'    => "Trapped in the Bluffs",
			'journal' => "<<1>> tells me his son, <<2>>, has gone missing somewhere in the oasis near Bergama.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3497 => array(
			'name'    => "Wave Event Template Quest",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3509 => array(
			'name'    => "Vines and Villains",
			'journal' => "A Red Rook messenger was captured near the Dwynnarth Ruins Lion Guard camp while he was on his way to a secret meeting between the Red Rooks and the Bloodthorn cult.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5366 => array(
			'name'    => "Group Layering Test",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3695 => array(
			'name'    => "Aggressive Negotiations",
			'journal' => "Tanval Indoril has fled towards the Tormented Spire with the Coral Heart in hand. He wants to unleash Sadal, the Brother of Strife, from his imprisonment. Kragenmoor's <<1>> is the only one who can allow me access to the mountain.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6731 => array(
			'name'    => "Robhir's Final Delivery",
			'journal' => "I came across the body of a Redguard. It looks like he was in the middle of delivering something to Nass in the House of Whims in Fargrave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5012 => array(
			'name'    => "Rusty Daggers",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3530 => array(
			'name'    => "Destroying the Dark Witnesses",
			'journal' => "Defeating the Dark Witnesses can only be accomplished by killing the Hagraven, Uela. Some of the wyrd sisters up near Jackdaw Cove have been searching for where Uela is hiding, but they need help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6281 => array(
			'name'    => "Swamp Jelly Sonata",
			'journal' => "An eccentric Argonian by the name of <<1>> asked me to help her fulfill a dream of hers, somehow involving creatures called swamp jellies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5964 => array(
			'name'    => "A Web of Troubles",
			'journal' => "Lady Clarisse Laurent and her man-servant Stibbons, along with a number of research assistants on loan from the Mages Guild, have been studying the ruins of Bal Fell. Now, everyone but Lady Laurent has disappeared.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3587 => array(
			'name'    => "Delaying the Daggers",
			'journal' => "<<1>> asked me to help slow the Covenant's siege of Davon's Watch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3566 => array(
			'name'    => "Kingdom in Mourning",
			'journal' => "King Fahara'jad has informed me of dire circumstances in Bangkorai. King Eamond is dead and the Reachmen are marching on the kingdom's capital of Evermore. He has asked me to go there and try to rally the city's defenses.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3595 => array(
			'name'    => "Wayward Son",
			'journal' => "Furon is worried that his brother Giron has joined a \"circle of mages.\" He wants my help to understand what they're doing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6703 => array(
			'name'    => "The Key and the Cataclyst",
			'journal' => "We recovered the relic stolen from Lyranth. It's a portal key. Before we use it to find the base mentioned in the Waking Flame letter, Lyranth wants to make preparations.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3574 => array(
			'name'    => "Demo OR Quest",
			'journal' => "I met a man who had a wife, said he would keep her all his life. The one day he met a girl.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4968 => array(
			'name'    => "Caring for Kwama",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5301 => array(
			'name'    => "Pledge: Tempest Island",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to explore Tempest Island, a storm-tossed outcropping off the coast of Malabal Tor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3585 => array(
			'name'    => "Legacy of the Ancestors",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3588 => array(
			'name'    => "City Under Siege",
			'journal' => "Covenant forces have scaled the walls of Davon's Watch. Now they're threatening to take the city from within its walls. Tanval's ritual may be our only hope to end the assault.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4417 => array(
			'name'    => "Bloodied Waters",
			'journal' => "Imperial archers are preventing rebel troops from crossing the river.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5307 => array(
			'name'    => "Pledge: Selene's Web",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to explore Selene's Web, a spider-haunted grove in Reaper's March.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3589 => array(
			'name'    => "Quiet the Ringing Bell",
			'journal' => "With the Covenant driven from Davon's Watch, <<1>> has asked me to return the living weapon known as Balreth to its cell.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3590 => array(
			'name'    => "Through the Aftermath",
			'journal' => "The remnants of a broken siege lie outside Davon's Watch, broken by the appearance of an army of ghosts. A sergeant at the front, a Nord named Rhorlak, has asked me to decide how best to deal with the spirits.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6851 => array(
			'name'    => "Bane of Dragons",
			'journal' => "The Elsweyr Dragon Defense Force is training new members in Elsweyr. I agreed to undergo their trials to join their ranks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4486 => array(
			'name'    => "Down the Skeever Hole",
			'journal' => "With the Colovians removed from the village, Captain Odreth has planned a counterattack with his Rangers. We are to use a secret tunnel to gain access to the Colovian camp and take them out.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3591 => array(
			'name'    => "The Venom of Ahknara",
			'journal' => "Covenant troops have killed Captain Hjolm of Fort Arand, and they're threatening the safety of the recruits within the fort. Captain Llaari hatched a plan to respond to the threat.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3592 => array(
			'name'    => "Proving Trust",
			'journal' => "A volcanic eruption and an infestation of shalks have devastated the town of Senie. The townsfolk are wounded, but distrust between Dark Elves and Argonians delay efforts to heal them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4246 => array(
			'name'    => "Deception in the Dark",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3594 => array(
			'name'    => "Enslaved in Death",
			'journal' => "Even though they're dead, Ix-Utha's Argonian brethren still act as slaves to the risen Chimer. They've returned to serve their cruel spectral masters.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7307 => array(
			'name'    => "Ossified Bravery",
			'journal' => "I met Nithima who told me that the Fighters Guild needs recruits to help with an initiative to strike at the heart of their enemies in Coldharbour.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3597 => array(
			'name'    => "Generic Escort Quest",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5588 => array(
			'name'    => "Memories of Youth",
			'journal' => "<<1>> runs a finder's business and uses the Thieves Guild to track down his quarries. Helping him will surely reflect well on my skill as a thief and be lucrative to boot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3598 => array(
			'name'    => "Giving for the Greater Good",
			'journal' => "<<1>> is grateful the dead have been put to rest. She wants to give of herself to restore the Starved Plain. She's proposed a sacrifice from herself and her students that will allow plants to grow again on the plain.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3600 => array(
			'name'    => "Oath Breaker",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7320 => array(
			'name'    => "Unearthed and Unexplored",
			'journal' => "Naj-Caldeesh is an abandoned xanmeer with a legendary secret. Talen-Lah and his band of Wuj-Laxul necromancers overran the xanmeer as soon as it was discovered. Talen-lah wants to take the xanmeer's power for himself.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3602 => array(
			'name'    => "Percussive Ranching",
			'journal' => "<<1>> lost most of his kwama in a mine collapse. He asked me to help round up the survivors and convince them to return home.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5471 => array(
			'name'    => "Thicker Than Water",
			'journal' => "<<1>> continues to restore the long-lost reputation of Orc cuisine. She needs additional rare ingredients for a special dessert.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3604 => array(
			'name'    => "Challenge the Tide",
			'journal' => "Svanhildr wants to break the Oathbound offensive once and for all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7315 => array(
			'name'    => "[Template] No IC",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5974 => array(
			'name'    => "A Masterful Plate",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3605 => array(
			'name'    => "Ritual of Anguish",
			'journal' => "I encountered a tormented Dark Elf spirit in the Orc camp at Malak's Maw. She asked for aid in easing her torment.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3608 => array(
			'name'    => "The Medallions of Saint Veloth",
			'journal' => "Healer Ravel asked me to collect Saint Veloth medallions. As long as the monks are allowed to pray in peace, the medallions allow the power of the Tear of Saint Veloth to keep the Llodos plague in check on whoever wears one.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6298 => array(
			'name'    => "Puzzle Test Switches",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3612 => array(
			'name'    => "The Naked Nord",
			'journal' => "I met a confused Nord named Norgred on the road outside of Quarantine Serk. He said he was escorting a witch. Apparently, she turned on him and stole a valuable axe. He's anxious to get it back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3616 => array(
			'name'    => "Rending Flames",
			'journal' => "I awakened the spirit of an ancient Dark Elf scribe named Mavos Siloreth. Mavos's spirit can't remember how he bound Balreth in the original ceremony. To help me, he's going to perform a ritual and show me a vision of the past.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3617 => array(
			'name'    => "Quieting a Heart",
			'journal' => "Bala, an Ashlander, is searching for her deceased lover, Eanen Varam. In life, he was part of House Dunmer, so their status kept them apart. They promised each other to unite in death, but now, she can't find his tomb.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6403 => array(
			'name'    => "The Pride of Alkosh",
			'journal' => "The Order of the New Moon plans to attack Pridehome, a reclusive temple dedicated to the worship of Alkosh. The Dragonguard is determined to stop the cult's assault and uncover the motives behind the attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3977 => array(
			'name'    => "To Vernim Woods",
			'journal' => "A scout sent by Centurion Gjakil has found me near Fallowstone Hall. The scout reports Reachmen movement near <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6894 => array(
			'name'    => "And Now, Perhaps, Peace",
			'journal' => "The various threats to the Systres Archipelago have been dealt with and now Lady Arabelle wants to get on with the long-delayed peace talks. First, however, she has a few other tasks for me to take care of.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5037 => array(
			'name'    => "Stem the Tide",
			'journal' => "Covenant soldiers are massing within Softloam Cavern. It won't be long before they have the numbers to launch an attack on Kragenmoor and the rest of Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3618 => array(
			'name'    => "To Ash Mountain",
			'journal' => "I've learned all I can at Othrenis. Onuja says that Balreth is somewhere near Ash Mountain. <<1>> has led many Pact soldiers there with hopes of containing the Brother of Strife.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4610 => array(
			'name'    => "The Army of Meridia",
			'journal' => "I need to rebuild my army if we're going to have any chance of confronting Molag Bal and stopping the Planemeld. The mysterious Groundskeeper suggests I rescue as many people as I can and send them to the Hollow City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3981 => array(
			'name'    => "To Taarengrav",
			'journal' => "Netapatuu fears the Worm Cult may try to enslave Valdur's spirit, much as they tried with Hakra's.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7340 => array(
			'name'    => "Tamrielic Golden Sap",
			'journal' => "<<1>> asked me to gather golden sap from nature spirits across Tamriel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3622 => array(
			'name'    => "The Brothers Will Rise",
			'journal' => "<<1>> sent me into a deep vision with the help of an ancient ritual site. In this vision of the past, I should be able to interact with Balreth and Sadal. Then I can learn how the Brothers of Strife were born.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5097 => array(
			'name'    => "The Beast Pits",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7155 => array(
			'name'    => "The Forgotten Vault",
			'journal' => "The Blind Path, led by the Blind, invaded a subsection of Maelstrom called Bedlam Veil. Fa-Nuit-Hen asked me to figure out what the Blind is looking for and deal with the Blind Path army.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3624 => array(
			'name'    => "The Saving of Silent Mire",
			'journal' => "Leel-Vata asked me to rescue the captured Argonian villagers from the Khajiiti pirates.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4278 => array(
			'name'    => "A Village Awakened",
			'journal' => "<<1>> senses that a powerful Daedra has wrested control of the animals. As long as the Daedra remains, Merormo won't free the villagers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4435 => array(
			'name'    => "Simply Misplaced",
			'journal' => "My first trial in Sheogorath's realm awaits. Valaste and Shalidor have uncovered what they need to get me where I need to go.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3985 => array(
			'name'    => "Tracking the Game",
			'journal' => "I've encountered a wounded hunter in the wilds near Bleakrock Village. <<1>> is seriously wounded. He wants some help killing his prey.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7344 => array(
			'name'    => "Tamrielic Gleaming Resin",
			'journal' => "<<1>> asked me to gather gleaming resin from rare, powerful nature spirits across the territories of all three alliances in Tamriel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3626 => array(
			'name'    => "Protecting the Hall",
			'journal' => "The lands of the mighty Hrogar are overrun with Goblins and spiders. The Nord has lost many men to the beasts, and he's under siege.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3632 => array(
			'name'    => "Breaking Fort Virak",
			'journal' => "The Covenant has seized Fort Virak. They're serving under the command of General Serien. The Pact's army-led by the Indorils, <<1>>, and Walks-in-Ash-has besieged the fort in an attempt to retake it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7359 => array(
			'name'    => "Famished Fellowship",
			'journal' => "Armorer Reistaff asked me to prepare three of any provisions I can for the Stirk Fellowship. I agreed to help. He said I could ask nearby chefs and brewers if I needed recipes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5624 => array(
			'name'    => "Contract: Alik'r Desert",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3633 => array(
			'name'    => "Evening the Odds",
			'journal' => "I've infiltrated the ruins beneath Fort Virak. <<1>> asked me to help secure our foothold within the fort's main courtyard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7352 => array(
			'name'    => "Bows for the Fellowship",
			'journal' => "Armorer Reistaff asked me to craft three bows of any kind for soldiers in the Stirk Fellowship. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3634 => array(
			'name'    => "The General's Demise",
			'journal' => "Fort Virak is ours. All that remains is to kill General Serien, the leader of the Covenant forces in Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6578 => array(
			'name'    => "Into the Deep",
			'journal' => "I arrived at an abandoned mine in Deshaan and found a young Dark Elf named Drathas and a Dremora named Lyranth. Lyranth offered a deal of gold in exchange for assisting in her search for a lockbox she cannot access.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4209 => array(
			'name'    => "Teldur's End",
			'journal' => "Teldur has kidnapped Canonreeve Valano and taken him into the manor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3635 => array(
			'name'    => "City at the Spire",
			'journal' => "<<1>> warned me that Tanval will likely try to release the second Brother of Strife from Tormented Spire. To ascend the spire I'll need to speak to the grandmaster of House Dres in Kragenmoor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7347 => array(
			'name'    => "Helms for the Headstrong",
			'journal' => "Armorer Reistaff asked me to craft two helms for soldiers in the Stirk Fellowship. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3637 => array(
			'name'    => "Godrun's Dream",
			'journal' => "The Omen of Betrayal failed to manipulate General Godrun into attacking Wayrest, but has now trapped General Godrun within his dream. Abbot Durak has asked me to enter the dream and kill the Omen to save Godrun.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6209 => array(
			'name'    => "Kill 150 Enemy Players",
			'journal' => "Grand Warlord Zimmeron has authorized a bonus for any Pact soldier who slays 150 enemy combatants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3643 => array(
			'name'    => "What Was Done Must Be Undone",
			'journal' => "I've helped the survivors at Heimlyn Keep. Now I need to end the curse. Its mystical corruption emanates from a Telvanni reliquary.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4482 => array(
			'name'    => "Rat Problems",
			'journal' => "<<1>> has been having problems with rats down his well.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4857 => array(
			'name'    => "The Concealing Veil",
			'journal' => "Verandis completed his research and discovered that his missing memories lead to an ancient ruin in the Lorkrata Hills. He needs my help to discover the secret hidden within the ruins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3645 => array(
			'name'    => "The Sapling",
			'journal' => "<<1>> fears for the survival of the Hist Tree the Argonians transplanted from Black Marsh to Ebonheart. He thinks the Dark Elves of House Dres poisoned it, so he wants to fertilize it to encourage its growth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7097 => array(
			'name'    => "A Matter of Propriety",
			'journal' => "Grander Nemarcet was robbed of his dowry by a brigand and her pack of beasts. He asked me to retrieve his stolen belongings so he can finalize his marriage.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5244 => array(
			'name'    => "Pledge: Banished Cells I",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to delve the Banished Cells beneath Auridon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3646 => array(
			'name'    => "An Unwanted Twin",
			'journal' => "A frightened soldier on the road told me about a slave revolt at Sathram Plantation. Ulov Stormwall led troops to the plantation, intending to suppress the revolt. Mysterious circumstances induced him to turn his blade on his own troops.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3648 => array(
			'name'    => "A Story Told in Footprints",
			'journal' => "I found a note on a dead Khajiit with plans for the riot. Apparently, the riot started with fires, followed by the freeing of prisoners.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5069 => array(
			'name'    => "The Warrior's Call",
			'journal' => "I met a strange Imperial who was being held captive by the Daggerfall Covenant. He believes he knows how to defeat the cursed army of Anka-Ra that has laid siege to Craglorn since the Celestials appeared.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5248 => array(
			'name'    => "Pledge: Fungal Grotto II",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to brave Fungal Grotto in the caverns beneath Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3650 => array(
			'name'    => "Cold-Blooded Vengeance",
			'journal' => "An extremely angry Argonian named Azeenus has asked me to seek vengeance against the Khajiiti pirates in his name.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6345 => array(
			'name'    => "Dragon Lore: Stormcrag Crypt",
			'journal' => "Dragons have not been seen in Tamriel for generations, and the Khajiit are unsure how best to fight against them. The ancient Nords recorded Dragon fighting techniques on scrolls they keep in their crypts. I need to find these scrolls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3651 => array(
			'name'    => "The Trial of the Ghost Snake",
			'journal' => "Farseer Bodani has placed the fate of Raston Vendil in my hands. I agreed to undertake a trial and see if the Ghost Snake will counsel me on this matter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6338 => array(
			'name'    => "The Usurper Queen",
			'journal' => "After learning that Mulaamnir the Dragon considers Euraxia, the usurper queen of Rimmen, to be nothing more than a puppet, Abnur Tharn believes parley is our best course of action-especially considering the current state of the Khajiiti army.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3652 => array(
			'name'    => "Fighting Back",
			'journal' => "Minister Ralden and Giron are missing. Advisor Varon believes that Giron is working with the Maulborn and has abducted Ralden.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4481 => array(
			'name'    => "Small Town Problems",
			'journal' => "Clan Mother Shuzura has apparently been very hard on her assistants. There are problems in the little town of S'ren-ja. Perhaps I can help out.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3653 => array(
			'name'    => "Ratting Them Out",
			'journal' => "Guard Rila wants me to slow down a group of agitated citizens to keep them from getting killed. She asked me to disrupt their meeting by using something they fear. I need rats.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3656 => array(
			'name'    => "Search and Rescue",
			'journal' => "Noxious fumes have leaked into Deepcrag Den, a kwama mine. The workers fell ill and the normally docile kwama became enraged.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4116 => array(
			'name'    => "Snow and Flame",
			'journal' => "Fildgor the usurper fled to the ancient dragonshrine of Skuldafn to steal Queen Nurnhilde's spirit and force the Crown of Freydis to recognize him. I need to stop him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7376 => array(
			'name'    => "Lost Among the Ashes",
			'journal' => "Two academics, Sirilonwe and Terinvel, came to study Ashbound Hall, a vampire prison that arrived with the planemeld on Solstice. However, Terinvel went missing and Sirilonwe needs help finding him among the deadly vampires.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3658 => array(
			'name'    => "A Timely Matter",
			'journal' => "I agreed to help Madras rescue the missing villagers. However, I need to prove myself to him before he'll tell me where they're being held.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5891 => array(
			'name'    => "Falkreath's Demise",
			'journal' => "Falkreath Hold is under siege by an army of Reachmen and minotaurs. It may well fall without outside intervention, and soon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3660 => array(
			'name'    => "Hiding in Plain Sight",
			'journal' => "Councilor Ralden and Giron are missing, along with many villagers. Varon Davel believes Giron is working with the Maulborn. Apparently, Giron abducted Ralden. I need to find them before it's too late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3661 => array(
			'name'    => "Mechanical Murder",
			'journal' => "I came across the wreckage of a trade caravan on the road between Mournhold and Narsis. Attacks on caravans don't usually occur this close to Mournhold, and the presence of a Dwarven construct makes this incident even more unusual.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7380 => array(
			'name'    => "Discord Among Friends",
			'journal' => "The Blue Sash Guild, a group of tight knit adventurers, is on the verge of breaking up. Kuzam-jo asked me to help put an end to their infighting before it is too late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3662 => array(
			'name'    => "A Bit of Sport",
			'journal' => "The Covenant landed west of Ebonheart, but a band of Nord sawyers have refused to join the general defense. They sneeringly refer to the town's Dark Elves as \"fancy lads.\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5515 => array(
			'name'    => "Free Spirits",
			'journal' => "The vile sorcerer, Gorlar the Dark, continues to terrorize the people of Wrothgar. He has raised countless Orcs as undead thralls and trapped their spirits in Mundus. I must set their spirits free and put this necromancer to the sword.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3663 => array(
			'name'    => "That Which Matters Most",
			'journal' => "House Dres raiders and Dwarven constructs attacked a caravan carrying valuable cargo. The House Hlaalu noble who sponsored this caravan will want it back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3666 => array(
			'name'    => "Rules and Regulations",
			'journal' => "General Gavryn Redoran is acting erratically and completely out of character. Something has changed. I get the impression that Tal'Deic Fortress is in some kind of trouble.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5535 => array(
			'name'    => "A Double Life",
			'journal' => "<<1>> asked me for help in dealing with a personal matter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3667 => array(
			'name'    => "Night of the Soul",
			'journal' => "The Dunmer of Ebonheart have turned to <<1>> for comfort in the face of an expected assault by Covenant forces. Disciple Sildras, however, believes the Tribunal has rejected the town. His gloom is affecting the citizens' morale.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3671 => array(
			'name'    => "To Fort Virak",
			'journal' => "The Covenant has taken Fort Virak, a vital strategic chokepoint.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4317 => array(
			'name'    => "Where the Frostheart Grows",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3672 => array(
			'name'    => "The Game of Chance",
			'journal' => "I met a strange little man who requested that I engage him in a game. He proposes that I acquire three items from some of his fellow carnival members. Once I acquire the items, he will take one from me and turn it into something amazing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6042 => array(
			'name'    => "A Bitter Pill",
			'journal' => "<<1>> requested I gather herbalist's satchels which contain alchemical plants in Clockwork City. I'll also need to collect a sweet syrup to help subdue the bitterness of the tonic created from the satchel's plants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3673 => array(
			'name'    => "Death Trap",
			'journal' => "The Maulborn have occupied the Obsidian Gorge, the pass to Mournhold. House Hlaalu guards have cordoned off the area to keep anyone else from walking into a trap.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5913 => array(
			'name'    => "King Razor-Tusk Hunt",
			'journal' => "Huntmaster Sorim-Nakar of the Erabenimsun Ashlander tribe has given me a challenge. He wants me to hunt the powerful kagouti called King Razor-Tusk.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3674 => array(
			'name'    => "Warm Welcome",
			'journal' => "Shadowfen's Vicecanons are concerned about the Dominion running supply lines for their troops through Alten Corimont.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6362 => array(
			'name'    => "The Serpent's Stampede",
			'journal' => "According to the Baandari broker, Nisuzi, ensorcelled relics draw dozens of minotaurs to the Tomb of the Serpents. She asked me to break the enchantment's grip on the tomb so that her clan can return to looting the crypt's Akaviri treasures.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3676 => array(
			'name'    => "A Pirate Parley",
			'journal' => "<<1>> has a plan to steal treasure from Dominion troops before they find out what happened to <<2>>, their contact in <<3>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5276 => array(
			'name'    => "Pledge: Elden Hollow I",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to explore the ancient complex beneath Elden Root, the Elden Hollow, in the center of Grahtwood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3678 => array(
			'name'    => "Trials of the Burnished Scales",
			'journal' => "Due to archaic rules, <<1>> of the Aldmeri Dominion is allowed to undergo sacred Argonian trials at Hissmir. Success will allow him to commune with the Hist. To determine his true plans, I must pass the trials myself.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3679 => array(
			'name'    => "Dreams From the Hist",
			'journal' => "I completed the Trials of the Burnished Scales at Hissmir, which allows me to commune with the Hist. Doing so will give me insight why Captain Henaamo, a Dominion soldier, would undertake the trials.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5394 => array(
			'name'    => "Woodworker Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Woodworker goods to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3680 => array(
			'name'    => "Of Dubious Value",
			'journal' => "Someone stole several relics from a cart in Hissmir. I offered to help <<1>> find the culprit.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3681 => array(
			'name'    => "Lost Pilgrimage",
			'journal' => "A group of pilgrims failed to arrive in Hissmir as scheduled. <<1>> fears something has happened to them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4388 => array(
			'name'    => "A Life of Privilege",
			'journal' => "I met a Breton in the middle of the swamps of Shadowfen. She seems to be lost.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6370 => array(
			'name'    => "Ache for Cake",
			'journal' => "I received a voucher for a special-occasion cake from the renowned master baker, Chef Donolon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3684 => array(
			'name'    => "Bad Soldiers",
			'journal' => "It appears that soldiers known as Sentinels are terrorizing the fortress.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5275 => array(
			'name'    => "Pledge: Darkshade Caverns II",
			'journal' => "Maj al-Ragath has challenged me to explore the kwama mines of Darkshade Caverns beneath Deshaan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3685 => array(
			'name'    => "The Thin Ones",
			'journal' => "I met Kothringi from <<1>> who told me Worm Cult necromancers plague their village.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7404 => array(
			'name'    => "Representing the Faction",
			'journal' => "My faction leader hired me to represent the faction in a skirmish within the Night Market's districts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3686 => array(
			'name'    => "Three Tender Souls",
			'journal' => "The town of Stormhold is plagued with murders and abductions.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3687 => array(
			'name'    => "Getting to the Truth",
			'journal' => "<<1>> wants me to look into the disappearance of <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3690 => array(
			'name'    => "Scouring the Mire",
			'journal' => "<<1>> suspects the Dominion skin-stealers are based out of <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3696 => array(
			'name'    => "Saving the Son",
			'journal' => "I've discovered where Grandmaster Omin Dres' son is being held captive. The Vanguard is holding his son, Sen, in the tombs west of Kragenmoor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4147 => array(
			'name'    => "The Shackled Guardian",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3697 => array(
			'name'    => "A Gathering of Guar",
			'journal' => "A guar herder named <<1>> has lost control of his herd. Tanval's Vanguard drove them out of their pen, and Ral now fears for their safety.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7109 => array(
			'name'    => "Spinning Out",
			'journal' => "Spinner Urthrendir of the Recollection is enacting a strange ritual in the newly grown jungle. I agreed to stop him and burn the mushrooms providing power to his spell.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3698 => array(
			'name'    => "To the Tormented Spire",
			'journal' => "The grandmaster of House Dres has given me a key that should see me past the wards at Tormented Spire. Now all that remains is to make my way there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5567 => array(
			'name'    => "Dark Revelations",
			'journal' => "Count Carolus still owes us what he promised for taking on his contract-information concerning the Black Dragon. Astara wants to find out what the Count knows.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6642 => array(
			'name'    => "Glittering Lies",
			'journal' => "I encountered a projection of the Duke of Crows outside Arpenia. He and the rest of the Blackfeather Court are trapped against their will to help a mage open a connection to the Evergloam. I agreed to help the Duke free the members of the Court.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4273 => array(
			'name'    => "To the King",
			'journal' => "<<1>> is encamped at Trolhetta with the bulk of the Ebonheart Pact's army. The Worm Cult is dug in, and hoping to hold on until they can gather the shards of Wuuthrad for some nefarious purpose.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3699 => array(
			'name'    => "From the Wastes",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6064 => array(
			'name'    => "Casting the Bones",
			'journal' => "According to the Undaunted Trailblazer, Yisareh, the ruins of Fang Lair have been taken over by a cult of necromancers who are preparing for something big. She's enlisted my help in discovering and foiling their plans.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3703 => array(
			'name'    => "Honor Bound",
			'journal' => "The monastery of Muth Gnaar was attacked by the Vereansu, a violent tribe of Ashlanders. The peaceful monks were caring for the afflicted using an ancient relic called the Tear of Saint Veloth when the Ashlanders attacked.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4349 => array(
			'name'    => "Kill Enemy Players",
			'journal' => "Grand Warlord Sorcalin has authorized a bonus for any Dominion soldier who slays 20 enemy combatants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7303 => array(
			'name'    => "Slime Pickings",
			'journal' => "I found a note attached to a jar of Sload slime on the shore. The captain waiting in Shor's Stand seems desperate to get the jar back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6074 => array(
			'name'    => "Cogs of Fate",
			'journal' => "The people of Slag Town apparently seek predictions about the future from a factotum known as the Grand Prognosticator. Kamid, whose mother died during the last pilgrimage, plans to prove that the oracle is a fraud.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3705 => array(
			'name'    => "Payback",
			'journal' => "Scout Kanat says that the rest of the scouting party is dead or afflicted, too far gone to be saved. He also says the Maulborn are protected from the Llodos plague by arcane wards.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6394 => array(
			'name'    => "Uneasy Alliances",
			'journal' => "Nahfahlaar knows of a Dragon who may join our alliance, but has no idea where to find him. I need to find Yahgrondu and extend an offer to join us.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3708 => array(
			'name'    => "Wave Combat Quest",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5063 => array(
			'name'    => "Stone Cold",
			'journal' => "Fleeing undead to the north, a clan of Bosmer refugees wants to settle in Shadows Crawl, but stone men, known as Hollow, rose from the ground and attacked.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3709 => array(
			'name'    => "The Bargain's End",
			'journal' => "After returning the <<z:1>>, <<2>> made me the arbiter over the fate of <<3>>. Destroying the <<z:1>> will allow the villagers to die, but preserving it will trap them in their lifeless state.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5186 => array(
			'name'    => "The Blood of Nirn",
			'journal' => "The scholar Nendirume warned me that the Scaled Court has taken over the ruined city of Skyreach. She believes they're searching Skyreach Hold for more of the nirncrux element, which they refer to as \"the blood of Nirn.\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3712 => array(
			'name'    => "Captive Souls",
			'journal' => "A traveling merchant named <<1>> fell prey to Worm Cult necromancers camped below <<2>>. His guards were killed, and their souls are now trapped in animus geodes throughout the cultists' camp.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6092 => array(
			'name'    => "The Merchant's Heirloom",
			'journal' => "I found the abandoned journal of Dulza, an Orc merchant who somehow entered the Clockwork City during a fierce storm in Craglorn. Her greatest regret is that her clan in Fell's Run will never see their ancestral sword again.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3715 => array(
			'name'    => "Strange Guard Beasts",
			'journal' => "Churasu has a way to turn the giant wasps into an army to use against the Khajiiti pirates. She needs my help to make an alchemical concoction.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3718 => array(
			'name'    => "Into the Temple",
			'journal' => "After Drillk's memory returned, he told me how to obtain the keystone. It's locked away in the Temple of Sul.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3864 => array(
			'name'    => "The Dungeon Delvers",
			'journal' => "Sibling explorers Raynor and Kireth Vanos are investigating the Dwarven ruins of Bthanual.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7289 => array(
			'name'    => "To Know the Void",
			'journal' => "I met Sahmazim outside a ruin called Xul-Katama, an ancient monastery of the death-dancers. With the Worm Cult encroaching on the monastery, he hired me to rescue some stone tablets before the cultists discover them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3719 => array(
			'name'    => "Captured Time",
			'journal' => "I've found a book about Ayleid 'Alasil pilasters' within the Temple of Sul. Beside it was a small Ayleid relic that may be able to awaken the pilasters.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4924 => array(
			'name'    => "The Barefoot Breton",
			'journal' => "I met a somewhat drunk and totally bootless Breton named Parquier Gimbert outside of Northpoint. He said he lost a valuable mace while hunting for mudcrabs along the shore. He thinks that maybe a Hagraven was involved. Also, he has no shoes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5472 => array(
			'name'    => "A Feast To Remember",
			'journal' => "<<1>> has nearly restored the long-lost reputation of Orc cuisine. She needs a few more rare ingredients to complete her masterpiece.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3874 => array(
			'name'    => "The Light Fantastic",
			'journal' => "A Dwarven device fills a chamber deep within the ruins of Bthanual. Kireth has repaired it. Beams of light now shine through prisms set around the room.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3721 => array(
			'name'    => "Dissonant Commands",
			'journal' => "I met an artificer who's trying to learn more about the mysterious rod that was used to control Dwarven constructs from Mzithumz.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3724 => array(
			'name'    => "Lost to the Mire",
			'journal' => "<<1>> went to the <<2>>. He has yet to reply to <<3>>'s correspondence.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5975 => array(
			'name'    => "A Masterful Weapon",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3728 => array(
			'name'    => "Remembering Risa",
			'journal' => "I found a battered journal while walking the Coiled Path. A young woman named Risa did not survive this place. She lost her amulet fighting one of the snakes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3882 => array(
			'name'    => "Placeholder Start",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3729 => array(
			'name'    => "A Stranger Uninvited",
			'journal' => "<<1>> welcomed a traveling High Elf to <<2>>, but she believes he has something to do with the wisps' recent aggression.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7189 => array(
			'name'    => "Of Crown and Flowers",
			'journal' => "Tanlorin was shaken by the revelation that Wisteria betrayed the Garland Ring. They want to uncover the truth of the Ceythalmor plan and put a stop to this threat once and for all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3731 => array(
			'name'    => "Broken Apart",
			'journal' => "To rid the area of her corrupting influence, <<1>> and I cleansed the ancient vessel. We need it capture the wispmother in <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7182 => array(
			'name'    => "Mirrormoor Incursion",
			'journal' => "Daedra from Mirrormoor are launching attacks throughout West Weald. <<1>> hired me to join forces with other adventurers and defeat a Mirrormoor incursion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3732 => array(
			'name'    => "Overrun",
			'journal' => "The Aldmeri Dominion has set up a staging area south of Hissmir in a fishing village called <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5973 => array(
			'name'    => "A Masterful Glyph",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3734 => array(
			'name'    => "Restoring the Guardians",
			'journal' => "<<1>> led a force of Pact soldiers to Ash Mountain. They were following the Brother of Strife, Balreth. Using the information I learned from Mavos at Othrenis, we're going to imprison the Brother of Strife again.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4104 => array(
			'name'    => "In Search of Kireth Vanos",
			'journal' => "Raynor and Kireth Vanos, self-proclaimed Dwemer experts, left Morrowind to look for long lost knowledge and lore within Skyrim's Dwarven ruins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6899 => array(
			'name'    => "Imminent Hazard",
			'journal' => "Active volcanic vents threaten the safety of Galen. Druid Aishabeh hired me to gather allies and seal these vents before it's too late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3880 => array(
			'name'    => "A Blow for Order",
			'journal' => "The Ordinators are outnumbered. They asked me to reduce Vox's forces to help even things up a bit.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6096 => array(
			'name'    => "The Queen's Decree",
			'journal' => "Queen Ayrenn ordered the once-forbidden island of Summerset to open its borders and welcome outsiders to its expansive shores. Razum-dar, an agent of the Queen, has come to the island to make sure the decree is followed to the letter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3735 => array(
			'name'    => "The Death of Balreth",
			'journal' => "I've defeated Ash Mountain's guardians. Now I must enter Ash Mountain's caldera, defeat Balreth, and imprison him again.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7186 => array(
			'name'    => "The Garland Ring",
			'journal' => "Along the road near Vulkhel Guard in Auridon, I met a tattooed High Elf caught in an ice trap.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3736 => array(
			'name'    => "Close the Scamp Caves",
			'journal' => "Scamps are boiling up out of the ground, no doubt excited by the freeing of Balreth. <<1>> asked for my help dealing with them. Collapsing the entrances to caves in <<2>> should do the job.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6789 => array(
			'name'    => "The Lost Symbol",
			'journal' => "A ring Isobel gave to her friend Aurelia many years ago has been stolen. The ring was a symbol of their relationship, once upon a time, but here and now the loss of the signet ring could mean trouble for Isobel's family.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3890 => array(
			'name'    => "Pull the Last Fang",
			'journal' => "I left the ancient Argonian ruins with a single Fang of Sithis. According to <<1>>, the Renrijra pirates are in an uproar. They may be feeling the effects of the other Fang in their midst.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5247 => array(
			'name'    => "Pledge: Fungal Grotto I",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to explore the Fungal Grotto beneath Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6106 => array(
			'name'    => "A Matter of Tributes",
			'journal' => "The Bursar of Tribute demands compensation for the Blackfeather Court from mortals who insult them and chase them away.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3737 => array(
			'name'    => "In With the Tide",
			'journal' => "<<1>> told me of an assault force landing in the Ashlands west of Ebonheart. She's asked me to make for the land coral maze known as Vivec's Antlers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3744 => array(
			'name'    => "Shattered Stones",
			'journal' => "Lord Stictal had a unique power crystal that was stolen by Supernal Dreamers. They have shattered the crystal in order to use the pieces for Daedric summoning ritual.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3747 => array(
			'name'    => "Crystal Capture",
			'journal' => "I have been tasked by Lord <<1>> to recover shards of a crystal of power from the Supernal Dreamers who have stolen it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3748 => array(
			'name'    => "The Shattered Crystal",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5500 => array(
			'name'    => "Dousing the Fires of Industry",
			'journal' => "<<1>>, a former Legionary engineer, wants to slow down the bombardment of Imperial City from the Elven Gardens District.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3902 => array(
			'name'    => "A Son's Promise",
			'journal' => "I met a Dark Elf named Telbaril in a pocket of Evergloam known as Crow's Wood. He's trying to find his missing father, Rulantaril. He asked for my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3749 => array(
			'name'    => "Into the Mouth of Madness",
			'journal' => "Blessed by Veloth, I've come to Eidolon's Hollow to confront Magistrix Vox. It's time to end her threat once and for all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7044 => array(
			'name'    => "Breaking the Chains",
			'journal' => "Corlys the Chainmaker has been wreaking havoc with his menagerie of monsters. I agreed to help deal with Corlys and expose his suppliers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4795 => array(
			'name'    => "Scaled Captors",
			'journal' => "A Bosmer scout claims her band was ambushed by Argonian invaders. She managed to get away in the scuffle, but her allies are now captives of the Argonians.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3896 => array(
			'name'    => "Devotees of Knowledge",
			'journal' => "I met a Dunmer named <<1>> in Apocrypha's Gate. He's a devotee of Hermaeus Mora. He and his fellows were attacked by rival cultists of Mehrunes Dagon, and their sanctum was put to the torch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3751 => array(
			'name'    => "Hunting Invaders",
			'journal' => "The Covenant attacked the Argonian town of Lukiul Uxith. The residents repelled the attack. They seem to be quite confident about their safety.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7206 => array(
			'name'    => "Faded Scarlets",
			'journal' => "Hooke, an ex-thief, believes someone is murdering members of his old gang, the Scarlets. He asked me to help him find the remaining Scarlets and protect them from the killer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5987 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3756 => array(
			'name'    => "Q3756 Save the Son",
			'journal' => "Save Geoffroi's Son.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3759 => array(
			'name'    => "The Rescue",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7210 => array(
			'name'    => "Found Family",
			'journal' => "I found a note from a trapper near a snared wormmouth called Versidue. The note says if I return Versidue to Star-Gazer Sariyaah in Ontus, I'll be rewarded.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6007 => array(
			'name'    => "A Call For Aid",
			'journal' => "I found a notice stating that adventurers looking for rewarding work should report to the Hall of Justice.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3760 => array(
			'name'    => "Kill the Boss!",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4211 => array(
			'name'    => "To Tanzelwil",
			'journal' => "Queen Ayrenn asked me to join her at the ancient site Tanzelwil where she must commune with the dead. She's worried there may be … complications.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3761 => array(
			'name'    => "Escape Quest",
			'journal' => "The son of Lord Strictal was taken captive by the Supernal dreamers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5631 => array(
			'name'    => "Contract: Bangkorai",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3762 => array(
			'name'    => "Free Steve",
			'journal' => "The Supernal Dreamers imprisoned Steve Stictal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4330 => array(
			'name'    => "Lifting the Veil",
			'journal' => "I arrived in Skywatch to find a festival in honor of the High Kinlady Estre in full swing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7214 => array(
			'name'    => "The Deserter",
			'journal' => "I encountered an injured Legionary, Captain Dartorius. I agreed to find out what happened to her squadron after they ran into Daedra and one of her own soldiers attacked her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3764 => array(
			'name'    => "Free Harvey",
			'journal' => "Harvey was attempting to steal back his fathers shards when he was captured. He needs help escaping.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3766 => array(
			'name'    => "Dealing with the Dreadlord",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4136 => array(
			'name'    => "Shoot the Messenger",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7367 => array(
			'name'    => "U47 Fixture test",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6138 => array(
			'name'    => "Savage Truths",
			'journal' => "Mehdze the Maestro is on an exploratory journey into King's Haven Pass to research his next epic. Unfortunately, things aren't going according to plan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3769 => array(
			'name'    => "Crystal Champion",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4827 => array(
			'name'    => "Hunting the Troll",
			'journal' => "Ledronor, a member of a hunting order called the Hounds, asked for my help in hunting a troll.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3928 => array(
			'name'    => "Dangerous Union",
			'journal' => "The military has tracked Stral Blackthroat, the leader of the Reachmen, to Vernim Wood. When they arrived, they found Worm Cultists performing dark rituals with his warriors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7353 => array(
			'name'    => "Shields for the Fellowship",
			'journal' => "Armorer Reistaff asked me to craft two shields of any kind for soldiers in the Stirk Fellowship. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3783 => array(
			'name'    => "Lost Lions",
			'journal' => "I encountered a wounded Lion Guard member named <<1>> in the Bonesnap Ruins. He and his soldiers were sent to retrieve pieces of an artifact, but were ambushed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4988 => array(
			'name'    => "Rendezvous at the Pass",
			'journal' => "High King Emeric has arrived with the Lion Guard to help retake Bangkorai Garrison from the Imperial forces.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5667 => array(
			'name'    => "Contract: Eastmarch",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3948 => array(
			'name'    => "Stop the Dream",
			'journal' => "After retrieving the crystal shards, <<1>> wants me to defeat the leader of the Supernal Dreamers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7245 => array(
			'name'    => "Secrets in the Sand",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3787 => array(
			'name'    => "Vengeance of the Oppressed",
			'journal' => "I met a pair of Argonians, former slaves. They want revenge upon Betina Volek, the last surviving descendant of the family that once owned them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5518 => array(
			'name'    => "Meat for the Masses",
			'journal' => "The laborers helping to rebuild Orsinium need supplies, specifically fresh meat. The hills and valleys of Wrothgar are typically filled with beasts to hunt, but their numbers are dwindling due to poaching. I must help feed the city's workers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7238 => array(
			'name'    => "Pledge: Lep Seclusa",
			'journal' => "Urgarlag Chief-bane of the Undaunted has challenged me to free Lep Seclusa from Orpheon the Tactician's army.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6019 => array(
			'name'    => "A Masterful Weapon",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3788 => array(
			'name'    => "Vengeance for House Dres",
			'journal' => "The grandmaster of House Dres wants revenge against the Vanguard troops outside Kragenmoor. He's commanded me to slay their leader.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3942 => array(
			'name'    => "To the Rescue",
			'journal' => "Marcus has told me that he was captured by Supernal Dreamers. I have freed him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4930 => array(
			'name'    => "Shedding the Past",
			'journal' => "Teeba-Ja was a member of the Bitterhand Bandits, but when they became more violent, she decided to leave. She wants to start a new life in Shornhelm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3789 => array(
			'name'    => "This One's a Classic",
			'journal' => "<<1>> is struggling to understand humor. He heard a rumor about a sacred Dark Elf performance from the distant past.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5257 => array(
			'name'    => "BAR Switch Howto Example",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3791 => array(
			'name'    => "Outside Interference",
			'journal' => "I found a strange device in the Dominion camps at <<1>>. The instructions for its use were signed \"Alchemist <<2>>.\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4522 => array(
			'name'    => "The Hedoran Estate",
			'journal' => "The Hedorans were killed by bandits. <<1>> has asked me to recover anything I find in the remains of their home.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3956 => array(
			'name'    => "Message to Mournhold",
			'journal' => "Varon asked me to travel to Mournhold. I should inform the Ordinators about the events at the Obsidian Gorge.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3795 => array(
			'name'    => "Deep Disturbance",
			'journal' => "Something caused the creatures in Xal Ithix's swamps to become aggressive and attack the villagers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7246 => array(
			'name'    => "Dismantling the Worm",
			'journal' => "I met Ojouna, a Worm Cult defector. She asked for my help to free prisoners and stop some necromantic rituals inside the Lair of the Black Worm. She promised to reward me if I help her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3796 => array(
			'name'    => "Nothing Left to Waste",
			'journal' => "Kotholl, a hired hand from the merchants' camp, was ordered to guard three specific chests with his life. He states it would be bad for the contents to fall into the hands of the Maulborn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5389 => array(
			'name'    => "Clothier Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Clothier goods to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3944 => array(
			'name'    => "Kidnapped!",
			'journal' => "Kidnapped Guy has been kidnapped.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4598 => array(
			'name'    => "Into the Vice Den",
			'journal' => "A Khajiit named <<1>> has disappeared at the notorious Hadran's Caravan. The people who love him are losing hope of seeing him again. They have turned to me for help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6996 => array(
			'name'    => "Piteous Envoys",
			'journal' => "Hezehk, a courier for Madam Whim, was robbed by members of the Voidprowler clan. They stole a secret that he meant to trade for a boon from Hermaeus Mora. I agreed to help him track down the Voidprowlers, take back the secret, and finish his job.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4619 => array(
			'name'    => "Colorbox Dummy",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3799 => array(
			'name'    => "Scales of Retribution",
			'journal' => "The Dominion established a foothold in the small town of <<1>>. The Argonian Shadowscales intend to take back the town by force.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6672 => array(
			'name'    => "Love Among the Fire",
			'journal' => "Peek-Ereel gave me a message from Governor Keshu's spymaster, Tee-Wan. Tee-Wan needs my help with a private and urgent matter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5278 => array(
			'name'    => "Pledge: Wayrest Sewers I",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to explore the complex of sewers beneath Wayrest, dead center in the region called Stormhaven.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3804 => array(
			'name'    => "Missing in the Mire",
			'journal' => "The Argonians in Xal Ithix have been picking through trash piles. Now normally docile creatures from the swamp are attacking them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6235 => array(
			'name'    => "Old Baubles",
			'journal' => "Bolu of the Dead-Water tribe, called the Naga-Kur in the Argonian tongue, wants me to go to the Echoing Hollow and accomplish the task he set before me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3809 => array(
			'name'    => "Cultural Exchange",
			'journal' => "Rigurt the Brash, leader of the Expedition for Nord Cultural Exchange, has asked for my assistance. I've agreed to trade a Nord book, clothes, and relic for similar Dark Elf items.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4516 => array(
			'name'    => "Stolen Ashes",
			'journal' => "I found a stolen item with the name <<1>> etched on it, as well as an address.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3815 => array(
			'name'    => "Cracking the Egg",
			'journal' => "<<1>> wants to find out more about <<A:2>>. Until the vicecanons understand its danger, they can't root out the Dominion in Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4652 => array(
			'name'    => "The Colovian Occupation",
			'journal' => "The town of Arenthia has been occupied by a Colovian force. I've been tapped to help retake the city alongside a Khajiiti resistance force.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3816 => array(
			'name'    => "Mercenaries Astray",
			'journal' => "I found a Redguard mercenary named Ragara near the entrance to Zehtswater Cave. Her band was bitten and carried off by Lamias.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7270 => array(
			'name'    => "Ghishzor's Ritual",
			'journal' => "The College of Rituals has learned that a powerful Daedra is determined to complete an ancient ritual in eastern Solstice. I agreed to extinguish the ritual fires and defeat the maldrith known as Ghishzor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3820 => array(
			'name'    => "Restless Spirits",
			'journal' => "Some terrible catastrophe has befallen Selfora, and now hostile spirits overrun the place.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3822 => array(
			'name'    => "Schism",
			'journal' => "An Argonian outside <<1>> asked me to find his brother, <<2>>. He was in the nearby mine when Goblins overran it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7274 => array(
			'name'    => "Dubious Intentions",
			'journal' => "I met an injured hunter out in Eastern Solstice who asked me to help finish hunting a pack of bear-lizards. Apparently, they attacked and killed his friend, Golvyni. I agreed to help lure them to his friend's trap.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6071 => array(
			'name'    => "Morsels and Pecks",
			'journal' => "The Bursar of Tributes seeks items of flesh to add to the Blackfeather Court's hoard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3824 => array(
			'name'    => "A Last Reminder",
			'journal' => "While in the Goblin camp, I found a Nord corpse clutching a runestone that bears the name \"Hodmar Iron-Kettle.\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6515 => array(
			'name'    => "The Antiquarian's Art",
			'journal' => "I joined the Antiquarian Circle. Now I need to learn how to find and recover rare antiquities.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4662 => array(
			'name'    => "Articles of Faith",
			'journal' => "I found a set of robes in a bandit camp.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3826 => array(
			'name'    => "Climbing the Spire",
			'journal' => "Tanval Indoril is inside the Tormented Spire working to unleash the Brother of Strife. Tanval's Vanguard holds the approach. If we don't stop Tanval, he'll unleash Sadal and destroy Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6807 => array(
			'name'    => "The Sable Knight",
			'journal' => "An animated suit of armor known as the Sable Knight threatens priests on pilgrimage near Steadfast Manor. I have agreed to destroy the Sable Knight and cleanse the darkness it spreads.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4426 => array(
			'name'    => "Stacking the Odds",
			'journal' => "<<1>> asked me to deliver orders to a rebel spy in Imperial-held Cheydinhal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3860 => array(
			'name'    => "Exquisite Tears",
			'journal' => "A shady character named <<1>> contacted me at Davon's Watch. He said we could both profit if I \"procured\" a rare and expensive bottle of wine for him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7285 => array(
			'name'    => "The Deep Tombs of Xul-Haj",
			'journal' => "I learned that the Worm Cult is planning to raid a Stone-Nest necropolis to raise an army of undead. And Vanus Galerion may be imprisoned there as well. I need to stop the cult and rescue Vanus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3827 => array(
			'name'    => "The Tree-Minder's Fate",
			'journal' => "Mystery surrounds Forsaken Hamlet. An Argonian tree-minder lies murdered at the edge of her village.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3828 => array(
			'name'    => "Cold-Blooded Revenge",
			'journal' => "The Dominion slaughtered the Argonians living in a small hamlet in Shadowfen. Only one remains alive to take his vengeance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7028 => array(
			'name'    => "Pledge: Scrivener's Hall",
			'journal' => "Urgarlag Chief-bane of the Undaunted has challenged me to remove Valinna from her position as the leader of the Scribes of Mora in Scrivener's Hall.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4651 => array(
			'name'    => "The Champion Division",
			'journal' => "I have defeated the champions of the three major divisions in Thizzrini Arena. Now I am ready to face Ishalga and become the Grand Champion of Thizzrini Arena.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3831 => array(
			'name'    => "The Judgment of Veloth",
			'journal' => "The time to confront Vox has arrived-but it seems that to defeat her, I need to die by her hammer. The Blessing of Veloth should protect me, but can I return from the dead?",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3837 => array(
			'name'    => "Opening the Portal",
			'journal' => "With Tanval's help, we've lowered the final ward guarding the prison of Sadal, the Brother of Strife. The mages who originally imprisoned Sadal linger on the Tormented Spire as spirits, angered by Tanval's ritual.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6455 => array(
			'name'    => "Bound in Blood",
			'journal' => "I awoke in a cold stone cell after my caravan was attacked on the way into Western Skyrim. I have no clue where I am, but I need to find a way out of here.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3838 => array(
			'name'    => "The Wounds in the World",
			'journal' => "Priest Dilyne asked me to close the Daedric portals that Vox opened within the shrine. All kinds of vile creatures are emerging from these blasphemous wounds in the world.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6859 => array(
			'name'    => "The Siege of Vastyr",
			'journal' => "The Firesong and their Dreadsails allies are about to begin the siege of Vastyr. Sir Stefan needs my help to defend the city and defeat the invaders.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3840 => array(
			'name'    => "Saving the Relics",
			'journal' => "Shadowfen's Vicecanon Hrondar sent Saervild Steel-Wind to Bogmother's ruins to collect Argonian relics.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3841 => array(
			'name'    => "The Mystery of Razak",
			'journal' => "I discovered a battered Dwarven construct that seems to have killed its Imperial caretaker. The Imperial's notes imply that they are searching for something within the Dwemer ruin at <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3845 => array(
			'name'    => "And Throw Away The Key",
			'journal' => "The vicecanons tracked an ancient keystone to <<1>>. This artifact would be a major boon to Pact forces in <<2>>. However, the prisoners seem unwilling to negotiate directly with the vicecanons.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3846 => array(
			'name'    => "The Keystone",
			'journal' => "The keystone is located within the White Rose Prison Dungeon. I've agreed to find it and use it to free the prisoners here.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4809 => array(
			'name'    => "Nirnroot Wine",
			'journal' => "I agreed to gather Red Mountain Ash for <<1>>, so she can make Nirnroot wine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3847 => array(
			'name'    => "The Ones Left Behind",
			'journal' => "<<1>>, a former prisoner of White Rose Prison, is dead, but her grieving spirit lingers on.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3850 => array(
			'name'    => "The Covenant Infiltrator",
			'journal' => "I've encountered a Covenant scout named <<1>> in <<2>>. She was captured by Imperials and has asked for my aid.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3854 => array(
			'name'    => "A Goblin's Affection",
			'journal' => "While attempting to test a new spell, <<1>> transformed his brother, Dithis, into a female Goblin. The Goblin lass has been kidnapped by a local Goblin warlord named <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3856 => array(
			'name'    => "Anchors from the Harbour",
			'journal' => "The Guildmaster of the Fighters Guild wants me to perform a task for her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6885 => array(
			'name'    => "Critter Capture",
			'journal' => "Juline Courcelles wants to display several critters found around the old druid site of Fauns' Thicket. I need to go there and capture the critters for her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3858 => array(
			'name'    => "The Dangerous Past",
			'journal' => "Sees-All-Colors has had Doshia's journal translated. From it she believes she may have discovered information relating to the mysterious Mortuum Vivicus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5718 => array(
			'name'    => "Sacrament: Smuggler's Den",
			'journal' => "I received a contract from the Speaker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3861 => array(
			'name'    => "QUEST IO BUG CHECK",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3865 => array(
			'name'    => "Savages of Stonefalls",
			'journal' => "The wilderness of Stonefalls is filled with predators and hostile creatures. The citizens of the region would sleep better knowing the worst ones are dead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6554 => array(
			'name'    => "The Dark Heart",
			'journal' => "Verandis detected a growing concentration of dark energy-Void energy-within Nighthollow Keep. Perhaps we'll find the Dark Heart somewhere within those ancient ruins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3868 => array(
			'name'    => "Sadal's Final Defeat",
			'journal' => "I've accomplished great deeds with Tanval Indoril, <<1>>, and Walks-in-Ash. We've returned Tanval's Vanguard to the Ebonheart Pact, and convinced the spirits of Tormented Spire to open Sadal's prison.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4839 => array(
			'name'    => "Love Lost",
			'journal' => "While exploring Hildune's Secret Refuge, I found a journal written by Nadafa. It tells the story of how she was seduced by a vampire. The journal mentions that her husband, Giran, works in Northpoint at the cemetery and lives in the fishing village.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6891 => array(
			'name'    => "Three-Pronged Approach",
			'journal' => "Stonelore healers are having trouble mending a group of initiates who sustained wounds from a chimera. I need to gather alchemical components from each head of the chimera so the healers can craft a cure.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3872 => array(
			'name'    => "The Talking Quest",
			'journal' => "This is the Talking Quest.  The purpose of this quest is to talk to people and things, to  make sure the conversation system is still working.  Tally-ho!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3885 => array(
			'name'    => "The Prismatic Core",
			'journal' => "Guildmaster Sees-All-Colors has plans to make use of the crystal and tome we found in the depths of Mzeneldt.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3886 => array(
			'name'    => "The Fetish",
			'journal' => "I purchased a strange statue from an Ashlander merchant in the wilderness. It may be a religious fetish.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5227 => array(
			'name'    => "Kill Enemy Dragonknights",
			'journal' => "Grand Warlord Dortene has authorized a reward for any Covenant soldier who slays 20 enemy Dragonknights.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3888 => array(
			'name'    => "Buried in the Past",
			'journal' => "Researchers seeking ancient relics around the xanmeers found Renrijra pirates camped in the Argonian ruins they were planning to search.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3889 => array(
			'name'    => "The Fangs of Sithis",
			'journal' => "The artifacts buried beneath Sunscale Strand are dangerous. <<1>> warns that we may have trouble with them. I need to be careful when searching the ruins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5368 => array(
			'name'    => "Blacksmith Writ",
			'journal' => "I've taken a contract from one of the Writ Boards, an agreement to deliver Blacksmith goods to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5750 => array(
			'name'    => "The Fallen City of Shada",
			'journal' => "I found the ruins of a city called Shada's Tear in the middle of the Craglorn desert. A mercenary told me that some researchers from Belkarth went into the ruins, but that strange things started happening when they did.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3893 => array(
			'name'    => "By Invitation Only",
			'journal' => "Deshaan's Academy of Magic, Shad Astula, trains only the most promising mages to become leaders of the Ebonheart Pact. However, the Academy only accepts those who've received a special invitation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6586 => array(
			'name'    => "Betrayal at Briar Rock",
			'journal' => "I met a hunter named Medone by Briar Rock. Hagravens of the Thornroot clan imprisoned her clan and are turning them into briarhearts. Medone wants me to free her clan and punish the Thornroot clan by destroying their briarheart tree.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3900 => array(
			'name'    => "Into the Mire",
			'journal' => "Dominion troops invaded <<1>>'s village, trapping her husband and the other villagers. Whatever the soldiers are doing in <<2>>, they must be stopped.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4743 => array(
			'name'    => "The Lost Lute",
			'journal' => "I found Minstrel Idria's lute abandoned in the Grotto of Depravity.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3904 => array(
			'name'    => "Supply Run",
			'journal' => "Aamela Rethandus needs healing supplies to treat Selfora's wounded.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6459 => array(
			'name'    => "Long Journey Home",
			'journal' => "Attacks by unnatural forces plague the area around Morthal. Villagers report undead wolf attacks and visions of ghosts. I should find the source of these attacks and stop them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3905 => array(
			'name'    => "Clarity",
			'journal' => "<<1>>'s husband <<2>> won't speak since his ordeal with the Dominion. I must find a way to heal him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6594 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "I accepted a contract to craft supplies using woodworking for civilians displaced by the Three Banners War.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3908 => array(
			'name'    => "Vision Quest",
			'journal' => "Arch-Mage Valeyn thinks his visions are somehow connected to the evil Magistrix Vox. Unfortunately, before he could learn more, something blocked his magic.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4225 => array(
			'name'    => "To Nimalten",
			'journal' => "I've put a stop to the threat of the Reachmen, but the threat to the Rift is far from over. The Worm Cult is massing in force, and the settlements west of Nimalten need to be warned.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3909 => array(
			'name'    => "The Dominion's Alchemist",
			'journal' => "<<1>> tracked the Dominion's movement to <<2>>, where she hopes to end the Dominion invasion of Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3910 => array(
			'name'    => "The Dream of the Hist",
			'journal' => "<<1>> asked me to retreive <<A:2>> from <<3>>, the architect of the Dominion invasion in Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4294 => array(
			'name'    => "The Unveiling",
			'journal' => "I uncovered evidence linking important community members in Mathiisen to the Veiled Heritance. Now Raz has asked me to take the fight to them directly.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3911 => array(
			'name'    => "The Swamp's Embrace",
			'journal' => "<<1>> is one of the few surviving prisoners from the Dominion's march into Loriasel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4372 => array(
			'name'    => "Goblin Marq",
			'journal' => "<<1>> wants me to look for his friend, Marq, who seems to think he's a Goblin.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3914 => array(
			'name'    => "Missing Son",
			'journal' => "<<1>> asked me to find his son who hasn't returned to the village.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3915 => array(
			'name'    => "Decree of the Court",
			'journal' => "A talking crow claims to represent the Blackfeather Court. It says I must pay a toll to pass safely through the court's domain.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5635 => array(
			'name'    => "Ache For Cake",
			'journal' => "I received a voucher for a special-occasion cake from the renowned master baker, Chef Donolon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3916 => array(
			'name'    => "Long Lost Lore",
			'journal' => "A high-ranking member of the Mages Guild, <<1>> wants me to perform a task for her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3919 => array(
			'name'    => "Beneath the Stone",
			'journal' => "I've traveled to Shor's Stone, which is under attack by Reachmen. The Companions and Pact military have marshaled a defense.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5655 => array(
			'name'    => "Contract: Auridon",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3920 => array(
			'name'    => "Unearthed",
			'journal' => "I've breached the barrier to the mine beneath Shor's Stone. The Reachmen have assaulted the foreman and his crew. I need to find the foreman, Gandis, and figure out what the Reachmen are doing here.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4371 => array(
			'name'    => "Wisdom of the Ages",
			'journal' => "An ancient Argonian settlement called Haj Uxith occupies a dark corner of Coldharbour. The Argonians want to help me in my quest, but they seem to be divided over something related to their Hist tree.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3921 => array(
			'name'    => "Move out Miners",
			'journal' => "Some of the miners beneath Shor's Stone still live!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6354 => array(
			'name'    => "Sunspire Summons",
			'journal' => "A preacher from Sunspire invited me to travel to the temple to see the god Alkosh incarnate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4497 => array(
			'name'    => "Lost Daughter",
			'journal' => "While exploring <<A:1>> I came across a campsite with an inscribed vase. The inscription read as if it was given to someone, but I only found skeletons in the tomb.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3923 => array(
			'name'    => "The Farmer's Champion",
			'journal' => "The residents of Treva's Farm have fled. A Worm Cult attack drew the attention of nearby giants. The besieged farmers ran for their lives.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3924 => array(
			'name'    => "Song of Awakening",
			'journal' => "The sky over the bardic college of Skald's Retreat is something out of nightmares, and monsters stalk its paths.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3925 => array(
			'name'    => "Guard the Knowledge",
			'journal' => "There are tomes scattered across the islands of Skald's Retreat that all have something to do with Vaermina.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5776 => array(
			'name'    => "The Time-Lost Warrior",
			'journal' => "The Celestial Warrior is worried about his champion, Titus Valerius. Lost in time, Titus' memory has begun to break apart. He no longer remembers who he is or his connection to the Warrior. Without his memory, the Warrior can't send him home.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3927 => array(
			'name'    => "In His Wake",
			'journal' => "The city of Riften has been devastated by Sinmur's assault. The town guard is overwhelmed trying to help all the refugees. They're asking everyone to help with the recovery effort.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4573 => array(
			'name'    => "Frighten the Fearsome",
			'journal' => "The Wood Elves in this part of the Valenwood are opposed to the Dominion. I agreed to negotiate and attempt to convince them to swear allegiance to Queen Ayrenn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5786 => array(
			'name'    => "Dark Anchors in Alik'r Desert",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Alik'r Desert.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3929 => array(
			'name'    => "Shattered Realm",
			'journal' => "The Crystal of Power has been stolen and shattered, and  Geoffroi wants me to help him retrieve it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5657 => array(
			'name'    => "Contract: Auridon",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3930 => array(
			'name'    => "Slaying the Dreamers",
			'journal' => "I'm helping Geoffroi restore his Crystal of Power. The Supernal Dreamers have stolen and shattered his Crystal of Power into five pieces.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5413 => array(
			'name'    => "Provisioner Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver food and drink to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6625 => array(
			'name'    => "A Keep's Ransom",
			'journal' => "The Ivory Brigade garrison of Borderwatch has been taken over by a hostile mercenary force called the Painted Eye, which is holding a number of civilians hostage.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3931 => array(
			'name'    => "Eternal Quest",
			'journal' => "I met a noble, B. Stower, who's power crystal was stolen by the Supernal Dreamers and broken in 5 shards.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3933 => array(
			'name'    => "Murder for Crystals",
			'journal' => "Distraught noble Geoffroi's antique crystal was stolen by Supernal Dreamers and split it into shards for their Daedric rituals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3937 => array(
			'name'    => "Captive Son",
			'journal' => "Geoffroi's son has been kidnapped by Supernal Dreamers. I need to make sure he gets back safely.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5536 => array(
			'name'    => "Heist: Deadhollow Halls",
			'journal' => "I was tipped off about a stash of Daedric treasures ripe for the taking in the Deadhollow Halls. I don't have much time to secure the goods before they're moved, so I should act quickly on this lead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3938 => array(
			'name'    => "Prisoner",
			'journal' => "I'm helping a young man escape from captivity. The Dreamers have chained him in a cage.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6633 => array(
			'name'    => "Harrada Harvest",
			'journal' => "Adrahawn, an unusual Wood Elf with a passion for trees, claims that he is searching for an otherworldly seed rumored to be lost in the ruins known as the Silent Halls. I agreed to help him retrieve it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5804 => array(
			'name'    => "Broken Bonds",
			'journal' => "Vvardenfell promises new experiences and new adventures. I can't wait to see what the land of the Dark Elves has in store for me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3939 => array(
			'name'    => "About a Prisoner",
			'journal' => "I found a guy tied up in a cage. He wanted to be free.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6626 => array(
			'name'    => "Competition and Contracts",
			'journal' => "I found Bastian Hallix tied up outside of Deepscorn Hollow. He asked me to help him rescue Quistley Silvelle, the son of his employers. And he warned me about a mysterious Dark Elf who entered the cave ahead of us.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3940 => array(
			'name'    => "Rescue Bob",
			'journal' => "I met a prisoner held by the Supernal Dreamers, who asked that I set him free.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4257 => array(
			'name'    => "Unaccounted Crew",
			'journal' => "Captain Erronfaire is desperate to leave port, but she's lost track of some of her crew. She needs them found so she can set sail for Velyn Harbor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5531 => array(
			'name'    => "Partners in Crime",
			'journal' => "A thief named <<1>> needs a partner for a difficult job. I took her up on the offer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3941 => array(
			'name'    => "Bob's Dramatic Escape",
			'journal' => "I guess I should let this guy go.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3945 => array(
			'name'    => "Wreck Up the Place",
			'journal' => "Geoffroi wants me to murder the leader of the supernal dreamers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4404 => array(
			'name'    => "Lost Treasures",
			'journal' => "<<1>> and some of his inn workers were meeting a trader to restock supplies when pirates invaded <<2>>. Everyone scattered, and now his workers are lost in the marsh.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3946 => array(
			'name'    => "Killing Dreams",
			'journal' => "Julia Kalere has told me that I must kill the Twilight Boss to put an end to this madness.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3949 => array(
			'name'    => "Kill Remelie",
			'journal' => "Geoffroi has asked me to kill Remelie, the leader of the cultists. This should send them fleeing back to Stormhaven. She should be near one of the Dreamer camps.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5417 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3951 => array(
			'name'    => "Kill the Beast",
			'journal' => "I need to kill the Beast to stop the Dreamers once and for all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5687 => array(
			'name'    => "Contract: Stonefalls",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3952 => array(
			'name'    => "Bongo Boss Beatdown",
			'journal' => "Troggg has discovered that it was his brother, Clogg, who stole the bongos.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4403 => array(
			'name'    => "Captive Crewmembers",
			'journal' => "The slavers have captured the crew of a ship that crashed on the rocks. <<1>> has asked me to free them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3953 => array(
			'name'    => "Chateau of the Ravenous Rodent",
			'journal' => "I've been tasked with finding another book for the Mages Guild. Once again I need to return to the Shivering Isles, to a place called the Chateau of the Ravenous Rodent.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5814 => array(
			'name'    => "Madness in Alik'r Desert",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Alik'r Desert.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3957 => array(
			'name'    => "Gift of the Worm",
			'journal' => "The Reachmen are being magically augmented by the Worm Cult. They must be stopped.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5685 => array(
			'name'    => "Contract: Stonefalls",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3958 => array(
			'name'    => "The Llodos Plague",
			'journal' => "Maulborn cultists are causing trouble throughout Deshaan. Quarantine Serk was only the beginning. A House Hlaalu guard asked me to take word of what happened to Narsis, a town south of the Serk.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5808 => array(
			'name'    => "Darkness Blooms in Rivenspire",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Orc's Finger Ruins, located in Rivenspire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3959 => array(
			'name'    => "Smoke on the Horizon",
			'journal' => "Boulderfall Pass has been attacked by Sinmur and the Reachmen. The soldiers defending this area need my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4605 => array(
			'name'    => "The Hollow City",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5818 => array(
			'name'    => "Madness in Bangkorai",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Bangkorai.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3961 => array(
			'name'    => "Raise the Colors",
			'journal' => "A dying soldier named <<1>> has asked me to avenge his death.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6650 => array(
			'name'    => "Rites of the Sul-Xan",
			'journal' => "The Sul-Xan Argonians are a tribe of vicious cultists who kidnap people around Blackwood for human sacrifice. I agreed to rescue captives held by the Sul-Xan and eliminate the cultists.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3964 => array(
			'name'    => "Secrets of the Lost City",
			'journal' => "I met a Redguard named Paldeen, held prisoner by the Khajiit bandits in the Lost City of the Na-Totambu. He asked for my help in opening the sealed inner chamber.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3966 => array(
			'name'    => "Chasing the Magistrix",
			'journal' => "Magister Vox stole an item of power from the Tribunal Temple-the hammer known as Veloth's Judgment. She fled east, away from Mournhold. Almalexia asked me to track her down before she gains the full power of the artifact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7169 => array(
			'name'    => "Three Thieves in Daggerfall",
			'journal' => "Quen wants to engage the three gang leaders in solving the puzzle of the Codex Voernet. I need to help each one with a job and get to know them in the process as we seek to bring them together as a proper Thieves Guild.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5442 => array(
			'name'    => "Cultural Affections",
			'journal' => "Rigurt the Brash, an odd Nord ambassador, presides over the Ebonheart Pact's diplomatic mission to Wrothgar. But Rigurt has a problem. He's fallen in love with his Orc counterpart, Ambassador Lazdutha.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3968 => array(
			'name'    => "Through the Shroud",
			'journal' => "Our final goal at Vernim must be to kill Stral and end the threat of the Reachmen. Unfortunately he's protected by a death shroud in the central ruin.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5699 => array(
			'name'    => "Contract: Eastmarch Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of East March.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3980 => array(
			'name'    => "Research Subject",
			'journal' => "A researcher named Nila feels terrible about her colleague's capture. The Worm Cult was torturing him at the far end of the cavern.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3982 => array(
			'name'    => "Bound to the Bog",
			'journal' => "<<1>>, an agent of the Vicecanons, asked me to join the expedition to <<2>>. They'll be searching for Argonian relics.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5719 => array(
			'name'    => "Sacrament: Smuggler's Den",
			'journal' => "I received a contract from the Speaker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3984 => array(
			'name'    => "Forgotten Heroes",
			'journal' => "I'd been told previously Ulrich, a former Lion Guard soldier, died at the hands of the cultists. <<1>> believes he may actually be imprisoned somewhere within Silaseli instead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4561 => array(
			'name'    => "Dangerously Low",
			'journal' => "<<1>> is running low on supplies to help the many wounded in his care.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3987 => array(
			'name'    => "Hozzin's Folly",
			'journal' => "I met up with Seyne near Hozzin's Folly. Bandits have landed on the island. They're exploring an abandoned mine that was once home to Daedra worshipers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4554 => array(
			'name'    => "Shadowfen Smorgasbord",
			'journal' => "The drunken cook, <<1>>, is in trouble. She bet her hunter friend that she'd collect ingredients from beasts all over Shadowfen and cook him a feast in Hissmir. Now sober, she is afraid to venture out into the fen, for fear of these same creatures.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7438 => array(
			'name'    => "Defacement",
			'journal' => "I looted a statue head from an enemy in Sorrow's Friend. When properly situated, it is supposed to project a magical rune indicating the north rooftop oasis nearby. I can aid the members of my faction by finding the statue and returning its head.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3988 => array(
			'name'    => "Dangerous Webs",
			'journal' => "I found a Fighters Guild camp where an Argonian soldier is recovering from a fierce battle with spiders.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3990 => array(
			'name'    => "A Beginning at Bleakrock",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5840 => array(
			'name'    => "Reclaiming Vos",
			'journal' => "The powerful Telvanni mage, Mistress Dratha, teeters on the brink of death. Now her replacement, Master Savarak, rules Vos with an iron fist. The situation seems dire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3991 => array(
			'name'    => "Escape from Bleakrock",
			'journal' => "The Daggerfall Covenant has assaulted Bleakrock Village. Captain Rana sent the surviving refugees to Last Rest, a tomb near the town. Rana and I must clear a path through the tomb for the refugees.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7442 => array(
			'name'    => "Codex and Cartel",
			'journal' => "While our efforts have gotten the three gang leaders to start working together, we're not close to turning them into a proper Thieves Guild or finding the Nowhere Vault. Quen thinks we need to learn more about Librarian Henard and the Koldane Cartel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3992 => array(
			'name'    => "What Waits Beneath",
			'journal' => "I caught up with Darj near Skyshroud Barrow. Skeletons attacked and wounded him before he could find a way to get its door open.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3993 => array(
			'name'    => "Kings of the Grotto",
			'journal' => "I met a group of Dark Elves in the Fungal Grotto. They want to reach a shrine within the caves. A tribe of hostile Goblins occupies most of the area, pushing out a colony of dreugh. The two fighting sides block access to the rest of the cave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4632 => array(
			'name'    => "Special Delivery",
			'journal' => "I've agreed to deliver a sack of fertilizer for <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5477 => array(
			'name'    => "The Watcher in the Walls",
			'journal' => "Someone calling themselves The Rat left a note claiming that the Daedra are slaughtering people in the Imperial Arena. If I slay the Daedra, instead, The Rat promised to reward me with a secret about Molag Bal's plan for Imperial City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3995 => array(
			'name'    => "The Frozen Man",
			'journal' => "Rolunda came to Orkey's Hollow looking for her brother, Eiman. The cave is said to be haunted, and young Nords on the island sometimes enter it to prove their bravery. Eiman went in, but he hasn't come out.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7197 => array(
			'name'    => "The Wing of the Indrik",
			'journal' => "After an eventful patrol into an ancient ruin, Votary Nahlia and I have uncovered a mystic library and a forgotten form of magic. While Nahlia contacts the Mages Guild, it's down to me to learn more about Scribing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5470 => array(
			'name'    => "A Healthy Choice",
			'journal' => "<<1>> continues to restore the long-lost reputation of Orc cuisine. She hired me to collect additional rare ingredients throughout Wrothgar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3996 => array(
			'name'    => "At Frost's Edge",
			'journal' => "I found a note on a dead bandit. Their leader, a man named Oath-Breaker, has orders to capture villagers, so he can trade them for supplies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3997 => array(
			'name'    => "The Mad God's Bargain",
			'journal' => "Valaste and Shalidor have finished analysis of Sheogorath's books. Now I need to help them take possession of Eyevea, the Mages Guild Sanctuary.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7441 => array(
			'name'    => "Feed Your Curiosity",
			'journal' => "I pledged myself to the Thousand Eyes faction in the Night Market. Melandir, speaker for Molto the Whisperer, wants me to get to know the perils of the districts better before sending me out on more jobs for the Thousand Eyes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	3999 => array(
			'name'    => "Lost on Bleakrock",
			'journal' => "<<1>> has lost track of her husband and her dog. I've offered to help her look for both of them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5751 => array(
			'name'    => "The Trials of Rahni'Za",
			'journal' => "Outside the school of blades called Rahni'Za, I encountered an initiate of the school who told me that the Sword-Disciples have been turned into thralls by a traitor. This betrayer now hides in the cavern at the end of the school's five trials.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4016 => array(
			'name'    => "The Missing of Bleakrock",
			'journal' => "I found evidence that the Daggerfall Covenant may soon attack Bleakrock and the rest of the Pact. We need to evacuate as soon as possible, but anyone who doesn't rally to the village will be left behind.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4018 => array(
			'name'    => "Giant Problems",
			'journal' => "Giants have attacked the village of Lower Yorgrim, destroying everything in their path.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5749 => array(
			'name'    => "The Seeker's Archive",
			'journal' => "I met Ibrula, a friend of the Star-Gazers who is trying to get into a ruined temple called the Seeker's Archive. She hopes to discover information inside that will help defeat the Celestials.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4022 => array(
			'name'    => "Salt of the Earth",
			'journal' => "The Covenant sent an advance force into Bal Foyen. They're destroying the fields behind Dhalmora.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4392 => array(
			'name'    => "Truth, Lies, and Prisoners",
			'journal' => "While exploring Coldharbour, I came across the Tower of Lies. Instead of a tower, the place is actually a hole in the ground where members of the Fighters Guild have been imprisoned.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5872 => array(
			'name'    => "A Melodic Mistake",
			'journal' => "The Gnisis Egg Mine has been shuttered. Some kind of mysterious illness has taken hold there. Unfortunately, many workers remain trapped inside.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4023 => array(
			'name'    => "If By Sea",
			'journal' => "We've landed safely. Captain Rana is going to muster the remaining Bleakrock villagers and the people of Dhalmora. She wants me to ensure the fires are lit in the nearby watchtower so we can warn Davon's Watch, the port city in Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4024 => array(
			'name'    => "Finding the Family",
			'journal' => "An Argonian named <<1>> lost most of his guar herd. They were spooked by scary noises in the fields behind Dhalmora. He wants me to collect the missing ones.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6460 => array(
			'name'    => "Of Ice and Death",
			'journal' => "The Silver Cormorant was hit by a strange ice storm that seemed to come out of nowhere. The ship is locked in ice and unable to move. I offered to help the crew.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4026 => array(
			'name'    => "Zeren in Peril",
			'journal' => "I've made my decision. I'm standing with <<1>> and defending Fort Zeren.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4028 => array(
			'name'    => "Breaking the Tide",
			'journal' => "The Daggerfall Covenant has seized control of Foyen Docks. An injured Argonian fisherman saw Rana fighting through their ranks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6468 => array(
			'name'    => "The Blood of Old Karth",
			'journal' => "A harrowstorm destroyed the town of Karthwatch and left it overrun with monsters. I agreed to help with the efforts to rescue any survivors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4034 => array(
			'name'    => "A Friend in Mead",
			'journal' => "Stormfist soldiers ransacked Voljar's Meadery and slaughtered the family-all because they wanted free mead. Haraldar's wife was one of the Voljars killed in the attack. He wants to reopen the meadery, and he recruited his four brothers to help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4219 => array(
			'name'    => "The Serpent's Beacon",
			'journal' => "Maormer pirates took the lighthouse at South Beacon and are signalling more pirate ships to come into port. If they're not stopped, they'll have the numbers they need to move on the port of Vulkhel Guard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6770 => array(
			'name'    => "The Corrupted Grove",
			'journal' => "A corruption has upset the natural balance of Stonelore Grove, turning the creatures there aggressive. Druid Audrine asked me to assist her in removing the corruption and discovering its source before it spreads to the rest of the island.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4037 => array(
			'name'    => "A Brother's Revenge",
			'journal' => "Yraldar Snowpeak was wounded trying to drive the Stormfist soldiers away from the meadery. He wants their leader, an Orc named Kazok, dead and his head on a pike. He asked for my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7034 => array(
			'name'    => "Snatch and Run",
			'journal' => "Local merchants reported that their goods are not arriving in Necrom. Ordinator investigators tracked the merchandise to the Camonna Tong's den, Camonnaruhn. Ordinator Tilena asked me to retrieve the supplies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4167 => array(
			'name'    => "A Business Proposition",
			'journal' => "A merchant in Nimalten claims his silk was stolen by bandits. I've been asked to retrieve it for the angry businessman.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4038 => array(
			'name'    => "Unorthodox Tactics",
			'journal' => "<<1>> thinks he's found a way to use the otherwise docile netch against the Daggerfall Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6467 => array(
			'name'    => "The Gathering Storm",
			'journal' => "When I approached the gates of Solitude, a wounded man stumbled out, fleeing for his life. He left me a bundle of papers and told me to keep them safe before he died.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4041 => array(
			'name'    => "Crossroads",
			'journal' => "I returned to Dhalmora. All of the Bleakrock villagers are gone. Aera informed me of a large Covenant attack on Bal Foyen. Darj has lead some of the non-combatants to Fort Zeren. Captain Rana is leading a defense of the docks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4043 => array(
			'name'    => "Collector of Pelts",
			'journal' => "After giants and wolves overran the village of Lower Yorgrim, the survivors found shelter in a tiny fishing camp outside the village walls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7498 => array(
			'name'    => "The Missing Tooth",
			'journal' => "<<1>> runs a finder's business in Glenumbra and uses the Thieves Guild to track down lost, mislaid, and stolen items. Helping her will surely reflect well on my skill as a thief-and be lucrative to boot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4048 => array(
			'name'    => "The Direct Approach",
			'journal' => "<<1>> says that Shadowed Path cultists have taken over Haynote Cave, north of Vlastarus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6664 => array(
			'name'    => "Family Secrets",
			'journal' => "Bastian recently learned that he was never supposed to stay with the Silvelles. He should have been taken in by his older sister Clairene, who is married to Count Auzin of Wayrest. Bastian wants to find out what happened.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4051 => array(
			'name'    => "Warning Davon's Watch",
			'journal' => "I should head to Davon's Watch, so I can ensure the troops have been warned about the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4052 => array(
			'name'    => "Death to the Crone",
			'journal' => "Nisin Cave is now a danger to Vlastarus. Shadowed Path necromancers and a Dremora named <<1>> have taken up residence.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6159 => array(
			'name'    => "Culling Serpents",
			'journal' => "<<1>> of the Divine Prosecution requested assistance in curbing a growing reef viper population threatening Summerset's mainland.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4054 => array(
			'name'    => "Deadly Whispers",
			'journal' => "Praxin and his comrades from the Fighters Guild explained that spiders infest the Spindleclutch caves. And maybe something else. It whispers in the darkness.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4424 => array(
			'name'    => "A Debt Come Due",
			'journal' => "<<1>> wants an Imperial lieutenant killed, to hurt their army's morale and readiness.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6740 => array(
			'name'    => "An Order Gone Awry",
			'journal' => "Captain Kaleen was hired to recover a missing rogue named Jakarn and a pair of items he was suppose to deliver. She asked for my assistance rescuing him and retrieving the items from a mysterious group called the Ascendant Order.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4875 => array(
			'name'    => "Bounty: Shadowed Path",
			'journal' => "I accepted the Fighters Guild contract Arkas offered.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4055 => array(
			'name'    => "A Cure For Droi",
			'journal' => "The Nord soldier Droi burns with fever from a witch's curse. I need to collect a plant called Kyne's Breath to help reduce his fever.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4056 => array(
			'name'    => "For Kyne's Honor",
			'journal' => "The Keepers of the Kynesgrove shrine have become Daedric witches. The loss of their loved ones to war has made them lash out against Kyne. I need to find a way to resolve this.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6492 => array(
			'name'    => "Soldiers of Fortune and Glory",
			'journal' => "I found a notice seeking brave and able-bodied adventurers for work.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5785 => array(
			'name'    => "Dark Anchors in Rivenspire",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Rivenspire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4058 => array(
			'name'    => "Shadows Over Windhelm",
			'journal' => "I arrived in Eastmarch, in the Nord city of Windhelm. The place bustles with revelers as a celebration honoring the Skald-King is underway.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4696 => array(
			'name'    => "The High Cost of Travel",
			'journal' => "<<1>> has asked that I travel to <<2>> and inform his brother <<3>> that his wife is dead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5541 => array(
			'name'    => "Pious Intervention",
			'journal' => "The pirate captain Crafty Lerisa has come to Kvatch to convince her sister, Josseline, to abandon the Society of the Dragon, a cult headed by the Grand Chanter of the Akatosh Cathedral for believers not ready to commit to the priesthood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4059 => array(
			'name'    => "The Konunleikar",
			'journal' => "Thane Mera has named me the Champion of Windhelm and wants me to enter the Footrace of the Nine Holds.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7261 => array(
			'name'    => "Save the Ruins!",
			'journal' => "Lector Volonaro has asked me to place alchemical deterrents in the Sunport Bay Forest to aid in the efforts of restoring local ruins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5534 => array(
			'name'    => "Cleaning House",
			'journal' => "<<1>>, the guildmaster of the Abah's Landing Thieves Guild, believes someone sold out the guild to the Iron Wheel. She needs my help to deal with the matter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4060 => array(
			'name'    => "Windhelm's Champion",
			'journal' => "Thane Mera, enraged by the death of the racer Hadring, demands that the assassins pay for this crime. She wants me, Windhelm's Champion, to deal with this-personally.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4062 => array(
			'name'    => "Blindsided",
			'journal' => "A renegade clan of Nords known as the Stormfists launched a surprise attack against Fort Morvunskar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6491 => array(
			'name'    => "Poison's Sting",
			'journal' => "Hedrynne, sister of Hamvir, has been poisoned by the bite of a giant insect near the cave known as Chillwind Depths. I should see if I can help save her life.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4065 => array(
			'name'    => "An Evil Presence",
			'journal' => "<<1>> says a skeleton invaded Vlastarus a few days ago. She thinks it came from a graveyard to the southwest.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4260 => array(
			'name'    => "Breaking the Barrier",
			'journal' => "Firsthold is under assault by High Kinlady Estre. The First Auridon Marines have arrived to try and save the city, but the city is overrun with Daedra and the castle proper is surrounded by a magical barrier.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6500 => array(
			'name'    => "The Fading Fire",
			'journal' => "A priestess of Kyne named Yrsild asked me to investigate the ancient ruins of Labyrinthian in order to find her grandfather, Keldnyr, who has gone missing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4066 => array(
			'name'    => "Bear Essentials",
			'journal' => "A giant bear named <<1>> has been attacking travelers near Vlastarus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4123 => array(
			'name'    => "Gods Save the King",
			'journal' => "The Dreamwalker is lost in King Jorunn's nightmares. I must enter the Dreamstride and try to rescue both of them before the Daedric poison kills the King.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4067 => array(
			'name'    => "The Bard of Hounds",
			'journal' => "An Argonian named Bishalus is afraid that nix-hounds have devoured her friend, Gena. She claimed to have made a flute that could tame the beasts, but she has yet to return.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4068 => array(
			'name'    => "Mementos",
			'journal' => "Refugees fleeing the war were often preyed upon by bandits. Many of their possessions were stolen on the way to Vlastarus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6674 => array(
			'name'    => "Breaking the Chain",
			'journal' => "According to <<1>>, a group of Naga extremists in Xi-Tsei called Sul-Xan are capturing and sacrificing innocent people. His client wants me to infiltrate the temple and destroy the extremists' ritual tools, then return for my reward.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4069 => array(
			'name'    => "Making Amends",
			'journal' => "The Skald-King and the other Ebonheart Pact leaders have established a base at Jorunn's Stand. While the Skald-King recovers, the leaders need to decide how to deal with Fildgor Orcthane.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4199 => array(
			'name'    => "A Tangled Net",
			'journal' => "<<1>> wants to neutralize the Gold Coast mercenaries in Malabal Tor so his people will be safe from reprisal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4070 => array(
			'name'    => "Security Details",
			'journal' => "The sentry captain, Ula-Reen, keeps the camp secure. She asked me to help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7513 => array(
			'name'    => "Favor for Lady Arabelle",
			'journal' => "I agreed to complete a favor for Lady Arabelle Davaux.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4071 => array(
			'name'    => "Sleep for the Dead",
			'journal' => "The orders from Fildgor Orcthane suggest that the Stormfists are after something precious in the royal tomb. The spirit of Queen Nurnhilde and Lieutenant Koruni both asked me to return the relics. While I'm at it, I'll see what else is going on.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4636 => array(
			'name'    => "Moonhenge's Tear",
			'journal' => "I need to halt the Daedric invasion from Moonhenge.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4908 => array(
			'name'    => "Rare Imports",
			'journal' => "<<1>> needs couriers to make deliveries around <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4072 => array(
			'name'    => "Rock Bone Diplomacy",
			'journal' => "Rock Bone Goblins have made camp to the northeast of Vlastarus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6499 => array(
			'name'    => "Scraps Matter",
			'journal' => "The High Elf Neramo needs to acquire special Dwarven power nodes and a master scepter in order to conduct his experiment. He asked for my help retrieving them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4073 => array(
			'name'    => "Stolen Banner",
			'journal' => "<<1>>'s family banner has been stolen. He believes it was taken by bandits who visited Vlastarus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4646 => array(
			'name'    => "The Mnemic Egg",
			'journal' => "The Dominion took <<A:1>> to the Ayleid ruin of <<2>>. There they used its power to sever the Argonian connection with the Hist. As long as it remains in their possession, no Argonian eggs can hatch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6251 => array(
			'name'    => "The Guiding Light",
			'journal' => "A scholar by the name of Tharayya is searching for an elusive artifact hidden somewhere within the ruins of Garlas Malatar. She's enlisted me to help her deal with the hazards of the lost Ayleid city and recover her quarry.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4532 => array(
			'name'    => "Take Me Home",
			'journal' => "Deep inside a dungeon called Mehrunes' Spite, I found a Scamp with a labeled choker. It apparently belongs to a mage in Skywatch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4074 => array(
			'name'    => "For a Friend",
			'journal' => "To the east of Vlastarus lies Juras Falls, home to the restless undead, including the former Captain Juras.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4075 => array(
			'name'    => "A Right to Live",
			'journal' => "I met a frightened young woman. Her and her friend were attacked by monsters on their way back to Windhelm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5795 => array(
			'name'    => "Dark Anchors in Malabal Tor",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Malabal Tor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4076 => array(
			'name'    => "To the Front",
			'journal' => "The Ebonheart Pact is at war with the Daggerfall Covenant and the Aldmeri Dominion for control of Cyrodiil.\n\nEveryone is needed in the war effort.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4706 => array(
			'name'    => "Reporting for Duty",
			'journal' => "Taralin says there is much more for me to learn about the war in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4077 => array(
			'name'    => "The Dreamers",
			'journal' => "A Daedric cult called the Supernal Dreamers stole a Crystal of Power from <<1>>.  The cult shattered the Crystal in five <<m:2>> and are using them in Daedric rituals.  <<1>> has asked me to kill the cultists and bring back the shards.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6764 => array(
			'name'    => "Escape from Amenos",
			'journal' => "We have corroborating evidence that High King Emeric, Queen Ayrenn, and Prince Irnskar may have washed ashore on the prison island of Amenos. Lady Arabelle has a plan for rescuing them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5545 => array(
			'name'    => "Prison Break",
			'journal' => "<<1>> found out the Iron Wheel is planning to ship <<2>> to Taneth. As acting guildmaster, I've called on the Thieves Guild to do something about it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4079 => array(
			'name'    => "Essence of Flame",
			'journal' => "Flame atronachs are plaguing the Ayleid ruins at <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4080 => array(
			'name'    => "Season of Harvest",
			'journal' => "I entered a secluded grove near Daggerfall Harbor called Bad Man's Hallows. The people here are engaged in a festival honoring the Bad Man, an ancient harvest god.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6774 => array(
			'name'    => "A Father's Pride",
			'journal' => "Brokerock Mine is being attacked by creatures called Hadolids. Meryline Barthel asked me to help her deal with the Hadolids in the mine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4531 => array(
			'name'    => "A Brush With Death",
			'journal' => "I found a journal within the Mines of Khuras. It contained a message addressed to Odette Vinielle.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4081 => array(
			'name'    => "Silver Scales",
			'journal' => "<<1>> asked me to gather some silver scales for the local artist to use in her next project.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6696 => array(
			'name'    => "The Last Ambition",
			'journal' => "Lyranth thinks we need to find the Fourth Ambition and gather up Sombren and Calia before Mehrunes Dagon goes after them and launches his next plot to conquer Nirn. She has once again asked me to help her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4083 => array(
			'name'    => "Caged Heat",
			'journal' => "The Supernal Dreamers have retaliated against <<1>> for having me steal back his Crystal of Power.  The Daedric cult captured him and threw him into a cage.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4085 => array(
			'name'    => "A Tale Forever Told",
			'journal' => "I've found the temple of the spinners of Malabal Tor, where their \"story\" has gotten away from them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4086 => array(
			'name'    => "Strange Allies",
			'journal' => "Aspera Giant-Friend says the giants are about to go to war against the Stormfists that hold Cradlecrush because their mammoths were stolen. As long as I oppose the Stormfists, these giants will consider me to be an ally.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4456 => array(
			'name'    => "The Hound's Plan",
			'journal' => "<<1>> told me his clan had a hand in the Silvenar's alleged kidnapping by the Hound.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4087 => array(
			'name'    => "Sneak Peak",
			'journal' => "King Jorunn wants me to make my way to Skuldafn. He wants to deal with Fildgor Orcthane and then bring the rest of the Pact leaders to the ancient dragonshrine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4088 => array(
			'name'    => "Can't Take It With Them",
			'journal' => "<<1>>, a Wood Elf down in the Bad Man's tunnels, claims he is one of the mages that originally summoned the Dremora Lord now posing as the Bad Man.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4925 => array(
			'name'    => "Urenenya's Lament",
			'journal' => "In ruined Klathzgar, I found a note from an Altmeri scholar investigating an ancient mystery. An Ayleid princess named Urenenya vanished centuries ago, and the scholar came here after learning of her connection to the Dwemer architect Klazthgar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4089 => array(
			'name'    => "The Hound's Men",
			'journal' => "The Green Lady's sister, <<1>>, may have been kidnapped from <<2>>, a village famous for being the birthplace of several previous incarnations of the Green Lady as well.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6975 => array(
			'name'    => "A Hidden Fate",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5445 => array(
			'name'    => "A Treasure in Need of a Home",
			'journal' => "I found some ancient Nord relics. I'm sure someone will want to examine these. I'll take them with me and try to locate an interested party.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4091 => array(
			'name'    => "Fulfilling One's Fate",
			'journal' => "The Worm Cult invaded Ouze and captured its primary protector, <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7293 => array(
			'name'    => "A Traitor's Tyranny",
			'journal' => "The Worm Cult infiltrated a College of Sea and Sword lodge in Eastern Solstice. With the help of a traitorous lector, they invaded the lodge, trapped the apprentices, and stole a Daedric relic. I agreed to save the apprentices and recover the relic.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5566 => array(
			'name'    => "A Faded Flower",
			'journal' => "Velsa left a note, asking that I come to visit her at her garden. She has something she wants to discuss.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4092 => array(
			'name'    => "Back to Rest",
			'journal' => "The Worm Cult is trying to enslave the spirits of Ouze.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4095 => array(
			'name'    => "The Siege of Cradlecrush",
			'journal' => "Aspera and the giants are ready to begin the siege of Cradlecrush. Using the documents I recovered from the Stormfist scouts, they formulated a plan of battle.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4096 => array(
			'name'    => "Merriment and Mystery",
			'journal' => "I wandered into Cragwallow, a small settlement used by the Skald-King's royal work crews. After a lot of hard work maintaining the roads, they're preparing to throw a party.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4099 => array(
			'name'    => "A Dying Wish",
			'journal' => "Lothgar Steady-Hand was grievously wounded by the creatures that attacked Cragwallow. He appears to be dying.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6999 => array(
			'name'    => "Lovesick",
			'journal' => "Dacia Caro thinks her love, Nathyn Dreloth, is being held captive by his mother, Sathile Dreloth. I agreed to help her rescue him from the isolation tower of Tel Dreloth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4106 => array(
			'name'    => "The Better of Two Evils",
			'journal' => "Valeric believes vampires don't have to be monsters. He wants my help to stop his father, Majorn, who plans to create an army of bloodfiends.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6484 => array(
			'name'    => "A Clan Divided",
			'journal' => "An accident at the Mor Khazgur Stronghold trapped workers inside the mine and caused strange creatures to emerge from below. Any efforts to rescue them have been stalled by infighting and a difference of opinion over the chief's new methods.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4107 => array(
			'name'    => "Banishing the Banished",
			'journal' => "The Banished Cells have been overrun by Daedra. They have set free the immortal High Kinlord Rilis, an evil being of great magical power. I have agreed to help Keeper Cirion, the warden of the Banished Cells, deal with this crisis.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4108 => array(
			'name'    => "The Tale of the Green Lady",
			'journal' => "<<1>> offered to tell me the story of the Green Lady and show me the most sacred sites of Deepwoods.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6472 => array(
			'name'    => "The Lady of Blood",
			'journal' => "Gwendis of House Ravenwatch recruited me to help free captives trapped inside Greymoor Keep by Lady Essenia, a cruel vampiric sovereign. Gwendis's friend, Adusa-daro, is among those prisoners.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4111 => array(
			'name'    => "The Show Must Go On",
			'journal' => "Vicious raiders known as the Steel Shrikes attacked a troupe of traveling entertainers. They hold several performers captive at Fuller's Break.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5981 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4112 => array(
			'name'    => "Raise the Curtain",
			'journal' => "<<1>> wants to take his own type of revenge against the Steel Shrikes who destroyed his troupe's ship and livelihood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4115 => array(
			'name'    => "Eternal Slumber",
			'journal' => "Jorunn the Skald-King has been poisoned. Now he lies in an eternal slumber. The Ternion monks, who revere the Three Old Gods, attempt to cure him. But time is running out.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4117 => array(
			'name'    => "Songs of Sovngarde",
			'journal' => "Inside the tombs of Skuldafn, I discovered that Fildgor Orcthane crossed into Sovngarde. To defeat Fildgor, I must follow him there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6995 => array(
			'name'    => "Lost in Thought",
			'journal' => "Cipher Huzmargo, the overseer of the Rectory Corporea, needs an adventurer to protect him while he investigates why the apparatus housed within the Rectory is failing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4118 => array(
			'name'    => "Dark Deeds",
			'journal' => "Draugr have spilled from the barrow beneath Bonestrewn Crest. Now they roam the area, attacking the living.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4119 => array(
			'name'    => "The Storm's Call",
			'journal' => "Maormer Sea Vipers invaded Ilayas Ruins, searching for a weather-controlling relic called the Sphere of Storms. I've been asked to help the stormwardens defend the sphere until reinforcements arrive.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4124 => array(
			'name'    => "The Prisoner of Jathsogur",
			'journal' => "The Drublog clan of Wood Orcs captured <<c:1>>. I need to infiltrate Jathsogur and do whatever's necessary to free him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4126 => array(
			'name'    => "Labor Dispute",
			'journal' => "The Orcthane's troops have taken control of the mine in Lost Knife Cave. They're forcing the miners to produce ore to equip Fildgor's army.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6498 => array(
			'name'    => "Spellbound",
			'journal' => "I found a strange talking doll. The soul trapped inside insists that the Dragon Cult is responsible for imprisoning her. She seems confident that if I help gather her belongings, she can use them to reclaim her body.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4129 => array(
			'name'    => "Shipwrecked Sailors",
			'journal' => "Maormer attacked an Aldmeri Dominion ship off the coast of <<1>> and captured several members of its crew.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4131 => array(
			'name'    => "The Maormer's Vessels",
			'journal' => "<<1>> asked me to burn the Maormer sailboats in <<2>> to prevent further ambushes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4133 => array(
			'name'    => "Soul Harvest",
			'journal' => "Worm Cultists have attacked <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4134 => array(
			'name'    => "Something Rotten",
			'journal' => "Someone is stirring up chaos in <<1>>, and <<2>> asked me to help put a stop to it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4135 => array(
			'name'    => "Pulled Under",
			'journal' => "The people of Riften have been brought inside their shattered walls. While Sinmur is gone, however, Riften's people may face other threats.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4137 => array(
			'name'    => "For Everything a Season",
			'journal' => "Though most of the rotmeth was stolen, an unfinished batch remains. <<1>> needs some special ingredients to complete what's left of the brew.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7031 => array(
			'name'    => "War Within",
			'journal' => "The nightmare Daedra Ansuul the Tormentor is invading Warlock Vanton's dreams. Wizard Twelvane asked me to journey into Vanton's mind and stop Ansuul before she defeats him and enters the waking world.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4138 => array(
			'name'    => "Alchemical Analysis",
			'journal' => "Jomund and his miners have no idea what to make of the mysterious metal they found. However, Jomund has a friend at Fort Amol who might be able to help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6516 => array(
			'name'    => "The Aspiring Scholar",
			'journal' => "I met a scholar named Yvara Plouff near a broken down cart. She asked me to help her research some nearby Dragon mounds.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4139 => array(
			'name'    => "Shattered Hopes",
			'journal' => "The Ebonheart Pact has come together under King Jorunn's command to battle the Worm Cult at the ruins of Honrich Tower.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4140 => array(
			'name'    => "Soldier Down",
			'journal' => "<<1>>, an Argonian healer, told me that many Pact soldiers lie wounded around Honrich Tower.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6504 => array(
			'name'    => "Reinforcement for Kyne's Aegis",
			'journal' => "Sea Giants and vampires have invaded the island of Kyne's Aegis. I must rally a group of allies to face this threat from the north.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4143 => array(
			'name'    => "Restore the Silvenar",
			'journal' => "Indaenir, the new Silvenar, traveled across Malabal Tor to marry the Green Lady and fulfill his role as the spiritual leader of the Wood Elves. But the Hound arrived first and is trying to usurp his position.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4144 => array(
			'name'    => "The Misfortunate Minstrels",
			'journal' => "People from across the Aldmeri Dominion gather to celebrate the handfast of the Silvenar and the Green Lady. The Hound, however, caused many of them trouble. Among these are a group of minstrels led by <<1>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4149 => array(
			'name'    => "Party Planning",
			'journal' => "Rigurt the Brash has been named Master of Ceremonies for the celebration of King Jorunn's anniversary. Rigurt has many organizational duties to attend to and needs a little help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5893 => array(
			'name'    => "Divine Intervention",
			'journal' => "Lord Vivec's condition continues to deteriorate as his energy drains away. Archcanon Tarvus believes that Chodala's staff holds the key to saving the Warrior-Poet. However, it also makes the Ashlander practically impervious to harm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4152 => array(
			'name'    => "Swamp to Snow",
			'journal' => "After events in Shadowfen, Vicecanon Heita-Meen and I must travel to Eastmarch, in Skyrim.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4153 => array(
			'name'    => "Concealed Weapons",
			'journal' => "I've arrived in Nimalten to find the city under threat. Thane Fjora was attacked by her housecarl. Aera Earth-Turner, my old friend from Bleakrock Isle, is leading an investigation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4160 => array(
			'name'    => "Approaching Thunder",
			'journal' => "I've received a chilly welcome at Fullhelm Fort. The garrison is beset by undead, and the old Thane is not welcoming strangers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4161 => array(
			'name'    => "A Novel Idea",
			'journal' => "A spinner in Valeguard wants to finally transcribe a story into written word, but is having difficulties.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4163 => array(
			'name'    => "Onward to Shadowfen",
			'journal' => "<<1>>, an agent of Shadowfen's leadership, asked me to help <<2>> investigate a number of murders in Stormhold.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6523 => array(
			'name'    => "Dwemer Disassembly",
			'journal' => "The Coterie of Organized Scholars wants me to test a Dissonance Apparatus on Dwarven constructs in the Scraps. They would also like me to recover a Dwarven heartcog.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4164 => array(
			'name'    => "A Giant in Smokefrost Peaks",
			'journal' => "Even though we've won a victory over the Worm Cult, <<1>> tells me that <<2>> is headed toward Riften. I should follow in the wake of the giant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7043 => array(
			'name'    => "Arcane Residue",
			'journal' => "Lord Vivec has taken an interest in arcanist magic and wants to study it further. I agreed to defeat Runemaster Xiomara in Apocrypha and gather arcane residue from warriors he has slain.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4166 => array(
			'name'    => "The War Council",
			'journal' => "Thane Harvald want Lord Vurlop and I to go into the keep and check on King Jorunn. We need to make sure that the King and his war council are safe.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5888 => array(
			'name'    => "Divine Delusions",
			'journal' => "Archcanon Tarvus agrees that Chodala is somehow involved in Lord Vivec's loss of power. He wants me to confirm his involvement and find a way to get his staff without starting a war with the Ashlanders.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4173 => array(
			'name'    => "The Thunder Breaks",
			'journal' => "Preparations were completed just in time - the undead are attacking Fullhelm Fort! <<1>> has asked me to help defeat the attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6045 => array(
			'name'    => "Oasis in a Metal Desert",
			'journal' => "Lector Naril oversees the Everwound Wellspring, an oasis in the mechanical desert that is the Clockwork City. He has found a way to grow plants in the metallic soil. Once every year, he shares his harvest and selects a new apprentice.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4176 => array(
			'name'    => "Breaking the Coven",
			'journal' => "There's a coven of Hagravens in Northwind Mine. They need to die.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4177 => array(
			'name'    => "Victory at Morvunskar",
			'journal' => "Lieutenant Koruni asked me to deliver a dispatch detailing Fort Morvunskar's status to Thane Oda Wolf-Sister at Fort Amol.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6431 => array(
			'name'    => "Sourcing the Ensorcelled",
			'journal' => "The Senchal job broker, Guybert Flaubert, asked me to travel to the Forsaken Citadel and acquire cursed swords.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6053 => array(
			'name'    => "Pledge: Bloodroot Forge",
			'journal' => "<<1>> of the Undaunted has challenged me to explore the Bloodroot Forge.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4184 => array(
			'name'    => "To Pinepeak Caverns",
			'journal' => "The spirit Valdur worries about his fellow companion spirit, bound to Pinepeak Cavern. He's asked me to check on her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4185 => array(
			'name'    => "To Honrich Tower",
			'journal' => "Valdur spoke of a dark gathering at Honrich Tower.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7079 => array(
			'name'    => "Prisoner of Fate",
			'journal' => "Leramil the Wise called me to serve as proxy to Hermaeus Mora, Daedric Prince of Fate. The Dremora Torvesard seeks to unlock a secret hidden in the vaults of Apocrypha. If he succeeds, reality itself could be in danger.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4186 => array(
			'name'    => "Securing the Pass",
			'journal' => "The Worm Cult has barred the route to <<1>>. King Jorunn has asked me to clear the way.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4188 => array(
			'name'    => "Stomping Sinmur",
			'journal' => "We're almost there. We must fight through Worm Cultists to reach Sinmur and defeat him!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6558 => array(
			'name'    => "Halt the Harrowstorms",
			'journal' => "<<1>> charged me with finding allies to assault the harrowstorm ritual sites springing up across the Reach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5904 => array(
			'name'    => "Salothan's Curse",
			'journal' => "I've been hired to defeat the spirits which haunt Salothan Tomb.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4189 => array(
			'name'    => "Names of the Fallen",
			'journal' => "Ebonheart Pact soldiers carry amulets into battle, so they can be identified if they die. Collecting these amulets may bring solace to the families of the dead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6562 => array(
			'name'    => "Lost in the Reach",
			'journal' => "<<1>> asked me to look for her husband, <<2>>, and their pet bunny <<3>>. She thinks they might be lost in the Reach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4193 => array(
			'name'    => "House and Home",
			'journal' => "Redguard invaders attacked Velyn Harbor, while Drublog Wood Orcs entered the town from the hills. With the harbor's defenders either killed or scattered in the attack, only a few Fists of Thalmor continue the defense.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6433 => array(
			'name'    => "Rude Awakening",
			'journal' => "Dirge Truptor asked me to remove an Order of the New Moon recruiter operating in Stormhaven.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4194 => array(
			'name'    => "One Fell Swoop",
			'journal' => "The battle for Velyn Harbor continues. With a little luck and a lot of effort, the Redguard invaders and Drublog Wood Orcs can be driven from the town.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4195 => array(
			'name'    => "The Unkindest Cut",
			'journal' => "<<1>> isn't a resident of Velyn Harbor, but he's been caught up in their problems.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6427 => array(
			'name'    => "Witches Festival Writ",
			'journal' => "I accepted a crafting writ related to the Witches Festival.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4196 => array(
			'name'    => "Enemy of My Enemy",
			'journal' => "<<1>> hired Gold Coast mercenaries from Colovia to protect his tradepost, but suspects they will turn on him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4201 => array(
			'name'    => "A Walk Above the Clouds",
			'journal' => "I've discovered a Worm Cult cell has come to the ruins of a Dragon Cult stronghold. No good can come of this.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7095 => array(
			'name'    => "Appointed Executor",
			'journal' => "I found a journal next to the corpse of an Imperial called Haldain. Haldain's last wish was for someone to bring his belongings to his sister in Skingrad.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4202 => array(
			'name'    => "Force of Nature",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6574 => array(
			'name'    => "U28 REUSE",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5920 => array(
			'name'    => "Breaking Through the Fog",
			'journal' => "Someone extinguished Seyda Neen's lighthouse to cause a shipwreck. Deputy Governor Deminah Salvi suspects slavers and asked me to help her investigate the matter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4205 => array(
			'name'    => "Our Poor Town",
			'journal' => "Trolls overran the town of Darkwater Crossing and now the Argonians who lived there have taken refuge in a nearby cave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6077 => array(
			'name'    => "A Fine-Feathered Foe",
			'journal' => "I have accepted a contract to defeat the creature Wraith-of-Crows and other Daedric incursions in Clockwork City's Radius.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4208 => array(
			'name'    => "Silsailen Rescue",
			'journal' => "The town of Silsailen is under attack by imposters disguised as First Auridon Marines. The townspeople suspect these men and women are a part of a group called the Veiled Heritance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5928 => array(
			'name'    => "Relics of Ebernanit",
			'journal' => "Numani-Rasi of the Urshilaku Ashlander tribe has requested that I explore the ruins of the Daedric shrine Ebernanit and gather as many historic relics as I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4213 => array(
			'name'    => "EM IND CONTENT 2",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4217 => array(
			'name'    => "In the Name of the Queen",
			'journal' => "Queen Ayrenn seeks to perform rituals honoring her ancestors. When she arrived at Tanzelwil, however, she found the spirits angered and violent.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6983 => array(
			'name'    => "Hungry for Knowledge",
			'journal' => "I found a member of the Remnant named Zirokh standing outside the Apogee of the Tormenting Eye. He's worried about Dremnaken invading the Apogee and devouring its knowledge. I agreed to rescue some artifacts and purge the Dremnaken.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4218 => array(
			'name'    => "Best of the Best",
			'journal' => "The Argonian Steady-Hand believes he's the best hunter in Eastmarch, better than any Nord or Dark Elf alive. He's challenged me to a hunting contest. If I can shoot a number of rabbits in one minute, or catch the rabbit called Golden Jack, I win.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6590 => array(
			'name'    => "Vows and Oaths",
			'journal' => "The Reach clans Six-Fords and Eagleseers don't get along. They plan to rectify that through the marriage of two of their young members, Eslyn and Madearn. A handfasting rite must be performed to announce their intention to marry.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5936 => array(
			'name'    => "Ache for Cake",
			'journal' => "I received a voucher for a special-occasion cake from the renowned master baker, Chef Donolon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4221 => array(
			'name'    => "An Act of Kindness",
			'journal' => "In the ruins of Ezduiin, I found an enterprising researcher working with Telenger. She's concerned about the recent appearance of the spirits. She seeks a way to lay them to rest.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6093 => array(
			'name'    => "The Mage's Dog",
			'journal' => "I found the journal of Orinol, a dead mage from the city of Cormount, in Grahtwood. He arrived in Clockwork City by accident and died in the Radius sometime later. His dying wish was that someone would care for his dog, Jona.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4224 => array(
			'name'    => "Abominations",
			'journal' => "Two Weynon villagers were taken by the vampires of <<1>>. <<2>> fears they are dead and about to rise as bloodfiends.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4228 => array(
			'name'    => "Claw of Akatosh",
			'journal' => "The Claw of Akatosh has been stolen from Weynon Priory by the Black Dagger gang. I've been asked to recover it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6072 => array(
			'name'    => "A Matter of Respect",
			'journal' => "The Bursar of Tribute demands compensation for the Blackfeather Court from mortals who refuse to die so they can be pecked apart and eaten.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4229 => array(
			'name'    => "Sacred Prey, Hunt Profane",
			'journal' => "An Altmer doing research in Treehenge found some cultists poaching mammoths at the shrine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6475 => array(
			'name'    => "Prisoner of the Past",
			'journal' => "Alwyn Felsdottar has been haunted by visions ever since someone scattered her ancestor's bones and belongings in Midnight Barrow. I promised to help her retrieve his remains to uncover the truth about his past, and allow his spirit to rest again.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4244 => array(
			'name'    => "Black Dagger Supplies",
			'journal' => "The Black Daggers are using an abandoned house west of Weynon to store supplies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4247 => array(
			'name'    => "Guard Work is Never Done",
			'journal' => "<<1>> has asked me to travel to <<2>> and return <<3>> to rest.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6495 => array(
			'name'    => "The Preservation of Life",
			'journal' => "<<1>>, representing the Coterie of Organized Scholars, has asked me to recover research sampled from Dragonhome.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6117 => array(
			'name'    => "The Taste of Fear",
			'journal' => "The garrison at Cey-Tarn Keep has been terrorized by a spate of unusual disappearances and deaths. A monster hunter named Silurie has enlisted my help to stop the killings.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4248 => array(
			'name'    => "Geirmund's Guardian",
			'journal' => "Geirmund's Hall hosts a festival, and Thane Vofknir is looking for brave souls willing to participate in an epic challenge. Those who pass the tests are named Geirmund's Guardian.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4249 => array(
			'name'    => "Trial of the Body",
			'journal' => "The warrior Daljari Half-Troll oversees the Trial of the Body, a test of brawn over brain.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4250 => array(
			'name'    => "Trial of the Mind",
			'journal' => "Iroda, a member of the Thane's household, proctors the Trial of the Mind. She's going to pose riddles to me. I need to seek out the answers to each on the island, then return to her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4252 => array(
			'name'    => "Save Your Voice",
			'journal' => "A bunch of dumb, drunken bards have gotten themselves stuck in a giant camp near Ivarstead. They tried to charm the giants with music. Now I need to run over there and save them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6622 => array(
			'name'    => "A Foe Most Porcine",
			'journal' => "I received an invitation from Jester King Emeric to enter some kind of competition at the Hammerdeath Arena in Stormhaven.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4253 => array(
			'name'    => "Geirmund's Oath",
			'journal' => "I met a Nord named Eirfa at Geirmund's Hall. She asked if I'd be interested in participating in a drinking contest. Let's see how this goes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4259 => array(
			'name'    => "Foreign Vintage",
			'journal' => "Spies from the Aldmeri Dominion have infiltrated Sanguine's celebration. <<1>> gave me Sanguine's goblet and wants me to find the spies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4263 => array(
			'name'    => "Field of Fire",
			'journal' => "<<1>> asked that I eliminate four flame atronachs issuing from the lava fissure in Chorrol.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4264 => array(
			'name'    => "Plague of Phaer",
			'journal' => "The town of Phaer is beset by a mysterious plague. The town alchemist, Hendil, has taken steps to treat the ill.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6509 => array(
			'name'    => "Lost Along the Shore",
			'journal' => "The Swords of Solitude asked me to kill a Sea Giant on Western Skyrim's northern shore, and recover information gathered by scouts who fell victim to the brute.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4270 => array(
			'name'    => "The Cache",
			'journal' => "Lliae asked me to bring supplies to one of her caches out in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6141 => array(
			'name'    => "Culture Clash",
			'journal' => "Rigurt the Brash, an ambassador from the Nord Cultural Exchange, came to Summerset to broker peace. Unfortunately, he can't get an audience with the Proxy Queen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4272 => array(
			'name'    => "Depths of Madness",
			'journal' => "I found a lost member of the Queen's entourage near the entrance to a hidden Vale. She told me that her fellow entourage members have been taken. A mad mage apparently holds them in the Vale.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6513 => array(
			'name'    => "The Pale Man",
			'journal' => "Aerolf spent his life hunting the monster he calls the Pale Man which he claims abducted his brother. He asked me to help track down the creature so he can avenge his loss.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4274 => array(
			'name'    => "Read Two Books",
			'journal' => "I need to test out a quest where someone reads two books. Fortunately, Jacques know where I can find them. He's lost, but he knows where to find books.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4275 => array(
			'name'    => "The Long Road Back",
			'journal' => "This is the Background Journal Text",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4282 => array(
			'name'    => "The Skald and the Scholar",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6532 => array(
			'name'    => "Guild Listings",
			'journal' => "I found a notice about the opportunity to join a guild of like-minded adventurers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4283 => array(
			'name'    => "River of Names",
			'journal' => "Svein fears that the fate of the dead in <<1>> will never be known to their families in Riften.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4289 => array(
			'name'    => "Blood Upon the Soil",
			'journal' => "Frostmoon Farmstead has been attacked, resulting in several grisly murders. Guard Jorald needs help uncovering the killer or killers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4300 => array(
			'name'    => "Blessings of the Eight",
			'journal' => "Torinaan is a holy site dedicated to the Divines. It's been corrupted by Daedra. The loyal clergy there want my aid in cleansing the area.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6541 => array(
			'name'    => "The Burning City",
			'journal' => "The battle for the Imperial City surges once more. I should see if I can lend my strength to my alliance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4302 => array(
			'name'    => "Worm Cult Summoner",
			'journal' => "The Worm Cult is raising dead giants to fight Pact soldiers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4304 => array(
			'name'    => "Redguard on the Run",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7062 => array(
			'name'    => "The Cost of Knowledge",
			'journal' => "After witnessing their father get caught in an Apocryphal curse, Tralise asked me to help conduct a ritual to free him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4307 => array(
			'name'    => "Returning Winter's Bite",
			'journal' => "Sage Svari has asked me to find and return <<1>>. She believes it's being held at Faldar's Tooth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6024 => array(
			'name'    => "Glitter and Gleam",
			'journal' => "The Bursar of Tributes seeks a particular treasure to add to the Blackfeather Court's hoard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4309 => array(
			'name'    => "Eye Spy",
			'journal' => "I met Arfanel, a member of the Eyes of the Queen at North Beacon. He's tracking an elite Covenant soldier and wants my assistance supporting the troops in the field.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4316 => array(
			'name'    => "On a Dare",
			'journal' => "Three friends have drunkenly wandered into the catacombs beneath Windhelm after having a good time at the Konunleikar. They won't leave the catacombs until they're reunited, but it's not safe for them to search the tunnels alone.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4321 => array(
			'name'    => "Problems Into Profit",
			'journal' => "The farmer <<1>> requested my help in dealing with a glowfly infestation in his hay at Ivarstead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6561 => array(
			'name'    => "Veteran Vateshran's Rites",
			'journal' => "The Rites Matron challenged me to complete the Vateshran's Rites within Vateshran Hollows. My success determines whether I earn the right to stay in the Reach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4322 => array(
			'name'    => "Lost Crown",
			'journal' => "In the Hall of the Dead beneath Windhelm I met <<1>>. She plans to gain favor in King Jorunn's court by eliminating <<2>>, an ancient necromancer entombed in the Merethic Era.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7066 => array(
			'name'    => "Kill Enemy Arcanists",
			'journal' => "Grand Warlord Sorcalin has authorized a reward for any Dominion soldier who slays 20 enemy Arcanists.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4327 => array(
			'name'    => "Preventative Measure",
			'journal' => "I met an Argonian named <<1>> in the ruins of Quendeluun. She's asked me to help me secure her freedom.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4329 => array(
			'name'    => "Harsh Lesson",
			'journal' => "I arrived at a place called the College of Aldmeri Propriety. One of the students claims this Thalmor training ground heaps hardship and abuse upon the Wood Elf and Khajiit students. She's asked me to investigate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4331 => array(
			'name'    => "Wearing the Veil",
			'journal' => "I've been taken to the secret island stronghold of the Veiled Heritance. If I can pass their trials, I may be able to discover the true leader behind the Veil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6048 => array(
			'name'    => "The Light of Knowledge",
			'journal' => "My companions and I turned back the Daedric incursion into the Clockwork City and rescued Sotha Sil from his own shadow. Now, the Clockwork God has some wisdom to share before he leaves.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4333 => array(
			'name'    => "A Valuable Distraction",
			'journal' => "<<1>>, a member of a Pact scouting party, barely escaped capture after discovering a hidden Lion Guard base in the mountains of the Rift. She needs a distraction to buy some time, so she can return with reinforcements.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4336 => array(
			'name'    => "Ancient Remains",
			'journal' => "A tribe of Wood Orcs is protecting Canonreeve Oraneth in <<1>> . <<2>>, a member of the Mages Guild, wants to stop them, but can't attack the Orcs directly due to their association with <<3>>, who is a member of the Thalmor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4337 => array(
			'name'    => "Buyer Beware",
			'journal' => "A Khajiiti merchant calling himself <<1>> claims to have found a way inside Belarata, a long-sealed Ayleid ruin.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7094 => array(
			'name'    => "Spilled Wine",
			'journal' => "Orissa Dellinoi's caravan was transporting a rare and expensive wine when it was attacked by rogues in Legion's Rest. She asked for my help in retrieving her stolen wine and finding her loyal companion, Lazen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4339 => array(
			'name'    => "If the Dead Could Talk",
			'journal' => "A research team from <<1>> has rediscovered the ancient city of Root Sunder. Unfortunately they've also been mauled by wild animals. <<2>>, the last survivor of the research team, needs the journals of her dead comrades.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6056 => array(
			'name'    => "The Astronomer's Apprentice",
			'journal' => "Cultists have attacked the Mnemonic Planisphere in order to steal Sotha Sil's memories, manifested as glowing stars. Amili Lloryn has tasked me to rescue these stars and return them to the Planisphere's sky.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4341 => array(
			'name'    => "Cutting Off the Source",
			'journal' => "I cast Merien's White Mask, a spell to hide the villagers from the Bloodthorn invaders. Now Merien wants my help to stop the Bloodthorn activity in Eagle's Brook.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7111 => array(
			'name'    => "Loan Recall",
			'journal' => "Commandant Salerius of the Imperial Legion of West Weald hired me to retrieve tool packs on loan to the Haldain Lumber Camp.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4346 => array(
			'name'    => "Nobles' Rest",
			'journal' => "I must discover what has happened to the missing nobles of Direfrost Keep.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4351 => array(
			'name'    => "Through the Daedric Lens",
			'journal' => "I discovered the Library of Dusk, an Ayleid ruin, just beyond the Hollow City. Several treasure-hunters and scholars in the service of the Mages Guild have been trapped within the ruins by Daedra.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5709 => array(
			'name'    => "Contract: Stonefalls Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4352 => array(
			'name'    => "The Library of Dusk",
			'journal' => "The Mages Guild hired dungeon-delvers and scholars to bolster their expedition force. A group of these treasure hunters are trapped in the Library of Dusk, including Kireth Vanos' brother, Raynor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4355 => array(
			'name'    => "Through the Ashes",
			'journal' => "The town of Dawnbreak has been overrun by Daedra. In response, First Auridon Marines have arrived to aid the town.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6723 => array(
			'name'    => "The Celestial Palanquin",
			'journal' => "I met a mysterious woman who calls herself the Anchorite. A strange magic connects us that may have something to do with Lyranth the Dremora. The Anchorite asked me to help her find Lyranth and investigate the Order of the Waking Flame in Fargrave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4358 => array(
			'name'    => "Between Blood and Bone",
			'journal' => "Captain One-Eye was forced to kill her own crew, and now she's vowed to destroy the apparition that controlled the cursed sailors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6743 => array(
			'name'    => "Pledge: Shipwright's Regret",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Shipwright's Regret.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4362 => array(
			'name'    => "The Jester's Joke",
			'journal' => "A restless spirit of Castle Rilis named Lisondor has asked me to retrieve his soul from a mad jester's playroom.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4364 => array(
			'name'    => "A Thorn in Your Side",
			'journal' => "To save the Mages Guild members that were forced to compete in the Observer's game, I decided to side with Angof. Now I just need to find a way to win the game.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5725 => array(
			'name'    => "Sacrament: Sewer Tenement",
			'journal' => "I received a contract from the Speaker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4368 => array(
			'name'    => "To Skywatch",
			'journal' => "I have purged the Veiled Heritance from Mathiisen. Now I should meet up with Razum-dar in Skywatch to learn what other plans the Veiled Heritance has.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4373 => array(
			'name'    => "Export Business",
			'journal' => "A Bosmer named Peras escaped from bandits hiding in the Crimson Cove. He claims his ship was boarded by the raiders, and he has friends still inside the cove.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4374 => array(
			'name'    => "Old Bones",
			'journal' => "I encountered a talking skull in the Lost Fleet. It seems  he can resist the power of the Crown of Bones, but was broken to bits because of this.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4378 => array(
			'name'    => "Naval Intelligence",
			'journal' => "<<1>> is a Proctor for the Aldmeri Dominion. He has tracked a bandit group known as the Crimson Raiders to their hideout. The Raiders have been causing trouble for the Dominion fleet.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6238 => array(
			'name'    => "Old Enemies",
			'journal' => "Bolu of the Dead-Water tribe, called the Naga-Kur in the Argonian tongue, wants me to go to the Echoing Hollow and accomplish the task he set before me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4381 => array(
			'name'    => "Sphere Assembly",
			'journal' => "Rulorn in <<1>> has offered a bounty for Dwarven parts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4382 => array(
			'name'    => "Moment of Truth",
			'journal' => "<<1>> was sent by his mother to prove his courage. He'd like me to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4387 => array(
			'name'    => "Forbidden Love",
			'journal' => "I found an ancient letter in <<Ac:1>>. It speaks of an ancient, forbidden love.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4391 => array(
			'name'    => "Catch the Lightning",
			'journal' => "A researcher and his apprentice are experimenting with an amulet that protects the wearer from wamasu lightning.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6260 => array(
			'name'    => "Salty Meats",
			'journal' => "Cyrodilic Collections' operation in Murkmire is growing, and more explorers mean more mouths to feed. With my help, Jee-Lar plans to send cured meats to Black Marsh.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4395 => array(
			'name'    => "Enemies at the Gate",
			'journal' => "I met the Wood Orcs of <<1>> in <<2>>. They have come under attack from raiders and gradually weakened by an unknown source. I should work with <<3>> to track down the source.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4396 => array(
			'name'    => "Unsafe Haven",
			'journal' => "<<1>>, the largest port on the Long Coast, has been taken by a powerful band of pirates!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4398 => array(
			'name'    => "A Chief Concern",
			'journal' => "<<1>> has asked me to find and deal with <<2>>, the real force behind the attacks on <<3>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6248 => array(
			'name'    => "Old Scrolls",
			'journal' => "Varo Hosidias of Cyrodilic Collections wants me to enter the Tsofeer Cavern and gather any old Imperial scrolls I find, as well as an Imperial cypher ring, all for the benefit of a benefactor named Tia Doran.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4399 => array(
			'name'    => "Into the Woods",
			'journal' => "I came across an abandoned pack at the edge of a dark forest. The pack suggests that members of the Fighters Guild might have been here.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4405 => array(
			'name'    => "A Little on the Side",
			'journal' => "<<1>> wants my help to keep her \"side business\" going. She assures me it isn't technically illegal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6771 => array(
			'name'    => "Tower Full of Trouble",
			'journal' => "Strange magic is causing chaos around Tor Draioch. I need to investigate it and put a stop to it before it hurts anyone.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4406 => array(
			'name'    => "Forgotten Soul",
			'journal' => "A researcher named <<1>> is trapped in the ruins of Root Sunder.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4409 => array(
			'name'    => "The Racer",
			'journal' => "<<1>> has offered me a chance to enter the frog race. If my frog wins, we'll split the prize.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6663 => array(
			'name'    => "Vinnus's Research",
			'journal' => "I found the corpse of Vinnus Reburrus next to an unsettling stone in the Deadlands. His last note said the mysterious stone was linked to Mehrunes Dagon's plans.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4410 => array(
			'name'    => "Assisting the Assistant",
			'journal' => "<<1>>'s pet-a Dwarven construct-has broken down. She wants me to steal parts so that she can repair it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5312 => array(
			'name'    => "Taking the Undaunted Pledge",
			'journal' => "Maj al-Ragath wants to introduce me to the Undaunted pledge.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4411 => array(
			'name'    => "Final Blows",
			'journal' => "A list of various Veiled Heritance members was found. Egranor is ensuring the people listed are brought to justice.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6267 => array(
			'name'    => "Tools of Slaughter",
			'journal' => "Cyrodilic Collections needs someone to rescue some well preserved relics from the Blackguards occupying the Teeth of Sithis.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4420 => array(
			'name'    => "Keepsake",
			'journal' => "In the confusion of battle, Sylvian Herius left a family heirloom in the Imperial-held cathedral across the river.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4421 => array(
			'name'    => "Thorns in Our Side",
			'journal' => "Bandits called The Thorns stole all the Cheydinhal rebels' valuables when war broke out.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6787 => array(
			'name'    => "Green with Envy",
			'journal' => "Ember still hopes to find the last member of her found family, Song. Silverslip and Jhaka have been investigating her whereabouts and apparently attracted the attention of a merchant in Evermore, a city in Bangkorai. That might be our next lead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4422 => array(
			'name'    => "Spice",
			'journal' => "Vyctoria Girien asked me to taint Imperial food stores with a special spice.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6286 => array(
			'name'    => "Aloe That Heals",
			'journal' => "The healers of Root-Whisper Village are in need of a rare aloe that grows in Vakka-Bok. I'll need to make my way past the dangerous creatures that lurk there in order to gather this plant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5632 => array(
			'name'    => "Contract: Bangkorai",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4429 => array(
			'name'    => "Last Rites",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5789 => array(
			'name'    => "Dark Anchors in Deshaan",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Deshaan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4432 => array(
			'name'    => "Blood and Sand",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6290 => array(
			'name'    => "Leather That Protects",
			'journal' => "The hunters and warriors of the Root-Whisper tribe are in desperate need for hackwing hides in order to craft leather armor. I've agreed to hunt down these beasts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4433 => array(
			'name'    => "Ayleid Treasure",
			'journal' => "<<1>> collects Ayleid items. She hasn't been able to search for any since the war started.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4434 => array(
			'name'    => "For Piety's Sake",
			'journal' => "Sylvian Herius's troops have written petitions, asking the gods' intervention on the rebel side.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6175 => array(
			'name'    => "Lost Libations",
			'journal' => "Alamar has lost several bottles of wine while on a winery tour. He hired me to find and collect them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5797 => array(
			'name'    => "Dark Anchors in Reaper's March",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Reaper's March.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4440 => array(
			'name'    => "Baan Dar's Boast",
			'journal' => "I met a strange old Wood Elf outside the ancient ruin known as Thormar. She's invited me to a party she calls \"the greatest in all the worlds.\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4441 => array(
			'name'    => "The Toothmaul Ploy",
			'journal' => "I stumbled onto an investigator named Pirondil being threatened by an unknown Altmer assailant in a Goblin cave. He's enlisted me to help get to the bottom of the situation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6302 => array(
			'name'    => "Scars of the Past",
			'journal' => "Iokkas' companions are lost in the nearby oasis caverns, surrounded by noxious fumes and hostile undead. I offered to help rescue them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4445 => array(
			'name'    => "Forever Bound",
			'journal' => "A polite spirit named <<1>> haunts <<Ac:2>>. He asked for my help in stopping the necromantic ritual that binds him to this place.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5656 => array(
			'name'    => "Contract: Auridon",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4453 => array(
			'name'    => "A Favor Returned",
			'journal' => "I arrived in Mournhold, the capital city of the Ebonheart Pact. As the seat of Dunmer power, it is home to the Tribunal Temple. Due to recent events, the city is bustling with visitors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4454 => array(
			'name'    => "Innocent Scoundrel",
			'journal' => "A man named Jakarn was thrown into the Grave, a prison below <<1>>. Apparently the headman of the town accused him of stealing a valuable gem.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4455 => array(
			'name'    => "Trade Negotiations",
			'journal' => "Merchants from House Hlaalu are worried about their leader, Raston Vendil. He went to negotiate a trade agreement with the isolated Mabrigash tribe in the Vale of the Ghost Snake. He never returned.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4457 => array(
			'name'    => "Box of Riddles",
			'journal' => "<<1>> wants my help to prank her Subtle Knife rival, <<2>>, who has won their trickster duels for the last four years.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6839 => array(
			'name'    => "The Island Tour",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4458 => array(
			'name'    => "The Drublog of Dra'bul",
			'journal' => "<<1>> believes the Wood Orc clan of Dra'bul, the Drublog, would not have attacked Velyn Harbor under normal circumstances.\n\nShe needs to take some time to visit with local leaders in Velyn Harbor before going to Dra'bul.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6324 => array(
			'name'    => "Bright Moons, Warm Sands",
			'journal' => "I awoke in Star Haven Adeptorium, home to an order of peaceful Khajiit adepts. It seems that I was asleep for many days, recovering from a devastating Dragon attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4459 => array(
			'name'    => "The Mournhold Underground",
			'journal' => "I used the key to search the House Dres farmhouses outside Mournhold. I found a suspicious-looking trap door hidden under a rug. Naryu thinks we'll discover a connection to the Maulborn down there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4460 => array(
			'name'    => "Grim Situation",
			'journal' => "The Mane has released Dro-m'athra spirits on Fort Grimwatch! Twins <<1>> and <<2>>, Khajiit Champions, lead the fight against the creatures.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6318 => array(
			'name'    => "Dark Souls, Mighty Weapons",
			'journal' => "From ancient Khajiit legends, Zahari has learned that weapons imbued with enchantments powered by Daedric souls are particularly powerful against Dragons. I need to collect these souls to help fight back against the Dragon threat.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5664 => array(
			'name'    => "The Sweetroll Killer",
			'journal' => "A flier indicates that an Imperial named Reman Mevavius seeks someone for an urgent and dangerous job in the Gold Coast.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4461 => array(
			'name'    => "Grimmer Still",
			'journal' => "I've worked with the Lunar Champion Shazah to seal up corruption wells inside of Fort Grimwatch. Now I've met up with her sister Khali to explore an unearthed Khajiiti temple.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4464 => array(
			'name'    => "Nature's Best Friend",
			'journal' => "After eating too many moon-sugar candies, <<1>>'s Khajiit husband, <<2>>, ran screaming into the forest.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6187 => array(
			'name'    => "Pledge: Moon Hunter Keep",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Moon Hunter Keep.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4468 => array(
			'name'    => "Unearthing the Past",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5672 => array(
			'name'    => "Contract: Grahtwood",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4469 => array(
			'name'    => "Fires of Battle",
			'journal' => "I have entered the Blessed Crucible to test my strength and take the Brimstone Crown. Talres Voren has financially backed my bid to become champion with the promise of riches and rewards.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6698 => array(
			'name'    => "Reformatory Rescue",
			'journal' => "A Khajiit named Tufeh asked me to enter the Brandfire Reformatory, a Waking Flame cult stronghold. Her sister, Pibiha, was taken inside by cultists and Tufeh doesn't know what happened to Pibiha.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4471 => array(
			'name'    => "Izad's Treasure",
			'journal' => "I dug up a chest by the shipwreck near Saintsport. It contained a shovel and directions written by a Captain Izad. It supposedly contains clues to a treasure buried on Stros M'Kai.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6207 => array(
			'name'    => "Capture All 3 Towns",
			'journal' => "I accepted a mission to help capture the towns of Vlastarus, Cropsford, and Bruma for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5701 => array(
			'name'    => "Contract: Grahtwood Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Grahtwood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4472 => array(
			'name'    => "Rat in a Trap",
			'journal' => "<<1>> got locked in a cage just before the Goblin invasion of Cumberland's Watch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4473 => array(
			'name'    => "Revenge Against Rama",
			'journal' => "<<1>> has asked me to track down Rama and make him pay for his crimes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6727 => array(
			'name'    => "Salvaged Plans",
			'journal' => "A member of the Grasp named Zyr asked me to help track down a group of thieves who stole a large quantity of Redmaiden from Fargrave. Apparently, they fled to the Sever after leaving Fargrave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4474 => array(
			'name'    => "Daughter of Giants",
			'journal' => "The Prophet has contacted me across great distance for some important purpose.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6212 => array(
			'name'    => "Capture All 3 Towns",
			'journal' => "I accepted a mission to help capture the towns of Vlastarus, Cropsford, and Bruma for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4475 => array(
			'name'    => "Shock to the System",
			'journal' => "While exploring along the river, I found an Argonian enveloped in lightning. I found a journal nearby his body. It contained instructions on how to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6334 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5680 => array(
			'name'    => "Contract: Reaper's March",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4477 => array(
			'name'    => "A Wedding to Attend",
			'journal' => "The Silvenar assures me he will recover from his imprisonment in Jathsogur and I should not wait for him. He wants me to meet him in the city of Silvenar where he will marry the Green Lady.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4483 => array(
			'name'    => "Ezzag's Bandits",
			'journal' => "Word around town is that the farmer Ezzag is having trouble in his fields.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6331 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4484 => array(
			'name'    => "Haunting of Kalari",
			'journal' => "The rumor around town is that Kalari's new house is haunted.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5816 => array(
			'name'    => "Madness in Auridon",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Auridon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4485 => array(
			'name'    => "Loose Ends",
			'journal' => "I helped the people of S'ren-ja with their many and varied problems.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4492 => array(
			'name'    => "Desecrated Ground",
			'journal' => "In the wake of a Dro-m'Athra attack, a lunar priest has asked me to save a priceless relic from the vault behind Moonmont.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6350 => array(
			'name'    => "Pledge: Moongrave Fane",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Moongrave Fane.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5696 => array(
			'name'    => "Contract: Bangkorai Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Bangkorai.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4493 => array(
			'name'    => "A Fair Warning",
			'journal' => "A note posted at the entrance to Knife Ear Grotto by a scholar warns of danger inside.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6221 => array(
			'name'    => "Morrowind Gate Crashing",
			'journal' => "I accepted a mission to help capture Farragut Keep, Arrius Keep, and Kingscrest Keep for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4494 => array(
			'name'    => "You Have to Break a Few",
			'journal' => "I discovered the body of a Dark Elf in a cave. There's a note next to the body.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6742 => array(
			'name'    => "Wright of Passage",
			'journal' => "I encountered Za'ji, a Khajiiti sea captain, who claims to be searching Shipwright's Regret for a treasure of some kind. He's requested my help with his search.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4499 => array(
			'name'    => "Baan Dar's Bash",
			'journal' => "After our tricks at Thormar, Aniel has ushered me into Baan Dar's realm: The Five Finger Dance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6219 => array(
			'name'    => "High Rock Gate Crashing",
			'journal' => "I accepted a mission to help capture Fort Glademist, Fort Rayles, and Fort Warden for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4500 => array(
			'name'    => "A Nord in Need",
			'journal' => "While exploring <<1>> I came across a battered Nord shield.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5704 => array(
			'name'    => "Contract: Malabal Tor Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Malabal Tor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4501 => array(
			'name'    => "Gates of Fire",
			'journal' => "I've come upon an active battlefield, as Colovian forces attempt to take an Ayleid ruin from Dominion soldiers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6730 => array(
			'name'    => "Courier's Folly",
			'journal' => "I found the body of a Spiderkith, Telofasa. It looks like she died while attempting to deliver something.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4503 => array(
			'name'    => "Brothers and Bandits",
			'journal' => "<<1>> and his brother, <<2>>, came to the ruins in search of a plant which grows in the light of an Ayleid well. Its sap is rumored to cure any malady.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4505 => array(
			'name'    => "Seeds of Hope",
			'journal' => "The Bloody Hand Goblins have stolen Cropsford's seeds, needed for next season's crops.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6887 => array(
			'name'    => "Helpful Handbills",
			'journal' => "Juline Courcelles asked me to travel to the Sea Elf town of Llanshara and post several handbills. She hopes the notices will attract Sea Elf tour guides to her business venture.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4506 => array(
			'name'    => "Tarnish the Crown",
			'journal' => "<<1>> and his Goblins killed Gugnir's brothers and left him for dead. He asked me to avenge his brothers by killing the Goblin king.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4508 => array(
			'name'    => "Washed Ashore",
			'journal' => "A drunk named Shazeem seems to have lost his memory. He offered to sell me a trained monkey, claiming it can find buried treasure.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6237 => array(
			'name'    => "Grave Mementos",
			'journal' => "Bolu of the Dead-Water tribe, which calls itself the Naga-Kur, wants me to go to Bok-Xul and accomplish the task he set before me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4510 => array(
			'name'    => "The Spearhead's Captain",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6360 => array(
			'name'    => "Rifling Through Ruins",
			'journal' => "The Baandari job broker, Nisuzi, tasked me with finding enchanted charms and talismans in the forbidding Abode of Ignominy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4511 => array(
			'name'    => "Dead Man's Wrist",
			'journal' => "In a cave full of plunder, I found a dead man clutching a note to his sister. Though the bracelet accompanying the note has little monetary value, his sister may be glad to have it back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4512 => array(
			'name'    => "The Dead of Culotte",
			'journal' => "Ufgra gra-Gum says that undead creatures infest the ruins of Culotte to the southwest of Cropsford.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6758 => array(
			'name'    => "A Sheep in Need",
			'journal' => "Moric Guidroz believes that a group of Hadolids stole his prized sheep, Privette. They carried her into their lair at Breakwater Cave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4515 => array(
			'name'    => "The Ties that Bind",
			'journal' => "Fists of Thalmor were attacked by strange creatures in <<1>>. While many were slain, there may be survivors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4519 => array(
			'name'    => "Word from the Dead",
			'journal' => "A mortally wounded man in <<1>> asked me to take word of his death to his wife.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6255 => array(
			'name'    => "Anti-Venom Agitation",
			'journal' => "Jee-Lar informed me that a Wood Elf named Mindileth knows the recipe for a particularly potent anti-venom. This brew may prove vital for the company's explorers in Murkmire. Apparently, the ingredients-and Mindileth-can be found in Grahtwood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5877 => array(
			'name'    => "An Armiger's Duty",
			'journal' => "Captain Naros, a Buoyant Armiger, is investigating a rash of disappearances near Molag Mar. I agreed to assist her in getting to the root of the cause.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4520 => array(
			'name'    => "Bloody Hand Spies!",
			'journal' => "Bloody Hand skirmishers are lurking on the outskirts of town. Ufgra is very worried about them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4523 => array(
			'name'    => "The Bloodthorn Plot",
			'journal' => "The Bloodthorn Cult recently attacked Stonetooth Fortress. The Orcs repelled the attack but something was stolen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4524 => array(
			'name'    => "Repentant Son",
			'journal' => "I found the journal of a young man named Vareldur in the mine called Del's Claim. He died trying to stop the Heritance from killing the miners there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5885 => array(
			'name'    => "Ancestral Ties",
			'journal' => "A Dark Elf named Drelyth Hleran requested my aid in finding his family's lost grave site. He believes the Ashlanders camped near Ald'ruhn might know more about the lost tomb, but the tribes only share their secrets with those who earn their respect.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4528 => array(
			'name'    => "Garments by Odei",
			'journal' => "A tailor tried to take a shortcut through Cambray Pass and was attacked by a pack of wild wolves. He lost the tools of his trade as he fled. He asked me to retrieve his possessions and meet him at the northern end of the pass.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4533 => array(
			'name'    => "Fortune in Failure",
			'journal' => "I found a message within Cryptwatch Fort indicating that the Red Rooks are worried the Bloodthorn cult will turn against them. The note also contains recipes for salves and potions to combat the Bloodthorns' festering magic.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6271 => array(
			'name'    => "Ritual of Change",
			'journal' => "Though I've gathered all the tablets, the Xinchei-Konu remains powerless. Jekka-Wass Vozei wishes to perform a ritual to help the monument regain its magical properties, but needs the help of his mentor Nisswo Paraxeeh to do so.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5765 => array(
			'name'    => "Taken Alive",
			'journal' => "Iron Orcs, who usually don't take prisoners, have started abducting travelers near the Valley of Scars. I encountered a caravan master who survived an attack, though a number of his guards were dragged off by the Orcs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4536 => array(
			'name'    => "The Golden Claw",
			'journal' => "The lunar priests at the Do'Krin Monastery have been attacked by the Stonefire cult. The survivors fear the cultists mean to steal a sacred relic: The Golden Claw.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4537 => array(
			'name'    => "An Offering",
			'journal' => "I found the body of a Khajiiti woman on the terrace of Do'Krin Monastery. She was still clutching a letter and a small offering to the Bright Moons.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6791 => array(
			'name'    => "The Princess Detective",
			'journal' => "Isobel received another letter from Knight Commander Jourvel, wanting to chat. My companion believes it has to do with Aurelia and her pursuit of that mysterious conspiracy in Rivenspire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4538 => array(
			'name'    => "Eye of the Storm",
			'journal' => "Battlereeve Alduril would like me to assist her in her assault on Tempest Island.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6398 => array(
			'name'    => "The Horn of Ja'darri",
			'journal' => "Sai Sahan and I discovered the location of the Horn of Ja'darri, a powerful Dragonhorn made by the Dragonguard. It awaits us in a Dragon's lair, nestled within a dormant volcano in Southern Elsweyr.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5744 => array(
			'name'    => "Mascot Theft in Reaper's March",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Claw's Strike, located in Reaper's March.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4541 => array(
			'name'    => "Soul Survivors",
			'journal' => "Three villages were absorbed into a plane of Oblivion, in an attempt to bring them into Coldharbour: one Dunmer, one Khajiit, and one Orc. The Dunmer <<1>> has a plan to help herself and the other village leaders escape.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4546 => array(
			'name'    => "Retaking the Pass",
			'journal' => "An undead army surges through Driladan Pass and assaults Marbruk on the command of the Shade of Prince Naemon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5880 => array(
			'name'    => "Divine Inquiries",
			'journal' => "Lord Vivec fears that the mysterious affliction that drains his divine energy may be the result of an enemy's attack. He asked me to work with Archcanon Tarvus to determine the source of the attack by conducting investigations throughout Vvardenfell.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4549 => array(
			'name'    => "Back to Skywatch",
			'journal' => "<<1>> is under siege, but the commander at the gate doesn't seem worried.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4550 => array(
			'name'    => "The Fires of Dune",
			'journal' => "I've arrived at the city of Dune to find it under siege by Daedra and the forces of Javad Tharn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6287 => array(
			'name'    => "Mushrooms That Nourish",
			'journal' => "The farmers of Root-Whisper Village are in need of a rare mushroom that grows in Blight Bog Sump. I'll need to make my way past the dangerous creatures that lurk there in order to gather this plant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4552 => array(
			'name'    => "Chasing Shadows",
			'journal' => "The Prophet and I need to find a way to rescue Sai Sahan, the Redguard who hid the Amulet of Kings.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4553 => array(
			'name'    => "Threefold Folly",
			'journal' => "<<1>> lost his lute in a riddle game with <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4556 => array(
			'name'    => "Strength of the Father",
			'journal' => "In the depths of <<1>> I came across the corpse of <<2>>, cooked in his armor by a beast that dwells there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4558 => array(
			'name'    => "Taking the Fight to the Enemy",
			'journal' => "Darien mentioned that a small group of Lion Guard soldiers went to search for Angof in the north. I should head in that direction and attempt to find them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4560 => array(
			'name'    => "Riches Beyond Measure",
			'journal' => "<<1>>, a Khajiit investigating <<2>>, has asked for my assistance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6806 => array(
			'name'    => "The Tournament Begins",
			'journal' => "The Tales of Tribute tournament begins now! If I succeed in defeating all of my opponents, I will advance to the second round, bringing me closer to the title match.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4563 => array(
			'name'    => "Capstone Caps",
			'journal' => "<<1>> needs special mushrooms found only in <<2>> to help the wounded.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4564 => array(
			'name'    => "Lost and Alone",
			'journal' => "Many of Bruma's citizens are lost or missing in the ruins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5768 => array(
			'name'    => "Slithering Brood",
			'journal' => "A woman claiming to be the leader of the Scaled Court requested my help. This Regent Cassipia wants me to help her stop the Scaled Court's process for creating a new type of creature for the Serpent's growing army-the mantikora.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4565 => array(
			'name'    => "Do as I Say",
			'journal' => "I've found a scared bandit trying to hide in Portdun Watch. He wants me to take a token to his brother, though he was too frightened to tell me much more.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6303 => array(
			'name'    => "In Sickness and In Health",
			'journal' => "The adeptorium, S'rendarr's Cradle, suffers under a terrible drought. Unfortunately, something or someone halted the flow of water from the Anequina Aqueduct. Without the aqueduct, the adepts of S'rendarr's Cradle will not last long.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4568 => array(
			'name'    => "The Standing Stones",
			'journal' => "<<1>> needs his restoration scrolls charged.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4570 => array(
			'name'    => "Know thy Enemy",
			'journal' => "<<1>> challenged me to learn more about the enemy so I can appreciate the dangers the folk of Bruma face.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6301 => array(
			'name'    => "Preserving the Prowl",
			'journal' => "The Prowl recently suffered a devastating Dragon attack. In the aftermath, violent poachers seized control of the area. If left unchecked, these reckless hunters could ruin this hunting ground for generations.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4574 => array(
			'name'    => "Veil of Illusion",
			'journal' => "After the death of Prince Naemon, Vicereeve Pelidil assumed leadership of the Veiled Heritance and stole the Staff of Magnus. Razum-dar has tracked him to somewhere in the area of Woodhearth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6822 => array(
			'name'    => "A Special Reagent",
			'journal' => "An alchemist requested brine glands from molting Hadolids. There's a Hadolid molting ground on Amenos, guarded by a fierce Hadolid matron. If I want to get brine glands, I'll have to deal with the matron.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4579 => array(
			'name'    => "Bring Down the Magister",
			'journal' => "<<1>> believes a Shadowed Path Magister is leading Bruma's intruders.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6299 => array(
			'name'    => "The Demon Weapon",
			'journal' => "Imperial battlemage and former Grand Chancellor of the Empire, Abnur Tharn, requested my assistance with an important matter that could lead to the end of the Three Banners War.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4580 => array(
			'name'    => "Double Jeopardy",
			'journal' => "Evidence suggests that Vicereeve Pelidil and the Veiled Heritance are planning to attack the city of Woodhearth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5784 => array(
			'name'    => "Dark Anchors in Stormhaven",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Stormhaven.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4581 => array(
			'name'    => "The Unseen",
			'journal' => "<<1>> asked me to search the manor for hidden Dremora portals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4586 => array(
			'name'    => "The Witch of Silatar",
			'journal' => "The Veiled Heritants plan to harm the Wilderking. If I enter Aranias' past through Maruin's story, I can change her future.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4588 => array(
			'name'    => "Land Dispute",
			'journal' => "Thulvald's logging camp has been overrun by spriggans and trolls. He came to check on his workers and was chased off by the hostile creatures. Now he's worried about his workers and expects that his business is ruined.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5792 => array(
			'name'    => "Dark Anchors in the Rift",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in the Rift.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4589 => array(
			'name'    => "Jumping Ship",
			'journal' => "Shifty Tom, one of Captain Blackheart's underlings, wants to give up the pirate's life. He plans to leave Blackheart Haven, the Captain's island fortress, but wants me to help him steal two magical relics from the Captain, first.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6317 => array(
			'name'    => "Moonstruck in Manacles",
			'journal' => "Mazdurr the Moonstruck, a Khajiit dedicated to the Daedric Prince Azurah, is trapped by bandits in the caverns of the Tangle. I agreed to help him escape.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4590 => array(
			'name'    => "The Skin-Stealer's Lair",
			'journal' => "<<1>> was replaced by a Dominion skin-stealer. I need to find the imposter and stop the remaining skin-stealers in <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4593 => array(
			'name'    => "Audience with the Wilderking",
			'journal' => "To learn why the Wood Elves in this part of the Valenwood are opposed to the Dominion and to convince them to change their mind, I must negotiate with the Wilderking.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6315 => array(
			'name'    => "Jode's Core",
			'journal' => "The moons have begun to align. When the eclipse occurs, the Moon Gate of Anequina will open and grant access to Jode's Core. We need to try to prevent the Dragons from accessing the portal and attaining the power stored there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4596 => array(
			'name'    => "Handmade Guardian",
			'journal' => "A Wood Elf named Benduin wants to show me how to build a guardian with special powers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5800 => array(
			'name'    => "Cursed Baubles of Stormhaven",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Norvulk Ruins, located in Stormhaven.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4597 => array(
			'name'    => "The Plan",
			'journal' => "The Ancient Keepers, sworn to imprison the spirit of High Kinlord Rilis, have taken to forming Daedric pacts. Keeper Cirion has asked for help in defeating the corrupting force of Rilis, but will require fighting the Keepers directly.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6826 => array(
			'name'    => "Druidic Research",
			'journal' => "A mage studying the practices of druidic magic needs someone to collect ritual components from Coral Cliffs on Amenos to help advance their research.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4599 => array(
			'name'    => "On the Doorstep",
			'journal' => "As I arrived at the town of Greenhill, a very drunk man ran up to me and began raving about evil strangers in town.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6855 => array(
			'name'    => "Seeds of Destruction",
			'journal' => "The Dreadsails and their Firesong allies are preparing to attack the city of Vastyr. Sir Stefan has a plan to bolster the city's defense by securing the help of the other two circles of druids, the Stonelore and the Eldertide.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4602 => array(
			'name'    => "Light from the Darkness",
			'journal' => "This tower is a lightless oubliette, a prison of darkness that binds King Laloriaran Dynar, the last Ayleid King.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7310 => array(
			'name'    => "Justice for the Fallen",
			'journal' => "The Worm Cult has returned to launch attacks on the Fighters and Mages Guilds. Guildmaster Merric died in the last assault. Now Prince Azah seeks to rally allies, destroy their Worm Nest, and put an end to the Worm Cult. Again.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4608 => array(
			'name'    => "The Blight of the Bosmer",
			'journal' => "Most of the Bosmer entering Verrant Morass are being corrupted by a blight that turns them into feral beasts. Indaenir believes the corruption comes from the Shade of Naemon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5453 => array(
			'name'    => "A Khajiit's Tale",
			'journal' => "I met a Khajiit named Zhasim who was raised as part of an Orc clan. An old pack within the ancient ruins inside Bonerock Cavern contained a Khajiit medallion. Could it be a clue to Zhasim's origin?",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6990 => array(
			'name'    => "Tracing Shadows",
			'journal' => "I learned of a spy inside Ald Isra's Telvanni council from a Dark Elf named Folsi. The spy's identity needs to be uncovered before the leak can get plugged.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4621 => array(
			'name'    => "The Tempest Unleashed",
			'journal' => "The Maormer seek to prevent the Dominion from establishing themselves on <<1>>. They've launched a full-scale attack on <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4622 => array(
			'name'    => "The Search is Over",
			'journal' => "I met a man named Armin in the bottom of Divad's Chagrin mine. After seven years searching, he found his ancestors remains and wants to send word to his wife that he will soon return home.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7317 => array(
			'name'    => "The Gates of Mor Naril",
			'journal' => "The Stirk Fellowship prepares for the final confrontation with Mannimarco and the Worm Cult, but deadly soul flayers protect Mor Naril from assault. Prince Azah needs me to find a way to overcome the defenses of the Worm Cult's stronghold.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6984 => array(
			'name'    => "Personal Effects",
			'journal' => "I met an injured courier in the Telvanni Peninsula. He's asked me to deliver his parcel urgently to Solni Nrayn in Necrom.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4623 => array(
			'name'    => "The Soul-Meld Mage",
			'journal' => "I met Gadris the Dark Elf and Zur the Khajiit. These two mages share a single body, as their souls were melded at this Vile Laboratory when they arrived in Coldharbour. This soul-meld has weakened them. If they remain combined, they will surely die.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7326 => array(
			'name'    => "A Death-Dancer's Duty",
			'journal' => "Death-Dancer Teerad of the Tide-Born Argonians needs someone to locate the missing tidal votives of the three warriors who recently were killed, so he can perform a funerary ritual. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4624 => array(
			'name'    => "The Perils of Diplomacy",
			'journal' => "The Aldmeri Dominion seeks to secure the loyalty of <<1>>, but an old treaty with the Maormer prevents this.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5469 => array(
			'name'    => "Blood Price",
			'journal' => "<<1>>, the hearth-wife to <<2>>, believes he was murdered. She suspects <<3>>, <<2>>'s hunt-wife, conspired to kill him. <<1>> is convinced <<3>> secretly worships Boethiah, the Daedric Prince of Treachery.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6994 => array(
			'name'    => "Escape of the Sane",
			'journal' => "Dreamcarvers invaded the Feral Gallery and killed all its caretakers. Morian Zenas, a man of questionable sanity, asked me to help him escape the magical barrier keeping him inside the gallery.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4625 => array(
			'name'    => "Tears of the Two Moons",
			'journal' => "A powerful mage named <<1>> went missing in the Temple of the Mourning Springs. I agreed to help his apprentice <<2>> search the temple for signs of her master.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4626 => array(
			'name'    => "Vanus Unleashed",
			'journal' => "I came across a magical recording of Vanus Galerion. The message says that the mage decided to enter the Black Forge on his own. He plans to reach the Great Shackle and destroy it, thus completing our mission in Coldharbour.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4635 => array(
			'name'    => "Heirloom",
			'journal' => "I found a broken cart with a list of items that were supposed to be delivered to Lorene Menillet of Cropsford.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4638 => array(
			'name'    => "The Real Snake",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7333 => array(
			'name'    => "Blood on the Water",
			'journal' => "Suraj, a member of Captain One-Eye's crew, begged for my help in stopping Velehk Sain, a pirate captain pursuing their ship out of Coldharbour.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7000 => array(
			'name'    => "Call of the High Ordinator",
			'journal' => "I found a notice that says the Tribunal seeks adventurers for dangerous work. Apparently, Ordinators in Necrom will know more.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4639 => array(
			'name'    => "Better Late Than Never",
			'journal' => "I found a bundle of damaged gifts meant for <<1>> who lives in <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7342 => array(
			'name'    => "Pact Elder Tallow",
			'journal' => "<<1>> asked me to collect elder tallow from the rarest, toughest trolls, kagouti, guars, and such beasts in the territories of the Ebonheart Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4640 => array(
			'name'    => "Sands of Sentinel",
			'journal' => "Rasha has asked me to deliver a small pouch of sand to an old friend.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5485 => array(
			'name'    => "Those Truly Favored",
			'journal' => "<<1>>, one of the Malacath faithful, learned Winterborn raiders seek the <<2>>, an old relic of Malacath. She wants me to recover it before they can.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4641 => array(
			'name'    => "What Was Lost",
			'journal' => "I met a mage named <<1>> in Darkshade Caverns. She has asked for help investigating the Dwemer ruins beneath the mines.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4642 => array(
			'name'    => "Hall of Judgment",
			'journal' => "Some of the mages who helped open the initial portal to Coldharbour were captured when they arrived. Now they're prisoners in the Daedric Court of Contempt, awaiting the verdict of Judge Xiven in the Hall of Judgment.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4647 => array(
			'name'    => "A Foot in the Door",
			'journal' => "Thizzrini Arena is host to numerous arena games in a sprawling complex outside of Dune. To compete in the games, I must speak with the three Division Champions and earn the proper credentials.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6519 => array(
			'name'    => "Mother of Shadows",
			'journal' => "A local scholar made the unpleasant discovery that a terrible creature of legend known as the Shademother is very much real and currently working unholy magic around Morthal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4650 => array(
			'name'    => "The Swordmaster Division",
			'journal' => "I am competing in the Swordmaster Division of Thizzrini Arena. I must rise through the ranks of contenders before I can battle Zara, the Swordmaster Division Champion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7349 => array(
			'name'    => "Axes for the Assault",
			'journal' => "Armorer Reistaff asked me to craft three axes of any kind for the Stirk Fellowship to break through the Worm Cult's defenses. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7016 => array(
			'name'    => "Castoff Destinies",
			'journal' => "Ogle, a Watchling in the service of Hermaeus Mora, mistakenly sent a glyphic into the Underweave to be destroyed. Fearful of the dangers within, Ogle asked me to help him retrieve the glyphic from the Underweave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4655 => array(
			'name'    => "Hadran's Fall",
			'journal' => "<<1>>, the slave dealer who runs <<2>>, sent his thugs to kill me and failed. His business partner, <<3>>, has grown tired of his slave-trading operation and wants me to eliminate him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7358 => array(
			'name'    => "First Aid Provider",
			'journal' => "Armorer Reistaff asked me to craft three health potions of any kind for injured soldiers in the Stirk Fellowship. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4656 => array(
			'name'    => "Tharayya's Trail",
			'journal' => "I've found a note from a Dwemer researcher's journal named Tharayya. She appears to have been searching for an ancient artifact referred to as the \"Guardian's Eye.\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5501 => array(
			'name'    => "Watch Your Step",
			'journal' => "<<1>> has taken to hunting Daedra for sport and profit. She's asked for my help in exchange for a cut of the earnings.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4658 => array(
			'name'    => "Misplaced Knowledge",
			'journal' => "I found a notebook full of observations about the Ayleid wells around Cyrodiil. The book's owner may want it back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4659 => array(
			'name'    => "Lady Laurent's Favor",
			'journal' => "Lady Laurent is displeased with her butler, Stibbons, for leading the Elves to Salas En. To get back in her good favor, he's asked me to help him by gathering Laurent's tools from among the ruins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4660 => array(
			'name'    => "The Devils You Know",
			'journal' => "A suspicious woman asked me to relay a message to <<1>> in Bruma.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4664 => array(
			'name'    => "Moon-Sugar Medicament",
			'journal' => "I found a note in a smuggler camp. A young Khajiit was captured and taken to <<1>> by the <<m:2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5381 => array(
			'name'    => "Pledge: City of Ash II",
			'journal' => "Glirion the Redbeard has challenged me to explore beyond the portal in Veteran City of Ash in Greenshade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6407 => array(
			'name'    => "Masterpieces",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4666 => array(
			'name'    => "The Silver Flute",
			'journal' => "I found a small silver flute in a crate. An inscription on the flute reads, \"Halion of Chorrol.\" There might be a reward for the flute's return.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4668 => array(
			'name'    => "Lizard Racing",
			'journal' => "I'm now participating in Lizard Racing, where I place bets on which lizard finishes the race first.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7038 => array(
			'name'    => "Manifesting Hope",
			'journal' => "Fathoms Drift is a ships' graveyard in Hermaeus Mora's domain. Ordinator Tilena asked me to locate ship manifests there to help the Ordinators catalog which ships lie broken upon the Apocryphal shores.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4669 => array(
			'name'    => "Spikeball",
			'journal' => "I'm now participating in Spikeball, a foot race where I can throw a Spikeball to stun opponents.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5575 => array(
			'name'    => "Heist: Glittering Grotto",
			'journal' => "I was tipped off about rare gems stashed in the Glittering Grotto. I don't have much time to secure the goods before they're moved, so I should act quickly on this lead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4670 => array(
			'name'    => "Troll Arena",
			'journal' => "I'm now betting on the Troll Arena, where I place bets on which Troll wins the fight.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7237 => array(
			'name'    => "Upstart Emperor",
			'journal' => "Orpheon the Tactician, a former emperor of Cyrodiil, believes that the Elder Scroll housed on Lep Seclusa holds a prophecy that will legitimize his claim to the throne. His bloody campaign to take the scroll by force must be stopped.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7032 => array(
			'name'    => "Approaching a Nightmare",
			'journal' => "Researcher An'Xeh of the Mages Guild asked me for help. One of Vaermina's Daedric followers could be summoned into this realm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4671 => array(
			'name'    => "Distant Relatives",
			'journal' => "<<1>> at the <<2>> asked me to deliver a letter to his cousin Asvorn in <<3>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7374 => array(
			'name'    => "Choice and Consequence",
			'journal' => "I met Cariel, an Eye of the Queen. Cariel knows that the Worm Cult is working with Prince Naemon. She wants to find out what they're up to and stop them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4672 => array(
			'name'    => "Morwha's Curse",
			'journal' => "The chief of the Ash'abah and many of the tribe's warriors have been poisoned, most likely by the Withered Hand. I agreed to help Talia at-Marimah gather ingredients for an antidote.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7042 => array(
			'name'    => "An Ancient Omniscience",
			'journal' => "I agreed to defeat the Prime Cataloger and collect molted skins from the tomeshells that follow it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4673 => array(
			'name'    => "The Root of the Problem",
			'journal' => "There's a crisis at <<1>>. Something is wrong with their Great Tree, and the locals believe it has something to do with the tree's roots, which can be accessed from a cave below it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4674 => array(
			'name'    => "Spoils of War",
			'journal' => "Someone named Jena is being threatened. She apparently lives in Cheydinhal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4678 => array(
			'name'    => "Catch of the Day",
			'journal' => "I met <<1>> fishing in the cold waters of northern Cyrodiil. She asked if I could bring some of her catch to her friend <<2>> in <<3>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7436 => array(
			'name'    => "A Taste of the Finer Things",
			'journal' => "Sheogorath departed after the incident with the cheese thieves, frustrated and tired. Meanwhile, Noth has been tracking the tomeshell and the stolen power. She says Sheogorath's power leaves behind a taint of madness as it travels.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4681 => array(
			'name'    => "The Serpent Lord",
			'journal' => "Sea Elves have entered Serpent's Grotto, and one of the Maormer mages has broken the seals that bound a massive sea serpent here for centuries.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7060 => array(
			'name'    => "The Telvanni Secret",
			'journal' => "I came across Master Faras, a Telvanni mage, and his daughters. Master Faras claims to have an ancient tome of secrets in his possession. He wants me to witness this moment of triumph.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4683 => array(
			'name'    => "Medicinal Herbs",
			'journal' => "<<1>> needs herbs delivered to <<2>> in Bruma.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4686 => array(
			'name'    => "The Initiation",
			'journal' => "On his deathbed, the chief of the Ash'abah says he has foreseen that I must join the tribe and defeat the Withered Hand. I must participate in an initiation ritual to become an Ash'abah.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7381 => array(
			'name'    => "Blood on the Sands",
			'journal' => "A Khajiit named Dorfifar hired me to kill as many creatures in the Night Market as I can. He doesn't seem to care which enemies, just that I avenge his missing eye.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7048 => array(
			'name'    => "Numinous Grimoire, Volume 1",
			'journal' => "I found the body of a mortal scribe in Apocrypha. A note and a pack were lying on the ground beside the dead cipher.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4687 => array(
			'name'    => "A Traitor's Luck",
			'journal' => "The Eye of the Queen, Cariel, wants my help to uncover corruption in the little town of Pa'alat. She suspects a former Eye of the Queen has gone rogue.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4690 => array(
			'name'    => "Striking at the Heart",
			'journal' => "The Shade of Prince Naemon has seized control of Hectahame where he is corrupting the Heart of the Valenwood. I must assist Queen Ayrenn and Indaenir in defeating him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7076 => array(
			'name'    => "Ithelia's Fury",
			'journal' => "Hermaeus Mora senses instabilities in reality and I have once again been called upon to serve as the chosen of fate. I need to investigate these instabilities and determine if Prince Ithelia is responsible before the situation gets worse.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4699 => array(
			'name'    => "The Fading Tree",
			'journal' => "A merchant in the Shrouded Vale reports seeing a village that has mysteriously and suddenly appeared in a nearby cave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4701 => array(
			'name'    => "Crossing the Chasm",
			'journal' => "The army has been gathered and the time has come to take the battle to Molag Bal! But to reach the planar vortex at the heart of the Dark Lord's inner sanctum, we have to cross the well-defended Chasm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5392 => array(
			'name'    => "Blacksmith Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Blacksmith goods to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7406 => array(
			'name'    => "Keen of Mind",
			'journal' => "My faction leader hired me to complete an oddity in the Night Market.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4704 => array(
			'name'    => "Welcome to Cyrodiil",
			'journal' => "I've entered Cyrodiil, eager to help the Dominion win the war.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5549 => array(
			'name'    => "Forever Hold Your Peace",
			'journal' => "The merchant lord <<1>> claims to have found <<2>>'s dowry, and for his trouble she has agreed to marry him. <<3>> suspects <<4>>, the treacherous former guildmaster of the Thieves Guild, is behind it all. She wants him to pay for his betrayal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7074 => array(
			'name'    => "King Nantharion's Gambit",
			'journal' => "Beragon believes King Nantharion is using the Staff of Many Paths to prepare a new wildburn that will destroy Skingrad and cover the entire West Weald in forest. He thinks we can use the Skein of Secrets to find Nantharion, then we can stop him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4705 => array(
			'name'    => "Siege Warfare",
			'journal' => "I've reached the siege range. Zan advised me to try each weapon. I should also talk to Firamo and Miniel to learn more about the weapons.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4709 => array(
			'name'    => "The Path to Moonmont",
			'journal' => "Lunar Priestess Kauzanabi-jo has indicated I should make for the temple of Moonmont. The Champions, the new candidates for the Mane, will join me there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5400 => array(
			'name'    => "Enchanter Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Enchanter glyphs to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6563 => array(
			'name'    => "After the Storm",
			'journal' => "A strange and powerful storm disrupted the protective wards around the village of Karthwasten, turning a portion of its people feral. Apprentice Fialyn needs help restoring the wards.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4710 => array(
			'name'    => "Hallowed To Arenthia",
			'journal' => "The priestess Kauzanabi-jo told me my next goal should be to help the people of Arenthia. Apparently Mane Akkhuz-ri is somewhere amid the Colovian invaders of the city.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4711 => array(
			'name'    => "To Dune",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4712 => array(
			'name'    => "The First Step",
			'journal' => "I traveled to the city of Rawl'kha, where I'm to help the Lunar Champions begin their spiritual journey.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7080 => array(
			'name'    => "The Voice of the Village",
			'journal' => "I met two young Wood Elves near Vashabar, Laeni and Clendagal. They want to marry, but their fathers are unwilling to give permission. They asked for my help to leave town and start a new life.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4719 => array(
			'name'    => "The Moonlit Path",
			'journal' => "I saved Dune from the plots of Javad Tharn. Now I must aid the Lunar Champion as she walks the Two Moons Path, a mystical Khajiit ritual used to anoint a new Mane.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6442 => array(
			'name'    => "Little Lost Cat",
			'journal' => "I found a note next to a cat named Tender Claw. I should return him to his owner in Senchal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4727 => array(
			'name'    => "Reporting for Duty",
			'journal' => "Aurorelle Varin says there is much more for me to learn about the war in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6980 => array(
			'name'    => "Madness in the Wind",
			'journal' => "The scribes of Quires Wind, mortal and Daedra alike, have gone mad. Cipher Agrushna believes corrupt scrolls hidden among the stacks are responsible. I agreed to enter Quires Wind and destroy the scrolls for her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4731 => array(
			'name'    => "Malignant Militia",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4732 => array(
			'name'    => "To Honor the Fallen",
			'journal' => "I've been asked by a moon priestess named Felari to help with a project that could help firm up relations between the Khajiit and Wood Elves. They are working to return home relics and bones from ancient battle sites.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7102 => array(
			'name'    => "Restoring the Resurrected",
			'journal' => "Destozuno is close to being able to restore the book, Fynboar the Resurrected. She just needs some bookbinding supplies in order to turn the sheaves I brought her into a book.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4733 => array(
			'name'    => "Knowledge Gained",
			'journal' => "The Mages Guild has launched an expedition into Selene's Web to seek out and study an ancient forest spirit rumored to live here.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7301 => array(
			'name'    => "Ghosthunters",
			'journal' => "I met Oorbesh, a ghost-hunting Orc, outside the very haunted Corelanya Manor. She agreed to cut me in for a share of treasure that's supposed to be hidden inside the manor if I help her find it before a rival ghost hunter beats her to it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7096 => array(
			'name'    => "Unfinished Business",
			'journal' => "I met Faric Velain, a priest of Arkay, investigating the ruins of Nonungalo. He believes a group of undead necromancers are performing a ritual inside and asked me to help him stop it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4735 => array(
			'name'    => "The Staff of Magnus",
			'journal' => "Queen Ayrenn must deal with the aftermath of her brother's betrayal and settle matters in Grahtwood. In the meantime, she asked me to travel to Marbruk, in Greenshade, and secure the Staff of Magnus from its vault.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7099 => array(
			'name'    => "Cold Fires on the Watch",
			'journal' => "I found a dead legionary inside Varen's Watch. She held a scroll that ordered her to ignite the signal fires atop its towers and walls to call for reinforcements. I should finish the job.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4740 => array(
			'name'    => "Questionable Contract",
			'journal' => "The Wood Elf brewing town of Willowgrove is trying to expand their beehives. They've hired help to clear the forest. This help appears to have suffered some difficulties.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6991 => array(
			'name'    => "An Unhealthy Fate",
			'journal' => "Torvesard gained the first part of the lost memory, but two more remain. We need to stop Blightcrown and the corruption that weakens Apocrypha, and we need to find Vaermina, hidden in Master Shelreni's body, before she locates the remaining secrets.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4744 => array(
			'name'    => "Before the Storm",
			'journal' => "Pelidil's plans to lead the Maormer in attack against Valenwood have been exposed and Treethane Fariel suggests I go to Seaside Sanctuary to try to repel the Maormer invasion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5589 => array(
			'name'    => "The Sailor's Pipe",
			'journal' => "<<1>> runs a finder's business and uses the Thieves Guild to track down his quarries. Helping him will surely reflect well on my skill as a thief and be lucrative to boot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4747 => array(
			'name'    => "The Anguish Gem",
			'journal' => "I met a Dark Elf named Dralnas who claims that Dremora have taken his soul to feed to something he calls the Anguish Gem.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7118 => array(
			'name'    => "Baleful Bluffs",
			'journal' => "A powerful Daedra called Hessedaz the Baleful prowls the  bluffs southeast of Skingrad. I agreed to defeat the creature and recharge the sentry spheres in the area.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4749 => array(
			'name'    => "How Few Remain",
			'journal' => "The mine called Kuna's Delve has been assaulted by a pack of bloodthirsty trolls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7168 => array(
			'name'    => "The Codex Caper",
			'journal' => "A new chapter of the Thieves Guild is forming in Glenumbra. I was recruited to help the thief Quen with a caper to unite the gangs of Daggerfall. After all, a rich prize is the best way to bring thieves together.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4750 => array(
			'name'    => "Throne of the Wilderking",
			'journal' => "Aranias and Andur were sent by the Veiled Heritance to kill the Wilderking. They are now forcing their way through Greenheart towards the Wilderking's throne.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6993 => array(
			'name'    => "A Memory of Mystery",
			'journal' => "A group of vile mercenaries called the Painted Eye invaded Kemel-Ze to loot Sotha Sil's experiments. Thoryn Armas, a priest of the Tribunal Temple asked for my help to rid Kemel-Ze of the ransacking mercenaries.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4754 => array(
			'name'    => "Master of Leki's Blade",
			'journal' => "I've arrived at Leki's Blade, a school of swordsmanship in the sands of Alik'r.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4757 => array(
			'name'    => "What the Heart Wants",
			'journal' => "I met a Soul Shriven named Fatahala. She claims that this Manor of Revelry isn't what it appears to be. Furthermore, she wants me to help Stibbons, who she tells me is in mortal danger here.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5448 => array(
			'name'    => "Maelstrom Arena",
			'journal' => "I have been welcomed into the bizarre realm of the Daedric Demiprince Fa-Nuit-Hen. In order to test my mettle and demonstrate my skills, I've agreed to enter the Maelstrom Arena, where deadly challenges created by Fa-Nuit-Hen's Barons await me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6474 => array(
			'name'    => "The Mountain Bellows",
			'journal' => "I encountered a wounded High Elf named <<1>> who requested my assistance stopping a Reachmen's ritual in the Shadowgreen that could cause the eruption of the mountain above it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4759 => array(
			'name'    => "Hallowed to Rawl'kha",
			'journal' => "The priestess Kauzanabi-jo told me my next goal should be to make for the temple in Rawl'kha. I need to aid the Lunar Champions in their journey.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5605 => array(
			'name'    => "Looming Shadows",
			'journal' => "I accepted a bounty to investigate unnatural phenomena preventing the excavation of Tribune's Folly.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4760 => array(
			'name'    => "Whose Wedding?",
			'journal' => "The bandits at Sep's Spine have canceled hostilities for sake of the boss's daughter's wedding.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4761 => array(
			'name'    => "Trouble at Tava's Blessing",
			'journal' => "A courier came to me with an urgent request to go help Prince Azah fight in the small town of Tava's Blessing and to send the Scholar's apprentice, Shiri, back to Bergama.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4764 => array(
			'name'    => "The Tharn Speaks",
			'journal' => "Abnur Tharn seeks an audience with me away from the Harborage. I should see what he wants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4766 => array(
			'name'    => "Freedom's Chains",
			'journal' => "<<1>> is a town in Bangkorai fabled to have the power to cure any disease. <<2>> and his husband Draven seek whatever cure the village offers, though they don't know what the cost may be.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4767 => array(
			'name'    => "One of the Undaunted",
			'journal' => "I've been invited to join a group of adventurers called the Undaunted.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4770 => array(
			'name'    => "Forthor's Cursed Axe",
			'journal' => "I found an odd, glowing handaxe in a pack at the bottom of Gurzag's Mine. A note of sale indicated a Bosmer named Forthor sold it to the Orcs. He's part of the Marbruk Mages Guild.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4771 => array(
			'name'    => "Beasts of Falinesti",
			'journal' => "Thonoras and his wife, Pariel, were captured and tortured by Wood Orcs. Pariel begged Hircine to save them, and Hircine \"changed her.\" After helping her husband escape, Pariel disappeared into a cave beneath the Falinesti Spring Site.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4773 => array(
			'name'    => "Keeper of Bones",
			'journal' => "I found a talking skull named <<1>> near <<Ac:2>>. He wants me to piece him together so he can help thwart Worm Cultists in the area.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5464 => array(
			'name'    => "Forcing the Faith",
			'journal' => "A group of Orc artisans were heading to the new city of Orsinium when they were attacked by members of the Trinimac cult known as the Vosh Rakh and taken prisoner in the ruins of Paragon's Remembrance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6490 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4775 => array(
			'name'    => "A Night to Forget",
			'journal' => "I ran into a very hung-over mage in Rawl'kha. He had too much to drink, lost several magical relics, and now fears he'll never gain an apprenticeship to Telenger the Artificer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4777 => array(
			'name'    => "The List",
			'journal' => "Tappo's journal details his victims. He kept a keepsake from each.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6647 => array(
			'name'    => "Dog Days",
			'journal' => "I found a note by the corpse of an Imperial. His dying wish was for his love to know the truth about what happened to him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4778 => array(
			'name'    => "Razor's Edge",
			'journal' => "Whisper Grove has been turned to a City of Ash by the forces of Mehrunes Dagon. If I don't intervene, the remaining Bosmer will be slaughtered.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4783 => array(
			'name'    => "The Weight of Three Crowns",
			'journal' => "I convinced the leaders of the three alliances to meet to discuss a plan to attack Molag Bal's realm of Coldharbour. The meeting takes place on the historic island of Stirk. Hopefully, the meeting will go smoothly.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7025 => array(
			'name'    => "A Calamity of Fate",
			'journal' => "Blightcrown completed Master Shelren's rite and Vaermina entered Apocrypha. The Prince of Dreams' presence has disrupted the workings of the realm as she hunts for the lost memory. I need to find a way to stop her from reaching it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4786 => array(
			'name'    => "The Innkeeper's Daughter",
			'journal' => "<<1>>, an innkeeper in <<2>>, informed me of bandits who kidnapped his daughter <<3>>. He asked me to bring her home safely.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7019 => array(
			'name'    => "Dim and Distant Pasts",
			'journal' => "Dimik-ei, the sister from Sharp's forgotten past, wishes to meet with him again. Sharp thinks she might be able to help us track down his former oppressor, Sondivel the Reformer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4788 => array(
			'name'    => "The Falinesti Faithful",
			'journal' => "A small group of Wood Elves entered the Falinesti Spring Site, which has been taken over by Wood Orcs. When they petitioned the Wood Orcs to leave, they were taken prisoner. Egannor managed to escape, but his friends are still in the camp.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6506 => array(
			'name'    => "Pledge: Stone Garden",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Stone Garden.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4791 => array(
			'name'    => "The Artisan",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7039 => array(
			'name'    => "Reenactment Gone Awry",
			'journal' => "I learned that Vro-Kuul-Sha the Deepreaver recently defeated a group of Buoyant Armigers who were attempting to follow in Lord Vivec's footsteps.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4792 => array(
			'name'    => "A Tangled Knot",
			'journal' => "A stone being named Sumiril carries a book that contains a story about how he came to be.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5509 => array(
			'name'    => "Parts of the Whole",
			'journal' => "The eccentric artificer and dungeon-delver, Raynor Vanos, needs more data for his experiments. He's certain that the information he requires can be found in a Dwarven ruin called Zthenganaz.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4793 => array(
			'name'    => "Manthir's Debt",
			'journal' => "I met a Khajiit called Cold-Eyes who is looking to collect a debt. He'll give me a cut if I help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7216 => array(
			'name'    => "Paths Out Of Darkness",
			'journal' => "Zerith received a suspicious summons from High Cantor Viti to meet in Pa'alat. Perhaps she finally realized we share an enemy in the Rajaska, but Zerith-var suspects it may be a trap.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7037 => array(
			'name'    => "Secret Protection",
			'journal' => "Ordinator Tilena wants three perfect Dremnaken teeth. The Ordinators believe they can use the teeth in a ritual to protect their secrets from Hermaeus Mora.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4798 => array(
			'name'    => "Eye on Arenthia",
			'journal' => "A shady character told me I was needed at the city of Arenthia. This Orthoron said someone of my prowess would come in handy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7365 => array(
			'name'    => "The Judgment of Garth",
			'journal' => "A member of the Skaal tribe believes the mood of his bear, Garth, during New Life, will determine the fate of Tamriel. He believes honeycomb might make him happy. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4799 => array(
			'name'    => "To Saifa in Rawl'kha",
			'journal' => "A strange priest at an old Khajiiti ruin told me that my path is laid out before him. He suggests I go to the city of Rawl'kha to learn more.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4808 => array(
			'name'    => "Test of Faith",
			'journal' => "I found a journal that belonged to a monk of the Lunar faith named Yenadar. The journal mentions that he has a sister named Kala in S'ren-ja. I should return the journal to her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4811 => array(
			'name'    => "Nirnroot Wine",
			'journal' => "I agreed to gather Red Mountain Ash for <<1>>, so he can make Nirnroot wine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4813 => array(
			'name'    => "No Second Chances",
			'journal' => "Master Pellingare, a Wayrest nobleman, has gone missing. His guards, Yahyif and Gedric, have come to the sewers looking for him after he was last seen heading into the undercity tunnels.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5504 => array(
			'name'    => "The Skin Trade",
			'journal' => "The Dragonstar Caravan Company's job broker, Guruzug, has tasked me with traveling to Argent Mine to slay werewolves. A wealthy client will pay handsomely for their skins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7232 => array(
			'name'    => "Fallen Leaves of West Weald",
			'journal' => "Gandrinar, a Wood Elf researcher, is working on a book about the Ayleids and their connection to the Daedra. He asked me to help him search for Ayleid remnants scattered across West Weald and assemble them into a bundle for his research.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4814 => array(
			'name'    => "News of Fallen Kin",
			'journal' => "I discovered the body of one of the Fighters Guild soldiers in Mal Sorra's Tomb. She started writing a note to her cousin, Grundskar, another soldier.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4815 => array(
			'name'    => "Pact Advocate",
			'journal' => "Nature itself has turned against the Bosmer village of Karthdar and its citizens are desperate to restore peace.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7057 => array(
			'name'    => "Bastion Nymic - Writhing Wastes",
			'journal' => "Ordinator Tandasea told me of a dangerous threat to Bastion Nymic, a stronghold in Hermaeus Mora's realm of Apocrypha. I agreed to find my way to the bastion and defeat Hermaeus Mora's enemies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4818 => array(
			'name'    => "To Auridon",
			'journal' => "After defeating the Maormer assault on Khenarthi's Roost, Razum-dar has asked me to report to <<1>> in the Auridon city of Vulkhel Guard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7247 => array(
			'name'    => "The Skull of Gandelec",
			'journal' => "The Sea Elf Loraviel needs assistance to remove the necrotically charged skull of her ancestor from Carapace Cavern. We have to proceed carefully so as not to release the spirits bound within.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4821 => array(
			'name'    => "Report to Marbruk",
			'journal' => "I've been given a letter to deliver to Queen Ayrenn to bring her up to speed on the events that have occurred and inform her of our victory over Vicereeve Pelidil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5512 => array(
			'name'    => "Scouting the Nobles District",
			'journal' => "My Alliance needs scouts to secure rally points across the Imperial City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7071 => array(
			'name'    => "Paths of Chaos",
			'journal' => "West Weald is beset by Daedric incursions and mysterious disturbances. Leramil the Wise fears that an ancient force of chaos is present in this land. I agreed to help her investigate these troubles.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4824 => array(
			'name'    => "Troublemakers",
			'journal' => "A Bosmer named Adamir, and his associate Timiwe, are attempting to animate an assemblage of rodent parts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4828 => array(
			'name'    => "Hunting the Wasp",
			'journal' => "Radreth, a new member of the hunting company the Hounds, asked for my help in hunting a giant wasp.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4832 => array(
			'name'    => "Council of the Five Companions",
			'journal' => "I received a summons to return to the Harborage in preparation for our battle against Molag Bal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4833 => array(
			'name'    => "Bosmer Insight",
			'journal' => "I've discovered an order of hunters called Brackenleaf's Briars. They protect a tree they say was planted by Y'ffre at the creation of the world.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7073 => array(
			'name'    => "Relics of the Three Princes",
			'journal' => "Recollection forces search Ayleid ruins while their Daedric allies continue to harry West Weald forces. Meanwhile, Ithelia remains hidden. I agreed to help Beragon learn more about our enemy's true plans.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4834 => array(
			'name'    => "A Past Remembered",
			'journal' => "While exploring the Tribulation Crypt, I encountered a ghost that beckoned me deeper into the tomb. The ghost led me to an ancient sword engraved with the following message: \"Marcelle Stenric of Shornhelm.\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7067 => array(
			'name'    => "Kill Enemy Arcanists",
			'journal' => "Grand Warlord Dortene has authorized a reward for any Covenant soldier who slays 20 enemy Arcanists.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4836 => array(
			'name'    => "Halls of Torment",
			'journal' => "Varen Aquilarios and Abnur Tharn located Sai Sahan within the Halls of Torment. I need to venture into these halls and save the final, missing companion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7263 => array(
			'name'    => "Candleflies for the Dead",
			'journal' => "Lector Volonaro has asked me to release candleflies in Tainted Leel to honor the fallen Argonians haunting the village.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4837 => array(
			'name'    => "Valley of Blades",
			'journal' => "Varen Aquilarios asked me to accompany Sai Sahan on a trip back to his ancestral home. An old friend, Kasura, may have knowledge of how to sneak into Sancre Tor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4840 => array(
			'name'    => "Ancient Power",
			'journal' => "I met a Mages Guild novice named Anenya just inside the Erokii Ruins. She was sent here on an assignment to recover a relic called the Tear of Anurraame, but she can't carry it out. I agreed to recover the relic and bring it back to Shornhelm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4841 => array(
			'name'    => "Trouble at the Rain Catchers",
			'journal' => "The Rain Catcher Fields, a sandy expanse of johads gathering water in large pools, has recently been overrun by beetles from the desert.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4843 => array(
			'name'    => "A Traitor's Tale",
			'journal' => "Undead soldiers have been haunting Traitor's Tor ever since the end of Ranser's War, but no one's ever been able to find out why.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7264 => array(
			'name'    => "Gaulm's Lair",
			'journal' => "Gaulm the vahath behemoth has taken over Tuniria, an ancient Corelanyan ruin south of Sunport. Its pet voriplasms are slowly destroying the site. I agreed to defeat Gaulm and burn the voriplasms left behind.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7085 => array(
			'name'    => "When the Ground Shook",
			'journal' => "Disaster struck the village of Ostumir. After the ground violently shook, villagers were killed or trapped. A Wood Elf named Beragon started rescue efforts while searching for his friend, Greenspeaker Sorilen. I agreed to help him rescue survivors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4846 => array(
			'name'    => "The Misuses of Knowledge",
			'journal' => "I have found the notes of a mage named Mercano from Woodhearth in the depths of an Ayleid ruin where the Veiled Heritants were raising undead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4847 => array(
			'name'    => "God of Schemes",
			'journal' => "The time has come to confront Molag Bal and reclaim my soul.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4848 => array(
			'name'    => "Deadly Ambition",
			'journal' => "I discovered the remains of a mage outside a cave in Shademist Moors. While there are no shortage of victims around here, the presence of the Mages Guild urges me to investigate what might have happened to the rest of them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7089 => array(
			'name'    => "Hold the Line",
			'journal' => "I met Tribune Alea Idolus on her way to assist in the defense of Feldagard Keep. A Daedric army has laid siege to the Imperial Legion there. She thinks the legionaries there need any help they can get. I agreed to join the fight for the keep.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4850 => array(
			'name'    => "Shades of Green",
			'journal' => "The Shade of Prince Naemon is spreading corruption across Greenshade. I must help Indaenir cleanse the corruption.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7083 => array(
			'name'    => "The Missing Cohort",
			'journal' => "A cohort of Imperial Legionaries under the command of Tribune Alea Idolus have disappeared. They were supposed to guard Rustwall, but now the entire estate appears deserted. I agreed to help in the search.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4852 => array(
			'name'    => "Claim to Fame",
			'journal' => "The Torog's Spite Bandits are dedicated to profit and vengeance from targeting families descended from those who sacked Orsinium. An Orc historian named Maaga wants to record the bandits' deeds but needs to know which Orc clan deserves credit.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4853 => array(
			'name'    => "Woodhearth",
			'journal' => "Before losing Aranias' memories, the Wilderqueen told me that she knew about the Veiled Heritance plot to steal the Staff of Magnus. She suggested I go to the city of Woodhearth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5544 => array(
			'name'    => "A Profitable Venture",
			'journal' => "Captain Jimila has been suffering under the oppressive dock fees in Anvil. She asked for my help in stealing the dock master's private stash to pay off her debts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4863 => array(
			'name'    => "Troll's Dessert",
			'journal' => "I met a Bosmer Adventurer named Gluineth locked in a cell in Troll's Toothpick. I've agreed to spring her from the cell and escort her out of the cave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4864 => array(
			'name'    => "A Favor Between Kings",
			'journal' => "I found a Knight of St. Pelin in the Crypt of the Exiles who died while on an important quest. She sought to recover a sacred Redguard artifact, the Memory Stone of Makela Leki, and return it to a Redguard scholar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4867 => array(
			'name'    => "Shadow of Sancre Tor",
			'journal' => "Sai Sahan recovered from his experiences in the Halls of Torment and is ready to guide me to the Amulet of Kings. He hid the artifact in Sancre Tor, an ancient Imperial catacomb.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4869 => array(
			'name'    => "Bounty: Black Daggers",
			'journal' => "I accepted a Fighters Guild contract in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6211 => array(
			'name'    => "Capture Any Nine Resources",
			'journal' => "I accepted a mission to help capture farms, lumber mills, and mines for the Dominion. Conquering any nine of these resources will complete my mission.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4870 => array(
			'name'    => "Bounty: Gray Vipers",
			'journal' => "I accepted a Fighters Guild contract in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4873 => array(
			'name'    => "Bounty: Black Daggers",
			'journal' => "I accepted a Fighters Guild contract from Arkas at the Southern Morrowind gate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4874 => array(
			'name'    => "Bounty: Gray Vipers",
			'journal' => "I accepted a Fighters Guild contract in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4876 => array(
			'name'    => "Bounty: Goblins",
			'journal' => "I accepted a Fighters Guild contract from Arkas at the Southern Morrowind gate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4877 => array(
			'name'    => "Bounty: Black Daggers",
			'journal' => "I accepted a Fighters Guild contract from Ikran at the Northern High Rock gate in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4878 => array(
			'name'    => "Bounty: Gray Vipers",
			'journal' => "I accepted a Fighters Guild contract in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6728 => array(
			'name'    => "A Gem of a Mystery",
			'journal' => "I found the body of a Scamp alongside a note and bag in the Deadlands.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4879 => array(
			'name'    => "Bounty: Shadowed Path",
			'journal' => "I accepted a Fighters Guild contract in Cyrodiil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4880 => array(
			'name'    => "Bounty: Goblins",
			'journal' => "I accepted a Fighters Guild contract from Ikran at the Northern High Rock gate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6738 => array(
			'name'    => "Web of Lies",
			'journal' => "The legendary mage, Divayth Fyr, came to Fargrave in search of a powerful Daedric artifact called the Ebony Blade. However, the owners hid it away under mysterious circumstances. I agreed to help Divayth uncover the truth of the missing sword.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4881 => array(
			'name'    => "The Will of the Woods",
			'journal' => "The princess of Evermore is being cared for in the Viridian Woods, but her future is uncertain.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4882 => array(
			'name'    => "The Wandering Minstrel",
			'journal' => "<<1>>'s husband, a minstrel named Sorion, went missing after going on tour through <<2>>. I agreed to help track him down.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4885 => array(
			'name'    => "A Lasting Winter",
			'journal' => "The <<1>>, one of the legendary resting places of the Walking City of Falinesti, is buzzing with activity. It seems <<2>> has begun an excavation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4888 => array(
			'name'    => "The Emerald Chalice",
			'journal' => "Lady Clarisse Laurent is on the trail of the Emerald Chalice, a relic stolen from the Mages Guild by the exiled Nord mage, Malofar. She tracked him here to Breagha-Fin.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5221 => array(
			'name'    => "Kill Enemy Templars",
			'journal' => "Grand Warlord Dortene has authorized a reward for any Covenant soldier who slays 20 enemy Templars.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6756 => array(
			'name'    => "Pursuit of Freedom",
			'journal' => "Louna Jolvanne, a Breton prisoner on Amenos, wants to rescue her partner Anton from Whalefall. The jailers hunt prisoners there as a game. The promised reward for survival is freedom from Amenos.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4891 => array(
			'name'    => "The Parley",
			'journal' => "The Reachmen threat has been averted, but Evermore is still in danger, and Queen Arzhela needs my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6750 => array(
			'name'    => "Honest Toil",
			'journal' => "Fasaria, a devotee of Zenithar, told me that completing deeds for Zenithar would earn his favor on this holiday.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4893 => array(
			'name'    => "Flipping the Coin",
			'journal' => "On approaching <<1>>, I came upon <<2>>, a Khajiit in trouble with the local authorities.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4894 => array(
			'name'    => "A Letter for Deshaan",
			'journal' => "The Llodos plague is ravaging the Deshaan countryside. I met someone from a town known as the Serk. He's on his way to warn people about the plague.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6744 => array(
			'name'    => "The Force of Change",
			'journal' => "I encountered Valsirenn trapped in a sychroseisiac state, out of phase with reality and in need of assistance. She asked me to help her locate a sage named Master Rrvenk, who is supposed to be a master of the Eleventh Force: Change.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4895 => array(
			'name'    => "Phantom Guilt",
			'journal' => "I found a strange glowing amulet on a corpse in the woods.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4896 => array(
			'name'    => "The Great Tree",
			'journal' => "<<1>> has heard rumblings of discontent in <<2>>, the Aldmeri Dominion's capital. She wants me to travel ahead of her and assess the situation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5229 => array(
			'name'    => "Kill Enemy Nightblades",
			'journal' => "Grand Warlord Sorcalin has authorized a reward for any Dominion soldier who slays 20 enemy Nightblades.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4899 => array(
			'name'    => "Beyond the Call",
			'journal' => "Reachmen attacked Northglen. They must be stopped before they can attack the city of Evermore.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4900 => array(
			'name'    => "Raiders at the Crossing",
			'journal' => "Imperials have been raiding Martyr's Crossing, and the Knights of Saint Pelin have wound up having to depend on the help of Baandari merchants for assistance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4901 => array(
			'name'    => "The Road to Rivenspire",
			'journal' => "A bloody conflict has erupted between the noble houses of Rivenspire and High King Emeric has asked me to go there on his authority and attempt to restore order.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6263 => array(
			'name'    => "A Taste for Toxins",
			'journal' => "According to Jee-Lar, poison-sniffing skinks can provide some much-needed security in the wilds of Murkmire. He needs me to acquire one of these valuable lizards in Alik'r.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4906 => array(
			'name'    => "A Quest",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6760 => array(
			'name'    => "Tournament of the Heart",
			'journal' => "Knight Commander Jourvel has promised the hand of her daughter Aurelia in marriage. Isobel Veloise, a Knight Aspirant in the Sapphire Tourney, has sworn to free Aurelia from her mother's promise.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4911 => array(
			'name'    => "Present in Memory",
			'journal' => "Scholar <<1>> and his guide, <<2>> became separated in the Sunken Road.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4912 => array(
			'name'    => "Storming the Garrison",
			'journal' => "Imperial forces have taken control of Bangkorai Garrison, the strategic keep that divides northern and southern Bangkorai. King Emeric and the Lion Guard have come to aid Queen Arzhela and the Knights of Saint Pelin in reconquering the garrison.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5245 => array(
			'name'    => "Holding Court",
			'journal' => "Little Leaf has asked me to find her friend, Regent Cassipia, in Dragonstar. She claims that the two of them are working from the inside to stop the Scaled Court.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4914 => array(
			'name'    => "The Wakening Dark",
			'journal' => "The ruins of the ancient Wood Elf village of Gil-Var-Delle is under the corrupting influence of Molag Bal. The Worm Cult is active here, and my help is needed to put an end to their schemes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4916 => array(
			'name'    => "Guar Gone",
			'journal' => "<<1>> is distraught. Her adorable pet, Puddles, ran away, and she's afraid that the little creature is lost.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6259 => array(
			'name'    => "Death and Dreaming",
			'journal' => "The riddles I uncovered with Xukas and Jaxsik-Orrn pointed us to a mysterious locale known as the Swallowed Grove. According to Famia, I will have to undertake a dream-wallow vision to learn the truth of the Remnant of Argon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4918 => array(
			'name'    => "The Shifting Sands of Fate",
			'journal' => "Hallin's Stand is ready to take back their town from the Imperial's Seventh Legion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4920 => array(
			'name'    => "The Lady's Keepsake",
			'journal' => "I met <<1>> and <<2>> at their camp along the roadside. They're fleeing for their lives from their ransacked farmstead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5319 => array(
			'name'    => "Birdsong's Curse",
			'journal' => "According to a note I discovered in Coldperch Cavern, a hagraven named Kraala uses a totem to transform people into birds.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4926 => array(
			'name'    => "Assassin Hunter",
			'journal' => "Adusa has further need of my help. She's learned of another plot, an assassin contracted by allies of House Montclair.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6776 => array(
			'name'    => "The Large Delegate",
			'journal' => "<<1>>, delegate-at-large for the Ebonheart Pact Cultural Exchange, misplaced his scepter of office after a night of wild festivities. He asked me to help him find it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4927 => array(
			'name'    => "The Assassin's List",
			'journal' => "Adusa has discovered that the Montclairs have assembled a list of targets they have marked for elimination.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4928 => array(
			'name'    => "Threat of Death",
			'journal' => "I returned to find Adusa and Castle Ravenwatch under attack. The list of targets puts Adusa right at the top.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6275 => array(
			'name'    => "Frog Totem Turnaround",
			'journal' => "I found a pack with a romantic poem and frog totem in the middle of the swamp. If I return these items to their rightful owner, I may receive a reward.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4934 => array(
			'name'    => "In the Doghouse",
			'journal' => "<<1>> has asked me to find her husband Ignobert, who went missing after a vampire attack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6671 => array(
			'name'    => "Shrine of Nocturnal",
			'journal' => "The Daedric Prince, Nocturnal, tasked me with recovering a relic that was stolen from her shrine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4936 => array(
			'name'    => "The Crown of Shornhelm",
			'journal' => "Now that the threat to Rivenspire has been dealt with, there's still the matter of crowning a true ruler for the region.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7180 => array(
			'name'    => "Another Death in Ontus",
			'journal' => "Legionary-turned-constable Yabren deputized me to help her investigate a series of suspicious deaths in the town of Ontus, including the death of her father Ulbren, the previous town constable.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4937 => array(
			'name'    => "The Sanctifying Flames",
			'journal' => "Countess Tamrith has sent a priest to sanctify her dead soldiers at Camp Tamrith. Those particularly devoted to the Light of Arkay have a profound fear of turning into undead creatures.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6295 => array(
			'name'    => "Death-Hunts",
			'journal' => "I found a notice calling for warriors to join a Death-Hunt.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4938 => array(
			'name'    => "The Heart of the Beast",
			'journal' => "Fallen Grotto is under the control of <<1>> and his Reachmen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6804 => array(
			'name'    => "A New Venture",
			'journal' => "Now that I'm an official member of the Roisters Club, Master Razhamad wants me to gain some experience playing Tribute.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4939 => array(
			'name'    => "Until Death",
			'journal' => "<<1>> has lost track of her husband and her dog. I've offered to help her look for both of them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6792 => array(
			'name'    => "Balki's Map Fragment",
			'journal' => "I found the body of a Khajiit, <<1>>. It looks like he died on the way to deliver something to the Dufort Shipyards.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4943 => array(
			'name'    => "The Honor of the Queen",
			'journal' => "All is not well in <<1>>. The Khajiit lack an embassy, which is the cause of tensions in the capital city of the Aldmeri Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4944 => array(
			'name'    => "Friend of Trolls",
			'journal' => "I discovered a research journal kept by an Orc named Grularz in Crestshade Mine. She was apparently studying the trolls until they decided to kill her. The journal referred to a brother named Gothurg in Shornhelm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5277 => array(
			'name'    => "Pledge: Elden Hollow II",
			'journal' => "Maj al-Ragath has challenged me to explore the ruins beneath Grahtwood, the Veteran Elden Hollow.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4945 => array(
			'name'    => "A Spy in Shornhelm",
			'journal' => "Adusa fears that Montclair agents still operate out of Shornhelm. She wants me to follow a courier in hopes the courier will lead me to the Montclair informer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6673 => array(
			'name'    => "Conflict of Interest",
			'journal' => "<<1>> asked me to collect crow eggs from <<2>> for two potential clients. One client wants the eggs saved, and the other wants them destroyed. It's up to me to decide what to do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4946 => array(
			'name'    => "A Silken Garb",
			'journal' => "<<1>> is concerned for her friend <<2>>. He recently entered a cave full of deadly spiders and hasn't been seen since.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4950 => array(
			'name'    => "Storgh's Bow",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6687 => array(
			'name'    => "Bounties of Blackwood",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4952 => array(
			'name'    => "Dearly Departed",
			'journal' => "Jowan and Mayla want to give their family a proper burial. They asked me to accompany them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6311 => array(
			'name'    => "The Riverhold Abduction",
			'journal' => "Mizzik Thunderboots has requested my help investigating an abduction in Riverhold.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4954 => array(
			'name'    => "Light in the Darkness",
			'journal' => "I met an old scholar near the Ayleid ruins who came to power a crystal for his research, but found the ruins swarming with Worm Cultists.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4955 => array(
			'name'    => "A Lucky Break",
			'journal' => "I found a journal that belonged to a thief called Nedras the Sly. He apparently turned to crime in order to support his ailing sister in Northpoint.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7321 => array(
			'name'    => "Pledge: Naj-Caldeesh",
			'journal' => "Urgarlag Chief-bane of the Undaunted challenged me to explore Naj-Caldeesh and get rid of the necromancers inside.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4956 => array(
			'name'    => "Last Words",
			'journal' => "I found a journal on the body of Klaandor Axe-Bearer. It details his transformation into a vampire. Seems like he was looking for someone to kill him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6814 => array(
			'name'    => "The Intoxicating Mix",
			'journal' => "A woman named Genalda enlisted my help gathering ingredients for a new drink she and her partner, Hilgrim, are hoping to make. Apparently, Hilgrim drank the last of their stock.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4957 => array(
			'name'    => "Breaking the News",
			'journal' => "I found a journal that belonged to a monk of the lunar faith named Yenadar. The journal mentions that he has a sister named <<1>> in S'ren-ja. I should return the journal to her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5136 => array(
			'name'    => "Summary Execution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6685 => array(
			'name'    => "Quaking Dread",
			'journal' => "I met a pair of Imperial Battlemages named Martus and Lucilla outside an abandoned prison known as the Dread Cellar. Martus asked for my help investigating magical activity that caught the Battlespire's notice.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4958 => array(
			'name'    => "Northpoint in Peril",
			'journal' => "Lleraya Montclair and her forces have taken control of Northpoint. They've sealed the gates and locked out the legitimate city guards. But Baron Dorell has a plan to liberate Northpoint. It just requires a small group willing to infiltrate the city.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6808 => array(
			'name'    => "Wildhorn's Wrath",
			'journal' => "A dangerous faun lord called Glemyos Wildhorn kills any hunters who enter its territory. Recently, Wildhorn slaughtered a band of noble hunters. The bloodshed must stop.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4959 => array(
			'name'    => "Trials and Tribulations",
			'journal' => "After their defeat at Bangkorai Garrison, the Imperial forces retreated south, and King Emeric believes they will make their last stand here at the ancient temple known as the Hall of Heroes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4960 => array(
			'name'    => "To Walk on Far Shores",
			'journal' => "Imperial Magus-General Septima Tharn captured King Emeric and took him into the Chamber of Passage. It's apparently a gateway to the plane known as the Far Shores, where Redguards go in the afterlife.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4965 => array(
			'name'    => "Children of Yokuda",
			'journal' => "I met Nicolene in the Dead Wolf Inn in Shornhelm. She and Captain Kaleen are here looking for one of King Fahara'jad's former spies, Naruk. Naruk has turned against the Daggerfall Covenant and is now inciting war, but Kaleen is missing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6307 => array(
			'name'    => "Descendant of the Potentate",
			'journal' => "The Dark Elf Ashur has come to Hakoshae to investigate Magnate Feina-Darak. Rumor has it that she's the descendant of the Akaviri Potentate. I agreed to help him in his investigation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4966 => array(
			'name'    => "Wanted: Sgolag",
			'journal' => "I found a wanted poster offering a reward for the head of an Orc called Sgolag.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7212 => array(
			'name'    => "The Knot Awaits",
			'journal' => "A Dremora called Xoryn is racing to get his hands on the arcane knot, a powerful and unstable object. Keshargo of the Scribes of Mora asked me to help him stop Xoryn and bring the knot to the null arca where it can be contained.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4969 => array(
			'name'    => "A Marriage in Ruins",
			'journal' => "A Daedra has taken over Nilata Ruins. Having possessed a farmer's wife, the Daedra is luring travelers into her lair-likely to their doom.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6327 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4970 => array(
			'name'    => "To Aid the Enemy",
			'journal' => "Something has been luring Imperial soldiers into the ruins. Many have been captured by the giant spiders lurking there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6836 => array(
			'name'    => "Pledge: Earthen Root Enclave",
			'journal' => "Urgarlag Chief-bane of the Undaunted has challenged me to rid Earthen Root Enclave of the Firesong invaders.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4971 => array(
			'name'    => "The Arch-Mage's Boon",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4972 => array(
			'name'    => "The Liberation of Northpoint",
			'journal' => "We found a way to sneak into the sealed city of Northpoint-a tunnel used by smugglers and black marketeers. Now we need to head inside and find a way to free the city from Lleraya Montclair's grasp.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6701 => array(
			'name'    => "An Apocalyptic Situation",
			'journal' => "A strange messenger claims that someone she calls the Dread Lady wants to meet with me. She further claims that this involves a situation that could have apocalyptic consequences.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4974 => array(
			'name'    => "Brackenleaf's Briars",
			'journal' => "While traveling in <<1>>, I found a shrine dedicated to the animals of the Valenwood. <<2>> said the shrine was created by <<3>>'s Briars, an elite group of hunters.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6824 => array(
			'name'    => "Challenges of the Past",
			'journal' => "The second Tribute tournament is about to begin! Unfortunately, Sorinne's past threatens to derail her future in the upcoming matches. The Gonfalon Chapter needs her help to succeed, but she may need our help to overcome her demons.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4975 => array(
			'name'    => "The Wounded Wood",
			'journal' => "I met a priest of Y'ffre who seeks to heal the scarred ruin of Gil-Var-Delle by mixing blessed water into the falls that feed into the river of the vale. She asked me to help her in her task.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4976 => array(
			'name'    => "Carnival Conundrum",
			'journal' => "<<1>>'s Carnival came from the Summerset Isle to spread High Elf culture. Unfortunately, they've suffered numerous setbacks in performing for the Wood Elves of <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5309 => array(
			'name'    => "Pledge: Vaults of Madness",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to explore the Vaults of Madness, a dark labyrinth at the heart of Coldharbour.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6705 => array(
			'name'    => "Death Stalks the Weak",
			'journal' => "An Orc named Khazasha plans to enter the Daedric citadel called Ardent Hope. She tracked a Daedric creature called the Ravener inside and plans to destroy it for murdering her entire clan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4978 => array(
			'name'    => "Striking Back",
			'journal' => "High King Emeric wants to liberate southern Bangkorai from Imperial control. He believes the key to that is tracking down Imperial Magus General Septima Tharn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6198 => array(
			'name'    => "The Towers' Remains",
			'journal' => "Loremaster Celarus and the Psijic scryers have finally discovered the four remaining pieces of the Staff of Towers. I must travel to remote locations to find and recover these artifacts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4979 => array(
			'name'    => "Publish or Perish",
			'journal' => "<<1>> asked me to search the ruins for texts from other scholars who may have previously studied here.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6699 => array(
			'name'    => "Deadlight",
			'journal' => "We destroyed the Riven Cataclyst at Fort Grief, but Sister Celdina escaped. We believe she retreated to a hidden stronghold in a forgotten realm known as Deadlight. We need to find this realm and deal with the cult once and for all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4980 => array(
			'name'    => "A Grave Matter",
			'journal' => "Llotha asked me to help her unlock a lockbox by solving an old mystery about the Blacklight Raiders. The password to the lock is a combination of the Raiders' names and professions.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6719 => array(
			'name'    => "Careless Contamination",
			'journal' => "Taupezu Azzida, a Mantikora in the Sever, poisons its prey. The Saraathu Tong wants to study the Mantikora's victims to make an antidote out of the venom. They want the Mantikora killed so no one can use the venom to harm mortals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4984 => array(
			'name'    => "A Lucrative Scheme",
			'journal' => "I found a journal in a bandit camp. It detailed the murder of an Orc chieftain's two wives. It also mentioned a third wife named <<1>> who lives in a place called Barkbite.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6215 => array(
			'name'    => "Capture Any Nine Resources",
			'journal' => "I accepted a mission to help capture farms, lumber mills, and mines for the Covenant. Conquering any nine of these resources will complete my mission.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4986 => array(
			'name'    => "A Token Trophy",
			'journal' => "<<1>>, an injured Orcish scout near <<2>>, seeks to rally others to his banner by acquiring Captain Helenus's Helm from the Seventh Legion <<3>> who devastated his Orcish brethren.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6846 => array(
			'name'    => "Shaky Job Prospects",
			'journal' => "Hierot Birian needs to perform a mineral survey of Embervine cave for House Mornard. He says Firesong Druids have opened lava fissures within the cave. He asked for my help dealing with the druids so he can finish his job.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4989 => array(
			'name'    => "A Handful of Stolen Dreams",
			'journal' => "As <<1>> and a group of refugees traveled through southern Bangkorai, they were attacked by Imperials. Several of them lost priceless heirlooms.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4991 => array(
			'name'    => "Dark Wings",
			'journal' => "The Wyress Freyda warned me away from Jackdaw Cove as dark forces are at work in northern Bangkorai.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4992 => array(
			'name'    => "Searching for the Searchers",
			'journal' => "A group of soldiers from Bangkorai Garrison lead by Sir Dubroc went out in search of the Reachman leader in Jackdaw Cove.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4997 => array(
			'name'    => "One of the Undaunted",
			'journal' => "<<1>> has invited me to join a group of adventurers called the Undaunted.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6339 => array(
			'name'    => "Lost Lute",
			'journal' => "Someone's lost an item and needs help finding it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	4998 => array(
			'name'    => "Cadwell's Silver",
			'journal' => "Cadwell has turned the Harborage into his new home-for the time being, at any rate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5000 => array(
			'name'    => "Cadwell's Gold",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5004 => array(
			'name'    => "Test",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6862 => array(
			'name'    => "Fauns in Peril",
			'journal' => "At Fauns' Thicket, a wolf pack threatens the local Faun herd. Druid Edaen asked me to plant a berry that the wolves hate but the Fauns love to eat. It should keep the wolves from attacking the Fauns.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5005 => array(
			'name'    => "Drink, Drink, and Be Merry",
			'journal' => "I found a keg containing wine with magical properties.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5007 => array(
			'name'    => "Shroud Hearth Barrow",
			'journal' => "Something strange is happening inside Shroud Hearth Barrow that is frightening the villagers of Ivarstead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5008 => array(
			'name'    => "A Diamond in the Root",
			'journal' => "I found an abandoned campfire at the entrance to <<1>>. I should figure out what happened to the campers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5009 => array(
			'name'    => "The Siege of Velyn Harbor",
			'journal' => "The Dominion's lifeline to Malabal Tor is under siege. If I want to assist in the defense of <<1>>, I should speak with <<2>> near the gate to <<3>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5011 => array(
			'name'    => "Beneath the Surface",
			'journal' => "A Pact soldier and his brother were captured by bandits after being caught spying. They were investigating the cult-like activity of the bandits.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6355 => array(
			'name'    => "Relatively Speaking",
			'journal' => "A Khajiit spirit has awakened from eternal slumber. His descendent Saviwa needs me to find a proper place for the urn in Rimmen Necropolis, so that his ancestor can be at peace, and Saviwa can have some peace and quiet.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5014 => array(
			'name'    => "Crimes of the Past",
			'journal' => "The Ring of Daggers has tracked down a war criminal named Gerard Althen, who once served as King Ranser's spymaster. He's hiding somewhere in Sanguine Barrows and I've agreed to find him and bring him out.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6751 => array(
			'name'    => "Ascending Doubt",
			'journal' => "Jakarn is on the hunt for his friend, the mage Valessea, who disappeared from her workshop. I agreed to help him search for Valessea as well as the medallions his employer, Lady Arabelle, asked him to retrieve from her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5016 => array(
			'name'    => "Children of the Hist",
			'journal' => "<<1>> overheard Dominion soldiers brag of capturing <<Ac:2>>, where Argonian children in <<3>> are born.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5349 => array(
			'name'    => "The King's Gambit",
			'journal' => "At Paragon's Remembrance, I learned that the Vosh Rakh plan to harm Chief Bazrag and High Priestess Solgra before they attack the chiefs at the king's moot in Orsinium. I need to find a way to stop them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6375 => array(
			'name'    => "The Connoisseur",
			'journal' => "I found a note offering a reward for three mysterious bottles of stolen wine. I should find the bottles and bring them to Hofgrad Kjisorssen in the Stitches to receive the reward.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5018 => array(
			'name'    => "Fell's Justice",
			'journal' => "I managed to drive the bandits from Fell's Run, but the bandit's leader, Leonce Gavendien, escaped with the stolen jewels.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7385 => array(
			'name'    => "The Hermit in the Hills",
			'journal' => "I met a courier named Wuleen-Kur who asked me to help him make an urgent delivery to a hermit named Hecicla.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5020 => array(
			'name'    => "The Wayward Son",
			'journal' => "Captain Thayer is having a difficult time recruiting new soldiers for the Shornhelm Guard. She asked me to help convince a man in the stocks to enlist.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6878 => array(
			'name'    => "Sunflower Stamina",
			'journal' => "Druid Gastoc requested that I kill the forest wraith haunting the Valley of the Watcher. While I'm there, he'd like me to collect some of the giant sunflowers as a request from within House Mornard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5021 => array(
			'name'    => "The Lover",
			'journal' => "Captain Thayer thinks her last prospect will pose the greatest challenge of all. His name is Julien Rissiel and he's reluctant to enlist because he's in love.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7440 => array(
			'name'    => "Madness Comes to Wayrest",
			'journal' => "Sheogorath is adamant on continuing his holiday despite our Daedrat encounter. He wants to try the Wayrest Cremant cheese next and has made arrangements to meet a merchant there. I agreed to accompany him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6749 => array(
			'name'    => "The Unrefusable Offer",
			'journal' => "Amminus Varo, a priest of Zenithar, asked me to help him make an offering to Zenithar and recover the blessed awl, a sacred object which was stolen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5022 => array(
			'name'    => "The Bandit",
			'journal' => "Captain Thayer has asked me to recruit a woman named Shuga into the Shornhelm Guard. Thayer suspects Shuga is actually a bandit who doesn't have much respect for the guard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5024 => array(
			'name'    => "Puzzle of the Pass",
			'journal' => "We put an end to Reezal-Jul and Lleraya. Now we need to deal with Baron Montclair. But to reach Montclair in his citadel atop the Doomcrag, we first need to find a way through the Shrouded Pass.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5028 => array(
			'name'    => "South Winds Derby",
			'journal' => "As part of the South Winds Festival, there is a mounted race being proctored by the priests of Kynareth. Participants of the derby will be rewarded with a blessing of good health if they can prove themselves worthy by beating a timed trial.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5033 => array(
			'name'    => "The Star-Gazers",
			'journal' => "A Herald has come with tales of a dire threat in the distant land of Craglorn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5035 => array(
			'name'    => "Calling Hakra",
			'journal' => "Lieutenant Slonn led a team of scholars to Pinepeak Caverns to awaken the spirit of Hakra, one of the five hundred companions, but the team hasn't returned.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7456 => array(
			'name'    => "Adding to the Collection",
			'journal' => "I found a scroll from Gazmod the Collector asking whoever found it to locate some curios in the Parch to add to their collection.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6765 => array(
			'name'    => "To Catch a Magus",
			'journal' => "Now that the alliance leaders have been rescued from the prison island of Amenos, Lady Arabelle wants to find the meeting place of the Ascendant Order and attempt to capture their leaders, the Lord and the Magus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5038 => array(
			'name'    => "The Truth about Spiders",
			'journal' => "I found a soldier outside a spider cave in Stonefalls. She's suffering from a spider bite she received while trying to search the Hollow.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6888 => array(
			'name'    => "Marking the Path",
			'journal' => "Juline Courcelles expects Y'ffre's Path to be a popular spot on her tour. She wants me to travel there and place trail markers along the best path through the area.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5039 => array(
			'name'    => "Darkvale Brews",
			'journal' => "A High Elf named Caalorne was ambushed by a group of Argonians on the road to Kragenmoor. They stole a number of potions from him, and he wants them back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6769 => array(
			'name'    => "Coral Conundrum",
			'journal' => "Brusic, a sentry for House Mornard, asked me to find Veslyn, another sentry sailing with him, and the prisoners in their care. Brusic followed their footprints, but they disappeared into the Coral Cliffs after Brusic's ship wrecked.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5042 => array(
			'name'    => "Assisting Davon's Watch",
			'journal' => "I met a soldier at Ash Mountain who said the defense of Davon's Watch could use some help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5043 => array(
			'name'    => "A Higher Priority",
			'journal' => "A soldier at Fort Virak told me the Pact military is focusing their efforts at the land coral maze known as Vivec's Antlers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6763 => array(
			'name'    => "Spies in the Shallows",
			'journal' => "Titus Stolo requested my aid in dealing with some intelligence he claims is important to my alliance that was lost when Dreadsail pirates interrupted their recovery from vessels that crashed into Shipwreck Shoals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5044 => array(
			'name'    => "To the Mountain",
			'journal' => "A Pact officer at Vivec's Antlers suggested I assist the generals at Ash Mountain.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6279 => array(
			'name'    => "The Skin Taker",
			'journal' => "The skinned bodies of Argonians are being placed on display around Murkmire by a killer dubbed \"the Skin Taker.\" I have been asked to help find this killer before anyone else falls victim to their grisly methods.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5050 => array(
			'name'    => "Waiting for Word",
			'journal' => "Captain Ronag at Shinji's Scarp asked me to deliver a dispatch to Clan Murtag soldiers near the at-Tura estate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5051 => array(
			'name'    => "The Last of Them",
			'journal' => "I ran into Blademaster Qariar in Northpoint's back alleys. He's hunting down the remaining vampires that were working with Lady Lleraya Montclair.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6904 => array(
			'name'    => "Dragon Frog Dilemma",
			'journal' => "I found a note referencing a plot to steal a dragon frog from a Hammerfell merchant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5055 => array(
			'name'    => "Missive to the Queen",
			'journal' => "A guardswoman in Skywatch has asked me to deliver a sealed message scroll to Vulkhel Guard for her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5070 => array(
			'name'    => "Broken Gazer",
			'journal' => "Something about the way the book was placed in front of the statue makes it appear to be an offering.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5071 => array(
			'name'    => "Curinure's Invitation",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6801 => array(
			'name'    => "Dueling Tributes",
			'journal' => "The Gonfalon Bay Club is offering rewards for any Tales of Tribute players who can win matches against other members of the club.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5074 => array(
			'name'    => "Rudrasa's Invitation",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5075 => array(
			'name'    => "Hilan's Invitation",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6795 => array(
			'name'    => "The Long Way Home",
			'journal' => "I helped Captain Eshmadin gather the lost fragments of a treasure map. Now he asked me to help him collect the treasure.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5076 => array(
			'name'    => "Nemarc's Invitation",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5077 => array(
			'name'    => "Basile's Invitation",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6282 => array(
			'name'    => "A Life in Carvings",
			'journal' => "I found a grave-stake lying next to a note indicating that it was stolen. I should find out who to return the grave-stake to.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5079 => array(
			'name'    => "The Seeker's Archive",
			'journal' => "I met Ibrula, a friend of the Star-Gazers who is trying to get into a ruined temple called the Seeker's Archive. She hopes to discover information inside that will help defeat the Celestials.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6815 => array(
			'name'    => "Seek and Destroy",
			'journal' => "House Dufort needs someone to enter Breakwater Cave on the southern coast of High Isle and destroy the crates of supplies stolen by the Hadolids that reside there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5080 => array(
			'name'    => "The Flower of Youth",
			'journal' => "An aging Bosmer named Hartmin brought his dying husband to a grove near Greenheart where they share memories from their youth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7324 => array(
			'name'    => "Pledge: Black Gem Foundry",
			'journal' => "Urgarlag Chief-bane of the Undaunted challenged me to explore Black Gem Foundry in Coldharbour and put a stop to production there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5081 => array(
			'name'    => "The Fallen City of Shada",
			'journal' => "I found the ruins of a city called Shada's Tear in the middle of the Craglorn desert. A mercenary told me that some researchers from Belkarth went into the ruins, but that strange things started happening when they did.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5085 => array(
			'name'    => "The Trials of Rahni'Za",
			'journal' => "Outside the school of blades called Rahni'Za, I encountered an initiate of the school who told me that the Sword-Disciples have been turned into thralls by a traitor. This betrayer now hides in the cavern at the end of the school's five trials.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5087 => array(
			'name'    => "Assaulting the Citadel",
			'journal' => "People venturing into Hel Ra Citadel have mysteriously vanished, including a group of Undaunted led by Kailstig the Axe. I should enter the Citadel to find out what happened to them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5088 => array(
			'name'    => "Naemon's Return",
			'journal' => "It seems that Prince Naemon has indeed returned from the dead and now leads an undead army that's pushed its way to the city gates of Marbruk.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6310 => array(
			'name'    => "The Lunacy of Two Moons",
			'journal' => "The Khajiiti temple of Two Moons at Tenmar has been afflicted by mass hysteria. The priests blindly follow the bizarre prophecies of their Moon-Bishop, which grow more deranged and deluded by the day.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5091 => array(
			'name'    => "Hallowed To Grimwatch",
			'journal' => "The priestess Kauzanabi-jo told me my next goal should be to help the Lunar Champions Rid-Thar spoke of. I should seek them out at the fort known as Grimwatch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5092 => array(
			'name'    => "The Champions at Rawl'kha",
			'journal' => "Shazah told me to travel to the city of Rawl'kha. There the next steps can be taken to name a new Mane.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5098 => array(
			'name'    => "The Trial of Air",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6829 => array(
			'name'    => "The Final Round",
			'journal' => "I reached the final round of the Tribute Tournament! I will have to summon up all my skill to win the title and take my place as a Tribute Master.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5102 => array(
			'name'    => "The Mage's Tower",
			'journal' => "I've discovered the Aetherian Archive, an enigmatic tower that had previously been sealed. Mighty Mordra and her Undaunted had planned to climb the Archive, but her allies are missing. If I choose to, I could explore it and reach the top before them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5103 => array(
			'name'    => "Mournhold Market Misery",
			'journal' => "A shipment from Artisans Craftworks in Mournhold never made it to the merchant, Blivisi Beleth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6833 => array(
			'name'    => "Bloody Reunion",
			'journal' => "Vampire slayer Mel Adrys has traveled to Western Skyrim in hopes of cleansing the remnants of the Gray Host.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5106 => array(
			'name'    => "Waters Run Foul",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6326 => array(
			'name'    => "An Animal's Grim Abode",
			'journal' => "A young Khajiiti mage named Narama-ko sought my help outside the Abode of Ignominy. Apparently, the notes and memory stones of a powerful mage lie within this crumbling, subterranean temple. She assures me she could put them to good use.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5107 => array(
			'name'    => "Supreme Power",
			'journal' => "Refugees outside of Elinhir told me that the city has been ransacked by mad mages. They sent me to Minerva Lauzon, who is gathering survivors in the sewers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5108 => array(
			'name'    => "Critical Mass",
			'journal' => "A giant object fell from the heavens when the Mage constellation disappeared. Sara Benele is examining the object, which she calls the \"Mage's Staff.\" She believes the staff is unstable and needs my help to fix it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6314 => array(
			'name'    => "Scariest in Show",
			'journal' => "A Khajiit named Tahara asked me to find an escaped monster called Burtle. She promised to pay me handsomely for its safe return.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5111 => array(
			'name'    => "Strange Lexicon",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6847 => array(
			'name'    => "The Hidden Lord",
			'journal' => "Lady Arabelle hopes to finally commence the peace talks, but keeps encountering delays. She has asked me to help her investigate potential threats and ensure the talks progress safely.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5112 => array(
			'name'    => "Message Unknown",
			'journal' => "Hermaeus Mora asked me to bring him unique relics from across Craglorn in exchange for knowledge of the Celestials. Perhaps he'll be interested in this strange writing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5317 => array(
			'name'    => "Broken Promises",
			'journal' => "I found a letter from High-King Emeric offering amnesty to the bandits who reside in the ruins of Watcher's Hold. The <<z:1>> also brought a chest of gold as a sign of the king's good faith.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7356 => array(
			'name'    => "A Call for Arm Cops",
			'journal' => "Armorer Reistaff asked me to craft two arm cops of any kind for soldiers in the Stirk Fellowship. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5113 => array(
			'name'    => "Edge of Darkness",
			'journal' => "Nerien'eth the lich has returned to the Crypt of Hearts to find the ghosts he imprisoned there arrayed against him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6852 => array(
			'name'    => "The Dream of Kasorayn",
			'journal' => "We ended the Dreadsails siege on Vastyr, but at the cost of a great many lives-including Count Mornard and Archdruids Rahval and Ellic. And the Firesong now have all three sacred seeds. We need to find a way to stop Archdruid Orlaith.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5115 => array(
			'name'    => "The Missing Guardian",
			'journal' => "Hara has decided it's time to do something about the third guardian, the Thief. The Thief has been mysteriously absent except for when she appears to speak to me in private.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5120 => array(
			'name'    => "Return to Ash",
			'journal' => "<<1>> attempted to enter an Oblivion Gate with a group of Undaunted to save his wife, <<2>>. He failed. I'm to help him get her back-if she still lives.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5130 => array(
			'name'    => "The Shattered and the Lost",
			'journal' => "I encountered a Star-Gazer who told me that two High Elf women inside the Crossroad Tavern claim to be aspects of the Celestial Mage. I should talk to them and investigate their claims.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6821 => array(
			'name'    => "Ascendant Shadows",
			'journal' => "Two Ascendant Order fanatics threaten travelers in western High Isle. The Ascendant Executioner and the Ascendant Harrower capture their victims with dark magic and subject them to horrible torment.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5151 => array(
			'name'    => "The Truer Fangs",
			'journal' => "Safa al-Satakalaam, a priest of Satakal, is at odds with the Scaled Court. Three of her most powerful acolytes have deserted her to worship the Celestial Serpent. She wants me to punish these traitors and deny their power to the Scaled Court.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5174 => array(
			'name'    => "Taken Alive",
			'journal' => "Iron Orcs, who usually don't take prisoners, have started abducting travelers near the Valley of Scars. I encountered a caravan master who survived an attack, though a number of his guards were dragged off by the Orcs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5194 => array(
			'name'    => "Slithering Brood",
			'journal' => "A woman claiming to be the leader of the Scaled Court requested my help. This Regent Cassipia wants me to help her stop the Scaled Court's process for creating a new type of creature for the Serpent's growing army-the mantikora.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5222 => array(
			'name'    => "Kill Enemy Templars",
			'journal' => "Grand Warlord Zimmeron has authorized a reward for any Pact soldier who slays 20 enemy Templars.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5228 => array(
			'name'    => "Kill Enemy Dragonknights",
			'journal' => "Grand Warlord Zimmeron has authorized a reward for any Pact soldier who slays 20 enemy Dragonknights.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7469 => array(
			'name'    => "Favor for Holgunn One-Eye",
			'journal' => "I agreed to do a favor for Holgunn One-Eye in Ebonheart Pact territory.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5230 => array(
			'name'    => "Kill Enemy Nightblades",
			'journal' => "Grand Warlord Dortene has authorized a reward for any Covenant soldier who slays 20 enemy Nightblades.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6901 => array(
			'name'    => "Imminent Hazard",
			'journal' => "Active volcanic vents threaten the safety of Galen. Druid Aishabeh hired me to gather allies and seal these vents before it's too late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5231 => array(
			'name'    => "Kill Enemy Nightblades",
			'journal' => "Grand Warlord Zimmeron has authorized a reward for any Pact soldier who slays 20 enemy Nightblades.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5234 => array(
			'name'    => "Kill Enemy Sorcerers",
			'journal' => "Grand Warlord Zimmeron has authorized a reward for any Pact soldier who slays 20 enemy Sorcerers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5237 => array(
			'name'    => "Instance Example",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5243 => array(
			'name'    => "A Leaf in the Wind",
			'journal' => "Regent Cassipia has asked me to find her seneschal, Little Leaf, somewhere along the road through the northeast passage. She wants me to offer my aid to Little Leaf and help her foil another one of the Scaled Court's plans.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5246 => array(
			'name'    => "Pledge: Banished Cells II",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to delve the Banished Cells beneath Auridon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5258 => array(
			'name'    => "The Time-Lost Warrior",
			'journal' => "The Celestial Warrior is worried about his champion, Titus Valerius. Lost in time, Titus' memory has begun to break apart. He no longer remembers who he is or his connection to the Warrior. Without his memory, the Warrior can't send him home.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5273 => array(
			'name'    => "Pledge: Spindleclutch II",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to explore the cavern called Spindleclutch beneath Glenumbra.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5274 => array(
			'name'    => "Pledge: Darkshade Caverns I",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to explore the kwama mine called Darkshade Caverns, which lies beneath Deshaan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5282 => array(
			'name'    => "Pledge: Wayrest Sewers II",
			'journal' => "Maj al-Ragath of the Undaunted has challenged me to explore the tunnels and caverns that lie beneath Wayrest, the Veteran Wayrest Sewers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5283 => array(
			'name'    => "Pledge: Crypt of Hearts I",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to explore the ruined halls of the Crypt of Hearts. It lies beneath the stones of Rivenspire, a former school for might and magic.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5284 => array(
			'name'    => "Pledge: Crypt of Hearts II",
			'journal' => "Glirion the Redbeard has challenged me to explore the ruins and hallways of Veteran Crypt of Hearts, beneath Rivenspire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5288 => array(
			'name'    => "Pledge: Arx Corinium",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to explore the ruins of Arx Corinium, the old Imperial pile sinking into the marsh of Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7020 => array(
			'name'    => "Light the Way to Freedom",
			'journal' => "Sharp says we finally have a way forward to find the kidnapped Dimik-ei and stop Sondivel Ulres the Reformer once and for all.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5289 => array(
			'name'    => "Provisioner Certification",
			'journal' => "If I can prove I know my way around a cooking fire, <<1>> will certify me as a provisioner.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5303 => array(
			'name'    => "Pledge: Volenfell",
			'journal' => "Glirion the Redbeard of the Undaunted has challenged me to explore the Dwemer tunnels of Volenfell, deep beneath the stones of Bangkorai.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5310 => array(
			'name'    => "Clothier Certification",
			'journal' => "If I can prove I know how to use a needle and thread, <<1>> will certify me as a clothier.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5313 => array(
			'name'    => "The Gray Passage",
			'journal' => "I found a book describing an ancient Nedic hero named Ranev the Coal-Eyed Wanderer and her legendary pilgrimage along \"The Gray Passage.\" According to the text, those who complete the pilgrimage in the allotted time gain the favor of the Stars.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5316 => array(
			'name'    => "An Unexpected Fall",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5320 => array(
			'name'    => "The Durzog Whistle",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5321 => array(
			'name'    => "A Heart of Brass",
			'journal' => "I met <<1>>, a member of a research expedition looking into the Dwarven ruin of Rkindaleft. While exploring the ruin, they were attacked by strange creatures. I told her I would look for any survivors.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6720 => array(
			'name'    => "Retrieval Gone Awry",
			'journal' => "A previous group of heroes left the Saraathu Tong's liminal rods in a Mantikora's lair. Vaveli Indavel tasked me with destroying the rods and killing Taupezu Azzida, the Mantikora.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5326 => array(
			'name'    => "Quarry Conundrum",
			'journal' => "Several Orc miners in the Graystone Quarry have vanished under mysterious circumstances. A High Elf researcher likely holds the key to unraveling the mystery.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5328 => array(
			'name'    => "Hidden History",
			'journal' => "I met a researcher named <<1>>. He's interested in learning what fundamentally altered the Orcs in <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5335 => array(
			'name'    => "Silver Linings",
			'journal' => "I met a member of the Silver Dawn werewolf hunters who might possibly be the last surviving member of the group. She barely escaped an attack on their headquarters in the Argent Mine by the werewolves of the Magnar Pack.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7068 => array(
			'name'    => "Kill Enemy Arcanists",
			'journal' => "Grand Warlord Zimmeron has authorized a reward for any Pact soldier who slays 20 enemy Arcanists.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5337 => array(
			'name'    => "A Question of Succession",
			'journal' => "A mortal wound to <<1>> has thrown Shatul clan into crisis.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5342 => array(
			'name'    => "Planemeld Obverse",
			'journal' => "The Daedra have occupied the White-Gold Tower, and I have agreed to help Moth Priest Terran Arminus retrieve an Elder Scroll left inside the Tower Library.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5348 => array(
			'name'    => "To Save a Chief",
			'journal' => "High Priestess Solgra sent Eveli to recruit me for some sort of secret mission. It appears to involve a rescue, but she wants to give me the details in person.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5352 => array(
			'name'    => "Into the Maw",
			'journal' => "The Khajiit's ancient enemy, the dro-m'Athra, have emerged from the Dark Behind the World and claimed the Temple of Seven Riddles for the Ghost Moon, Lorkhaj.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5367 => array(
			'name'    => "AvA Quest Branch Example",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6768 => array(
			'name'    => "Blood, Books, and Steel",
			'journal' => "I met with a squire named Rayan who fears for his knight, Dame Madach. Rayan asked if he could accompany me into the tower called the Spire of the Crimson Coin to find his missing knight.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5374 => array(
			'name'    => "Clothier Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Clothier goods to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5377 => array(
			'name'    => "Blacksmith Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Blacksmith goods to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5382 => array(
			'name'    => "Pledge: Imperial City Prison",
			'journal' => "<<1>> has challenged me to explore the Imperial City Prison in the Imperial City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5395 => array(
			'name'    => "Woodworker Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Woodworker goods to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7077 => array(
			'name'    => "Fate of the Forgotten Prince",
			'journal' => "We restored the relics once used to find and imprison Ithelia, and we crafted a new relic, the Mirror of Truth. Now we need to go to Apocrypha, find Ithelia, and end the threat she poses to all of existence.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5407 => array(
			'name'    => "Enchanter Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Enchanter glyphs to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7266 => array(
			'name'    => "Tidewash Stranded",
			'journal' => "A salvage crew sent by the College of Sea and Sword are trapped on a beach with the giant coral crab known as Tidespite. I need to defeat Tidespite and rescue the crew members.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5409 => array(
			'name'    => "Provisioner Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver food and drink to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5412 => array(
			'name'    => "Provisioner Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver food and drink to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5414 => array(
			'name'    => "Provisioner Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver food and drink to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5418 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5422 => array(
			'name'    => "Sightseeing TEST",
			'journal' => "Find the thing for the thing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6981 => array(
			'name'    => "Turning a Page",
			'journal' => "I came across Ysgild and her brother Vorm, who somehow turned himself into a floating book. Ysgild believes that the secret to turning Vorm back into his normal form can be found inside the Disquiet Study.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5439 => array(
			'name'    => "Draugr Dilemma",
			'journal' => "Draugr from a nearby barrow harass the Orcs of Gloorot's clan. A legendary hero named Narsis Dren agreed to help them, but he has yet to return from the ancient barrow.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5446 => array(
			'name'    => "Where Loyalty Lies",
			'journal' => "Lady Sovelle, an ambassador from Daggerfall, was sent to negotiate a trade agreement with the Orcs. But the fleet was blown off course and wrecked on the ice, and now Lady Sovelle is missing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6668 => array(
			'name'    => "Beauty Amid Danger",
			'journal' => "Deetum-Jas asked me to take notes on things I observe inside a Daedric ruin for a local poet who is looking for inspiration.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5449 => array(
			'name'    => "Riekr Revenge",
			'journal' => "Novice hunters from an Orc clan ran into an unusually organized band of Riekrs. Some hunters were captured, while others went to attempt a rescue and never returned. The hunt leader asked me to find the hunters and acquire the Riekr chief's totem.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6793 => array(
			'name'    => "Ferone's Map Fragment",
			'journal' => "I found the body of a Dark Elf, <<1>>. It looks like she died on the way to deliver something to the Dufort Shipyards.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5452 => array(
			'name'    => "Corgrak's Cairn",
			'journal' => "<<1>> built a cairn for his grandfather Corgrak, but couldn't retrieve Corgrak's helm from <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5454 => array(
			'name'    => "Kindred Spirits",
			'journal' => "<<1>> wants to restore the long-lost reputation of Orc cuisine, but her assistant is missing. I agreed to help her until he arrives.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5458 => array(
			'name'    => "In the Name of the King",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6809 => array(
			'name'    => "Arcane Research",
			'journal' => "A mage needs someone to enter the Firepot, a cave to the southwest of All Flags Islet, to help with some sort of arcane research.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5468 => array(
			'name'    => "The Anger of a King",
			'journal' => "King Kurog seems to be in a foul mood, which has affected everyone in the royal court. His shield-wife indicates that tensions at the keep are on the verge of boiling over. I wonder what happened to darken the king's usually boisterous spirits?",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5473 => array(
			'name'    => "Of Brands and Bones",
			'journal' => "Worm Cultists are digging up skeletons in the Memorial District and raising them as undead thralls. Some of these skeletons appear to be searching for something underground. I've agreed to help the Drake of Blades find out what they're looking for.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5474 => array(
			'name'    => "One Ugly Mug",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5480 => array(
			'name'    => "The Bleeding Temple",
			'journal' => "The minions of Molag Bal are rounding up the Imperial City's clergy and mercilessly torturing them. The Drake of Blades has asked for my help in rescuing one of these prisoners-a priest named Egnatius.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6700 => array(
			'name'    => "Against All Hope",
			'journal' => "We defeated Sister Celdina, but Valkynaz Nokvroz captured the Anchorite. He wants to use her blood as fuel for his own cataclyst device. Lyranth thinks Nokvroz is holding her in his fortress, Ardent Hope. We need to rescue her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5481 => array(
			'name'    => "Blood on a King's Hands",
			'journal' => "All of the paths into the moot have been sealed except the one through the temple in Orsinium. Chief Bazrag wants me to get to the temple, help High Priestess Solgra, and then reach the moot before Kurog assassinates the clan chiefs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5483 => array(
			'name'    => "The Imperial Standard",
			'journal' => "I met a scout who serves an Imperial resistance fighter named Captain Caudex. The captain, and a small group of legionary loyalists, have held a garrison at the heart of the Nobles District for nearly two months.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6825 => array(
			'name'    => "Pirate Problems",
			'journal' => "Pirates from Shipwreck Shoals have stolen crates of uniforms from alliance ships that crashed along the coastlines between High Isle and Amenos. An agent representing all the alliances wants the uniforms found and destroyed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5484 => array(
			'name'    => "Of Sentimental Value",
			'journal' => "One of the survivors of the frozen fleet asked me to keep an eye open for a gold pendant he lost. It was meant to be a gift for his betrothed when they are finally reunited.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5487 => array(
			'name'    => "City on the Brink",
			'journal' => "The Ebonheart Pact is locked in an epic struggle for control of the Imperial City. The forces of Molag Bal hold the city in an iron grip, but enemy bannermen also vie for power. The Pact needs my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5491 => array(
			'name'    => "Speaking For The Dead",
			'journal' => "<<1>> has a strange plan to spy on the Worm Cult, but he needs help to see it through.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5496 => array(
			'name'    => "City on the Brink",
			'journal' => "The Aldmeri Dominion is locked in an epic struggle for control of the Imperial City. The forces of Molag Bal hold the city in an iron grip, but enemy bannermen also vie for power. The Dominion needs my help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5498 => array(
			'name'    => "Historical Accuracy",
			'journal' => "A concerned clerk named <<1>> believes there is a grave threat to the well-being of future generations if the Imperial recordkeeping falls to war.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7362 => array(
			'name'    => "The Mudman",
			'journal' => "I met a Wood Elf in Grahtwood trying to construct a Mudman effigy for the New Life festival to bless all of Tamriel's green. I agreed to help her gather mud and complete the effigy since her kin aren't there to help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5505 => array(
			'name'    => "Fire in the Hold",
			'journal' => "The perfidious occupants of Watcher's Hold have regrouped. Even now, they stockpile supplies in preparation for more attacks on the people of Wrothgar. Sergeant Oufa has signed a contract to ensure that the situation is dealt with.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7233 => array(
			'name'    => "Infinite Avenger: Fabled",
			'journal' => "A ghost named Esalinwe lost all her threads of fate in the Infinite Archive. She wants retribution, and asked me to kill the Fabled maligraphies who had a hand in causing her death.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5506 => array(
			'name'    => "Scouting the Memorial District",
			'journal' => "My Alliance needs scouts to secure rally points across the Imperial City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5507 => array(
			'name'    => "Breakfast of the Bizarre",
			'journal' => "The Dragonstar Caravan Company job broker, Guruzug, has tasked me with retrieving some mysterious eggs from Coldperch Cavern.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5511 => array(
			'name'    => "Scouting the Elven Gardens",
			'journal' => "My Alliance needs scouts to secure rally points across the Imperial City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6732 => array(
			'name'    => "Uxark's Treasure",
			'journal' => "Nass told me that the planestones I brought to her open a portal to the hidden and valuable treasure of Uxark, a Xivilai warrior.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5513 => array(
			'name'    => "Scouting the Temple District",
			'journal' => "My Alliance needs scouts to secure rally points across the Imperial City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5519 => array(
			'name'    => "Scholarly Salvage",
			'journal' => "A caravan bringing books to Orsinium has been waylaid by ogres. The books have likely been torn to pieces. If I'm to reassemble them for their rightful owner, these creatures will need to be dealt with.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7249 => array(
			'name'    => "Support the Fellowship",
			'journal' => "Skordo the Knife called on me to help aid the war effort against the Worm Cult. The Stirk Fellowship established camps along the Writhing Wall in Solstice to hold back the cultists. They need material support in order to break through the wall.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5522 => array(
			'name'    => "Heresy of Ignorance",
			'journal' => "The Worm Cult has established a foothold in Wrothgar and seeks to summon a beast called Zandadunoz the Reborn. The king's servant, Arzorag, demands that I disrupt their scheme.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5523 => array(
			'name'    => "Snow and Steam",
			'journal' => "A Khajiit named Birkhu the Bold has stumbled upon a number of dangerous Dwarven mechanisms, including the massive Dwarven Centurion, Nyzchaleft. These machines pose a threat to Wrothgar and must be dealt with.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5532 => array(
			'name'    => "The Long Game",
			'journal' => "<<1>> has concerns about an ambitious merchant lord named <<2>>. She wants to learn if his interests overlap with those of the Thieves Guild.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7265 => array(
			'name'    => "Guardians Gone Bad",
			'journal' => "A thief stole ancient relics, angering the voskrona guardians of the Shrine of Vakkan. I need to return the stolen relics to their proper places, and destroy any hostile voskrona guardians I find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5538 => array(
			'name'    => "Voices in the Dark",
			'journal' => "Rumors abound that the Dark Brotherhood, assassins of legend, are on the lookout for members worthy to wield their killing blade. This might be why so many innocents have been murdered of late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5540 => array(
			'name'    => "Signed in Blood",
			'journal' => "To prove myself to the Dark Brotherhood, Speaker Terenus wants me to murder the Imperial noble named Quintus Jarol.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5543 => array(
			'name'    => "Shell Game",
			'journal' => "Walks-Softly needs help dealing with a personal and delicate situation that involves an old friend from his past.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5547 => array(
			'name'    => "The Vampire's Prey",
			'journal' => "Vampire hunter Mel Adrys has come to Hrota Cave to track down the dangerous vampire lord Zalar-do, who has been preying on the people of Anvil. I promised to help him slay the vampire and rescue the victims she kidnapped.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6889 => array(
			'name'    => "The Moth Study",
			'journal' => "Druid Gastoc asked that I kill the forest wraith in the Valley of the Watcher and gather some radiant phoenix moths for a friend of his to study.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5548 => array(
			'name'    => "Debts of War",
			'journal' => "Razum-dar tracked an Imperial war criminal to the Garlas Agea ruins to make her pay for attacking innocent noncombatants in Bravil. I agreed to help him dispatch this Captain Jena Apinia and recover the items they stole.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5554 => array(
			'name'    => "The Dark Moon's Jaws",
			'journal' => "A Khajiit named Bakhum is worried about a family member who has been writing him unsettling letters about the \"Maw of Lorkhaj.\" He has asked me and eleven friends to help escort him to the Temple of Seven Riddles to make sure everything is all right.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7419 => array(
			'name'    => "Wolf Trap",
			'journal' => "I helped the three gang leaders come up with a plan to set a trap for the Koldane Cartel and convinced Captain Sansonne and the Lion Guard to assist. Now we just need to put everything in motion and see what happens.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5572 => array(
			'name'    => "Heist: The Hideaway",
			'journal' => "I was tipped off about treasures stashed in The Hideaway. I don't have much time to secure the goods before they're moved, so I should act quickly on this lead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5573 => array(
			'name'    => "Heist: Underground Sepulcher",
			'journal' => "I was tipped off about treasures stashed in Underground Sepulcher. I don't have much time to secure the goods before they're moved, so I should act quickly on this lead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6796 => array(
			'name'    => "The All Flags Curse",
			'journal' => "Stibbons has asked me to help him free his mistress, Lady Clarisse Laurent, from a curse that plagues All Flags Islet. Apparently, this site was once a monument dedicated to the All Flags Navy, but it has fallen into disrepair.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5577 => array(
			'name'    => "Heist: Secluded Sewers",
			'journal' => "I was tipped off about treasures stashed in the Secluded Sewers. I don't have much time to secure the goods before they're moved, so I should act quickly on this lead.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5581 => array(
			'name'    => "That Which Was Lost",
			'journal' => "Another note from Velsa. It seems Narahni is in need of assistance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7309 => array(
			'name'    => "Fleeing the Shadows",
			'journal' => "I met Nalgorfin, the leader of a band of Shadow Walkers who escaped from Coldharbour. Most of the clan was captured by the Worm Cult when they arrived. Nalgorfin wants my help to rescue them in Calindvale Gardens.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6976 => array(
			'name'    => "Conclave of Fate",
			'journal' => "After the events in the Infinite Panopticon, our only real lead is Master Shelreni and the Black Book she stole from Meln. I need to return to the Telvanni Peninsula and start looking into her dealings with the Hidden Kindred.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5582 => array(
			'name'    => "Master of Heists",
			'journal' => "Velsa let me in on one of the perks of being a member of the Thieves Guild: Heists. She's sending me to the guild's heist master to get vetted for work.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7313 => array(
			'name'    => "Roots in Stone",
			'journal' => "Mani-Kai, a Bright-Throat Argonian aiding the Stirk Fellowship, wishes to speak with the leader of Xor-Hist, but the village is under threat of a Worm Cult attack. She needs my help to light the village's signal pyres.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5586 => array(
			'name'    => "The Lost Pearls",
			'journal' => "<<1>> runs a finder's business and uses the Thieves Guild to track down his quarries. Helping him will surely reflect well on my skill as a thief and be lucrative to boot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5587 => array(
			'name'    => "Thrall Cove",
			'journal' => "<<1>> runs a finder's business and uses the Thieves Guild to track down his quarries. Helping him will surely reflect well on my skill as a thief, and be lucrative to boot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5591 => array(
			'name'    => "Contract: Glenumbra",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7327 => array(
			'name'    => "The Rogue Mage",
			'journal' => "Vanus Galerion has summoned me to the Aetherian Archive. A rogue mage has cracked the wards protecting the space, and now threatens to claim its power for his own.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5592 => array(
			'name'    => "Contract: Glenumbra",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6812 => array(
			'name'    => "Venting the Threat",
			'journal' => "Druid Peeska is worried about the increasing activity at the volcanic vents on the island. She hired me to gather allies and disrupt the activity before the volcanic vents can erupt and the situation becomes dire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5593 => array(
			'name'    => "Contract: Glenumbra",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5595 => array(
			'name'    => "A Lesson in Silence",
			'journal' => "Recent lectures in Kvatch by the Grand Sermonizer of Akatosh and her priests paint the Dark Brotherhood as powerless pretenders not to be feared. Astara wants the Grand Sermonizer dealt with-permanently.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5596 => array(
			'name'    => "A Special Request",
			'journal' => "Governor Fortunata of Anvil has taken a contract with the Dark Brotherhood. She's added a special request, however. She wants to meet with the assassin personally before completing the deal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7325 => array(
			'name'    => "A Study in Discipline",
			'journal' => "<<1>>, a seasoned adventurer, claims to know the ways of Tamriel's many mystical and martial disciplines. She has offered to teach me how to teach myself, to open up my mind to new possibilities.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6992 => array(
			'name'    => "Forgotten Glories",
			'journal' => "Dreamcarvers have invaded the Remnants' fortress, the Cenotaph. The attackers seek a pact that provides the Remnant clan its power. I agreed to purge the Dreamcarvers from the Cenotaph and defend their pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5598 => array(
			'name'    => "The Wrath of Sithis",
			'journal' => "The Black Hand has decreed that Lyra the Black Dragon must die for her crimes against the Brotherhood. Green-Venom-Tongue has gone to Knightsgrave to perform the task. Astara has asked me to back him up.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5600 => array(
			'name'    => "Filling the Void",
			'journal' => "Speaker Terenus wants to cut off the head of the group that has been moving against the Dark Brotherhood-he wants us to kill Artorius, the Primate of Akatosh. He wants the death to happen quickly, before the Order of the Hour attacks the Sanctuary.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7329 => array(
			'name'    => "Hunt for the Great Mage",
			'journal' => "After breaking the Worm Cult's defenses at Gristmung Hold, it may finally be time to turn our attention to finding and rescuing Vanus Galerion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5602 => array(
			'name'    => "City on the Brink",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5603 => array(
			'name'    => "Buried Evil",
			'journal' => "Cleric Arvina in Anvil has posted a bounty for any group brave enough to enter Hrota Cave and destroy Exulus, a wispmother claiming the cave as her own. He also asked that the pauldrons of his fallen warriors be retrieved.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7323 => array(
			'name'    => "Deadly Armaments",
			'journal' => "I met Dalenor the Summoner outside of a portal to Coldharbour. He barely escaped the Black Gem Foundry, a forge that manufactures soul gem-studded arms and armor for the Worm Cult invasion on Solstice. I agreed to help him shut down the foundry.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5604 => array(
			'name'    => "The Common Good",
			'journal' => "I accepted a contract to destroy the beast named <<1>>. It's being held captive deep in the ruins of Garlas Agea by the Gold Coast Trading Company. Additionally, the job calls for me to appropriate company ledgers from their operation there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5606 => array(
			'name'    => "The Roar of the Crowds",
			'journal' => "The arena master has posted an open challenge for warriors brave enough to fight one of the top gladiators to the death in Kvatch Arena.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5609 => array(
			'name'    => "Plucking Fingers",
			'journal' => "Bards and entertainers are failing to pay their dues and badmouthing the guild's members. The Thieves Guild wants to remind them why they pay protection. I should look for games, musical instruments, or children's toys, but any treasures will do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5610 => array(
			'name'    => "Idle Hands",
			'journal' => "The guild has had laborers around their territory failing to pay their dues and badmouthing its members. The Thieves Guild wants to remind them why they pay protection. I should look for smithing, fishing, or other tools, but any treasures will do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7341 => array(
			'name'    => "Dominion Heavy Pelts",
			'journal' => "<<1>> asked me to gather heavy pelts from rare beasts within Aldmeri Dominion territories.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5614 => array(
			'name'    => "Contract: Stormhaven",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5617 => array(
			'name'    => "Contract: Rivenspire",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5618 => array(
			'name'    => "Contract: Stormhaven",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5619 => array(
			'name'    => "Contract: Stormhaven",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7339 => array(
			'name'    => "Covenant Undead Essences",
			'journal' => "<<1>> asked me to gather undead essences from the territories of the Daggerfall Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5620 => array(
			'name'    => "Contract: Stormhaven",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7023 => array(
			'name'    => "Adversarial Adventures",
			'journal' => "Azandar asked to continue our explorations into his frayed fates. The arcanist wants more information on the mysterious entity he refers to as his adversary.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5621 => array(
			'name'    => "Contract: Stormhaven",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5623 => array(
			'name'    => "Brotherhood Contract",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5625 => array(
			'name'    => "Contract: Alik'r Desert",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7364 => array(
			'name'    => "Betnihk's Boon",
			'journal' => "I met a group of Orcs in Stonetooth Fortress breaking boulders for Betnikh's Boon during the New Life Festival. Their leader proposed I search an area where he fought Daedra for a stronger boulder to break. I accepted his challenge.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5627 => array(
			'name'    => "Contract: Alik'r Desert",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7357 => array(
			'name'    => "A Call for Jacks",
			'journal' => "Armorer Reistaff asked me to craft two jacks of any kind for soldiers in the Stirk Fellowship. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7024 => array(
			'name'    => "Tempting Fates",
			'journal' => "It's time to end the Fateweaver experiment. Azandar al-Cybiades wants to confront his adversary, and end him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5630 => array(
			'name'    => "Contract: Bangkorai",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5633 => array(
			'name'    => "Contract: Bangkorai",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5634 => array(
			'name'    => "Litany of Blood",
			'journal' => "The Listener sent a mysterious Dark Elf to the Sanctuary to deliver a strange tome by the statue of the Night Mother. She mentioned that it was meant for the killer who tasted death.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5636 => array(
			'name'    => "Pledge: Ruins of Mazzatun",
			'journal' => "<<1>> of the Undaunted has challenged me to explore the Ruins of Mazzatun.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5639 => array(
			'name'    => "Idle Hands",
			'journal' => "The guild has had laborers around their territory failing to pay their dues and badmouthing its members. The Thieves Guild wants to remind them why they pay protection. I should look for smithing, fishing, or other tools, but any treasures will do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5640 => array(
			'name'    => "Idle Hands",
			'journal' => "The guild has had laborers around their territory failing to pay their dues and badmouthing its members. The Thieves Guild wants to remind them why they pay protection. I should look for smithing, fishing, or other tools, but any treasures will do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5641 => array(
			'name'    => "Plucking Fingers",
			'journal' => "Bards and entertainers are failing to pay their dues and badmouthing the guild's members. The Thieves Guild wants to remind them why they pay protection. I should look for games, musical instruments, or children's toys, but any treasures will do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5642 => array(
			'name'    => "Plucking Fingers",
			'journal' => "Bards and entertainers are failing to pay their dues and badmouthing the guild's members. The Thieves Guild wants to remind them why they pay protection. I should look for games, musical instruments, or children's toys, but any treasures will do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5643 => array(
			'name'    => "Under Our Thumb",
			'journal' => "Some law-abiding citizens have been meddling in the affairs of the Thieves Guild. The guild wants its pickpockets to teach the meddlers to mind their own business. I should look for ornaments, writings, or other oddities, but any treasures will do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6473 => array(
			'name'    => "Crisis at Dragon Bridge",
			'journal' => "Supplies for the Dragon Bridge Garrison keep mysteriously disappearing, which puts a strain on their operations. I should investigate and see what I can find out.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5644 => array(
			'name'    => "Under Our Thumb",
			'journal' => "Some law-abiding citizens have been meddling in the affairs of the Thieves Guild. The guild wants its pickpockets to teach the meddlers to mind their own business. I should look for ornaments, writings, or other oddities, but any treasures will do.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5645 => array(
			'name'    => "Crime Spree",
			'journal' => "Rumors are floating around that the Thieves Guild isn't a threat anymore. The Thieves Guild wants to quash the rumors with a series of conspicuous thefts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6293 => array(
			'name'    => "Unsuitable Suitors",
			'journal' => "As the Bright-Throat tribe prepares to commence their most important ritual of the year, things may be in for a rocky start as none of their visiting guests seem in any hurry to participate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5647 => array(
			'name'    => "Crime Spree",
			'journal' => "Rumors are floating around that the Thieves Guild isn't a threat anymore. The Thieves Guild wants to quash the rumors with a series of conspicuous thefts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5649 => array(
			'name'    => "Contract: Deshaan",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5650 => array(
			'name'    => "Contract: Deshaan",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5651 => array(
			'name'    => "Contract: Deshaan",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5652 => array(
			'name'    => "Contract: Deshaan",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5653 => array(
			'name'    => "Contract: Eastmarch",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5654 => array(
			'name'    => "Contract: Auridon",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5658 => array(
			'name'    => "Contract: Auridon",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5659 => array(
			'name'    => "Contract: Grahtwood",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5661 => array(
			'name'    => "Contract: Grahtwood",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5662 => array(
			'name'    => "Contract: Malabal Tor",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5663 => array(
			'name'    => "Contract: Malabal Tor",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5665 => array(
			'name'    => "Contract: Malabal Tor",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5666 => array(
			'name'    => "Contract: Malabal Tor",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5668 => array(
			'name'    => "The Cutpurse's Craft",
			'journal' => "Before the Thieves Guild sends me after more lucrative targets, I need to prove I know how to pick pockets.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5669 => array(
			'name'    => "Contract: Eastmarch",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5671 => array(
			'name'    => "Contract: Grahtwood",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5673 => array(
			'name'    => "Contract: Greenshade",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5674 => array(
			'name'    => "Contract: Greenshade",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5675 => array(
			'name'    => "Contract: Reaper's March",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6505 => array(
			'name'    => "Method and Madness",
			'journal' => "Gwendis is investigating the schemes of a mad alchemist named Arkasis and his secret laboratory known only as the Stone Garden. I agreed to help Gwendis discover what Arkasis is up to and stop him if necessary.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5676 => array(
			'name'    => "Contract: Greenshade",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5678 => array(
			'name'    => "Contract: Reaper's March",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6325 => array(
			'name'    => "Home Sweet Home",
			'journal' => "Sweetwater Farmstead in Merryvale seeks workers to replace those that have gone missing. I should investigate this mystery and see if I can help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5679 => array(
			'name'    => "Contract: Reaper's March",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5683 => array(
			'name'    => "Contract: Shadowfen",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5684 => array(
			'name'    => "Contract: Shadowfen",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5689 => array(
			'name'    => "Contract: Stonefalls",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5690 => array(
			'name'    => "Contract: Stonefalls",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5691 => array(
			'name'    => "Contract: The Rift",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5693 => array(
			'name'    => "Contract: Alik'r Desert Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of the Alik'r Desert.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5694 => array(
			'name'    => "Contract: Auridon Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Auridon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6213 => array(
			'name'    => "Kill 150 Enemy Players",
			'journal' => "Grand Warlord Sorcalin has authorized a bonus for any Dominion soldier who slays 150 enemy combatants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5695 => array(
			'name'    => "Contract: The Rift",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5697 => array(
			'name'    => "Contract: The Rift",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5698 => array(
			'name'    => "Contract: Deshaan Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Deshaan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5700 => array(
			'name'    => "Contract: Glenumbra Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Glenumbra.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5702 => array(
			'name'    => "Silk and Shadow",
			'journal' => "An Argonian Shadowscale named Tsatva-Lan went missing while investigating the lair of a rival group of assassins known as the Silken Ring. His former student, Hides-the-Ashes, asked me to help her track down her mentor and rescue him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5703 => array(
			'name'    => "Contract: Greenshade Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Greenshade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6412 => array(
			'name'    => "Skooma Sequela",
			'journal' => "A band of smugglers have turned the sacred Moonlit Cove into a skooma distillery and poisoned the waters with their shoddy spirits.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5705 => array(
			'name'    => "Contract: Reaper's March Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Reaper's March.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5706 => array(
			'name'    => "Contract: Rivenspire Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Rivenspire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5707 => array(
			'name'    => "Contract: Shadowfen Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6537 => array(
			'name'    => "Potent Poison",
			'journal' => "I found a strange note. The author promises a reward if I return it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5708 => array(
			'name'    => "Contract: Kvatch",
			'journal' => "I agreed to be initiated into the ways of the Dark Brotherhood's murder-for-hire contracts. There are no stipulations regarding methods or timing, so I may execute the target at my discretion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6357 => array(
			'name'    => "Dragon Hunt",
			'journal' => "To combat the Dragon menace, I accepted a contract from Battlereeve Tanerline to kill them at their hunting grounds.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5711 => array(
			'name'    => "Contract: The Rift Spree",
			'journal' => "I accepted one of the Dark Brotherhood's murder-for-hire contracts, which requests the indiscriminate slaughter of citizens of the Rift.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5713 => array(
			'name'    => "Sacrament: Trader's Cove",
			'journal' => "I received a contract from the Speaker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5714 => array(
			'name'    => "Sacrament: Trader's Cove",
			'journal' => "I received a contract from the Speaker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6553 => array(
			'name'    => "Help Wanted in Markarth",
			'journal' => "A notice indicates that someone seeks adventurers in Markarth for dangerous work.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5724 => array(
			'name'    => "Sacrament: Sewer Tenement",
			'journal' => "I received a contract from the Speaker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5726 => array(
			'name'    => "Sacrament: Sewer Tenement",
			'journal' => "I received a contract from the Speaker.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5733 => array(
			'name'    => "Ancient Armaments in Bangkorai",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in the Crypt of the Exiles, located in Bangkorai.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5734 => array(
			'name'    => "The Spirit Trap in Malabal Tor",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Shael Ruins, located in Malabal Tor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7450 => array(
			'name'    => "The Finer Things",
			'journal' => "Tachien asked me to help some factionless individuals in the Night Market's districts. She said that was the best way for me to prove my skills and help get the Glittering Goad closer to their goals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5735 => array(
			'name'    => "Ancestor Wards in Deshaan",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in the Corpse Garden, located in Deshaan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6444 => array(
			'name'    => "Dawn of the Dragonguard",
			'journal' => "I accepted a dangerous bounty to kill a Dragon!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5737 => array(
			'name'    => "Dwarven Relics of Stonefalls",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in the Inner Sea Armature, located in Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5738 => array(
			'name'    => "Ayleid Trinkets in Grahtwood",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Wormroot Depths, located in Grahtwood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5739 => array(
			'name'    => "Red Rook Ransack in Glenumbra",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Ilessan Tower, located in Glenumbra.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5742 => array(
			'name'    => "The Witchmother's Bargain",
			'journal' => "I summoned one of the fabled witchmothers, who are rumored to appear only during the Witches Festival! Legends say these strange beings can be bargained with for power.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6389 => array(
			'name'    => "Ruddy Fang Retrieval",
			'journal' => "An Imperial named <<1>> asked me to recover her stolen belongings from a nearby camp of Ruddy Fang bandits.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5743 => array(
			'name'    => "The Oldest Ghost",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5745 => array(
			'name'    => "Molten Pearls of Alik'r Desert",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Aldunz, located in Alik'r Desert.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5746 => array(
			'name'    => "The Corrupted Stone",
			'journal' => "The guardian star sign of the Warrior has failed to appear to protect the mortal world from the Serpent. One of the Star-Gazers needs help uncovering what might be happening.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5747 => array(
			'name'    => "The Star-Gazers",
			'journal' => "A Herald has come with tales of a dire threat in the distant land of Craglorn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5748 => array(
			'name'    => "The Warrior's Call",
			'journal' => "I met a strange Imperial who was being held captive by the Daggerfall Covenant. He believes he knows how to defeat the cursed army of Anka-Ra that has laid siege to Craglorn since the Celestials appeared.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5754 => array(
			'name'    => "Waters Run Foul",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5755 => array(
			'name'    => "Supreme Power",
			'journal' => "Refugees outside of Elinhir told me that the city has been ransacked by mad mages. They sent me to Minerva Lauzon, who is gathering survivors in the sewers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6585 => array(
			'name'    => "Discarded Treasures",
			'journal' => "A scout of the Ard's Stonehands encountered a new type of Dwarven automaton in a nearby cave that appeared to be dropping scrap metal. The Stonehands asked me to investigate. I need to destroy the construct and gather the scrap.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5756 => array(
			'name'    => "Critical Mass",
			'journal' => "A giant object fell from the heavens when the Mage constellation disappeared. Sara Benele is examining the object, which she calls the \"Mage's Staff.\" She believes the staff is unstable and needs my help to fix it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5757 => array(
			'name'    => "Gem of the Stars",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5758 => array(
			'name'    => "Strange Lexicon",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6277 => array(
			'name'    => "The Assassin's Arbitration",
			'journal' => "A local dispute over a saltrice farm has now gotten to the point where Sulahkeesh, a former Shadowscale, must arbitrate for a resolution to be found. I volunteered to ask the former assassin for her help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5759 => array(
			'name'    => "Message Unknown",
			'journal' => "Hermaeus Mora asked me to bring him unique relics from across Craglorn in exchange for knowledge of the Celestials. Perhaps he'll be interested in this strange writing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5762 => array(
			'name'    => "The Reason We Fight",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5763 => array(
			'name'    => "The Shattered and the Lost",
			'journal' => "I encountered a Star-Gazer who told me that two High Elf women inside the Crossroad Tavern claim to be aspects of the Celestial Mage. I should talk to them and investigate their claims.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5766 => array(
			'name'    => "Iron and Scales",
			'journal' => "I met an Iron Orc shaman who had been banished by a younger rival. What's more, this rival, named Agganor, seems to be responsible for the Iron Orc and Scaled Court alliance. The old shaman wants me to avenge him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6476 => array(
			'name'    => "Dark Clouds Over Solitude",
			'journal' => "The Icereach Coven and its allies can call down devastating and unnatural harrowstorms on Western Skyrim. Fennorian thinks these storms can turn people into harrowfiends and mindless harrowed. We need to inform Solitude.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5769 => array(
			'name'    => "The Serpent's Fang",
			'journal' => "A Wood Elf named Little Leaf claims she's on a mission to stop the Iron Orcs from building a troll army. She tried to appeal to the Orc leaders, but diplomacy didn't work. Now she wants me to come along and try a more forceful approach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5770 => array(
			'name'    => "Souls of the Betrayed",
			'journal' => "A devout knight of the Divines discovered that the dead have awakened in the depths of Skyreach Catacombs. If the legend of Virmaril is true, then a powerful Elven necromancer could be the source of the trouble.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6144 => array(
			'name'    => "Pearls Before Traitors",
			'journal' => "Captain Telomure has asked me to destroy a pearl that's been stolen by a member of the Divine Prosecution, Justiciar Avanaire. The pearl is somewhere in Tor-Hame-Khard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5774 => array(
			'name'    => "A Leaf in the Wind",
			'journal' => "Regent Cassipia has asked me to find her seneschal, Little Leaf, somewhere along the road through the northeast passage. She wants me to offer my aid to Little Leaf and help her foil another one of the Scaled Court's plans.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5775 => array(
			'name'    => "Holding Court",
			'journal' => "Little Leaf has asked me to find her friend, Regent Cassipia, in Dragonstar. She claims that the two of them are working from the inside to stop the Scaled Court.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5777 => array(
			'name'    => "The Gray Passage",
			'journal' => "I found a book describing an ancient Nedic hero named Ranev the Coal-Eyed Wanderer and her legendary pilgrimage along \"The Gray Passage.\" According to the text, those who complete the pilgrimage in the allotted time gain the favor of the Stars.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5778 => array(
			'name'    => "Give and Take in Shadowfen",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Atanaz Ruins, located in Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5779 => array(
			'name'    => "Icy Intrigue in Eastmarch",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Chill Hollow, located in Eastmarch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5780 => array(
			'name'    => "Pledge: Cradle of Shadows",
			'journal' => "<<1>> of the Undaunted has challenged me to explore the Cradle of Shadows.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5787 => array(
			'name'    => "Dark Anchors in Bangkorai",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Bangkorai.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6617 => array(
			'name'    => "Weapons of Destruction",
			'journal' => "The presence of the Ambitions in Gideon has painted a target on the city. I need to find a way to keep Sombren, Calia, and Destron safe while also drawing the danger away from Gideon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5788 => array(
			'name'    => "Dark Anchors in Stonefalls",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Stonefalls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6160 => array(
			'name'    => "Struck from Memory",
			'journal' => "Dangerous Worm Cult tomes are circulating throughout Summerset. The Office of Divine Prosecution wants some of these forbidden books recovered and disposed of.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5790 => array(
			'name'    => "Dark Anchors in Shadowfen",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5791 => array(
			'name'    => "Dark Anchors in Eastmarch",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Eastmarch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5794 => array(
			'name'    => "Dark Anchors in Grahtwood",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Grahtwood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5796 => array(
			'name'    => "Dark Anchors in Greenshade",
			'journal' => "<<1>> of the Fighters Guild learned of Worm Cult activity in Greenshade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5798 => array(
			'name'    => "Veiled Darkness in Auridon",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in <<2>>, located in Auridon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5799 => array(
			'name'    => "A Hireling of House Telvanni",
			'journal' => "Two Argonian slaves named Eoki and Sun-in-Shadow have hatched a plot to win their freedom from the Dark Elves of House Telvanni.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5802 => array(
			'name'    => "Inflamed Pyres of the Rift",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Shroud Hearth Barrow, located in the Rift.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5811 => array(
			'name'    => "Snow Bear Plunge",
			'journal' => "<<1>>, a New Life Herald, spoke of a Lower Yorgrim New Life Festival in Eastmarch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5819 => array(
			'name'    => "Madness in Deshaan",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Deshaan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6649 => array(
			'name'    => "Goblin Research",
			'journal' => "The Toad-Tongue Goblin tribe took over an old ruin. And they ate a scholar who ventured in to study them. I agreed to confront War Chief Zathmoz and recover any notes I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5820 => array(
			'name'    => "Madness in Eastmarch",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Eastmarch.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6192 => array(
			'name'    => "Woe of the Welkynars",
			'journal' => "Z'Maja, a powerful Sload sorceress, has filled the city of Cloudrest with an unnatural shadow which saps the life of all it touches. I've agreed to help Olorime defeat Z'Maja and save the city.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5822 => array(
			'name'    => "Madness in Glenumbra",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Glenumbra.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5824 => array(
			'name'    => "Madness in Greenshade",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Greenshade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5826 => array(
			'name'    => "Madness in Reaper's March",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Reaper's March.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5827 => array(
			'name'    => "Madness in Rivenspire",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Rivenspire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5828 => array(
			'name'    => "Madness in Shadowfen",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Shadowfen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5830 => array(
			'name'    => "Madness in Stormhaven",
			'journal' => "<<1>> of the Mages Guild wants me to recover a dangerous relic from Stormhaven.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5832 => array(
			'name'    => "Like Blood from a Stone",
			'journal' => "A Dark Elf businessman named Malur Rethan is convinced that his brother and partner, Benar, is hiding something in the Vassir-Didanat ebony mine. He asked me to investigate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5834 => array(
			'name'    => "The Trial of Five-Clawed Guile",
			'journal' => "<<1>>, a New Life Herald in Eastmarch, spoke of <<2>>'s New Life Festival in Reaper's March.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5836 => array(
			'name'    => "Bound by Love",
			'journal' => "Llonas Givyn desires to marry his love, Tirwin. Unfortunately, Tirwin is a slave belonging to Llonas's mother, Faras, who disapproves of the pairing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6208 => array(
			'name'    => "Capture Any Nine Resources",
			'journal' => "I accepted a mission to help capture farms, lumber mills, and mines for the Pact. Conquering any nine of these resources will complete my mission.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5838 => array(
			'name'    => "Mud Ball Merriment",
			'journal' => "<<1>>, a New Life Herald in Eastmarch, spoke of a Skywatch New Life Festival in Auridon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5839 => array(
			'name'    => "Signal Fire Sprint",
			'journal' => "<<1>>, a New Life Herald in Eastmarch, spoke of <<2>>'s New Life Festival in Alik'r Desert.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5841 => array(
			'name'    => "At Any Cost",
			'journal' => "Mistress Dratha, having woken from a crippling illness, made it clear that she wants to beat death. Seven Daedric relics called the Stones of Cold Fire may be the key to achieving immortality.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5852 => array(
			'name'    => "War Orphan's Sojourn",
			'journal' => "<<1>>, a New Life Herald in Eastmarch, spoke of <<2>>'s New Life Festival in Grahtwood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5853 => array(
			'name'    => "Culinary Justice in Greenshade",
			'journal' => "<<1>>, one of the Undaunted, gave me a challenge to complete in Naril Nagaia, located in Greenshade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5856 => array(
			'name'    => "Stonetooth Bash",
			'journal' => "<<1>>, a New Life Herald in Eastmarch, spoke of the New Life Festival in Betnikh.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5857 => array(
			'name'    => "A Smuggler's Last Stand",
			'journal' => "The feared Camonna Tong crime syndicate attacked a group of people smugglers at Khartag Point. Khartag and a group of slaves retreated to a nearby cavern. They'll be killed if help doesn't arrive soon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5859 => array(
			'name'    => "Rising to Retainer",
			'journal' => "The recently freed Argonian slave, Sun-in-Shadow, seeks to continue her climb through the ranks of House Telvanni. She needs my help to claim the next title.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5862 => array(
			'name'    => "The Scarlet Judge",
			'journal' => "According to a masked vigilante known as the Scarlet Judge, a corrupt marshal named Hlaren is oppressing the citizens of Suran. I must aid the Judge in rooting out this corruption.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5864 => array(
			'name'    => "Nothing to Sneeze At",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5866 => array(
			'name'    => "Oxen Free",
			'journal' => "I've been hired to defeat a nix-ox that has been eating mushrooms the wizard Bravosi Felder is attempting to research. I'm also required to collect samples of the mushrooms while in the area.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5876 => array(
			'name'    => "A Dangerous Breed",
			'journal' => "A mad breedsman named Drovos Nelvayn has created a dangerous new genus of kwama. These insects attack without provocation and resist any attempts at domestication. If the creatures escape, native kwama and the miners that tend them will suffer.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5879 => array(
			'name'    => "The New Life Festival",
			'journal' => "<<1>>, a New Life Herald in Eastmarch, spoke of the New Life Festival celebrated across Tamriel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6588 => array(
			'name'    => "Old Life Observance",
			'journal' => "<<1>> told me about the Old Life tradition in which people write messages to those that have passed on and burn them at special shrines.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5881 => array(
			'name'    => "A Hidden Harvest",
			'journal' => "According to a Khajiiti farmer named Halinjirr, a flock of bandits claimed the cavern of Zainsipilu and seized his stock of valuable crops.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5882 => array(
			'name'    => "Ancestral Adversity",
			'journal' => "Elfbetta, personal assistant to the famed explorer Narsis Dren, informed me that the adventurer is investigating the ancestral tombs that dot the Vvardenfell landscape. They apparently uncovered an old scroll that hints at hidden treasure.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5883 => array(
			'name'    => "Hatching a Plan",
			'journal' => "Another crisis looms in the Gnisis Egg Mine. Kwama workers, driven mad by the tonal magicks of the Dwarven resonator, killed their queen. Without the queen, the colony dies. If I do not act, the mining town of Gnisis will die along with it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6256 => array(
			'name'    => "Reeling in Recruits",
			'journal' => "Jee-Lar asked me to help recruit adventurers to join Cyrodilic Collections' expedition to Murkmire. Posting recruitment handbills and performing a traditional Argonian dance will help drum up interest.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5886 => array(
			'name'    => "The Heart's Desire",
			'journal' => "The scholar, Neramo, has a problem. One of his less savory competitors, Artisan Lenarmen, stole his Dwemer control rod, and seeks to use it to find an ancient artifact. I must stop Lenarmen before he discovers the secret of Nchulefthingth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5887 => array(
			'name'    => "Fleeing the Past",
			'journal' => "A House Redoran Councilor seeks information regarding his missing daughter, Veya Releth. After exhausting House resources, he's turned to outside help in a concerted effort to find her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5889 => array(
			'name'    => "Blood for Blood",
			'journal' => "I encountered an escaped captive named Filand who warned me that the Reachmen are kidnapping people to labor at a strange, ancient Forge in this valley. He asked me to help rescue the other prisoners before the Reachmen work them to death, or worse.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7235 => array(
			'name'    => "Undying Requiem",
			'journal' => "I met Lucilla Caprenia outside the Exiled Redoubt. She wants to find out what's happening to the soldiers the Albus family kidnaps and, ultimately, stop the Albuses from kidnapping anyone else regardless of their alliance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5894 => array(
			'name'    => "Forging the Future",
			'journal' => "Strange contraptions have emerged from a rift in the caverns beneath Tel Fyr and killed most of the Telvanni who were growing the tower. The renowned mage Divayth Fyr has asked me to accompany him in investigating this threat.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6217 => array(
			'name'    => "Kill 150 Enemy Players",
			'journal' => "Grand Warlord Dortene has authorized a bonus for any Covenant soldier who slays 150 enemy combatants.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5900 => array(
			'name'    => "Echoes of a Fallen House",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5901 => array(
			'name'    => "Objections and Obstacles",
			'journal' => "With my help, Sun-in-Shadow achieved the rank of retainer in the Great House Telvanni. If she is to help free Eoki, she will need to climb the ranks even further. Unfortunately, becoming an oathman may present a greater challenge than she expects.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6549 => array(
			'name'    => "The Ravenwatch Inquiry",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5903 => array(
			'name'    => "The Memory Stone",
			'journal' => "An elderly Dark Elf named <<1>> contracted me to transport a memory stone to locations across Vvardenfell. He wishes to archive his memories for future generations.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5905 => array(
			'name'    => "Divine Restoration",
			'journal' => "Archcanon Tarvus revealed himself to be Barbas, the hound of Clavicus Vile. In an effort to locate and take control of Clockwork City, he tricked me into helping him drain Lord Vivec's energy. I need to stop Barbas and recover Lord Vivec's energy.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5907 => array(
			'name'    => "Great Zexxin Hunt",
			'journal' => "Huntmaster Sorim-Nakar of the Erabenimsun Ashlander tribe has given me a challenge. He wants me to hunt the deadly nix-ox called Great Zexxin.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5908 => array(
			'name'    => "Tarra-Suj Hunt",
			'journal' => "Huntmaster Sorim-Nakar of the Erabenimsun Ashlander tribe has given me a challenge. He wants me to hunt the deadly nix-hound called Tarra-Suj.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7251 => array(
			'name'    => "Charging the Lens",
			'journal' => "<<1>> asked me to collect vitrified souls from Worm Cultists and Coldharbour Daedra to charge the Meridian Lens.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5910 => array(
			'name'    => "Mother Jagged-Claw Hunt",
			'journal' => "Huntmaster Sorim-Nakar of the Erabenimsun Ashlander tribe has given me a challenge. He wants me to hunt the giant mudcrab called Mother Jagged-Claw.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5911 => array(
			'name'    => "Ash-Eater Hunt",
			'journal' => "Huntmaster Sorim-Nakar of the Erabenimsun Ashlander tribe has given me a challenge. He wants me to hunt the dire ash beetle called Ash-Eater.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7271 => array(
			'name'    => "Soulcaller's Curse",
			'journal' => "Several ancient stone tablets were unearthed at a site northeast of Xul-Katama in eastern Solstice. However, a dangerous Voidmother named Soulcaller reportedly now haunts the area. I need to defeat Soulcaller and retrieve the tablets.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5914 => array(
			'name'    => "The Magister Makes a Move",
			'journal' => "My ambitious companion, Sun-in-Shadow, succeeded in replacing Magister Gothren's Mouth, Ralasa Delvi. But even success carries risk in Sadrith Mora. Rumor has it that Magister Gothren leaves no slight unpunished.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5915 => array(
			'name'    => "Tribal Troubles",
			'journal' => "The Ashlanders of Zainab have been run out of Nchuleft, stopping them from scavenging the ruins for items to trade. The Temple wishes to better their relations with the tribe by gathering tools and parts which were left behind.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5918 => array(
			'name'    => "A Creeping Hunger",
			'journal' => "I've been hired by researchers to defeat a Hunger that's protecting a ruin they wish to study. I'm also required to gather relics scattered around the ruin.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6565 => array(
			'name'    => "By Love Betrayed",
			'journal' => "Adusa-daro needs my help to rescue her friend Kathad from a clan of evil vampires known as the Lothid, and their leader, Boss Trenlor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5919 => array(
			'name'    => "Of Faith and Family",
			'journal' => "Even though I confirmed that Veya is safe, Councilor Eris continues to press me to arrange a meeting between him and his daughter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5921 => array(
			'name'    => "Springtime Flair",
			'journal' => "Jester Queen Ayrenn has tasked me with enforcing cheer at her festivities by finding any festival goers who aren't acting the spirit of things and pelting them with flower petals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5922 => array(
			'name'    => "The Heart of a Telvanni",
			'journal' => "After a harrowing experience at the hands of Magister Gothren, Sun-in-Shadow finds herself at a crossroads-torn between a mage's ambition and the fruits of companionship. Dedicating herself to one may well destroy her chances at the other.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5923 => array(
			'name'    => "The Lost Library",
			'journal' => "The miniature model of Vvardenfell is nearly complete. One section remains missing, which Librarian Bradyn believes is the key to finding the lost Library of Andule.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5925 => array(
			'name'    => "Relics of Assarnatamat",
			'journal' => "Numani-Rasi of the Urshilaku Ashlander tribe has requested that I explore the ruins of the Daedric shrine Assarnatamat and gather as many historic relics as I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5927 => array(
			'name'    => "Relics of Ashurnabitashpi",
			'journal' => "Numani-Rasi of the Urshilaku Ashlander tribe has requested that I explore the ruins of the Daedric shrine Ashurnabitashpi and gather as many historic relics as I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5929 => array(
			'name'    => "Relics of Dushariran",
			'journal' => "Numani-Rasi of the Urshilaku Ashlander tribe has requested that I explore the ruins of the Daedric shrine Dushariran and gather as many historic relics as I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5930 => array(
			'name'    => "Relics of Ashalmawia",
			'journal' => "Numani-Rasi of the Urshilaku Ashlander tribe has requested that I explore the ruins of the Daedric shrine Ashalmawia and gather as many historic relics as I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5931 => array(
			'name'    => "A Noble Guest",
			'journal' => "Jester King Jorunn requested that I steal his bride-to-be away from her minders, so she may join him in his Feast of Fools.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5933 => array(
			'name'    => "A Purposeful Writ",
			'journal' => "With the revelation that Veya's brother is dead, Councilor Eris has asked me to keep his daughter away from Balmora and House Redoran while he looks into what happened. In the meantime, Ashur asked me to talk to Veya.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5934 => array(
			'name'    => "Tax Deduction",
			'journal' => "I have been asked to retrieve taxes and records stolen by bandits.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6581 => array(
			'name'    => "Notes of the Void",
			'journal' => "A defector from a local Reach clan informed the Ard's Stonehands that his clan's witch matron is attempting a dangerous ritual. I agreed to investigate and put a stop to the ritual, as well as search for any of the witch matron's notes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5935 => array(
			'name'    => "The Missing Prophecy",
			'journal' => "An oracle of Azura named Rhea Opacarius needs my help. Some malevolent force conspires to destroy her order and, perhaps, all of Tamriel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6265 => array(
			'name'    => "The Sounds of Home",
			'journal' => "A local crafter, Nesh-Deeka, hopes to establish a trade agreement with Captain Jimila to sell an Argonian musical instrument abroad. He wants me to act as his agent and secure a deal for his vossa-satl.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5948 => array(
			'name'    => "Family Reunion",
			'journal' => "Veya thinks we should go and find Vatola Telem's squad that's scouting the old fort outside of Balmora. She said the fort's overrun with cultists and we need to reach the soldiers before anything happens to them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5949 => array(
			'name'    => "For Glory",
			'journal' => "I found a pamphlet describing the Battlegrounds, a place where contests of strategy and skill are held daily.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5950 => array(
			'name'    => "The Ancestral Tombs",
			'journal' => "Librarian Bradyn seeks the location of the lost Library of Andule, the secret repository of knowledge of the ancient Great Houses, by building a miniature reproduction of Vvardenfell.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5953 => array(
			'name'    => "Let the Games Begin",
			'journal' => "Battlemaster Rivyn has challenged me to participate in five Battlegrounds matches.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5954 => array(
			'name'    => "Test of Mettle",
			'journal' => "Battlemaster Rivyn has challenged me to show my individual skill as a player by earning Battleground Medals.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5956 => array(
			'name'    => "Daedric Disruptions",
			'journal' => "The Sunken Vaults of Ashalmawia are infested with Worm Cultists performing unclean rites to summon Daedra.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7299 => array(
			'name'    => "The Daedric Scourge",
			'journal' => "The residents of Deetra Grotto have been afflicted with a deadly disease called the Daedric scourge. Their healer, Keeshka, can make a curative, but since the illness struck him, he needs help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5958 => array(
			'name'    => "Unsettled Syndicate",
			'journal' => "The Camonna Tong are thought to be active in the illegal slave trade at Khartag Point. Their efforts need to be disrupted.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6156 => array(
			'name'    => "Snuffing Out the Light",
			'journal' => "The Divine Prosecution has requested I travel to Eton Nir Grotto and destroy the cocoons of the Daedra roaming the cavern. I'll also need to burn any sacrilegious text I find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5961 => array(
			'name'    => "Planting Misinformation",
			'journal' => "A faction of bandits in a cavern called Zainsipilu are attempting to grow moon-sugar grass. I've been asked to re-direct their methods through misinformation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5962 => array(
			'name'    => "Kwama Conundrum",
			'journal' => "Through unknown breeding and feeding practices, the kwama miners of Matus-Akin have been developing a new, more aggressive breed of kwama. Though this breed needs to be contained, the Temple also wishes to learn more about their breeding methods.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5972 => array(
			'name'    => "A Masterful Weapon",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6172 => array(
			'name'    => "The Psijics' Calling",
			'journal' => "Loremaster Celarus, a prestigious Psijic monk, invited me to join the Psijic Order. My first task involves closing breaches in time on Summerset Isle.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5977 => array(
			'name'    => "A Masterful Feast",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5978 => array(
			'name'    => "Masterful Tailoring",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5979 => array(
			'name'    => "Masterful Leatherwear",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6297 => array(
			'name'    => "The Final Order",
			'journal' => "Before we killed the Usurper Queen, Euraxia issued a final order. We need to ascertain what she ordered her soldiers to do and see how it connects to the plans of the Dragons and the necromancers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5980 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5982 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6638 => array(
			'name'    => "Sand, Snow, and Blood",
			'journal' => "I found a notice about traveling to Craglorn, the Wrothgar Mountains, and the Imperial City. If I complete tasks in these places, I will be rewarded.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5984 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5985 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5986 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5988 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5989 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5990 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7194 => array(
			'name'    => "A Moonlit Shadow",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	5991 => array(
			'name'    => "A Masterful Concoction",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6003 => array(
			'name'    => "Divine Blessings",
			'journal' => "Lord Vivec wants to reward those who helped restore his power and save Vvardenfell from Barbas and Clavicus Vile. He asked me to help him accomplish that.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6004 => array(
			'name'    => "A Late Delivery",
			'journal' => "<<1>>, a merchant, was late delivering a shipment of rare mushrooms to the Telvanni mage Divayth Fyr. Though she now has the mushrooms, she fears Fyr may still punish her for delivering them late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6010 => array(
			'name'    => "Kill Enemy Wardens",
			'journal' => "Grand Warlord Sorcalin has authorized a reward for any Dominion soldier who slays 20 enemy Wardens.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6011 => array(
			'name'    => "Kill Enemy Wardens",
			'journal' => "Grand Warlord Zimmeron has authorized a reward for any Pact soldier who slays 20 enemy Wardens.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6014 => array(
			'name'    => "Whitestrake's Mayhem",
			'journal' => "I ran into a group of priests sermonizing about a holiday.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6017 => array(
			'name'    => "A Masterful Glyph",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6018 => array(
			'name'    => "A Masterful Plate",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6020 => array(
			'name'    => "A Masterful Shield",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6527 => array(
			'name'    => "Problem Growth",
			'journal' => "An explorer reported the discovery of a gigantic Dwarven colossus that appears to be building upon itself using scrap it finds. They believe that this construct will pose an even greater threat if left unchecked.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6021 => array(
			'name'    => "Masterful Tailoring",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7363 => array(
			'name'    => "Those Who Would Rule",
			'journal' => "The Curator told me a little bit about the mysterious Night Market which recently opened in Fargrave. He said it was a good place to gain power and influence.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6022 => array(
			'name'    => "Masterful Leatherwear",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7354 => array(
			'name'    => "A Call for Epaulets",
			'journal' => "Armorer Reistaff asked me to craft two epaulets of any kind for Stirk Fellowship soldiers he befriended. I agree to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6023 => array(
			'name'    => "Of Knives and Long Shadows",
			'journal' => "A junior member of the Mages Guild named Skaldir insists that he and his associates have made a significant magical discovery-one that could prove disastrous if left uninvestigated.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6220 => array(
			'name'    => "Elsweyr Gate Crasher",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6025 => array(
			'name'    => "Deepening Shadows",
			'journal' => "In the wake of the Shadow of Sotha Sil's attack, Gascone has gone missing.  I should find him and see if he has any insight into what the shadow is and why it was in the Brass Fortress.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6415 => array(
			'name'    => "Pledge: Icereach",
			'journal' => "<<1>> of the Undaunted challenged me to explore Icereach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6037 => array(
			'name'    => "Fuel for our Fires",
			'journal' => "<<1>> requested I gather oil from the water within Clockwork City. I'll also need to gather supplies to create more tools for future collecting.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6038 => array(
			'name'    => "A Daily Grind",
			'journal' => "<<1>> requested I gather a strange material called flash stone which can be turned into a useful powder. I'll also need to collect metal pellets that help in the grinding process.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6039 => array(
			'name'    => "Loose Strands",
			'journal' => "<<1>> requested I gather the metal strands weaved within cloths in Clockwork City. I also need to collect components for an artificial lodestone that is used in harvesting the metal strands.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6040 => array(
			'name'    => "A Sticky Solution",
			'journal' => "<<1>> requested I gather the husks of Clockwork trees for an adhesive. I'll also need to collect artificial coal to help keep the sap warm.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6416 => array(
			'name'    => "Unhallowed Grave",
			'journal' => "A mysterious band of necromancers and graverobbers breached the defenses of the Unhallowed Grave. The burial site's last living defender, Shelaria, asked for my help in ridding the tomb of these invaders.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6046 => array(
			'name'    => "Unto the Dark",
			'journal' => "The Shadow of Sotha Sil, having claimed the Skeleton Key from the Blackfeather Court, is making his way to the center of the Clockwork City-The Cogitum Centralis. Proctor Luciana and I must find some way to follow him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6049 => array(
			'name'    => "The Shadow Cleft",
			'journal' => "One of Sotha Sil's Clockwork Apostles, an adjunct named Daro, asked me to use an unusual device called a perception tabulator to find his associate and investigate a mysterious portal that appeared in the Radius.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6050 => array(
			'name'    => "To The Clockwork City",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6052 => array(
			'name'    => "Lost in the Gloam",
			'journal' => "According to Gascone, a murder of crows stole a mysterious key from his office in the Clockwork Basilica-the same key that Clavicus Vile and the mysterious shadow cultists seek. I must find this key before the enemy does.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6054 => array(
			'name'    => "Pledge: Falkreath Hold",
			'journal' => "<<1>> of the Undaunted has challenged me to defend the besieged city of Falkreath Hold.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6252 => array(
			'name'    => "Pledge: Depths of Malatar",
			'journal' => "<<1>> of the Undaunted has challenged me to explore the Depths of Malatar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6057 => array(
			'name'    => "In Search of a Sponsor",
			'journal' => "The High Elf explorer, Neramo, and his two associates, Kireth and Raynor, have a plan to win the sponsorship of an influential Clockwork Apostle named Varuni Arvel. We'll need her support to become bonafide citizens of the Clockwork City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6058 => array(
			'name'    => "The Halls of Regulation",
			'journal' => "The Halls of Regulation have begun to break down, threatening the water supply throughout Clockwork City. I've agreed to help <<1>> make repairs by protecting her as we gather parts and fix machinery.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6059 => array(
			'name'    => "Tarnished Truffles",
			'journal' => "Brengolin, the cook in Slag Town, needs some assistance in creating some unconventional cuisine and I have agreed to lend him a hand.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6061 => array(
			'name'    => "A Matter of Tenderness",
			'journal' => "Brengolin, the cook in Slag Town, needs some assistance in creating some unconventional cuisine. His meat stores are presently empty, so I agreed to assist him with restocking.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6063 => array(
			'name'    => "The Strangeness of Seht",
			'journal' => "Divayth Fyr is convinced that something sinister is driving Sotha Sil's recent change of character. He asked me to help him root out the cause of the Clockwork God's new and peculiar behavior.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6065 => array(
			'name'    => "Plans of Pestilence",
			'journal' => "<<1>> has requested my aid in helping him journey through a disease infested temple, located within the hazards of Scalecaller Peak.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6066 => array(
			'name'    => "The Precursor",
			'journal' => "Associate Zanon has asked for my assistance in rebuilding an ancient factotum. Doing so will allow him to better understand the creation of factotums, a mystery to all but Sotha Sil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6268 => array(
			'name'    => "Grave Subject Matter",
			'journal' => "Cyrodilic Collections needs someone to collect old remains from the Teeth of Sithis for study.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6073 => array(
			'name'    => "A Shadow Misplaced",
			'journal' => "The Clockwork Apostles have sent explorers into the Shadow Cleft to find out more about the mysterious realm and the reason it's connected to Clockwork City. One explorer hasn't returned, and I agreed to try to find him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6075 => array(
			'name'    => "The Oscillating Son",
			'journal' => "Now that I found all of the Precursor's parts, Associate Zanon wishes to present the factotum to his mentor.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6393 => array(
			'name'    => "The Dark Aeon",
			'journal' => "Sai Sahan received a letter from Abnur Tharn. It seems the old battlemage discovered something about a threat to Southern Elsweyr and needs our help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6076 => array(
			'name'    => "Inciting the Imperfect",
			'journal' => "I've accepted a contract to defeat Sotha Sil's creation, the Imperfect, in order to test its capabilities. I've also been asked to gather its lost sequence plaques.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6448 => array(
			'name'    => "The Herbalist's Product",
			'journal' => "An herbalist hired me to deliver his product, an herbal remedy, to multiple customers in Senchal. He's being somewhat cryptic, but he offered to pay well if I hurry.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6078 => array(
			'name'    => "Family Feud",
			'journal' => "After an argument with his sister Kireth concerning Raynor's behavior during the Shadow Crisis, the young artificer stormed off and vowed to finish exploring the Clockwork City on his own. Kireth asked me to go help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6597 => array(
			'name'    => "A Challenge of Worth",
			'journal' => "A Reachman named Angair challenged me to complete a clan ritual called the Vateshran's Rites. He says it involves combat against creatures from Oblivion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6079 => array(
			'name'    => "Again Into the Shadows",
			'journal' => "The Clockwork Apostles sent a member of the Mages Guild through the portal to explore the Shadow Cleft. She hasn't returned.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6080 => array(
			'name'    => "A Shadow Malfunction",
			'journal' => "A Clockwork Apostle entered the Shadow Cleft and hasn't returned. What's worse, he forgot to take his perception tabulator with him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6081 => array(
			'name'    => "Oiling the Fans",
			'journal' => "The Clockwork Apostles have tasked me to continue maintenance in the Halls of Regulation. I need to grease a ventilation fan using oil found within the repair factotums.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6082 => array(
			'name'    => "The Sickening Sea",
			'journal' => "The Divine Prosecution has tasked me with taking coral crab eggs from the reef between Lillandril and the Corgrad Wastes for the Sapiarchs to inspect for signs of corruption.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7302 => array(
			'name'    => "Wealth from the Sea",
			'journal' => "I found a note attached to a cache of goods. The captain waiting at the inn in Shor's Stand seems desperate to get the goods back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6083 => array(
			'name'    => "Taming the Wild",
			'journal' => "<<1>> tasked me with killing Caanerin, the indrik matriarch, and restraining her spirit by activating warding stones scattered around her ancestral glade.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6084 => array(
			'name'    => "The Abyssal Alchemist",
			'journal' => "The Divine Prosecution has charged me with clearing out Welenkin Cove and eliminating the threat posed by B'Korgen, the Sea Sload alchemist.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6086 => array(
			'name'    => "Never Forgotten",
			'journal' => "The Office of Divine Prosecution suffered a number of casualties to a creature called Graveld in Sil-Var-Woad.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6088 => array(
			'name'    => "Changing the Filters",
			'journal' => "The Clockwork Apostles have tasked me to continue maintenance in the Halls of Regulation. I need to replace several water filters in order to stop the possibility of contamination.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6090 => array(
			'name'    => "Saints' Mercy",
			'journal' => "Three Dark Elf Saints were granted the gift of immortality, in the form of ageless mechanical bodies created by the god Sotha Sil, at the price of their sanity. Now their madness poses a significant threat to the Clockwork City.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6464 => array(
			'name'    => "Greymoor Rising",
			'journal' => "High King Svargrim revealed his alliance with the Gray Host and their plans to unleash a harrowstorm over Solitude. I need to help Svana find a way to stop him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6094 => array(
			'name'    => "The Broken Brassilisk",
			'journal' => "I found an inactive brassilisk in the Radius of the Clockwork City with the name \"Frynga\" carved into its jaw. If I find Frynga and return her mechanical pet, perhaps she will reward me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6095 => array(
			'name'    => "The Registrar's Request",
			'journal' => "I found a sequence plaque on the road in the Clockwork City Radius. An engraving on the plaque asks that it be returned to the Clockwork Registrar in the Brass Fortress.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6097 => array(
			'name'    => "Through a Veil Darkly",
			'journal' => "I received an enchanted stone from the Mages Guild that contains an important message from Vanus Galerion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6098 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7318 => array(
			'name'    => "The Passages Beneath",
			'journal' => "The Stirk Fellowship drove the Worm Cult out of Sunport, but we learned that the cult plans to move something important through a hidden route known as the Passages Beneath. I agreed to look into the Worm Cult's plans and disrupt them if I can.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6099 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6101 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6102 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7306 => array(
			'name'    => "Vault of Agony",
			'journal' => "There's a dangerous relic known as the Dolorous Cista in the Ossein Cage. Captain Lyn of the Fighters Guild tasked me with helping their mission to destroy this relic before it is unleashed on Tamriel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6103 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6300 => array(
			'name'    => "Iron in the Blood",
			'journal' => "While investigating Darkpool Mine, a Khajiit named Mara'dahni, and her partner, Hadaz, were set upon by Daedric cultists and their minions. Mara'dahni escaped. Hadaz wasn't so lucky. I agreed to investigate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6105 => array(
			'name'    => "Alchemist Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver alchemical products to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6107 => array(
			'name'    => "A Matter of Leisure",
			'journal' => "The Bursar of Tribute demands compensation for the Blackfeather Court from mortals who refuse to allow them to play with their games and toys.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6109 => array(
			'name'    => "The Dreaming Cave",
			'journal' => "The Ritemaster used scrying magic to recall some of his missing memories. He took the Heart of Transparent Law into the Dreaming Cave beneath Ceporah Tower, but he still can't remember what happened after that. I need to help him remember.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6480 => array(
			'name'    => "One Last Adventure",
			'journal' => "A High Elf woman named Telline is worried about her husband, Viggol. He wants to go on one last adventure that involves hunting rumored creatures in the mine.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6110 => array(
			'name'    => "Glitter and Gleam",
			'journal' => "The Bursar of Tributes seeks ornate items to add to the Blackfeather Court's hoard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6112 => array(
			'name'    => "A Pearl of Great Price",
			'journal' => "Ritemaster Iachesis of the Psijic Order plans to use an augury ritual to reveal the secrets of the strange pearl we recovered from the Monastery of Serene Harmony.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6113 => array(
			'name'    => "Lost in Translation",
			'journal' => "Ritemaster Iachesis remembers a woman's voice, whispering ancient words over and over. He thinks these words are tied to the place where he hid the Heart of Transparent Law. Before we breach the barrier, we need to determine what the words mean.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6114 => array(
			'name'    => "Manor of Masques",
			'journal' => "Rinyde asked me to find her brother Larydel and stop him from joining the House of Reveries. I must audition as well in order to figure out who he is.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7334 => array(
			'name'    => "Cold War",
			'journal' => "I met a Nord in Windhelm who told me her group celebrates New Life by hurling snowballs at each other, but they're one player short. I agreed to participate in their festivity.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6115 => array(
			'name'    => "Illusions of Grandeur",
			'journal' => "Students in the Illumination Academy have taken on bizarre personae and don't appear to know who or where they are. I must help them reclaim their lost identities.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6116 => array(
			'name'    => "A Tale of Two Mothers",
			'journal' => "A distraught mother named Lanarie cannot find her son, Arathel. The youth surrendered to the authorities-saying he was part of a plot to kill a Sapiarch. But he disappeared from his cell shortly thereafter. I promised Lanarie I would find her son.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6118 => array(
			'name'    => "Lauriel's Lament",
			'journal' => "I met a High Elf bailiff outside the ruins of the Direnni Acropolis. He told me that an ancient curse lingers within its walls. I must find the source of this corruption and cleanse it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7322 => array(
			'name'    => "Crossing the Wall",
			'journal' => "The Stirk Fellowship is ready to challenge the Worm Cult, but the Writhing Wall defies the Light of Meridia. Prince Azah asked me to find out what impedes the Light and deal with it. The Writhing Wall must fall to open the way into Eastern Solstice.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6119 => array(
			'name'    => "The Ebon Sanctum",
			'journal' => "I agreed to escort the scholar Andewen as she investigates an ancient temple, hidden within the mysterious forest of Ebon Stadmont.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6316 => array(
			'name'    => "Blood and Tears",
			'journal' => "Travelers in the Weeping Scar are falling prey to an ancient clan of vampires called the Tenarr Zalviit, but I met an exile of this clan named Yushiha, who is determined to stop the bloodshed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6121 => array(
			'name'    => "Untamed and Unleashed",
			'journal' => "I met a keeper of the Royal Menagerie in Sil-Var-Woad. He warned me that dangerous animals have escaped into the park grounds, but he is searching for a tame indrik that may have escaped during the chaos.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6496 => array(
			'name'    => "The Fate of the Frozen",
			'journal' => "I found a wounded, former Frostbite Raider named Magreta who asked me to reunite her with her partner, Nelfthea, who is still with their raiding band on the Frozen Coast.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6126 => array(
			'name'    => "The Crystal Tower",
			'journal' => "I negotiated an alliance of sorts with Clavicus Vile and Mephala. They agreed to help us get into the Crystal Tower-mostly. Once inside, I need to find a way to stop Nocturnal before she uses the tower to remake reality.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6127 => array(
			'name'    => "Half-Formed Understandings",
			'journal' => "Ulliceta, an apprentice studying under the Psijics, took a powerful seeing stone called the Eye of Remembrance to the forbidden Traitor's Vault. It may reveal the secrets hidden there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6129 => array(
			'name'    => "The Perils of Art",
			'journal' => "Tindoria, a High Elf artist, has hired me to acquire a list of rare art supplies for her next masterpiece.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6130 => array(
			'name'    => "Room to Spare",
			'journal' => "Felande Demarie, a friendly landlord, is looking for a tenant of good reputation to give her inn room to.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7350 => array(
			'name'    => "Cuirasses for the Courageous",
			'journal' => "Armorer Reistaff asked me to craft two cuirasses of any kind for soldiers in the Stirk Fellowship. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6131 => array(
			'name'    => "Storming the Walls",
			'journal' => "A massive storm is brewing over the fortress of Sea Keep that is the work of a Maormer Sea Mage. It will devastate the entire coast unless the Sea Mage's spell is stopped.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7338 => array(
			'name'    => "Pact Creature Fat",
			'journal' => "<<1>> asked me to collect creature fat from trolls, alits, guars, and kagouti in the territories of the Ebonheart Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6135 => array(
			'name'    => "Old Wounds",
			'journal' => "A prominent High Elf named Halimorion was murdered by a folkloric assassin called the Ghost of the Green. But there may be more to this case than meets the eye.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6332 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6137 => array(
			'name'    => "Looting the Light",
			'journal' => "Seeks-the-Dark was hired to steal tokens from a cult within Eton Nir Grotto. Unexpectedly, the cult has been massacred by Daedra who now lurk within. I've been hired to collect the tokens and find Seek-the-Dark's assistant, Erilthel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6139 => array(
			'name'    => "The Hulkynd's Heart",
			'journal' => "Tableau is an artist fascinated by the outcasts known as hulkynd, and wishes to meet one. She's requested I help arrange a meeting with one who was seen outside of Rellenthil.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6140 => array(
			'name'    => "Bantering with Bandits",
			'journal' => "Tableau is debuting as an artist. Manacar fears that she may be in danger. He's asked me to help things run smoothly.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6512 => array(
			'name'    => "Halt the Harrowstorms",
			'journal' => "<<1>> charged me with finding allies to assault the harrowstorms forming around Western Skyrim.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6142 => array(
			'name'    => "The Tower Sentinels",
			'journal' => "The Sea Sload altered the Ritemaster's memories and forced him to steal the Heart of Transparent Law and hide it away. While he works to unlock his memories, he wants Valsirenn and I to warn the keepers of the Crystal Tower, the Sapiarchs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6145 => array(
			'name'    => "The Runaway's Tale",
			'journal' => "Esterdel asked me to find <<1>>, his missing bride-to-be. He tracked her to Archon's Grove, which has been overrun by werewolves. He's hired me to rescue Niralin, or at least collect proof that she's died.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6146 => array(
			'name'    => "Wasting Away",
			'journal' => "A group of outcasts living in the seclusion of the Corgrad Wastes has seen their number dwindle after a string of disappearances. They believe the source of the trouble lies in the old buried ruins exposed by the recent sinkholes plaguing the region.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6149 => array(
			'name'    => "Lost at Sea",
			'journal' => "An Altmer woman approached me with questions about her father, Curtaros. His angler fleet is long overdue from their voyage to Wasten Coraldale and she's concerned for his safety. I have agreed to locate her father for her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6152 => array(
			'name'    => "Pilgrimage's End",
			'journal' => "<<1>> wants me to recover articles of faith on behalf of the cult of Phynaster.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6153 => array(
			'name'    => "A New Alliance",
			'journal' => "With the Crystal Tower saved, Valsirenn wishes to call a meeting of Summerset's most influential leaders to ensure its future protection.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6166 => array(
			'name'    => "Sinking Summerset",
			'journal' => "Battlereeve Tanerline charged me with finding allies to attack the abyssal geysers and destroy the pearls that power them before the Sea Sload cabal's plan to sink Summerset comes to fruition.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6168 => array(
			'name'    => "Ache for Cake",
			'journal' => "I received a voucher for a special-occasion cake from the renowned master baker, Chef Donolon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6174 => array(
			'name'    => "Whispers from the Deep",
			'journal' => "A number of adventurers have fallen prey to the magic of a mind trap. A mage named Oriandra has a plan to rescue them, but she needs my help to succeed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6176 => array(
			'name'    => "His Final Gift",
			'journal' => "I found the corpse of Nafarion, a High Elf who turned into a vampire. His dying wish was for his betrothed to know the truth about what happened to him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6178 => array(
			'name'    => "The Forest Vandal",
			'journal' => "A High Elf named Vinafwe is being plagued by a vandal who is ruining her carefully arranged botanical grounds with foreign flowers and even dangerous stranglers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6179 => array(
			'name'    => "Gryphon Grievance",
			'journal' => "Miranrel and her companions were attacked by gryphons after entering their nesting grounds. She's offered me a reward if I help find her lost friends.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6180 => array(
			'name'    => "Obedience Issues",
			'journal' => "I encountered a noblewoman along the beach who has lost her husband and her prized hound. I have agreed to help reunite them with her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6181 => array(
			'name'    => "Breaches On the Bay",
			'journal' => "Loremaster Celarus's apprentice, Josajeh, charged me with sealing more time breaches for the Psijic Order-this time in the Iliac Bay region.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6185 => array(
			'name'    => "Breaches of Frost and Fire",
			'journal' => "A new cluster of time breaches appeared in northeastern Tamriel-specifically the lands of Stonefalls. Eastmarch, and The Rift. As an agent of the Psijic Order, I need to seal them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6188 => array(
			'name'    => "The Great Hunt",
			'journal' => "The Ashlander Hanu has recruited me to join her hunting team within Hircine's Hunting Grounds. We must hunt down an indrik and present its heart to Hircine in order for Hanu to win a boon from the Daedric Prince and save her father.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6195 => array(
			'name'    => "Time in Doomcrag's Shadow",
			'journal' => "Even with all eight fragments of the Staff of Towers collected, time breaches continue to open all across Tamriel. I must seal a new cluster of these rifts in the land of Rivenspire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6196 => array(
			'name'    => "A Breach Beyond the Crags",
			'journal' => "Loremaster Celarus feels confident that the threat posed by the Staff of Towers is nearly dealt with. Unfortunately, there is one final cluster of time breaches in the land of Craglorn. I need to seal them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6199 => array(
			'name'    => "The Towers' Fall",
			'journal' => "Loremaster Celarus's apprentice, Josajeh, stole the eight fragments of the Staff of Towers and fled to Tamriel. She no doubt plans to reforge the staff and use it to alter time. The results could be catastrophic. I must stop her before it's too late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6203 => array(
			'name'    => "High Rock Gate Crashing",
			'journal' => "I accepted a mission to help capture Fort Glademist, Fort Rayles, and Fort Warden for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6204 => array(
			'name'    => "Masterful Jewelry",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6206 => array(
			'name'    => "Elsweyr Gate Crasher",
			'journal' => "I accepted a mission to help capture Castle Faregyl, Castle Black Boot, and Castle Bloodmayne for the Pact.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6210 => array(
			'name'    => "Capture Any Three Keeps",
			'journal' => "I accepted a mission to help capture any three keeps for the Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6214 => array(
			'name'    => "Capture Any Three Keeps",
			'journal' => "I accepted a mission to help capture any three keeps for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6216 => array(
			'name'    => "Capture All 3 Towns",
			'journal' => "I accepted a mission to help capture the towns of Vlastarus, Cropsford, and Bruma for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6224 => array(
			'name'    => "High Rock Gate Reclaimation",
			'journal' => "I accepted a mission to help reclaim Fort Glademist, Fort Rayles, and Fort Warden for the Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6227 => array(
			'name'    => "Jewelry Crafting Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Jewelry to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6228 => array(
			'name'    => "Jewelry Crafting Writ",
			'journal' => "I've taken a contract from one of the Writ boards, an agreement to deliver Jewelry to an interested organization.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6234 => array(
			'name'    => "Old Growth",
			'journal' => "Bolu of the Dead-Water tribe, which he calls the Naga-Kur, wants me to go to the Echoing Hollow and accomplish the task he set before me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6239 => array(
			'name'    => "She Who Eats the Light",
			'journal' => "The Teeth of Sithis, a pair of ancient xanmeer temples, has been overtaken by the servants of its heretical, former high priestess, Shuxaltsei. She has taken the custodians of the Teeth captive and is sacrificing them in some profane blood rite.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6240 => array(
			'name'    => "The Lost Legion",
			'journal' => "Dradeiva, a local Argonian, seeks to prove that the Imperial Ninth Legion disappeared in the Tsofeer Caverns. She hopes to show that her ancestor didn't betray Murkmire. Unfortunately, the caves may also be home to the monstrous voriplasm, Wuju-Ka.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6245 => array(
			'name'    => "By River and Root",
			'journal' => "After a lengthy chase and a harrowing battle with Kassandra, Famia and I finally retrieved the Remnant of Argon. Now, we need to learn how to release the souls that reside within it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6247 => array(
			'name'    => "The Weight of Words",
			'journal' => "Cyrodilic Collections seeks scriptures relating to ancient Argonian worship of Sithis.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6249 => array(
			'name'    => "Lock and Keystone",
			'journal' => "I encountered an expedition party searching for a Dwarven vault in a glacier. Their leader, a scholar named Tharayya, made me an offer to assist her in locating an artifact known as the Wrathstone.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6264 => array(
			'name'    => "Sacred Candles",
			'journal' => "Varo Hosidias of Cyrodilic Collections wants me to enter the Tsofeer Cavern and place sacred Argonian candles at the behest of a benefactor named Tia Doran.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6266 => array(
			'name'    => "Missing in Murkmire",
			'journal' => "My companion from the xanmeer, Famia Mercius, needs my help. Several of her employees have gone missing in recent days, and she fears the worst. I need to try to find out what happened to them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6269 => array(
			'name'    => "The Black Gauntlet",
			'journal' => "Erilthel has requested that I help save her boss Seeks-the-Dark by completing a gauntlet held by the Blackguards. I'll have to make my way through the many foes of Blackrose Prison and emerge victorious.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6270 => array(
			'name'    => "Monument of Change",
			'journal' => "Twelve tablets were stolen from the Xinchei-Konu, an ancient Argonian calendar. Jekka-Wass Vozei, the keeper of the calendar, has tasked me to return the first missing tablet.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6276 => array(
			'name'    => "Lost in Murkmire",
			'journal' => "<<1>> asked if I could head into the swamp and look for her missing wife, Varona, and their pet Drooler.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6278 => array(
			'name'    => "The Burnt Branch",
			'journal' => "Hands-That-Heal has lost a wooden box with great sentimental value to him after being attacked by hackwings. He's offered a reward if I successfully return the box to him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6288 => array(
			'name'    => "Envoys Who Cower",
			'journal' => "Several envoys from the Root-Whisper Village have gone missing, and Tuwul suspects that Blackguards are the ones responsible.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6289 => array(
			'name'    => "Offerings That Hide",
			'journal' => "Several offerings collected by the Root-Whisper tribe have been stolen by the Blackguards. If these offerings aren't recovered, the other tribes will become deeply offended.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6304 => array(
			'name'    => "Two Queens",
			'journal' => "After successfully defending Riverhold from Euraxia's forces, Abnur Tharn and the war council think the time has come to assault Rimmen and remove the Usurper Queen from the throne.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6313 => array(
			'name'    => "Hunting the Hunters",
			'journal' => "A vain Khajiit named Sarazi challenged me to kill all the master hunters lurking on Predator Mesa, as well as a feisty gryphon called Quillmane.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6320 => array(
			'name'    => "The Singing Crystal",
			'journal' => "The ancient Khajiiti mural known as the Rithana-di-Renada has been completed, but Iraya feels like something is still missing. Legends say the mural emits some kind of music.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6321 => array(
			'name'    => "The Witch of Azurah",
			'journal' => "Hundreds of Khajiiti undead emerge from the ruins of the Ashen Scar. A mysterious High Elf mage named Vastarie is trying to keep the corpses at bay, but without my help, the waves of undead might overwhelm Elsweyr.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6322 => array(
			'name'    => "Path of the Hidden Moon",
			'journal' => "An ancient Khajiiti lich named Arum-Khal threatens to emerge from the depths of the Ashen Scar. If Vastarie and I are to have any hope of stopping him, I'll need to learn more about the monastic order he betrayed: The Order of the Hidden Moon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6330 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6336 => array(
			'name'    => "A Rage of Dragons",
			'journal' => "Battlemage and former Imperial advisor Abnur Tharn summoned me to Northern Elsweyr to help him and the Khajiiti deal with a rage of Dragons.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6337 => array(
			'name'    => "A Battle of Silk and Flame",
			'journal' => "A Redguard warrior, Khud af-Hadajja, requested my help in destroying a Daedric device inside a mysterious doomvault before either the servants of Mephala or Mehrunes Dagon get their hands on it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6342 => array(
			'name'    => "Goblin Demonstration",
			'journal' => "Zahari asked me to help train new recruits for the Khajiit Defense Legion to fight enemies they may not have encountered in training.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6346 => array(
			'name'    => "Dragon Lore: Icehammer's Vault",
			'journal' => "Dragons have not been seen in Tamriel for generations, and the Khajiit are unsure how best to fight against them. The ancient Nords recorded Dragon fighting techniques on scrolls they keep in their crypts. I need to find these scrolls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6349 => array(
			'name'    => "The Sanguine Successor",
			'journal' => "Chevalier Renald enlisted my aid in stopping his comrade, Grundwulf, from consuming the blood of a Dragon that he's trapped inside a ruined Khajiiti temple with the help of the Hollowfang vampire clan.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6351 => array(
			'name'    => "The Azure Blight",
			'journal' => "I met a couple of Wood Elves who want to enlist a mage's and a nature spirit's help to kill a Dragon that threatens Elden Root.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6353 => array(
			'name'    => "The Return of Alkosh",
			'journal' => "Word has spread that the Divine Alkosh has descended to his mountain temple of Sunspire to be the Khajiit's savior in these troubled times.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6359 => array(
			'name'    => "Wisdom in the Winds",
			'journal' => "Euraxia's troops have descended upon Desert Wind Caverns, scouring the ruins for ancient scrolls. Nisuzi, the Baandari job broker, wants me to claim these texts before the Imperials.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6361 => array(
			'name'    => "The Hungry Cat's Favor",
			'journal' => "According to Nisuzi, some of Elsweyr's most terrifying beasts stalk Predator Mesa. Hunting these creatures and making blood offerings to the Daedric Prince, Hircine, might earn Nisuzi's clan the favor that they seek.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6363 => array(
			'name'    => "Tangled Tea Leaves",
			'journal' => "Nisuzi's Baandari caravan suffers a tea shortage, but not just any tea will do as a replacement. Apparently, rare Samar tea shrubs grow in the Tangle, and the pekoe leaves they produce are the finest in Tamriel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6364 => array(
			'name'    => "Sweet Rotmeth Brew",
			'journal' => "I found a keg of sweet rotmeth brew. If I return it to its rightful owner, I may receive a reward.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6371 => array(
			'name'    => "Cross-cultural Confusion",
			'journal' => "Rigurt the Brash was sent to Elsweyr to improve relations with the Khajiit, but he's already lost his collection of Nord peace offerings. I agreed to help Rigurt deliver his missing gifts, and complete his diplomatic mission.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6373 => array(
			'name'    => "Chaos Magic",
			'journal' => "The Daedric Prince Sheogorath appeared at my alliance's camp in Cyrodiil, promising to show me how to make war more exciting.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6374 => array(
			'name'    => "Love and Guar",
			'journal' => "A Khajiit named <<1>> needs me to burn the wasp nests in his fields and find his lost guar, Gordon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6378 => array(
			'name'    => "The Traders' Terror",
			'journal' => "An eerily clever terror bird called Wily Kee'va has been harassing sugar traders and devouring their camels near Scar's End. Left unchecked, this beast could bring trade in southwestern Anequina grinding to a halt.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6379 => array(
			'name'    => "A Waking Nightmare",
			'journal' => "An ancient dro-m'Athra called Zalsheem lurks in the ruins of a Hidden Moon shrine south of Predator Mesa. Her corrupting influence will surely spread if I don't act quickly.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6380 => array(
			'name'    => "Another Day, Another Death",
			'journal' => "A powerful necromancer named Na'ruzz the Boneweaver lurks in the Bone Pits-a cursed gulch near Orcrest. His dark influence presents a danger to any Khajiit who strays too close.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6381 => array(
			'name'    => "A Dastardly Duo",
			'journal' => "According to Ri'hirr, a pair of Khajiiti criminals named Akumjhargo and Zav'i bleed Northern Elsweyr dry through a combination of theft, murder, and extortion. Honest trade cannot flourish as long as these lawbreakers remain alive.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6384 => array(
			'name'    => "Dragon Hunt",
			'journal' => "To combat the Dragon menace, I accepted a contract from Battlereeve Tanerline to kill them at their hunting grounds.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6390 => array(
			'name'    => "Kill Enemy Necromancers",
			'journal' => "Grand Warlord Dortene has authorized a reward for any Covenant soldier who slays 20 enemy Necromancers.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6396 => array(
			'name'    => "Chiaroscuro Crossroads",
			'journal' => "I met the Twilight Cantor, Adara'hai, and her sister, Ka'ishka, in the village of Black Heights. Ka'ishka's husband, Vijari, is terminally ill and needs to perform the last rights of his faith, but something is not right at the Wall of Life.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6397 => array(
			'name'    => "New Moon Rising",
			'journal' => "Kaalgrontiid's plan to fulfill the prophecy of the Dark Aeon and become a god continues as he sent his island, Dragonhold, into the sky. Now we need to find a way to reach the island.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6400 => array(
			'name'    => "A Childhood in Flames",
			'journal' => "A good-hearted scoundrel named Daini wants to rob local bandits of their ill-gotten goods. She means to use their wealth to leave Elsweyr with her brother, but seems intent on settling scores before she departs.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6401 => array(
			'name'    => "The Dragon's Lair",
			'journal' => "A Dragon lairs on a nearby island, and Sai Sahan believes it plans to attack Senchal. I agreed to help Sai sail to the island and slay the beast before it can harm the city.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6402 => array(
			'name'    => "The Pride of Elsweyr",
			'journal' => "We reformed the Dragonguard and defeated Kaalgrontiid. I should see if there's anything else we need to take care of here in Southern Elsweyr.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6405 => array(
			'name'    => "Taking Them to Tusk",
			'journal' => "Dirge Truptor tasked me with intercepting a courier for the Order of the New Moon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6409 => array(
			'name'    => "Reformation",
			'journal' => "Sai Sahan is reforming the Dragonguard and wants to know if our companions will join us.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6419 => array(
			'name'    => "Tomes of the Tsaesci",
			'journal' => "The Senchal job broker, Guybert, urged me to search the Forsaken Citadel for sealed Akaviri tomes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6420 => array(
			'name'    => "In Defense of Pellitine",
			'journal' => "I found a notice urging me to take up arms in defense of Southern Elsweyr.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6424 => array(
			'name'    => "Scholarly Observations",
			'journal' => "A scholar made an ill-advised trek to a guarded temple. Now I must destroy the temple guardian and gather the scholar's scattered notes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6428 => array(
			'name'    => "Sticks and Bones",
			'journal' => "Dirge Truptor said the New Moon are scheming with a necromancer in the Bone Orchard of Grahtwood.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6429 => array(
			'name'    => "Digging Up the Garden",
			'journal' => "Dirge Truptor informed me that an Akaviri solder was interred in a crypt in Deshaan. He's asked me to search it for secrets of the Dragonguard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6430 => array(
			'name'    => "File Under D",
			'journal' => "Dirge Truptor told me to recover some old Dragonguard scrolls secreted away beneath the Reliquary of Stars.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6432 => array(
			'name'    => "Solace By Candlelight",
			'journal' => "The Senchal job broker, Guybert Flaubert, urged me to place enchanted candle orbs in the halls of the Forsaken Citadel. According to his client, the candles may help the restless dead that dwell in the Citadel find peace.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6434 => array(
			'name'    => "The Dragonguard's Quarry",
			'journal' => "Dragons continue to rage across Elsweyr. Chizbari the Chipper charged me with slaying some of these great beasts in the Dragonguard's name.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6435 => array(
			'name'    => "The Dragonguard's Quarry",
			'journal' => "Dragons continue to rage across Elsweyr. Chizbari the Chipper charged me with slaying some of these great beasts in the Dragonguard's name.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6437 => array(
			'name'    => "Moonlit Mushrooms",
			'journal' => "The Senchal job broker wants me to harvest the mushrooms found in Moonlit Cove. The fungus' unique flavor is greatly prized by some Khajiiti cooks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6439 => array(
			'name'    => "Witches Festival Writ",
			'journal' => "I accepted a crafting writ related to the Witches Festival.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6441 => array(
			'name'    => "Song of the Sand-Whale",
			'journal' => "A traveler from Skyrim named Vigwenn Owl-Watcher claims to have the recipe for a secret concoction that allows the imbiber to see the invisible sand-whales of Elsweyr and hear their haunting song.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6447 => array(
			'name'    => "Out of Murkmire",
			'journal' => "Nomu and his friends from Murkmire were chased out of the quarry by cultists and now his friends are missing. I promised to try to find them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6451 => array(
			'name'    => "Masterful Jewelry",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6452 => array(
			'name'    => "Masterful Jewelry",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6453 => array(
			'name'    => "The Good Bits",
			'journal' => "A man named <<1>> recently arrived at the Dragonguard Sanctum, claiming we should be gathering ingredients from Dragon corpses on our hunts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6456 => array(
			'name'    => "The Gray Host",
			'journal' => "High King Svargrim refuses to act, so Svana has decided to take matters into her own hands. With Fennorian's help, she plans to develop a strategy for fighting back against the Gray Host.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6462 => array(
			'name'    => "Danger in the Holds",
			'journal' => "With the high queen's murderer brought to justice and a clear connection established between the assassin and the Icereach Coven, Svana thinks we have enough evidence to convince High King Svargrim that the threat to his kingdom is real.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6463 => array(
			'name'    => "The Coven Conundrum",
			'journal' => "Lyris Titanborn and I uncovered a conspiracy involving the witches of the Icereach Coven. The trail leads to an isolated inn somewhere in the wilds of Eastmarch. We need to determine what the coven has planned before it's too late.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6466 => array(
			'name'    => "The Vampire Scholar",
			'journal' => "The high king has finally taken steps, but it isn't enough. Svana thinks we need to develop a defense against the harrowstorms. To that end, she wants to recruit the vampire scholar, Fennorian, to aid us.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6469 => array(
			'name'    => "Orchestrations",
			'journal' => "Leiborn, a member of the Bards College, asked me to recover a famous instrument called Petraloop. Apparently, this lute's safe return might help two rival factions in the College reconcile.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6482 => array(
			'name'    => "A Salskap to Remember",
			'journal' => "Leiborn intends to host a salskap-a bard's gathering to celebrate the recovery of the Bards College instruments. It's sure to be quite a show.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6486 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6487 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6494 => array(
			'name'    => "A Trail Gone Cold",
			'journal' => "The Coterie of Organized Scholars has asked me to discover evidence of an undiscovered tribe living in Chillwind Depths.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6497 => array(
			'name'    => "The Tones of the Deep",
			'journal' => "Kireth Vanos enlisted my aid in helping her and her brother, Raynor, investigate the ancient Dwarven ruin of Nchuthnkarst.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6501 => array(
			'name'    => "Cultural Conciliation",
			'journal' => "Rigurt the Brash is in Western Skyrim on behalf of the Skald-King. He needs my help to complete his mission.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6503 => array(
			'name'    => "The Fight for Kyne's Aegis",
			'journal' => "An unlikely alliance of Sea Giants and vampires invaded Kyne's Aegis. If the survivors are to have any hope of escape, I must drive these monsters back into the sea. Hopefully, the island's defenders will rally behind me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6520 => array(
			'name'    => "Precious Bark",
			'journal' => "The Coterie of Organized Scholars has tasked me with collecting fresh spriggan bark.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6524 => array(
			'name'    => "Spiritual Release",
			'journal' => "The Coterie of Organized Scholars tasked me to destroy several umbric urns in Midnight Barrow.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6526 => array(
			'name'    => "Feasting in the Dark",
			'journal' => "The Swords of Solitude tasked me with hunting down vampires who are dragging their victims underground to feast on them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6528 => array(
			'name'    => "Halt the Harrowstorms",
			'journal' => "<<1>> charged me with finding allies to assault the harrowstorms forming around Western Skyrim.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6534 => array(
			'name'    => "Inguya's Mining Samples",
			'journal' => "I found a dead Nord woman in the caverns of Blackreach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6536 => array(
			'name'    => "Ghamborz's Mining Samples",
			'journal' => "I found a dead miner in the passages of Blackreach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6538 => array(
			'name'    => "Adanzda's Mining Samples",
			'journal' => "I found a note in the passages of Blackreach. There was no sign of anyone around.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6550 => array(
			'name'    => "The Despot of Markarth",
			'journal' => "Count Verandis Ravenwatch seeks to uncover a plot to conquer the Reach by an army of vampires and werewolves that calls itself the Gray Host. To succeed, he needs to earn the trust of the region's sovereign: the dreaded Despot of Markarth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6555 => array(
			'name'    => "The Gray Council",
			'journal' => "Gwendis and I discovered a letter from Rada al-Saran to her mentor, Verandis, in a hidden room in Ravenwatch Castle. She asked me to accompany her to investigate the secret island, Grayhome, and the Gray Host castle mentioned in the letter.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6556 => array(
			'name'    => "Namira's Perversions",
			'journal' => "The Ard's Stonehands are concerned about a witch matron of a rogue Reach clan attempting a dangerous ritual. I need to put a stop to this ritual and rescue the innocent Reachfolk that were abducted to carry it out",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6560 => array(
			'name'    => "Kingdom of Ash",
			'journal' => "Rada al-Saran used the orrery to reach his sanctuary realm in Oblivion, where he will perform the Darkstorm ritual that will destroy the Reach and make the Gray Host truly immortal. To stop him, we must follow him into the Void.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6564 => array(
			'name'    => "Glory of the Undaunted",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6566 => array(
			'name'    => "A Feast of Souls",
			'journal' => "The Dark Heart has awakened and grows more powerful with every beat. While Verandis attempts to deal with Rada al-Saran, I need to go to Markarth and stop Lady Belain from sacrificing the entire city to the Void.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6569 => array(
			'name'    => "What's Hers is Ours",
			'journal' => "<<1>> explained that his contacts in the Wildblood clan think a hagraven matron is influencing the Thornroot clan. I need to take a powerful totem in her possession in order to weaken her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6570 => array(
			'name'    => "Second Chances",
			'journal' => "Rada al-Saran is dead and the victims of the Gray Host's assault on Markarth have been restored to life thanks to Verandis.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6573 => array(
			'name'    => "Unhatched Menace",
			'journal' => "A crazed mage performing experiments in Gloomreach has a strange egg that has the potential to be dangerous if it hatches. I volunteered to destroy the egg.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6575 => array(
			'name'    => "Endeavor in the Gloom",
			'journal' => "I met a recent recruit of the Psijic Order. He asked for my help investigating strange behavior in the local plant life, which he fears may be a result of his own errant magic. I agreed to accompany him into the cave to take a closer look.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6582 => array(
			'name'    => "Ruptures in the Reach",
			'journal' => "The Ard's Stonehands asked me to investigate a Reach clan's witch matron and put a stop to a dangerous ritual. Supposedly, the ritual creates tears in reality. I also need to close any I find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6589 => array(
			'name'    => "Red Eagle's Song",
			'journal' => "Lyris Titanborn requested my assistance. She found something at the Reachwind Ritual Site and needs help solving a problem.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6592 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "I accepted a contract to craft supplies using blacksmithing for civilians displaced by the Three Banners War.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6593 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "I accepted a contract to craft supplies using the clothier craft for civilians displaced by the Three Banners War.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6595 => array(
			'name'    => "A Charitable Contribution",
			'journal' => "I accepted a contract to craft supplies using provisioning for civilians displaced by the Three Banners War.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6598 => array(
			'name'    => "The Symbol of Gulibeg",
			'journal' => "I came across the body of <<1>>. He apparently died after stealing a sacred relic of Hircine bound for a shrine in Markarth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6600 => array(
			'name'    => "Remnants of the Deep Folk",
			'journal' => "The Wayward Guardians need Dwarven scrap to augment their arms and armor. Ardanir asked me to search the ruins of Bthardamz for the precious metal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6602 => array(
			'name'    => "The Light of Arkthzand",
			'journal' => "The shamans of the Wayward Guardians claim that certain Dwarven constructs contain gemstones imbued with powerful magic. Ardanir wants me to acquire some of these gemstones.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6605 => array(
			'name'    => "The Language of the Night",
			'journal' => "The Wayward Guardians seek writings from Nighthollow Keep so their witches can learn more about the magic of the Nighthollow vampires.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6609 => array(
			'name'    => "The Symbol of Uricanbeg",
			'journal' => "High Shaman Glynroch sent Apprentice Orla to find the Symbol of Uricanbeg, the final sacred relic needed to dedicate the shrine, at Lost Valley Redoubt. She hasn't returned yet, so he asked me to go aid her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6610 => array(
			'name'    => "Vateshran's Rites",
			'journal' => "The Rites Matron challenged me to complete the Vateshran's Rites within Vateshran Hollows. My success determines whether I earn the right to stay in the Reach.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6613 => array(
			'name'    => "An Abundance of Stibbons",
			'journal' => "Lady Clarisse Laurent's manservant Stibbons broke a relic in a nearby xanmeer and unleashed a curse. Now the Sithis priest Somarz demands that she make things right. With Stibbons somehow incapacitated, Lady Laurent needs help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6614 => array(
			'name'    => "Pyre of Ambition",
			'journal' => "Vandacia and the Order of the Waking Flame seem to be determined to attack Gideon and capture the Ambitions. Governor Keshu promised to protect them, but wants to take them somewhere more defensible than the city. Somewhere such as Fort Redmane.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6619 => array(
			'name'    => "A Mysterious Event",
			'journal' => "With the twins safely under the protection of Keshu's Black Fin Legion, Eveli and I need to figure out how to find the other Ambitions before Vandacia and his cultists beat us to it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6620 => array(
			'name'    => "The Rising Winds",
			'journal' => "A resident of the Stonewastes named Nuxul explained that his village was attacked by a warlord named Naxhosa who stole their supplies. Nuxul asked me to help him find the remaining Three Winds, famous Argonian protectors, and ask for their help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6623 => array(
			'name'    => "The Golden Anvil",
			'journal' => "A priest of Zenithar asked me to help him locate a lost artifact of his faith, the Golden Anvil, in Zenithar's Abbey.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6624 => array(
			'name'    => "Idol Threats",
			'journal' => "An Argonian named Jela asked for my help to rescue her friends from the Sul-Xan, a dangerous tribe that seeks to use them to summon a spirit of destruction using something called the Idol of Xulunaht.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6630 => array(
			'name'    => "A Hidden Vault",
			'journal' => "Eveli Sharp-Arrow asked me to accompany her to Gideon as we run down the clue provided by the deciphered Imperial documents and try to find the hidden vault that contains the Four Ambitions.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6632 => array(
			'name'    => "The King's Spoils",
			'journal' => "<<1>> needs help gathering fermented mubcrab apples to brew into a special festival cider of her own design. They have all been harvested for King Boar in the Festival Arena and I'll need to compete with the hungry boar to get them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6634 => array(
			'name'    => "The Ghostwriter of Giovesse",
			'journal' => "A romance novelist named Yisara asked me to help her explore the haunted Castle Giovesse. She seems convinced that the castle's tragic history will help inspire her next novel.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6637 => array(
			'name'    => "In Need of a Bath",
			'journal' => "An Argonian named Hokatsei claims the Nereid of the ruins of Vunalk, Sister of Pools, turned her brother, Deet-Loh, into a pig. She needs help to return to the ruins and break the spell over her brother.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6641 => array(
			'name'    => "Wisdom of the Druids",
			'journal' => "I met a Stonelore Circle druid named Jend. He's worried about the recent activity around the volcanic vents in the nearby area. He asked me to help investigate said vents and find a solution to the problem.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6645 => array(
			'name'    => "Legend of the Man-Bull",
			'journal' => "I learned that the Minotaur Bhrum-Koska is terrorizing an archeological site in southern Blackwood. I agreed to defeat Bhrum-Koska and remove any unearthed heirlooms that might be drawing Minotaurs to the area.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6652 => array(
			'name'    => "The Trophy-Taker",
			'journal' => "Xeemhok the Trophy-Taker, a brutal Argonian behemoth, lurks in the ruins near Vunalk. Dozens of brave warriors have challenged him and died. I agreed to take him on, and search the fallen for tokens by which they can be identified.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6658 => array(
			'name'    => "Making a Racket",
			'journal' => "The owners of the Happy Averno Shipping Company, Iulius and Scipion Averno, asked for my help dealing with a gang in Blackwood called the Fang-Furls.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6661 => array(
			'name'    => "Haunted Legacy",
			'journal' => "The people of Gideon believe that a woman named Cerise Gemain murdered a beloved priestess sixty years ago, but her grandson, Faric, insists she's innocent of the charges. He enlisted me to help him prove it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6662 => array(
			'name'    => "Things Lost, Things Found",
			'journal' => "",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6665 => array(
			'name'    => "Shadow Tactics",
			'journal' => "<<1>>'s client heard rumors of ancient caches containing old information about the Morag Tong in Bloodrun Cave. I need to go to Bloodrun Cave, find the documents, and destroy them on behalf of the client.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6667 => array(
			'name'    => "Dead Weight",
			'journal' => "Mirri remains concerned about her mother's obsession with finding her father's stolen puzzle box. Mirri believes it's only a matter of time before Irenni goes searching again, so she wants to take up the search herself.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6670 => array(
			'name'    => "The Pirate's Gift",
			'journal' => "I found a message written by a pirate of the Topal Bay in a bottle by the shore. It appears to be directions to buried treasure.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6675 => array(
			'name'    => "Insect Savior",
			'journal' => "<<1>> asked me to enter <<2>> on behalf of a local bug enthusiast to bring back a special breed of torchbug before the cavern floods.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6686 => array(
			'name'    => "Pledge: The Dread Cellar",
			'journal' => "<<1>> of the Undaunted has challenged me to explore the Dread Cellar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6693 => array(
			'name'    => "Hope Springs Eternal",
			'journal' => "Mehrunes Dagon has been defeated and his plot to take over Tamriel has been thwarted. Eveli thinks we need to check in with our allies and rendezvous in Leyawiin.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6694 => array(
			'name'    => "Peace's Wretched Price",
			'journal' => "Tephilaz, a Daedra living in Wretched Spire, died suddenly on the street. The locals suspect someone murdered him to stop a treaty. A Dremora named Noroth seeks the killer before more violence erupts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6695 => array(
			'name'    => "Plucking the Crow",
			'journal' => "Witchmother Taerma asked for my help to defeat a Wraith-of-Crows from the Evergloam called the Crowborne Horror. She tasked me with collecting feathers from vile creatures in order to form an effigy that can bring the creature forth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6697 => array(
			'name'    => "Ambition's End",
			'journal' => "Sombren decided to join Mehrunes Dagon. He stole Calia's power and added it to his own. Now we must protect the Anchorite and figure out how to find and use the egonymic if we hope to defeat Sombren and Mehrunes Dagon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6707 => array(
			'name'    => "The Durance Vile",
			'journal' => "We destroyed an Incarnate at Burning Gyre Keep, but Lyranth believes that the Waking Flame plans to create more. She wants to find the Riven Cataclyst before they do-and she thinks the Bladebearer Dremora can point us to our goal.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6708 => array(
			'name'    => "Born of Grief",
			'journal' => "The mad prisoner Elegian, creator of the Riven Cataclyst, told me we can get to the device by going to Annihilarch's Summit. I need to locate the cataclyst before Sister Celdina unleashes more Incarnates to attack Fargrave and Nirn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6709 => array(
			'name'    => "Mercenary in the Making",
			'journal' => "I found a notice that says House Hexos in Fargrave seeks adventurers for dangerous work.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6710 => array(
			'name'    => "Propaganda Suppressed",
			'journal' => "A merchant named Fyna commissioned Luna Beriel and the Gleaners of Aurbis to sneak into the Brandfire Refomatory and destroy the propaganda the Order of the Waking Flame is making there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6711 => array(
			'name'    => "Brandfire Reformatory Relics",
			'journal' => "Luna Beriel tasked me with recovering relics from the Brandfire Reformatory, a fearsome building in the Deadlands where the Waking Flame cult takes prisoners and doesn't release them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6712 => array(
			'name'    => "Forgotten Grave Goods",
			'journal' => "Luna Beriel has witness accounts of some valuables lying in the False Martyrs' Folly from when the Martyrs there were turned to stone. She wants me to go to the Folly and bring the items back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6713 => array(
			'name'    => "Flowers in the Folly",
			'journal' => "Luna Beriel tasked me with going into the False Martyrs' Folly to retrieve some martyrs' copperleaf plants for Tuleenith, a healer in Fargrave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6714 => array(
			'name'    => "Clandestine Communications",
			'journal' => "Luna Beriel gave me some papers containing important information from the Gleaners founder, Pieron Desant. The letters need to be dropped in letterboxes throughout the Blood Pit for the Gleaners' agents imprisoned there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6717 => array(
			'name'    => "The Mind's Eye",
			'journal' => "Vaveli Indavel asked me to place seeing stones around the den of the Unmaker and kill the creature on the behalf of Vares Dredayn, who is studying the creature's death cycle.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6718 => array(
			'name'    => "Unmaker's Hoard",
			'journal' => "The Unmaker presents a challenge to even the strongest Daedra, so many try to test their might against it and fail. Vaveli Indavel hired me to kill the Unmaker, as well as the Havocrel around it, and bring back armor for the Saraathu Tong.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6722 => array(
			'name'    => "Getting the Band Together",
			'journal' => "The Breton Samuel Gourone has a plan to liven up the Jester's Festival. He asked for my help in gathering minstrels for a band that's \"out of this realm!\"",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7432 => array(
			'name'    => "District Field Journal",
			'journal' => "I discovered an unfinished field journal covered in webs. I should check the journal in my inventory and see if I can complete the unfinished notes and turn them in for a reward at a collection box on a rooftop oasis.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6741 => array(
			'name'    => "Pledge: Coral Aerie",
			'journal' => "<<1>> of the Undaunted has challenged me to explore <<2>>.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6752 => array(
			'name'    => "Of Knights and Knaves",
			'journal' => "Lady Arabelle Davaux needs help investigating a mystery that threatens a crucial conference to be held on High Isle, in the Systres Archipelago.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6753 => array(
			'name'    => "People of Import",
			'journal' => "The Ascendant Order wrecked the ships carrying the alliance leaders to High Isle, but they were not washed ashore. Lady Arabelle believes that our best chance to find the missing delegates is to investigate the Order and learn who's behind this.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6754 => array(
			'name'    => "Deadly Investigations",
			'journal' => "We found a map at Castle Navire that shows several possible meeting sites used by the Ascendant Order. Lady Arabelle believes investigating these sites is our best chance to track down the missing alliance leaders.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7448 => array(
			'name'    => "A Madcap Mystery",
			'journal' => "Sheogorath was poisoned at the cheese festival and his staff was stolen. We traced Othuk and Lazaki to the Violet Vvardvark Tavern. I need to find out where they took Sheogorath's staff.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6757 => array(
			'name'    => "Scalding Scavengers",
			'journal' => "A High Elf named Vinilsare asked me to escort her through the Firepot, a dangerous cavern, to collect vulk'esh scoria and ensure her wife, Tarrinore's, safety.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6761 => array(
			'name'    => "A King's Retreat",
			'journal' => "Jakarn and I recovered the invitation medallions and returned them to Lady Arabelle. She asked for my continued help. She wants me to deliver medallions to the Mages Guild and Fighters Guild delegates.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6767 => array(
			'name'    => "Race for Honor",
			'journal' => "Cynric, the ghostly guardian of Garick's Rest, challenged me to complete three Trials of Worth. He wants me to honorably earn Lady Garick's Sacred Shield before the necromancer Amunara steals it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6772 => array(
			'name'    => "An Experiment with Peace",
			'journal' => "A strange malady is affecting the patients of Steadfast Manor. The healers are lost on how to treat it. If I find the cause, perhaps they can determine a solution.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6773 => array(
			'name'    => "The Missing Prowler",
			'journal' => "Captain Jimila has lost her ship, the Prowler. She and her quartermaster, Oblan, discovered it missing after coming ashore for supplies. I agreed to help find the ship so they can get off Skulltooth Coast.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6778 => array(
			'name'    => "Aiding the Archipelago",
			'journal' => "I found a notice for adventurers seeking work on the islands of High Isle and Amenos. Sounds right up my alley.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6781 => array(
			'name'    => "A Chance for Peace",
			'journal' => "We defeated the Ascendant Magus at All Flags Islet and saved the alliance leaders. Lady Arabelle believes there's still a chance to salvage Lord Bacaro's peace talks.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6783 => array(
			'name'    => "Reavers of the Reef",
			'journal' => "Admiral Galvendier of the Dufort Flotilla requires soldiers for an assault on the Dreadsail pirate fleet's headquarters.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6784 => array(
			'name'    => "All Hands on Deck",
			'journal' => "I met a sailor recruiting for war against the Dreadsail pirate fleet. I agreed to join a landing party for a mission.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6794 => array(
			'name'    => "Rhadh's Map Fragment",
			'journal' => "I found the body of a Redguard, Rhadh. It looks like he died before retrieving a map for <<1>> at the Dufort Shipyards.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6799 => array(
			'name'    => "Tales of Tribute",
			'journal' => "A new card game called Tales of Tribute has captured the imagination of cardsharps and collectors all across Tamriel. I can begin my own Tribute journey in Gonfalon Bay.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6800 => array(
			'name'    => "Cards Across the Continent",
			'journal' => "The Gonfalon Chapter's daily broker, Kishka, challenged me to seek out rival Tribute players and defeat them. If I succeed, I will earn both rank and rewards!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6803 => array(
			'name'    => "Avarice of the Eldertide",
			'journal' => "A pair of Eldertide theurges have taken control of a local lord's land, searching it for magical geodes. I need to defeat the theurges and collect the geodes they've unearthed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7496 => array(
			'name'    => "The Quill Pen",
			'journal' => "<<1>> runs a finder's business in Glenumbra and uses the Thieves Guild to track down lost, mislaid, and stolen items. Helping her will surely reflect well on my skill as a thief-and be lucrative to boot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6805 => array(
			'name'    => "A Final Peace",
			'journal' => "An old knight of the Order of the Albatross wants someone to brave the ruins of Death's Valor Keep and retrieve the skulls of the knights who fell there long ago so they can finally be put to rest.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6813 => array(
			'name'    => "Venting the Threat",
			'journal' => "Druid Peeska is worried about the increasing volcanic activity at the volcanic vents on the island. She hired me to gather allies and disrupt the activity before the volcanic vents can erupt and the situation becomes dire.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6816 => array(
			'name'    => "The Serpent Caller",
			'journal' => "A Sea Elf fanatic named Serpent Caller Vinsha is manipulating snakes native to High Isle, magically altering them to make them more dangerous. The Stonelore Circle Druids want her stopped.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6831 => array(
			'name'    => "Cards Across the Continent",
			'journal' => "The Gonfalon Chapter's daily broker, Kishka, challenged me to seek out rival Tribute players and defeat them. If I succeed, I will earn both rank and rewards!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6832 => array(
			'name'    => "Dueling Tributes",
			'journal' => "The Gonfalon Chapter's daily broker, Marunji, challenged me to seek out adventurers like myself and defeat them in Tales of Tribute. If I succeed, I will earn both rank and rewards!",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6835 => array(
			'name'    => "The Stonelore Defense",
			'journal' => "Help Druid Laurel stop the Firesong Circle Druids and Archdruid Devyric from getting the Stonelore circle's sacred seed.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6837 => array(
			'name'    => "Haunted Depths",
			'journal' => "Dhulef and I crashed onto an uncharted island. The storm that wrecked us rose up from the island without warning. We have no way off without another ship.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6838 => array(
			'name'    => "Pledge: Graven Deep",
			'journal' => "Urgarlag Chief-bane of the Undaunted has challenged me to explore Graven Deep.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6848 => array(
			'name'    => "The Ivy Throne",
			'journal' => "Lord Bacaro, who we learned is secretly the Ascendant Lord, plans to steal the Druid King's regalia and proclaim himself the new Druid King. Lady Arabelle wants me to help stop him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6853 => array(
			'name'    => "Guardian of Y'ffelon",
			'journal' => "Archdruid Orlaith demanded Sir Stefan surrender Frii. She threatened to kill captives she took from Vastyr unless he complied. Sir Stefan has a plan of his own to thwart the Firesong.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6854 => array(
			'name'    => "Cursed and Coniferous",
			'journal' => "I found Joslin and Octin, a pair of Breton thieves who broke a stone tablet. Now they're cursed. They asked me to fix the tablet before Joslin turns into a tree and Octin's transformation into a sprite is permanent.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6858 => array(
			'name'    => "A Heart Turned Black",
			'journal' => "Elder Pitof of the Eldertide Druids explained that Firesong Druids attacked his home. His wife, Gwenaelle, was lost in the chaos. He asked for my help to find her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6860 => array(
			'name'    => "The Freebooters",
			'journal' => "Lanazha and Kiko the bird are the sole survivors of a Dreadsails attack while searching for an ancient treasure. I agreed to help Lanazha solve the bird's riddles and find the Gloomlantern.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6880 => array(
			'name'    => "Soldier of the Isles",
			'journal' => "I found a notice that says House Mornard in Vastyr seeks adventurers for dangerous work across Galen.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6881 => array(
			'name'    => "A Wailing Wood",
			'journal' => "A group of House Mornard hirelings have been taken captive by a forest wraith in the Valley of the Watcher. I need to defeat the wraith and rescue the hirelings.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6883 => array(
			'name'    => "Radiant Souvenirs",
			'journal' => "Juline Courcelles needs me to travel to Embervine on the eastern coast of Galen and collect radiant cores. She thinks they will make good souvenirs for guests on her tour.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6884 => array(
			'name'    => "Volcanic Virtuoso",
			'journal' => "Juline Courcelles asked me to travel to Embervine on the eastern coast of Galen and sketch the various natural phenomena within.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6886 => array(
			'name'    => "Flower Fancier",
			'journal' => "Juline Courcelles wants me to collect kingsblood blossoms in Fauns' Thicket. She believes the flowers would show Galen's natural beauty to patrons of Savage Systres Tours.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6890 => array(
			'name'    => "Recovered Relics",
			'journal' => "Druid Gastoc asked me to venture to the Grove of the Chimera to retrieve some druid relics that have fallen into the hands of the Firesong. I need to recover the relics and defeat the empowered chimera.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6892 => array(
			'name'    => "Shrines on Shaky Ground",
			'journal' => "Increasing activity from Mount Firesong has driven a chimera into a destructive frenzy. Druid Gastoc asked me to deal with the chimera and check on a series of Stonelore shrines.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6896 => array(
			'name'    => "Unstuck From Time",
			'journal' => "As I arrived at the stronghold of Bal Sunnar, several Dark Elves recognized me and ran away. A mage named Saresea has come to find the cause of a temporal distortion. I agreed to help her contain its source.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6897 => array(
			'name'    => "Pledge: Bal Sunnar",
			'journal' => "<<1>> of the Undaunted has challenged me to explore Bal Sunnar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6905 => array(
			'name'    => "Spice of Life",
			'journal' => "Chef Rhazeez has enlisted me as his assistant. He wants to create the spiciest dish in Tamriel using local Galen ingredients.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6967 => array(
			'name'    => "Eye of Fate",
			'journal' => "Leramil the Wise claims that the Daedric Prince of Fate, Hermaeus Mora, needs my help to stop a threat that could destroy both Mora's realm of Apocrypha and the mortal realm of Nirn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6972 => array(
			'name'    => "Keeper of the Fate",
			'journal' => "Curate Gadayn is suspicious of Prelate Faram, recently arrived at the Necrom Necropolis. The prelate closed the catacombs due to a mysterious illness. I should determine if this is related to the threads of fate I was sent to investigate.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6973 => array(
			'name'    => "Spirit of Fate",
			'journal' => "Leramil the Wise met me near the abandoned tower of Tel Rendys. She senses that both of us will be required to learn why Tel Rendys is important to stopping the threat facing Hermaeus Mora's realm and Nirn.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6974 => array(
			'name'    => "Fate's Lost Dream",
			'journal' => "I found the Watchling Scruut brooding outside Alavelis. She was drawn here by some sort of contradiction in fate that may involve Daedric cultists. I agreed to help Scruut investigate the situation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6978 => array(
			'name'    => "Underwritten",
			'journal' => "The Kwama Consortium of Necrom dispatched an auditor named Cindieth to inspect the Anchre Egg Mine after a sudden halt in production. Expecting trouble, Cindieth hired me to accompany her as a bodyguard.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6979 => array(
			'name'    => "The Traitor's Trade",
			'journal' => "A vile criminal named Luaffyn Romavel captured several Argonian soldiers in hopes of extracting a ransom from the Ebonheart Pact. With my help, Ashur of the Morag Tong intends to execute Romavel and set the captives free.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6982 => array(
			'name'    => "Destiny's Tide",
			'journal' => "I came across a lost High Elf sailor named Landolvel that washed ashore at Fathoms Drift. He has asked for my help in finding his father, Naanurrel, the ship's captain.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6985 => array(
			'name'    => "Llaro's Headache",
			'journal' => "Balver Bemis, a retainer of House Telvanni, asked me to collect some grisly ingredients from across the Telvanni Peninsula to help him brew a tincture for his superior, Llaro Ralas.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6997 => array(
			'name'    => "Under Malign Stars",
			'journal' => "A cipher by the name of Meenai-Shai seems to be at the center of a mysterious plot housed within the Sidereal Cloisters.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	6998 => array(
			'name'    => "A Thief to Catch a Thief",
			'journal' => "Sailenmora is home to the Shrine of Saint Vorys, which houses the Talisman of Saint Vorys, a sacred relic of the patron saint of House Telvanni. A dashing rogue named the Lark of Rosgard intends to steal it. Armiger Rilasi enlisted me to stop him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7013 => array(
			'name'    => "Bastion Nymic - Glasscrag",
			'journal' => "Ordinator Tandasea told me of a dangerous threat to Bastion Nymic, a stronghold in Hermaeus Mora's realm of Apocrypha. I agreed to find my way to the bastion and defeat Hermaeus Mora's enemies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7015 => array(
			'name'    => "In a Troubled House",
			'journal' => "A Dark Elf by the name of Garalo Givyn wants to find the research of a reclusive portal mage. He asked me to accompany him to the nearby island of Gorne.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7017 => array(
			'name'    => "The Double Edge",
			'journal' => "Mercenary Sharp-as-Night needs my help tracking down someone named Reynila Treviri. He thinks Reynila sent rival mercenaries to throw him off her trail. I agreed to help Sharp gather information on Reynila's last known whereabouts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7018 => array(
			'name'    => "Between a Rock and a Whetstone",
			'journal' => "Sharp has been looking for his former job broker, Federo, to get information on the Reformer, the mysterious Telvanni wizard whose experiments resulted in Sharp's lost memory.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7021 => array(
			'name'    => "The Fateweaver Key",
			'journal' => "A talkative man named Azandar attempted to craft a magic item in Apocrypha. Now he's plagued by something he calls frayed fates. He wants my help to track down an old friend of his and fix his Fateweaver Key.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7208 => array(
			'name'    => "Mini Menagerie",
			'journal' => "I found a note from a trapper named Marcien in the wilderness, along with a snared adolescent guzzard. According to the note, if I deliver the guzzard to Star-Gazer Sariyaah in Ontus, I'll be rewarded.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7029 => array(
			'name'    => "Burdensome Beasts",
			'journal' => "Daedric creatures from Plokun's menagerie run wild across Blackwood. They can cause untold destruction if I don't find a way to return them to their master.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7033 => array(
			'name'    => "Egg Raider",
			'journal' => "Ordinator Tilena asked me to enter the Anchre Egg Mine. The eggs mined there are a rare commodity and in high demand, but the miners have stopped producing.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7040 => array(
			'name'    => "Tracking Nightmares",
			'journal' => "The Walking Nightmare is terrorizing the Telvanni Peninsula, and most recently killed a scholar researching it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7049 => array(
			'name'    => "Numinous Grimoire, Volume 2",
			'journal' => "I found the body of Cipher Dreyla in Apocrypha. It looked like he died suddenly, leaving a note and a pack beside his body.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7056 => array(
			'name'    => "Bastion Nymic - Fungal Downs",
			'journal' => "Ordinator Tandasea told me of a dangerous threat to Bastion Nymic, a stronghold in Hermaeus Mora's realm of Apocrypha. I agreed to find my way to the bastion and defeat Hermaeus Mora's enemies.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7061 => array(
			'name'    => "The Margins of Ire",
			'journal' => "Tho'at Replicanum is creating enemies of ink from books in the Infinite Archive. Master Malkhest has a plan to summon Tho'at Replicanum, but he needs my help to track down the pieces of her nymic and defeat her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7063 => array(
			'name'    => "Turn the Page",
			'journal' => "Pages of a stolen Apocryphal tome have been scattered after its curse was triggered. I was tasked with gathering as many as I can find.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7248 => array(
			'name'    => "Party Pains",
			'journal' => "Acolytes of Dibella entered the Vale of Revelry, a party dedicated to the Daedric Prince of Revelry, Sanguine, and have not returned. Their high priest, Caroril, asked me to journey into the vale, find the missing acolytes, and help them leave.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7069 => array(
			'name'    => "Aiding the Infinite Archive",
			'journal' => "Filer Pix, a Watchling under the service of a Master Malkhest, asked me to help defend the mysterious Infinite Archive.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7405 => array(
			'name'    => "Fleet and Swift",
			'journal' => "My faction leader hired me to complete a daring race in the Night Market's districts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7072 => array(
			'name'    => "Seeds of Suspicion",
			'journal' => "Beragon believes the best way to get to the bottom of West Weald's troubles is to find the source of the wildburn seeds. He thinks that the seed we found can lead us to the Recollection … or the Recollection can lead us to more seeds.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7075 => array(
			'name'    => "The Untraveled Road",
			'journal' => "Shardmarshal Vargas's Mirrormoor forces still threaten West Weald, but Beragon thinks they're mostly meant to keep us from interfering in whatever Ithelia plans to do next. We need to find the Prince and prevent her from destroying our reality.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7078 => array(
			'name'    => "In Memory Of",
			'journal' => "Even though Ithelia has been exiled and Torvesard defeated, the threat to all of existence posed by the tears in reality remains. I need to find a way to repair them before the fabric of fate fully unravels.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7082 => array(
			'name'    => "Where There's a Will",
			'journal' => "Mizzik Thunderboots is helping execute the will of the late Lady Weatherleah. A cryptic will and family secrets lead him to suspect foul play, but he needs my help in uncovering the truth and resolving ownership of the inheritance.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7084 => array(
			'name'    => "The Many Paths",
			'journal' => "Leramil the Wise claims that the threads of fate have formed a dangerous tangle around the Outcast Inn. She's blocked from entering by her own ability to perceive the threads. I agreed to go inside, find the tangle's source, and end the threat.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7088 => array(
			'name'    => "Vesper Valente",
			'journal' => "Fennorian, a vampire of House Ravenwatch, asked me to help investigate a suspicious auction for a wine called Vesper Valente hosted by Gallio Valente at the Valente Vineyards and Winery.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7091 => array(
			'name'    => "Restorer of Theories",
			'journal' => "Destozuno requested my help in gathering pages of a vast body of work called Theories on Apocrypha.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7272 => array(
			'name'    => "Wo-Xeeth's Wrath",
			'journal' => "<<1>> told me of dangerous Corelanya relics left in the lair of Wo-Xeeth, an Argonian behemoth changed somehow from the effects of the Writhing Wall. I agreed to dispose of Wo-Xeeth and recover the relics.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7093 => array(
			'name'    => "Caught in a Trap",
			'journal' => "Leftwheal Trading Post has fallen victim to an attack from within driven by shining beasts and cultists. Baroness Eliana Cossa has requested help in rescuing her citizens and ending the invasion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7098 => array(
			'name'    => "The Silent Bell",
			'journal' => "A band of deserters have taken control of Fort Colovia and driven out the small Legion cohort of raw recruits. Captain Dromius wants me to take Legionary Calvo back inside and ring the warning bell to alert the rest of the Legion to the danger.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7100 => array(
			'name'    => "Seeking Armistice",
			'journal' => "Goblins in West Weald are getting ready to go to war. Fearing for their lives, the local farmers want to hire hunters to take care of the Goblins, but others are looking for a way to stop the war and save the Goblins.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7101 => array(
			'name'    => "A Bookbinder's Bind",
			'journal' => "A librarian named Destozuno wants me to collect the pages from a book called Fynboar the Resurrected, which have been scattered throughout the Infinite Archive.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7104 => array(
			'name'    => "The Second Era of Scribing",
			'journal' => "Votary Nahlia, a Knight in the Order of the Lamp, is exploring some kind of magical anomaly. She's requested assistance through the Mages Guild to investigate this source of unknown and untamed power.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7105 => array(
			'name'    => "Victory Through Strife",
			'journal' => "A group called the Recollection has invaded the Oathsworn Pit. Bazrag asked me to help him repel these invaders.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7107 => array(
			'name'    => "Skingrad Seeks Adventurers!",
			'journal' => "I found a notice that says the West Weald Legion seeks adventurers for dangerous work. <<1>> will know more.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7108 => array(
			'name'    => "Venom Hunt",
			'journal' => "Commandant Salerius asked me to enter Legion's Rest and gather venom crystals from the crystaljack wasps nesting there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7110 => array(
			'name'    => "Mirrormoor Incursion",
			'journal' => "Daedra from Mirrormoor are launching attacks throughout West Weald. <<1>> hired me to join forces with other adventurers to defeat a Mirrormoor incursion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7114 => array(
			'name'    => "A Study in Tharriker",
			'journal' => "<<1>>  asked me to investigate Fyrelight Cave to see if the tharriker have established dens and are laying eggs. He wants to learn as much as possible about the newly arrived creatures.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7116 => array(
			'name'    => "Fate-Eater",
			'journal' => "I agreed to gather allies and head to the far north of Skingrad to dispatch a Daedra called Stri the Fate-Eater and recover the armaments of the legionaries originally sent to deal with the creature.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7296 => array(
			'name'    => "The Gift of Death",
			'journal' => "Prince Azah fears the Stirk Fellowship can't defeat Wormblood and his followers with swords or spells, but we have something they need: the Gift of Death. We need to find a way to use the Gift against the Worm Cult.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7117 => array(
			'name'    => "Training Camp",
			'journal' => "A group of Goblins have set up a training camp for tharrikers. I agreed to destroy their training supplies and slay the tharrikers guarding the camp.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7119 => array(
			'name'    => "Recollection Rendezvous",
			'journal' => "The Legion learned that Recollection leaders are meeting to the west of Skingrad at Frontier's Cradle. I agreed to dispatch the leaders and retrieve any information I find there.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7120 => array(
			'name'    => "Hazardous Waters",
			'journal' => "Oakenclaw, a powerful aggression of root, has taken control of Lake Olo to the east of Skingrad, putting its citizens in danger. I agreed to slay Oakenclaw and place bulletins warning passersby of the threat.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7187 => array(
			'name'    => "Thorns and Blossoms",
			'journal' => "Tanlorin received an invitation to meet with the Gardener, the leader of the Garland Ring. Tan hopes there's been progress in the search for answers after the Ceythalmor attack on them and their friends.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7188 => array(
			'name'    => "Uprooted",
			'journal' => "A traitor hides in the ranks of the Garland Ring. Tanlorin, eager to follow up on this threat, received a message from Hyacinth that asked us to investigate further.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7207 => array(
			'name'    => "Beyond a Veil of Twilight",
			'journal' => "Zerith-var received a letter from Cantor Krin'ze asking us to meet with her. Zerith believes it might have something to do with the Rajaska.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7209 => array(
			'name'    => "Flocking Together",
			'journal' => "I found a note from a trapper named Marcien in the wilderness, along with an old snared guzzard. The note says if I deliver the guzzard to Star-Gazer Sariyaah in Ontus, I'll be rewarded.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7213 => array(
			'name'    => "Aiding the Expedition",
			'journal' => "An Imperial named Valaria Calidius was a part of an expedition to the Lucent Citadel. She said they were attacked and need help.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7218 => array(
			'name'    => "Burnt Biscuits",
			'journal' => "While traveling across West Weald I encountered a legionary in the service of Calantius, the Count of Skingrad. He says that the count's pet bear cub, Burnbiscuit, has gone missing. I agreed to help him recover the bear cub.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7219 => array(
			'name'    => "Self-Preservation",
			'journal' => "I met Belienne, a sorceress whose experiments with Daedric magic shattered her personality into multiple facets. I agreed to help her track down these facets and convince them to merge back with her.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7220 => array(
			'name'    => "The Wing of the Crow",
			'journal' => "The final wing of the Scholarium lies open. The unmarked door is actually the Wing of the Crow. The Crow will be conducting my final trial, as I seek to complete my journey to restore the Scribing Altar.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7221 => array(
			'name'    => "The Dark Behind the World",
			'journal' => "Zerith's gauntlet beckons him to Khenarthi's Rise, a place that was ruins even in his day. I should help him investigate this unusual summons, as he feels it might relate to our goal to stop Ravith-morna.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7229 => array(
			'name'    => "An Ambition Fulfilled",
			'journal' => "I came across a Daedra by the name of Ezhkel who needs help infiltrating a Worm Cult ritual. They want to reform in Coldharbour and need my help to do it.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7230 => array(
			'name'    => "Infinite Avenger: Marauder",
			'journal' => "A ghost named Esalinwe lost all her threads of fate in the Infinite Archive. She wants retribution, and asked me to kill a Marauder since they played a part in her death.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7244 => array(
			'name'    => "Mystery of the Broken Temple",
			'journal' => "Inside the ruined Broken Light Temple rests a font of the Daedric Prince Meridia's power. High Priest Eraamine seeks to access this power so her order can restore the temple to its former glory. To do that, she needs to overcome the dangers inside.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7250 => array(
			'name'    => "Dominion Lightweight Furs",
			'journal' => "<<1>> asked me to gather lightweight furs from beasts within the Aldmeri Dominion.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7262 => array(
			'name'    => "Sanguine's Solace",
			'journal' => "Lector Volonaro asked me to retrieve three specimens of the planta Sanguine's solace from the Vale of Revelry.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7269 => array(
			'name'    => "Fragments of Fear",
			'journal' => "Lector Volonaro asked me to enter the Sea and Sword Lodge and destroy the anchor fragments tearing rifts around the lodge.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7284 => array(
			'name'    => "The Worm Turns",
			'journal' => "With the fall of the Writhing Wall, Prince Azah prepares to send the Stirk Fellowship advance force into the newly opened east side of the island. But first, we need to determine what awaits us beyond the mountain pass.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7286 => array(
			'name'    => "The Final Dark",
			'journal' => "The Stirk Fellowship is ready to attack the Worm Cult's final stronghold, Mor Naril. Prince Azah formulated a desperate plan to breach the defenses and destroy the wraithforge before Mannimarco completes his sinister schemes.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7290 => array(
			'name'    => "A Guild in Crisis",
			'journal' => "First, the Fighters Guild suffered several attacks by an unidentified enemy. Then Merric, its guildmaster, disappeared. Prince Azah wants to track down Merric, find the enemy, and stop them.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7294 => array(
			'name'    => "The Regent of Sunport",
			'journal' => "The leaders of the Stirk Fellowship are assembling in Sunport to confront the Worm Cult. I agreed to help Prince Azah formalize an alliance against our common foe with Regent Karinwe of House Corelanya, governor of Sunport.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7295 => array(
			'name'    => "The Writhing Wall",
			'journal' => "A prisoner of the Worm Cult, Vanus Galerion managed to send a cryptic message to the Stirk Fellowship. I agreed to help Gabrielle Benele establish contact with Vanus and find out what more he has learned about the Worm Cult's plans.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7300 => array(
			'name'    => "The Everlasting Fair",
			'journal' => "Sergeant Alydine, a member of the Fighters Guild here with the Stirk Fellowship, asked me to investigate the Everlasting Fair. Her team, including an Alfiq named Sashii, went inside and haven't returned.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7305 => array(
			'name'    => "Striking the Deal",
			'journal' => "Captain Genevé Delitian wants me to join her when she meets with the client who paid for the cargo I found strewn across the shore of Solstice. She said there was something odd about the client and she wants someone she can trust to watch her back.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7311 => array(
			'name'    => "A Pirate's Debt",
			'journal' => "A Sea Elf pirate named Cirwynia is indebted to Captain Artellas. She asked for my help finding an item she can sell to pay off her debt.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7314 => array(
			'name'    => "The Stirk Fellowship",
			'journal' => "The Worm Cult has resurfaced on the remote island of Solstice. I agreed to join champions from across Tamriel gathering under the banner of the Stirk Fellowship to put an end to the Worm Cult's threat.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7343 => array(
			'name'    => "Covenant Undead Ichor",
			'journal' => "<<1>> asked me to collect undead ichor from rare undead within the lands of the Daggerfall Covenant.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7351 => array(
			'name'    => "Staves for the Fellowship",
			'journal' => "Armorer Reistaff asked me to craft three restoration staves of any kind for the Stirk Fellowship's healers. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7360 => array(
			'name'    => "An Enchanting Request",
			'journal' => "Armorer Reistaff asked me to craft two glyphs of any kind with a ta aspect rune for the Stirk Fellowship. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7366 => array(
			'name'    => "The Desert and the Sea",
			'journal' => "I met a wandering traveler out in Sentinel on pilgrimage to give fallen Yokudan mages Anseilak, a ritual that brings them peace. However, since he's unable to complete the journey, I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7372 => array(
			'name'    => "Heartsick Follies",
			'journal' => "Kuzam-jo, a Priest of Mara, asked me to help a struggling theater troupe find the passion they need to perform this Hearts Week.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7375 => array(
			'name'    => "A Toast to Malcontents",
			'journal' => "Kuzam-jo, the Priest of Mara overseeing the Hearts Week holiday festivities, has another task for me. This time I'm to accompany Samara Gautier around Belkarth.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7377 => array(
			'name'    => "Make Some Noise",
			'journal' => "I pledged myself to the Ruckus faction in the Night Market. My patron, Kharakhal Moon-Eye, wants me to better understand the perils of the districts before sending me out on more jobs for the Ruckus.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7391 => array(
			'name'    => "A Cold Grave",
			'journal' => "I met the spirit of Tesamoril in Western Solstice standing next to his frozen corpse. He asked me to find his wife, Healer Namreras, and inform her of his death.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7392 => array(
			'name'    => "Matters of the Heart",
			'journal' => "Kuzam-jo thanked me for the help I offered throughout the Hearts Week festivities. He has one more favor to ask of me.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7393 => array(
			'name'    => "The Golden Knight",
			'journal' => "Darien asked me to go on one last adventure with him to discover whatever secret Mannimarco knew about Darien's light and how it connects to why he's still among the living. I agreed to help him figure it all out.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7400 => array(
			'name'    => "Argent Eradication",
			'journal' => "My faction leader hired me to kill an argent enemy in the Night Market.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7403 => array(
			'name'    => "Brazen Eradication",
			'journal' => "My faction leader hired me to kill brazen in the Night Market's districts.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7426 => array(
			'name'    => "Racing Into Nowhere",
			'journal' => "Quen coaxed the location of the Nowhere Vault out of the Codex Voernet, we acquired at least one Nowhere Key, and we're ready to make our play for the Thieves' Moon. Unfortunately, the Koldane Cartel is after the same treasure.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7433 => array(
			'name'    => "Missing Persons",
			'journal' => "I found a notice from the Grasp of the Stricture offering a reward for detailed information on three missing persons in the Skittering Precinct.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7435 => array(
			'name'    => "Sullied Reputations",
			'journal' => "I found a note tacked onto a notice board telling me that the other factions put up horrible propaganda about my faction. For a reward, I can clean up the district and protect our reputation.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7437 => array(
			'name'    => "Curio Collection",
			'journal' => "I discovered an incomplete requisition for some unusual items scattered throughout the Skittering Precinct. If I find them, I can deliver them to the northern rooftop oasis for a reward and gain reputation within my faction.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7444 => array(
			'name'    => "The Last Laugh",
			'journal' => "Sheogorath the God of Madness set aside his power to take a holiday. That power manifested as a being called Maddy, which fled to the Shivering Isles. The Mad God needs my help to recover his power before poison kills his mortal form.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7449 => array(
			'name'    => "Chasing Madness",
			'journal' => "The Daedric Prince Sheogorath was poisoned while on his holiday in mortal form. To stave off death, he must reclaim his power. I agreed to help him.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7453 => array(
			'name'    => "A Guild Grows In Glenumbra",
			'journal' => "After the brutal attack by the Koldane Cartel on Basil-and the revelation that the cartel includes werewolves!-Captain Sansonne may be interested in working with us to end the threat of the cartel. Now Quen and I just need to convince the others.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7454 => array(
			'name'    => "Free the Caged",
			'journal' => "I found a note asking for aid in freeing caged animals captured by poachers. If I free the animals, I can return this note to a collection box on a rooftop oasis to receive payment.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7455 => array(
			'name'    => "Fatal Flask",
			'journal' => "I found a note in the Parch about a bottle of some deadly concoction misplaced by Alchemist Narinyor in one of three locations in the Parch. He offered a reward for its return to a delivery box in the northern rooftop oasis.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7457 => array(
			'name'    => "Restocking the Bizarre",
			'journal' => "I discovered a written request for bizarre objects found within Sorrow's Friend. If I find them, I can deliver them for a reward and gain reputation within my faction.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7458 => array(
			'name'    => "Scavenger's Riddles",
			'journal' => "I found a curious missive that hints at a hidden treasure somewhere in the Parch. If I solve this riddle, I might learn more.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7461 => array(
			'name'    => "Too Many Spiders",
			'journal' => "I discovered a scroll asking me to repair spider deterrent devices in the Skittering Precinct. The author of the scroll seemed really concerned that the devices were going to overheat and stop working.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7477 => array(
			'name'    => "Favor for Battlereeve Urcelmo",
			'journal' => "I agreed to do a favor for Battlereeve Urcelmo.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7484 => array(
			'name'    => "Quenneth Lumena's Warrant",
			'journal' => "The bounty on Quen has escalated. What was originally the broader charge of crimes against the realm has increased to the charge of murder. Captain Sansonne warned us that the Lion Guard would be hunting her soon.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
	7499 => array(
			'name'    => "The Family Jewels",
			'journal' => "<<1>> runs a finder's business in Glenumbra and uses the Thieves Guild to track down lost, mislaid, and stolen items. Helping her will surely reflect well on my skill as a thief-and be lucrative to boot.",
			'internalName' => "",
			'zoneId' => 0,
			'zone' => "",
			'type' => 0,
		),
);

